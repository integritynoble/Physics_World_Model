...
# Solver registry for multispectral_sat
SOLVERS = {
    "traditional_cpu": {
        "name": "RDA [proxy]",
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    Args:
        solver_key: One of ['traditional_cpu', 'best_quality', 'ms_dl']
...
def run_best_quality(y: np.ndarray, operator: Any = None, cfg: Optional[Dict] = None) -> np.ndarray:
    """SAR-DL [proxy]. CPU only.
    Reference: Richardson 1972, JOSA
    """
    return run_solver("best_quality", y, operator, cfg)

def run_ms_dl(y: np.ndarray, operator: Any = None, cfg: Optional[Dict] = None) -> np.ndarray:
    """MS-Pansharpening-DL [proxy]. CPU only.
    Reference: Richardson 1972, JOSA
    """
    return run_solver("ms_dl", y, operator, cfg)
