...
# Inject reconstruction results (best-run images + metrics) from summary JSON
_RESULTS_JSON = _Path(__file__).resolve().parent.parent / "static" / "img" / "results" / "results_summary.json"
if _RESULTS_JSON.exists():
    import json as _json
    _results = _json.loads(_RESULTS_JSON.read_text())
    for _key, _data in _results.items():
        if _key in MODALITY_DATABASE:
            MODALITY_DATABASE[_key]["recon_results"] = _data

# Inject paper setup figures + citations from manifest JSON
_PAPER_JSON = _Path(__file__).resolve().parent.parent / "static" / "img" / "paper_setups" / "paper_setups_manifest.json"
if _PAPER_JSON.exists():
    import json as _json2
    _papers = _json2.loads(_PAPER_JSON.read_text())
    for _key, _data in _papers.items():
        if _key in MODALITY_DATABASE:
            MODALITY_DATABASE[_key]["paper_setup_url"] = _data["image"]
            MODALITY_DATABASE[_key]["paper_setup_citation"] = _data["citation"]
            MODALITY_DATABASE[_key]["paper_setup_link"] = _data["link"]


# ── Forward-Model Mismatch Data ────────────────────────────────────────────
# For each modality, documents what goes wrong when the widefield Gaussian
# blur fallback (sigma=2.0, shape-preserving, real-valued, self-adjoint) is
# used instead of the correct physics operator, and how to correct it.

_MODALITY_MISMATCHES: dict[str, dict] = {
...
# Merge mismatch data into MODALITY_DATABASE introductions
for _key, _mm in _MODALITY_MISMATCHES.items():
    if _key in MODALITY_DATABASE and "introduction" in MODALITY_DATABASE[_key]:
        MODALITY_DATABASE[_key]["introduction"].update(_mm)
