"""Tests for CasePack: SAR.

Template: sar_graph_v2
Chain: generic_source -> sar_backprojection -> generic_sensor -> complex_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackSar:
    """CasePack acceptance tests for the sar modality."""

    def test_template_compiles(self):
        """sar_graph_v2 template compiles without error."""
        tpl = _load_template("sar_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "sar_graph_v2", **tpl_clean})
