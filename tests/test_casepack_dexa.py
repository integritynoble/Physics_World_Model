"""Tests for CasePack: DEXA.

Template: dexa_graph_v2
Chain: xray_source -> dual_energy_beer_lambert -> photon_sensor -> poisson_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackDexa:
    """CasePack acceptance tests for the dexa modality."""

    def test_template_compiles(self):
        """dexa_graph_v2 template compiles without error."""
        tpl = _load_template("dexa_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "dexa_graph_v2", **tpl_clean})
