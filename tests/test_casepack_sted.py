"""Tests for CasePack: STED.

Template: sted_graph_v2
Chain: photon_source -> saturation_depletion -> conv2d -> photon_sensor -> poisson_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackSted:
    """CasePack acceptance tests for the sted modality."""

    def test_template_compiles(self):
        """sted_graph_v2 template compiles without error."""
        tpl = _load_template("sted_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "sted_graph_v2", **tpl_clean})
