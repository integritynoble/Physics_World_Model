"""Tests for Holography: template, forward, adjoint recon, W2 correction."""
import numpy as np
import pytest
...
def _build_graph():
    spec = OperatorGraphSpec.model_validate({
        "graph_id": "holography_graph_v2",
        "metadata": {"canonical_chain": True, "modality": "holography",
                      "x_shape": [32, 32], "y_shape": [32, 32]},
        "nodes": [
            {"node_id": "source", "primitive_id": "photon_source", "role": "source", "params": {"strength": 1.0}},
            {"node_id": "prop", "primitive_id": "fresnel_prop", "role": "transport", "params": {"wavelength": 0.633e-6, "distance": 5.0e-3, "pixel_size": 3.45e-6}},
...
class TestHolography:
    def test_template_compiles(self):
        graph = _build_graph()
        assert graph.graph_id == "holography_graph_v2"
    ...
    def test_adjoint_recon_psnr(self):
        ...
        prop = get_primitive("fresnel_prop", {"wavelength": 0.633e-6, "distance": 5.0e-3, "pixel_size": 3.45e-6})
        ...
        assert psnr > 15, f"Adjoint PSNR {psnr:.1f} < 15"

    def test_w2_nll_decreases(self):
        src = get_primitive("photon_source", {"strength": 1.0})
        def make_fwd(dist):
            p = get_primitive("fresnel_prop", {"wavelength": 0.633e-6, "distance": dist, "pixel_size": 3.45e-6})
        ...
        sigma_noise = 0.005
        y_measured = make_fwd(7.0e-3)(x_true) + rng.randn(32, 32) * sigma_noise
        nll_before = float(np.sum((y_measured - make_fwd(5.0e-3)(x_true))**2 / sigma_noise**2))
        best_nll = np.inf
        for td in np.linspace(3.0e-3, 10.0e-3, 15):
