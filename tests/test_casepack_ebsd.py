"""Tests for CasePack: EBSD.

Template: ebsd_graph_v2
Chain: generic_source -> reciprocal_space_geometry -> energy_resolving_detector -> poisson_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackEbsd:
    """CasePack acceptance tests for the ebsd modality."""

    def test_template_compiles(self):
        """ebsd_graph_v2 template compiles without error."""
        tpl = _load_template("ebsd_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "ebsd_graph_v2", **tpl_clean})
