"""Tests for CasePack: PALM STORM.

Template: palm_storm_graph_v2
Chain: photon_source -> blinking_emitter -> photon_sensor -> poisson_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackPalmStorm:
    """CasePack acceptance tests for the palm_storm modality."""

    def test_template_compiles(self):
        """palm_storm_graph_v2 template compiles without error."""
        tpl = _load_template("palm_storm_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "palm_storm_graph_v2", **tpl_clean})
