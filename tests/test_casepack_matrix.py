"""Tests for CasePack: Matrix.

Template: matrix_graph_v2
Chain: generic_source -> random_mask -> generic_sensor -> poisson_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackMatrix:
    """CasePack acceptance tests for the matrix modality."""

    def test_template_compiles(self):
        """matrix_graph_v2 template compiles without error."""
        tpl = _load_template("matrix_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "matrix_graph_v2", **tpl_clean})
