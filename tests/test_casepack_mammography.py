"""Tests for CasePack: Mammography.

Template: mammography_graph_v2
Chain: xray_source -> beer_lambert -> photon_sensor -> poisson_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackMammography:
    """CasePack acceptance tests for the mammography modality."""

    def test_template_compiles(self):
        """mammography_graph_v2 template compiles without error."""
        tpl = _load_template("mammography_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "mammography_graph_v2", **tpl_clean})
