"""Tests for CasePack: Polarization.

Template: polarization_graph_v2
Chain: photon_source -> conv2d -> photon_sensor -> poisson_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackPolarization:
    """CasePack acceptance tests for the polarization modality."""

    def test_template_compiles(self):
        """polarization_graph_v2 template compiles without error."""
        tpl = _load_template("polarization_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "polarization_graph_v2", **tpl_clean})
