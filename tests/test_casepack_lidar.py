"""Tests for CasePack: Lidar.

Template: lidar_graph_v2
Chain: photon_source -> scan_trajectory -> tof_gate -> spad_tof_sensor -> poisson_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackLidar:
    """CasePack acceptance tests for the lidar modality."""

    def test_template_compiles(self):
        """lidar_graph_v2 template compiles without error."""
        tpl = _load_template("lidar_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "lidar_graph_v2", **tpl_clean})
