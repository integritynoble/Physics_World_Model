"""Tests for CasePack: Panorama.

Template: panorama_graph_v2
Chain: photon_source -> conv2d -> frame_integration -> photon_sensor -> poisson_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackPanorama:
    """CasePack acceptance tests for the panorama modality."""

    def test_template_compiles(self):
        """panorama_graph_v2 template compiles without error."""
        tpl = _load_template("panorama_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "panorama_graph_v2", **tpl_clean})
