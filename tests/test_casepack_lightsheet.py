"""Tests for Lightsheet: template, forward, RL recon, W2 correction."""
import numpy as np
import pytest
...
def _build_graph():
    spec = OperatorGraphSpec.model_validate({
        "graph_id": "lightsheet_graph_v2",
        "metadata": {"canonical_chain": True, "modality": "lightsheet",
                      "x_shape": [32, 32], "y_shape": [32, 32]},
        "nodes": [
            {"node_id": "source", "primitive_id": "photon_source", "role": "source", "params": {"strength": 1.0}},
            {"node_id": "blur", "primitive_id": "conv2d", "role": "transport", "params": {"sigma": 1.0, "mode": "reflect"}},
            {"node_id": "sensor", "primitive_id": "photon_sensor", "role": "sensor", "params": {"quantum_efficiency": 0.9, "gain": 1.0}},
            {"node_id": "noise", "primitive_id": "poisson_gaussian_sensor", "role": "noise", "params": {"peak_photons": 15000.0, "read_sigma": 0.01, "seed": 0}},
        ],
        "edges": [
...
class TestLightsheet:
    def test_template_compiles(self):
        graph = _build_graph()
        assert graph.graph_id == "lightsheet_graph_v2"
    ...
    def test_rl_recon_psnr(self):
        ...
        y = ndimage.gaussian_filter(x_true, sigma=1.0) + np.random.RandomState(42).randn(64, 64) * 0.01
        ax = np.arange(-7, 8, dtype=np.float64)
        xg, yg = np.meshgrid(ax, ax)
        psf = np.exp(-(xg**2 + yg**2) / (2 * 1.0**2))
        psf /= psf.sum()
