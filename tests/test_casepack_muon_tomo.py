"""Tests for CasePack: Muon Tomo.

Template: muon_tomo_graph_v2
Chain: generic_source -> multiple_scattering -> track_detector_sensor -> poisson_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackMuonTomo:
    """CasePack acceptance tests for the muon_tomo modality."""

    def test_template_compiles(self):
        """muon_tomo_graph_v2 template compiles without error."""
        tpl = _load_template("muon_tomo_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "muon_tomo_graph_v2", **tpl_clean})
