"""Tests for CasePack: TIRF.

Template: tirf_graph_v2
Chain: photon_source -> evanescent_decay -> conv2d -> photon_sensor -> poisson_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackTirf:
    """CasePack acceptance tests for the tirf modality."""

    def test_template_compiles(self):
        """tirf_graph_v2 template compiles without error."""
        tpl = _load_template("tirf_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "tirf_graph_v2", **tpl_clean})
