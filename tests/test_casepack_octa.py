"""Tests for CasePack: OCTA.

Template: octa_graph_v2
Chain: photon_source -> angular_spectrum -> vessel_flow_contrast -> photon_sensor -> poisson_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackOcta:
    """CasePack acceptance tests for the octa modality."""

    def test_template_compiles(self):
        """octa_graph_v2 template compiles without error."""
        tpl = _load_template("octa_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "octa_graph_v2", **tpl_clean})
