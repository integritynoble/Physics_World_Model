"""Tests for CasePack: ToF Camera.

Template: tof_camera_graph_v2
Chain: photon_source -> tof_gate -> spad_tof_sensor -> poisson_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackTofCamera:
    """CasePack acceptance tests for the tof_camera modality."""

    def test_template_compiles(self):
        """tof_camera_graph_v2 template compiles without error."""
        tpl = _load_template("tof_camera_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "tof_camera_graph_v2", **tpl_clean})
