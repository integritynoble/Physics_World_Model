...
**OperatorGraph-first rule (foundation of PWM):**
- **All 26 modality forward models are represented as `OperatorGraphSpec`** (no modality-specific forward code on the hot path).
- **All reconstruction/calibration pipelines consume a `GraphOperator`** (or an adapter that exposes the same interface).
- Layer B (templates/CasePacks) only supplies **defaults** (graph_template_id, priors, presets, datasets), never bespoke physics.
...
## 2.3 OperatorGraph as the single source of truth (26 modalities embedded in Layer A)

**Yes — it is feasible** to embed all 26 modalities into Layer A, *as long as they are within PWM’s declared PhysicsScope.* The key is to treat every modality as:

- **Graph template**: `graph_templates.yaml` entry (skeleton DAG)
- **Parameterization**: θ (system + mismatch) + ϕ (noise)
- **Assets/blobs**: mask/PSF/trajectory/calibration captures stored in RunBundle
- **Execution**: always via `GraphCompiler → GraphOperator`

### What “embedded” means in code
- `modalities.yaml` keeps human-facing names/tags, but **never owns physics**.
- `graph_templates.yaml` becomes the canonical definition of each modality’s forward model.
- Old per-modality `Operator.forward()` implementations become **reference wrappers** used only for equivalence testing until fully retired.

### Nonlinear / hybrid modalities
Some modalities are nonlinear (e.g., holography) or use iterative physics solvers. They still fit by adding primitives like:
- `magnitude_sq`, `saturation`, `phase_wrap`
- `pde_wave_propagation` (expensive primitive, but still a node)
If the primitive exists in PhysicsScope, the modality can be embedded.

### Mandatory tests to guarantee “Layer A is the foundation”
Add these CI gates for all 26 modalities:

1) **Compile test**: template → `GraphOperator` compiles successfully  
2) **Serialization test**: `serialize()` roundtrip + SHA256 hashes stable  
3) **Adjoint test** (if linear): `check_adjoint()` passes  
4) **Equivalence test** (temporary): Graph output matches legacy operator output (within tolerance)  
5) **Solver integration test**: every registered solver can run using `GraphOperator` interface
...
### 4.5 Solver API unification (so reconstruction is OperatorGraph-based)

To make OperatorGraph the **foundation of PWM**, every solver must operate on the same operator interface:

```python
class LinearLikeOperator(Protocol):
    def forward(self, x: np.ndarray) -> np.ndarray: ...
    def adjoint(self, y: np.ndarray) -> np.ndarray: ...
    def shape(self) -> tuple[tuple[int, ...], tuple[int, ...]]: ...  # (x_shape, y_shape)
```

Rules:
- **Classical solvers** (GAP/ADMM/ISTA/FISTA/TV/PnP) must call `op.forward()` and `op.adjoint()` only.
- **Learning-based solvers** that need operator access must receive `op` (or its Jacobian/adjoint where defined).
- If a solver truly cannot use `adjoint()` (rare), it must declare `requires_adjoint: false` and use a compatible alternative (e.g., score-based methods with forward-only training), but the *measurement simulation* still comes from `GraphOperator`.

Deliverable:
- A single `SolverRunner(op: GraphOperator, y, config)` path used by `pwm run`, `pwm demo`, and paper experiments.
...
**Goal:** OperatorGraph becomes the **single source of truth**: all 26 modalities compile to `GraphOperator`, and reconstruction uses the GraphOperator interface.
...
- All reconstruction/calibration pipelines accept `GraphOperator` (no modality-specific forward model in the main pipeline)
