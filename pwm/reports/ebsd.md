...
| Modality ID | `ebsd` |
| Category | electron |
| Dataset | ebsd_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
- Modality key: `ebsd`
- Category: electron
- Forward model: y = Sensor(ReciprocalSpaceGeometry(GenericSource(x))) + n
- Default solver: pseudo_inverse
- Pipeline linearity: nonlinear

EBSD imaging pipeline with 1 element(s) in the forward chain. Reconstruction uses pseudo_inverse. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: ebsd_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `ebsd_benchmark`
...
SourceNode: generic_source — strength=1.0
  ↓
Element 1 (subrole=transport): reciprocal_space_geometry — sample_tilt_deg=70.0, detector_distance=15.0
  ↓
...
| 2 | geometry | reciprocal_space_geometry | transport | sample_tilt_deg=70.0, detector_distance=15.0 | gain | [0.5, 2.0] | uniform |
| 3 | sensor | energy_resolving_detector | sensor | n_channels=256, energy_range_ev=[0, 2000], qe=0.8 | gain | [0.5, 2.0] | uniform |
| 4 | noise | poisson_gaussian_sensor | noise | peak_photons=8000.0, read_sigma=0.015, seed=0 | — | — | — |
...
| 2 | geometry | (64, 64) | float64 | [0.0000, 1.0000] | `artifacts/trace/02_geometry.npy` |
...
- **Prompt used:** `"Simulate ebsd measurement of a phantom and reconstruct"`
...
- **Mode I recon using corrected operator A':**
  ...
  |--------|-------------------|----------------|-------|
  | PSNR   | 13.02 dB | 13.02 dB | +0.00 dB |
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (13.02 dB)
- [x] W2 operator correction (NLL decrease): PASS (78.0%)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 13.02 dB | >= 1 dB | PASS |
| W1 SSIM | ssim | 0.3874 | — | info |
| W1 NRMSE | nrmse | 0.2417 | — | info |
| W2 NLL decrease | nll_decrease_pct | 78.0% | >= 0% | PASS |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality ebsd --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): 36f46d22c5cc0004
- Output hash (x_hat): 0c9c565c465c968c

## Saved artifacts

- RunBundle: `runs/run_ebsd_exp_ecfbdefd/`
- Report: `pwm/reports/ebsd.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_ebsd_exp_ecfbdefd/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_ebsd_exp_ecfbdefd/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_ebsd_exp_ecfbdefd/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world EBSD datasets at full resolution
