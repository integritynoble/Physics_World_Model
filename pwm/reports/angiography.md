# Angiography — PWM Report

| Field | Value |
|-------|-------|
| Modality ID | `angiography` |
| Category | xray_variant |
| Dataset | angiography_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
- Modality key: `angiography`
- Category: xray_variant
- Forward model: y = Sensor(FluoroInteg(BeerLambert(XRaySource(x)))) + n
- Default solver: pseudo_inverse
- Pipeline linearity: nonlinear

Angiography imaging pipeline with 2 element(s) in the forward chain. Reconstruction uses pseudo_inverse. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: angiography_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `angiography_benchmark`
...
SourceNode: xray_source — strength=1.0
  ↓
Element 1 (subrole=transduction): beer_lambert — I_0=6000.0
  ↓
Element 2 (subrole=encoding): fluoro_temporal_integrator — n_frames=4, decay=0.85
  ↓
SensorNode: xray_detector_sensor — scintillator_efficiency=0.75, gain=1.0
  ↓
...
| 2 | transmission | beer_lambert | transduction | I_0=6000.0 | gain | [0.5, 2.0] | uniform |
| 3 | temporal_int | fluoro_temporal_integrator | encoding | n_frames=4, decay=0.85 | — | — | — |
| 4 | sensor | xray_detector_sensor | sensor | scintillator_efficiency=0.75, gain=1.0 | gain | [0.5, 2.0] | uniform |
...
- **Prompt used:** `"Simulate angiography measurement of a phantom and reconstruct"`
...
- **Operator definition:**
  ...
  - A_sha256: 13b85fe0a9a9c5f2
  - Linearity: nonlinear
  - Notes (if linearized): N/A
- **Mismatch specification:**
  - Mismatch type: synthetic_injected
  - Parameters perturbed: I_0 drift (6000 -> 7800)
...
- **Mode C fit results:**
  ...
  - NLL before correction: 16841762012.9
...
- **Mode I recon using corrected operator A':**
  ...
  |--------|-------------------|----------------|-------|
  | PSNR   | 75.13 dB | 75.13 dB | +0.00 dB |
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (75.13 dB)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 75.13 dB | >= 1 dB | PASS |
| W1 SSIM | ssim | 1.0000 | — | info |
| W1 NRMSE | nrmse | 0.0002 | — | info |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality angiography --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): eba5b5d883075923
- Output hash (x_hat): 4313877b710080ce

## Saved artifacts

- RunBundle: `runs/run_angiography_exp_b86daf42/`
- Report: `pwm/reports/angiography.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_angiography_exp_b86daf42/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_angiography_exp_b86daf42/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_angiography_exp_b86daf42/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world Angiography datasets at full resolution
