# Mammography — PWM Report

| Field | Value |
|-------|-------|
| Modality ID | `mammography` |
| Category | xray_variant |
| Dataset | mammography_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
- Modality key: `mammography`
- Category: xray_variant
- Forward model: y = Sensor(BeerLambert(XRaySource(x))) + n
- Default solver: pseudo_inverse
- Pipeline linearity: nonlinear

Mammography imaging pipeline with 1 element(s) in the forward chain. Reconstruction uses pseudo_inverse. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: mammography_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `mammography_benchmark`
...
SourceNode: xray_source — strength=1.0
  ↓
Element 1 (subrole=transduction): beer_lambert — I_0=8000.0
  ↓
...
| 2 | transmission | beer_lambert | transduction | I_0=8000.0 | gain | [0.5, 2.0] | uniform |
| 3 | sensor | photon_sensor | sensor | quantum_efficiency=0.85, gain=1.0 | gain | [0.5, 2.0] | uniform |
| 4 | noise | poisson_gaussian_sensor | noise | peak_photons=30000.0, read_sigma=0.008, seed=0 | — | — | — |
...
- **Prompt used:** `"Simulate mammography measurement of a phantom and reconstruct"`
...
- **Mode C fit results:**
  ...
  - NLL before correction: 25448981813.8
...
- **Mode I recon using corrected operator A':**
  ...
  |--------|-------------------|----------------|-------|
  | PSNR   | 76.92 dB | 76.92 dB | +0.00 dB |
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (76.92 dB)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 76.92 dB | >= 1 dB | PASS |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality mammography --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): 5f2da1426c38e896
- Output hash (x_hat): 7b0ae847e5b1078c

## Saved artifacts

- RunBundle: `runs/run_mammography_exp_2e52d989/`
- Report: `pwm/reports/mammography.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_mammography_exp_2e52d989/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_mammography_exp_2e52d989/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_mammography_exp_2e52d989/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world Mammography datasets at full resolution
