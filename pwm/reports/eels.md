...
| Modality ID | `eels` |
| Category | electron |
| Dataset | eels_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
- Modality key: `eels`
- Category: electron
- Forward model: y = Sensor(EnergyResolvingDetector(GenericSource(x))) + n
- Default solver: pseudo_inverse
- Pipeline linearity: linear

EELS imaging pipeline with 1 element(s) in the forward chain. Reconstruction uses pseudo_inverse. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: eels_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `eels_benchmark`
...
SourceNode: generic_source — strength=1.0
  ↓
Element 1 (subrole=transport): energy_resolving_detector — n_channels=1024, energy_range_ev=[0, 3000], qe=0.7
  ↓
...
| 2 | spectrometer | energy_resolving_detector | transport | n_channels=1024, energy_range_ev=[0, 3000], qe=0.7 | gain | [0.5, 2.0] | uniform |
...
| 2 | spectrometer | (64, 64) | float64 | [0.0000, 1.0000] | `artifacts/trace/02_spectrometer.npy` |
...
- **Prompt used:** `"Simulate eels measurement of a phantom and reconstruct"`
...
- **Mode I recon using corrected operator A':**
  ...
  |--------|-------------------|----------------|-------|
  | PSNR   | 26.69 dB | 26.69 dB | +0.00 dB |
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (26.69 dB)
- [x] W2 operator correction (NLL decrease): PASS (80.9%)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 26.69 dB | >= 1 dB | PASS |
| W1 SSIM | ssim | 0.9772 | — | info |
| W1 NRMSE | nrmse | 0.0501 | — | info |
| W2 NLL decrease | nll_decrease_pct | 80.9% | >= 0% | PASS |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality eels --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): 64d9b33a3134c20f
- Output hash (x_hat): 88dfcc551ad51538

## Saved artifacts

- RunBundle: `runs/run_eels_exp_32adf4d4/`
- Report: `pwm/reports/eels.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_eels_exp_32adf4d4/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_eels_exp_32adf4d4/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_eels_exp_32adf4d4/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world EELS datasets at full resolution
