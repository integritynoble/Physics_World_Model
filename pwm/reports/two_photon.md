# Two Photon — PWM Report

| Field | Value |
|-------|-------|
| Modality ID | `two_photon` |
| Category | microscopy |
| Dataset | two_photon_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
- Modality key: `two_photon`
- Category: microscopy
- Forward model: y = Sensor(Conv2D(NonlinearExcitation(PhotonSource(x)))) + n
- Default solver: richardson_lucy_2d
- Pipeline linearity: nonlinear

Two Photon imaging pipeline with 2 element(s) in the forward chain. Reconstruction uses richardson_lucy_2d. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: two_photon_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `two_photon_benchmark`
...
SourceNode: photon_source — strength=1.0
  ↓
Element 1 (subrole=interaction): nonlinear_excitation — n_photons=2
  ↓
Element 2 (subrole=transport): conv2d — sigma=1.5, mode=reflect
  ↓
...
| 2 | excitation | nonlinear_excitation | interaction | n_photons=2 | gain | [0.5, 2.0] | uniform |
| 3 | blur | conv2d | transport | sigma=1.5, mode=reflect | — | — | — |
| 4 | sensor | photon_sensor | sensor | quantum_efficiency=0.45, gain=1.0 | gain | [0.5, 2.0] | uniform |
| 5 | noise | poisson_gaussian_sensor | noise | peak_photons=5000.0, read_sigma=0.02, seed=0 | — | — | — |
...
| 2 | excitation | (64, 64) | float64 | [0.0000, 1.0000] | `artifacts/trace/02_excitation.npy` |
...
- **Prompt used:** `"Simulate two_photon measurement of a phantom and reconstruct"`
...
- **Mode C fit results:**
  ...
  - NLL before correction: 2735.0
  - NLL after correction: 1992.8
  - NLL decrease: 27.1%
- **Mode I recon using corrected operator A':**

  | Metric | A₀ (uncorrected) | A' (corrected) | Delta |
  |--------|-------------------|----------------|-------|
  | PSNR   | 11.77 dB | 11.24 dB | -0.53 dB |
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (11.77 dB)
- [x] W2 operator correction (NLL decrease): PASS (27.1%)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 11.77 dB | >= 1 dB | PASS |
| W1 SSIM | ssim | 0.1336 | — | info |
| W1 NRMSE | nrmse | 0.2793 | — | info |
| W2 NLL decrease | nll_decrease_pct | 27.1% | >= 0% | PASS |
| W2 PSNR delta | psnr_delta | -0.53 dB | — | info |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality two_photon --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): 0e45b4dd7b955b29
- Output hash (x_hat): 4d6b1e0ce5a2dada

## Saved artifacts

- RunBundle: `runs/run_two_photon_exp_4d788cb1/`
- Report: `pwm/reports/two_photon.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_two_photon_exp_4d788cb1/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_two_photon_exp_4d788cb1/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_two_photon_exp_4d788cb1/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world Two Photon datasets at full resolution
