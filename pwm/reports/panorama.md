...
| Field | Value |
|-------|-------|
| Modality ID | `panorama` |
| Category | computational |
| Dataset | panorama_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
- Category: computational
- Forward model: y = Sensor(FrameInteg(Conv2D(PhotonSource(x)))) + n
- Default solver: pseudo_inverse
- Pipeline linearity: linear

Panorama imaging pipeline with 2 element(s) in the forward chain. Reconstruction uses pseudo_inverse. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: panorama_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `panorama_benchmark`
...
SourceNode: photon_source — strength=1.0
  ↓
Element 1 (subrole=transport): conv2d — sigma=1.0, mode=reflect
  ↓
Element 2 (subrole=encoding): frame_integration — axis=0, T=1
  ↓
...
| 2 | defocus | conv2d | transport | sigma=1.0, mode=reflect | gain | [0.5, 2.0] | uniform |
| 3 | integrate | frame_integration | encoding | axis=0, T=1 | — | — | — |
| 4 | sensor | photon_sensor | sensor | quantum_efficiency=0.9, gain=1.0 | gain | [0.5, 2.0] | uniform |
| 5 | noise | poisson_gaussian_sensor | noise | peak_photons=10000.0, read_sigma=0.01, seed=0 | — | — | — |
...
| 2 | defocus | (64, 64) | float64 | [0.0000, 1.0000] | `artifacts/trace/02_defocus.npy` |
| 3 | integrate | (64, 64) | float64 | [0.0000, 1.0000] | `artifacts/trace/03_integrate.npy` |
...
- **Prompt used:** `"Simulate panorama measurement of a phantom and reconstruct"`
...
- **Mode C fit results:**
  ...
  - NLL before correction: 259080.4
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (8.26 dB)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 8.26 dB | >= 1 dB | PASS |
| W1 SSIM | ssim | 0.4152 | — | info |
| W1 NRMSE | nrmse | 0.4184 | — | info |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality panorama --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): 213959f5ad706d66
- Output hash (x_hat): c66fd4616e58b215

## Saved artifacts

- RunBundle: `runs/run_panorama_exp_c92db02b/`
- Report: `pwm/reports/panorama.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_panorama_exp_c92db02b/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_panorama_exp_c92db02b/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_panorama_exp_c92db02b/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world Panorama datasets at full resolution
