...
| Dataset | sar_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
- Modality key: `sar`
- Category: radar_sonar
- Forward model: y = Sensor(SARBackprojection(GenericSource(x))) + n
- Default solver: adjoint_backprojection
- Pipeline linearity: linear

SAR imaging pipeline with 1 element(s) in the forward chain. Reconstruction uses adjoint_backprojection. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: sar_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `sar_benchmark`
...
SourceNode: generic_source — strength=1.0
  ↓
Element 1 (subrole=encoding): sar_backprojection — n_pulses=64, swath_width=100.0
  ↓
...
| 2 | backproject | sar_backprojection | encoding | n_pulses=64, swath_width=100.0 | gain | [0.5, 2.0] | uniform |
| 3 | sensor | generic_sensor | sensor | gain=1.0 | gain | [0.5, 2.0] | uniform |
| 4 | noise | complex_gaussian_sensor | noise | sigma=0.01, seed=0 | — | — | — |
...
| 2 | backproject | (64, 64) | float64 | [0.0000, 1.0000] | `artifacts/trace/02_backproject.npy` |
...
- **Prompt used:** `"Simulate sar measurement of a phantom and reconstruct"`
...
- **Mode C fit results:**
  ...
  - NLL before correction: 491823638.7
...
- **Mode I recon using corrected operator A':**
  ...
  |--------|-------------------|----------------|-------|
  | PSNR   | 10.65 dB | 82.96 dB | +72.31 dB |
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (10.65 dB)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 10.65 dB | >= 1 dB | PASS |
| W1 SSIM | ssim | 0.6227 | — | info |
| W1 NRMSE | nrmse | 0.3175 | — | info |
| W2 NLL decrease | nll_decrease_pct | 100.0% | >= 0% | PASS |
| W2 PSNR delta | psnr_delta | +72.31 dB | — | info |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality sar --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): 3b71003f41e67189
- Output hash (x_hat): 41ddcfc067ca629f

## Saved artifacts

- RunBundle: `runs/run_sar_exp_a30c6288/`
- Report: `pwm/reports/sar.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_sar_exp_a30c6288/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_sar_exp_a30c6288/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_sar_exp_a30c6288/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world SAR datasets at full resolution
