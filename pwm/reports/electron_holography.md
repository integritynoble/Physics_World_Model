# Electron Holography — PWM Report

| Field | Value |
|-------|-------|
| Modality ID | `electron_holography` |
| Category | electron |
| Dataset | electron_holo_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
- Modality key: `electron_holography`
- Category: electron
- Forward model: y = Sensor(FresnelProp(GenericSource(x))) + n
- Default solver: pseudo_inverse
- Pipeline linearity: nonlinear

Electron Holography imaging pipeline with 1 element(s) in the forward chain. Reconstruction uses pseudo_inverse. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: electron_holo_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `electron_holo_benchmark`
...
SourceNode: generic_source — strength=1.0
  ↓
Element 1 (subrole=transport): fresnel_prop — wavelength=2.5e-12, distance=0.001, pixel_size=1e-10
  ↓
...
| 2 | propagate | fresnel_prop | transport | wavelength=2.5e-12, distance=0.001, pixel_size=1e-10 | gain | [0.5, 2.0] | uniform |
| 3 | sensor | photon_sensor | sensor | quantum_efficiency=0.8, gain=1.0 | gain | [0.5, 2.0] | uniform |
| 4 | noise | poisson_gaussian_sensor | noise | peak_photons=20000.0, read_sigma=0.01, seed=0 | — | — | — |
...
- **Prompt used:** `"Simulate electron_holography measurement of a phantom and reconstruct"`
...
- **Mode C fit results:**
  ...
  - NLL before correction: 46810.0
  - NLL after correction: 2048.4
  - NLL decrease: 95.6%
- **Mode I recon using corrected operator A':**

  | Metric | A₀ (uncorrected) | A' (corrected) | Delta |
  |--------|-------------------|----------------|-------|
  | PSNR   | 24.46 dB | 24.46 dB | +0.00 dB |
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (24.46 dB)
- [x] W2 operator correction (NLL decrease): PASS (95.6%)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 24.46 dB | >= 1 dB | PASS |
| W1 SSIM | ssim | 0.9676 | — | info |
| W1 NRMSE | nrmse | 0.0648 | — | info |
| W2 NLL decrease | nll_decrease_pct | 95.6% | >= 0% | PASS |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality electron_holography --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): 71be75687b85cbe3
- Output hash (x_hat): 3b73840094a66707

## Saved artifacts

- RunBundle: `runs/run_electron_holography_exp_f88967d2/`
- Report: `pwm/reports/electron_holography.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_electron_holography_exp_f88967d2/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_electron_holography_exp_f88967d2/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_electron_holography_exp_f88967d2/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world Electron Holography datasets at full resolution
