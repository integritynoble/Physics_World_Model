...
| Modality ID | `stem` |
| Category | electron |
| Dataset | stem_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
STEM imaging pipeline with 2 element(s) in the forward chain. Reconstruction uses pseudo_inverse. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: stem_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `stem_benchmark`
...
- **Prompt used:** `"Simulate stem measurement of a phantom and reconstruct"`
...
- **Mode C fit results:**
  ...
  - NLL before correction: 104027.3
...
- **Mode I recon using corrected operator A':**
  ...
  |--------|-------------------|----------------|-------|
  | PSNR   | 10.60 dB | 10.60 dB | +0.00 dB |
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (10.60 dB)
- [x] W2 operator correction (NLL decrease): PASS (98.0%)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 10.60 dB | >= 1 dB | PASS |
| W1 SSIM | ssim | -0.0039 | — | info |
| W1 NRMSE | nrmse | 0.3194 | — | info |
| W2 NLL decrease | nll_decrease_pct | 98.0% | >= 0% | PASS |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality stem --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): 29b5f2bfe5f26d87
- Output hash (x_hat): 0ea6b1e6dd539bb7

## Saved artifacts

- RunBundle: `runs/run_stem_exp_e7848a42/`
- Report: `pwm/reports/stem.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_stem_exp_e7848a42/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_stem_exp_e7848a42/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_stem_exp_e7848a42/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world STEM datasets at full resolution
