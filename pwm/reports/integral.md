...
| Field | Value |
|-------|-------|
| Modality ID | `integral` |
| Category | computational |
| Dataset | integral_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
Integral imaging pipeline with 1 element(s) in the forward chain. Reconstruction uses richardson_lucy_2d. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: integral_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `integral_benchmark`
...
SourceNode: photon_source — strength=1.0
  ↓
Element 1 (subrole=transport): conv2d — sigma=2.0, mode=reflect
  ↓
...
| 2 | blur | conv2d | transport | sigma=2.0, mode=reflect | gain | [0.5, 2.0] | uniform |
...
- **Prompt used:** `"Simulate integral measurement of a phantom and reconstruct"`
...
- **Mode I recon using corrected operator A':**
  ...
  |--------|-------------------|----------------|-------|
  | PSNR   | 23.05 dB | 26.02 dB | +2.97 dB |
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (23.05 dB)
- [x] W2 operator correction (NLL decrease): PASS (96.3%)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 23.05 dB | >= 1 dB | PASS |
| W1 SSIM | ssim | 0.9472 | — | info |
| W1 NRMSE | nrmse | 0.0762 | — | info |
| W2 NLL decrease | nll_decrease_pct | 96.3% | >= 0% | PASS |
| W2 PSNR delta | psnr_delta | +2.97 dB | — | info |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality integral --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): 9e4b05f729ab44eb
- Output hash (x_hat): f68b5930dcf278a3

## Saved artifacts

- RunBundle: `runs/run_integral_exp_96091347/`
- Report: `pwm/reports/integral.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_integral_exp_96091347/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_integral_exp_96091347/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_integral_exp_96091347/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world Integral datasets at full resolution
