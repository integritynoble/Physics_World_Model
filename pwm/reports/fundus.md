...
| Dataset | fundus_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
Fundus imaging pipeline with 1 element(s) in the forward chain. Reconstruction uses richardson_lucy_2d. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: fundus_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `fundus_benchmark`
...
SourceNode: photon_source — strength=1.0
  ↓
Element 1 (subrole=transport): conv2d — sigma=1.5, mode=reflect
  ↓
...
| 2 | retinal_psf | conv2d | transport | sigma=1.5, mode=reflect | gain | [0.5, 2.0] | uniform |
...
| 2 | retinal_psf | (64, 64) | float64 | [0.0000, 1.0000] | `artifacts/trace/02_retinal_psf.npy` |
...
- **Prompt used:** `"Simulate fundus measurement of a phantom and reconstruct"`
...
- **Mode I recon using corrected operator A':**
  ...
  |--------|-------------------|----------------|-------|
  | PSNR   | 23.66 dB | 30.19 dB | +6.53 dB |
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (23.66 dB)
- [x] W2 operator correction (NLL decrease): PASS (95.5%)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 23.66 dB | >= 1 dB | PASS |
| W1 SSIM | ssim | 0.9570 | — | info |
| W1 NRMSE | nrmse | 0.0711 | — | info |
| W2 NLL decrease | nll_decrease_pct | 95.5% | >= 0% | PASS |
| W2 PSNR delta | psnr_delta | +6.53 dB | — | info |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality fundus --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): 24ee943c73d4a975
- Output hash (x_hat): 0c8b908edeac8129

## Saved artifacts

- RunBundle: `runs/run_fundus_exp_1172ebf2/`
- Report: `pwm/reports/fundus.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_fundus_exp_1172ebf2/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_fundus_exp_1172ebf2/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_fundus_exp_1172ebf2/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world Fundus datasets at full resolution
