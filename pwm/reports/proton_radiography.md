# Proton Radiography — PWM Report

| Field | Value |
|-------|-------|
| Modality ID | `proton_radiography` |
| Category | particle |
| Dataset | proton_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
- Modality key: `proton_radiography`
- Category: particle
- Forward model: y = Sensor(MultipleScattering(ParticleAttenuation(GenericSource(x)))) + n
- Default solver: pseudo_inverse
- Pipeline linearity: nonlinear

Proton Radiography imaging pipeline with 2 element(s) in the forward chain. Reconstruction uses pseudo_inverse. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: proton_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `proton_benchmark`
...
SourceNode: generic_source — strength=1.0
  ↓
Element 1 (subrole=transduction): particle_attenuation — I_0=10000.0, cross_section=0.03
  ↓
Element 2 (subrole=transport): multiple_scattering — sigma=1.5
  ↓
...
| 2 | attenuation | particle_attenuation | transduction | I_0=10000.0, cross_section=0.03 | gain | [0.5, 2.0] | uniform |
| 3 | scatter | multiple_scattering | transport | sigma=1.5 | — | — | — |
| 4 | sensor | generic_sensor | sensor | gain=1.0 | gain | [0.5, 2.0] | uniform |
...
| 2 | attenuation | (64, 64) | float64 | [0.0000, 1.0000] | `artifacts/trace/02_attenuation.npy` |
| 3 | scatter | (64, 64) | float64 | [0.0000, 1.0000] | `artifacts/trace/03_scatter.npy` |
...
- **Prompt used:** `"Simulate proton_radiography measurement of a phantom and reconstruct"`
...
- **Operator definition:**
  ...
  - A_sha256: 609d34c8e4ffa726
  - Linearity: nonlinear
  - Notes (if linearized): N/A
- **Mismatch specification:**
  - Mismatch type: synthetic_injected
  - Parameters perturbed: I_0 drift (10000 -> 13000)
...
- **Mode C fit results:**
  ...
  - NLL before correction: 42064325334.0
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (1.86 dB)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 1.86 dB | >= 1 dB | PASS |
| W1 SSIM | ssim | -0.0094 | — | info |
| W1 NRMSE | nrmse | 0.8739 | — | info |
| W2 NLL decrease | nll_decrease_pct | 100.0% | >= 0% | PASS |
| W2 PSNR delta | psnr_delta | +4.24 dB | — | info |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality proton_radiography --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): 323d828e1001fdaa
- Output hash (x_hat): 83bc569fa9317010

## Saved artifacts

- RunBundle: `runs/run_proton_radiography_exp_e88e2a5d/`
- Report: `pwm/reports/proton_radiography.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_proton_radiography_exp_e88e2a5d/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_proton_radiography_exp_e88e2a5d/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_proton_radiography_exp_e88e2a5d/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world Proton Radiography datasets at full resolution
