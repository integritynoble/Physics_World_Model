# Diffusion MRI — PWM Report

| Field | Value |
|-------|-------|
| Modality ID | `diffusion_mri` |
| Category | mri_variant |
| Dataset | diffusion_mri_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
- Modality key: `diffusion_mri`
- Category: mri_variant
- Forward model: y = Sensor(MRI_kspace(SpinSource(x))) + n
- Default solver: adjoint_ifft
- Pipeline linearity: linear

Diffusion MRI imaging pipeline with 1 element(s) in the forward chain. Reconstruction uses adjoint_ifft. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: diffusion_mri_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `diffusion_mri_benchmark`
...
SourceNode: spin_source — strength=1.0
  ↓
Element 1 (subrole=encoding): mri_kspace — H=64, W=64, sampling_rate=0.3, seed=42
  ↓
SensorNode: coil_sensor — sensitivity=1.0
  ↓
NoiseNode: complex_gaussian_sensor — sigma=0.015, seed=0
  ↓
y
```
...
| 2 | kspace | mri_kspace | encoding | H=64, W=64, sampling_rate=0.3, seed=42 | gain | [0.5, 2.0] | uniform |
| 3 | sensor | coil_sensor | sensor | sensitivity=1.0 | gain | [0.5, 2.0] | uniform |
| 4 | noise | complex_gaussian_sensor | noise | sigma=0.015, seed=0 | — | — | — |
...
- **Prompt used:** `"Simulate diffusion_mri measurement of a phantom and reconstruct"`
...
- **Mode C fit results:**
  ...
  - NLL before correction: 336447955.7
...
- **Mode I recon using corrected operator A':**
  ...
  |--------|-------------------|----------------|-------|
  | PSNR   | 45.95 dB | 71.40 dB | +25.45 dB |
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (45.95 dB)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 45.95 dB | >= 1 dB | PASS |
| W1 SSIM | ssim | 0.9997 | — | info |
| W1 NRMSE | nrmse | 0.0055 | — | info |
| W2 NLL decrease | nll_decrease_pct | 100.0% | >= 0% | PASS |
| W2 PSNR delta | psnr_delta | +25.45 dB | — | info |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality diffusion_mri --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): f72e49556f6f2903
- Output hash (x_hat): 2dade9b57a117b46

## Saved artifacts

- RunBundle: `runs/run_diffusion_mri_exp_cbbefbe1/`
- Report: `pwm/reports/diffusion_mri.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_diffusion_mri_exp_cbbefbe1/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_diffusion_mri_exp_cbbefbe1/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_diffusion_mri_exp_cbbefbe1/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world Diffusion MRI datasets at full resolution
