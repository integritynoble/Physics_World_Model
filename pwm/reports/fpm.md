# Fourier Ptychographic Microscopy (FPM) — PWM Report

| Field | Value |
|-------|-------|
| Modality ID | `fpm` |
| Category | coherent |
| Dataset | FPM benchmark (synthetic proxy: complex transmission phantom) |
| Date | 2026-02-12 |
| PWM version | `a473f9a` |
...
- Modality key: `fpm`
- Category: coherent
- Forward model: y = |Angular_Spectrum(x)|² + noise, where Angular_Spectrum models angular-illumination-dependent propagation (λ=0.5 µm, z=1 mm, dx=1 µm), |·|² is intensity detection, and noise follows a Poisson-Gaussian model
- Default solver: backpropagation (sqrt → adjoint), nonlinear
- Pipeline linearity: nonlinear (magnitude_sq)

Fourier ptychographic microscopy (FPM) achieves super-resolution by illuminating the sample from multiple angles and capturing low-resolution intensity images. Each angular illumination shifts the sample's spectrum in Fourier space; iterative phase retrieval stitches these shifted spectra into a high-resolution complex image.
...
- Name: FPM benchmark (synthetic proxy: Gaussian blob phantom)
- Source: synthetic
- Size: 64x64 phantom
- Registered in `dataset_registry.yaml` as `fpm_benchmark`
...
- **Prompt used:** `"Simulate Fourier ptychographic microscopy and reconstruct using backpropagation"`
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality fpm --seed 42 --mode simulate,invert,calibrate
  ```
...
- RunBundle: `runs/run_fpm_exp_9ef7de4a/`
- Report: `pwm/reports/fpm.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`

## Next actions

- Implement multi-angle FPM with LED array scanning
- Add EPRY-FPM solver for improved convergence
- Extend to 3D FPM for thick specimen tomographic reconstruction
