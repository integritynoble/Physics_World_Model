# Muon Tomo — PWM Report

| Field | Value |
|-------|-------|
| Modality ID | `muon_tomo` |
| Category | particle |
| Dataset | muon_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
- Modality key: `muon_tomo`
- Category: particle
- Forward model: y = Sensor(MultipleScattering(GenericSource(x))) + n
- Default solver: pseudo_inverse
- Pipeline linearity: linear

Muon Tomo imaging pipeline with 1 element(s) in the forward chain. Reconstruction uses pseudo_inverse. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: muon_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `muon_benchmark`
...
SourceNode: generic_source — strength=1.0
  ↓
Element 1 (subrole=transport): multiple_scattering — sigma=2.0
  ↓
SensorNode: track_detector_sensor — n_layers=8, efficiency=0.95
  ↓
NoiseNode: poisson_gaussian_sensor — peak_photons=15000.0, read_sigma=0.01, seed=0
  ↓
y
```
...
| 2 | scatter | multiple_scattering | transport | sigma=2.0 | gain | [0.5, 2.0] | uniform |
| 3 | sensor | track_detector_sensor | sensor | n_layers=8, efficiency=0.95 | gain | [0.5, 2.0] | uniform |
| 4 | noise | poisson_gaussian_sensor | noise | peak_photons=15000.0, read_sigma=0.01, seed=0 | — | — | — |
...
| 2 | scatter | (64, 64) | float64 | [0.0000, 1.0000] | `artifacts/trace/02_scatter.npy` |
...
- **Prompt used:** `"Simulate muon_tomo measurement of a phantom and reconstruct"`
...
- **Mode C fit results:**
  ...
  - NLL before correction: 552067.7
...
- **Mode I recon using corrected operator A':**
  ...
  |--------|-------------------|----------------|-------|
  | PSNR   | 26.57 dB | 26.57 dB | +0.00 dB |
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (26.57 dB)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 26.57 dB | >= 1 dB | PASS |
| W1 SSIM | ssim | 0.9800 | — | info |
| W1 NRMSE | nrmse | 0.0508 | — | info |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality muon_tomo --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): 20b09dc77999617a
- Output hash (x_hat): afb9791fbe311b62

## Saved artifacts

- RunBundle: `runs/run_muon_tomo_exp_98f2adf1/`
- Report: `pwm/reports/muon_tomo.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_muon_tomo_exp_98f2adf1/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_muon_tomo_exp_98f2adf1/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_muon_tomo_exp_98f2adf1/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world Muon Tomo datasets at full resolution
