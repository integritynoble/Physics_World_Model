# Light Field — PWM Report

| Field | Value |
|-------|-------|
| Modality ID | `light_field` |
| Category | computational |
| Dataset | light_field_benchmark (synthetic proxy: phantom) |
| Date | 2026-02-12 |
| PWM version | `7394757` |
...
Light Field imaging pipeline with 1 element(s) in the forward chain. Reconstruction uses richardson_lucy_2d. Tested on a 64x64 synthetic phantom with seed=42.
...
- Name: light_field_benchmark (synthetic proxy: Gaussian-blob phantom)
- Source: synthetic
- Size: 1 image, 64x64
- Registered in `dataset_registry.yaml` as `light_field_benchmark`
...
| 2 | blur | conv2d | transport | sigma=1.5, mode=reflect | gain | [0.5, 2.0] | uniform |
...
- **Prompt used:** `"Simulate light_field measurement of a phantom and reconstruct"`
...
- **Mode I recon using corrected operator A':**
  ...
  |--------|-------------------|----------------|-------|
  | PSNR   | 23.76 dB | 26.76 dB | +3.00 dB |
...
- [x] W1 simulate: PASS
- [x] W1 reconstruct (PSNR reported): PASS (23.76 dB)
- [x] W2 operator correction (NLL decrease): PASS (96.4%)
...
|-------|--------|-------|-----------|--------|
| W1 PSNR | psnr | 23.76 dB | >= 1 dB | PASS |
| W1 SSIM | ssim | 0.9548 | — | info |
| W1 NRMSE | nrmse | 0.0702 | — | info |
| W2 NLL decrease | nll_decrease_pct | 96.4% | >= 0% | PASS |
| W2 PSNR delta | psnr_delta | +3.00 dB | — | info |
...
- Deterministic reproduction command:
  ```bash
  pwm_cli run --modality light_field --seed 42 --mode simulate,invert,calibrate
  ```
- Output hash (y): 515f01f261f04ec6
- Output hash (x_hat): b53ce6e576469300

## Saved artifacts

- RunBundle: `runs/run_light_field_exp_26370f0f/`
- Report: `pwm/reports/light_field.md`
- Scoreboard: `pwm/reports/scoreboard.yaml`
- Node trace (.npy): `runs/run_light_field_exp_26370f0f/artifacts/trace/*.npy`
- Node trace (.png): `runs/run_light_field_exp_26370f0f/artifacts/trace/png/*.png`
- W2 operator metadata: `runs/run_light_field_exp_26370f0f/artifacts/w2_operator_meta.json`

## Next actions

- Test on real-world Light Field datasets at full resolution
