#!/usr/bin/env python3
"""Two-Photon Microscopy — Phase B Experiment Script.

Two-photon: y = Sensor(Conv2D(NonlinearExcitation(x))) + noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_two_photon_experiment.py
"""
...
SEED = 42
MODALITY = "two_photon"
TEMPLATE_KEY = "two_photon_graph_v2"
H, W = 64, 64
PEAK_PHOTONS = 5000.0
READ_SIGMA = 0.02
PSF_SIGMA = 1.5
...
def build_operators(gain=1.0):
    ...
    excite = get_primitive("nonlinear_excitation", {"n_photons": 2})
    blur = get_primitive("conv2d", {"sigma": PSF_SIGMA, "mode": "reflect"})
    sens = get_primitive("photon_sensor", {"quantum_efficiency": 0.45, "gain": gain})
    def fwd(x): return sens.forward(blur.forward(excite.forward(src.forward(x))))
    return fwd
