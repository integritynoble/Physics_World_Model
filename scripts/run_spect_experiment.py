#!/usr/bin/env python3
"""SPECT (Single-Photon Emission Computed Tomography) — Phase B Experiment Script.

SPECT: y = Sensor(EmissionProjection(x)) + Poisson noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_spect_experiment.py
"""
...
def build_operators(gain=1.0):
    ...
    def fwd(x): return sens.forward(proj.forward(src.forward(x)))
...
def main():
    ...
    a_sha256 = hashlib.sha256(f"spect_gain".encode()).hexdigest()[:16]
