#!/usr/bin/env python3
"""Light-Sheet Fluorescence Microscopy — Phase B Experiment Script.

Lightsheet: y = Poisson(peak * QE * PSF ** x) + noise
Thin PSF (sigma=1.0) for sectioned illumination.

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_lightsheet_experiment.py
"""
...
SEED = 42
MODALITY = "lightsheet"
TEMPLATE_KEY = "lightsheet_graph_v2"
H, W = 64, 64
PSF_SIGMA = 1.0
...
def make_phantom(H, W, seed):
    ...
    x += 0.3 * np.exp(-((xx-0.3)**2+(yy+0.2)**2)/(2*0.12**2))
    for _ in range(5):
        cx, cy = rng.uniform(-0.6, 0.6, 2)
        x += rng.uniform(0.15, 0.4) * np.exp(-((xx-cx)**2+(yy-cy)**2)/(2*0.06**2))
