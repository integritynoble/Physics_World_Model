#!/usr/bin/env python3
"""Muon Tomography \u2014 Phase B Experiment Script.

Muon Tomo: y = TrackDetectorSensor(MultipleScattering(GenericSource(x))) + noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_muon_tomo_experiment.py
"""
...
SEED = 42
MODALITY = "muon_tomo"
TEMPLATE_KEY = "muon_tomo_graph_v2"
H, W = 64, 64
...
def build_operators(gain=1.0):
    ...
    scat = get_primitive("multiple_scattering", {"sigma": 2.0})
    sens = get_primitive("track_detector_sensor", {"n_layers": 8, "efficiency": 0.95})
    def base_fwd(x): return sens.forward(scat.forward(src.forward(x)))
    def base_adj(y): return src.adjoint(scat.adjoint(sens.adjoint(y)))
    # Wrap with gain multiplier for W2 mismatch
    def fwd(x): return gain * base_fwd(x)
    def adj(y):
        y_scaled = y / gain if gain != 0 else y
        out = base_adj(y_scaled)
        out = np.clip(np.real(out) if np.iscomplexobj(out) else out, 0, None)
        if out.max() > 0:
            out = out / out.max()
        return out
    return fwd, adj
...
def main():
    ...
    save_operator_meta(rb_dir, {"a_definition": "callable", "a_extraction_method": "graph_stripped",
        "a_sha256": a_sha256, "linearity": "linear", "linearization_notes": "Scattering + track detection",
