#!/usr/bin/env python3
"""X-ray Radiography — Phase B Experiment Script.

X-ray: y = Sensor(Scatter(BeerLambert(x))) + noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_xray_radiography_experiment.py
"""
...
TEMPLATE_KEY = "xray_radiography_graph_v2"
H, W = 64, 64
...
def build_operators(gain=1.0):
    ...
    sens = get_primitive("xray_detector_sensor", {"scintillator_efficiency": 0.8, "gain": gain})
    def fwd(x): return sens.forward(scat.forward(bl.forward(src.forward(x))))
    def adj(y):
        # Pseudo-inverse: undo sensor scaling, ignore scatter, invert Beer-Lambert
        y_unscaled = y / (0.8 * gain + 1e-12)
        return -np.log(np.clip(y_unscaled / 10000.0, 1e-8, None))
    return fwd, adj
...
def main():
    ...
    x_hat_w1 = adj_fn(np.maximum(y, 1e-8))
    ...
    a_sha256 = hashlib.sha256(f"xray_radiography_gain".encode()).hexdigest()[:16]
    ...
    x_hat_uncorrected = adj_uncorr(np.maximum(y_measured, 1e-8))
    ...
    x_hat_corrected = adj_corr(np.maximum(y_measured, 1e-8))
    ...
    save_operator_meta(rb_dir, {"a_definition": "callable", "a_extraction_method": "graph_stripped",
        "a_sha256": a_sha256, "linearity": "nonlinear", "linearization_notes": "beer_lambert is exp(-mu*x)",
