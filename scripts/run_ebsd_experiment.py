#!/usr/bin/env python3
"""EBSD (Electron Backscatter Diffraction) — Phase B Experiment Script.

EBSD: y = EnergyResolvingDetector(ReciprocalSpaceGeometry(GenericSource(x))) + Poisson-Gaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_ebsd_experiment.py
"""
...
def build_operators(gain=1.0):
    """Build forward chain with gain multiplier wrapping the base forward."""
    src = get_primitive("generic_source", {"strength": 1.0})
    rsg = get_primitive("reciprocal_space_geometry", {"sample_tilt_deg": 70.0, "detector_distance": 15.0})
    erd = get_primitive("energy_resolving_detector", {"n_channels": 256, "energy_range_ev": [0, 2000], "qe": 0.8})
    def fwd(x):
        return gain * erd.forward(rsg.forward(src.forward(x)))
    def adj(y):
        # reciprocal_space_geometry is non-linear (affine_transform), pseudo-inverse via clip+normalize
        return src.adjoint(erd.adjoint(y)) * gain
    return fwd, adj
...
def main():
    ...
    # Pseudo-inverse recon: clip + normalize
    ...
    a_sha256 = hashlib.sha256(f"ebsd_gain".encode()).hexdigest()[:16]

    rng2 = np.random.RandomState(SEED + 10)
    fwd_pert, _ = build_operators(gain=1.3)
    y_clean_pert = fwd_pert(x_true)
    scaled_pert = np.maximum(np.abs(y_clean_pert) * PEAK_PHOTONS, 0.0)
    y_measured = rng2.poisson(scaled_pert).astype(np.float64) / PEAK_PHOTONS + rng2.normal(0, READ_SIGMA, size=y_clean_pert.shape)
    ...
    # Uncorrected pseudo-inverse
    x_hat_uncorrected = np.clip(y_measured, 0, None)
    ...
    # Corrected: rescale measurement by nominal/best gain ratio
    x_hat_corrected = np.clip(y_measured / max(best_gain, 1e-12), 0, None)
    ...
    save_operator_meta(rb_dir, {"a_definition": "callable", "a_extraction_method": "graph_stripped",
        "a_sha256": a_sha256, "linearity": "nonlinear", "linearization_notes": "reciprocal_space_geometry uses affine_transform",
