#!/usr/bin/env python3
"""CBCT (Cone-Beam Computed Tomography) — Phase B Experiment Script.

CBCT: y = PhotonSensor(BeerLambert(Radon(XRaySource(x)))) + Poisson noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_cbct_experiment.py
"""
...
def compile_graph(key):
    ...
    tpl["metadata"]["x_shape"] = [H, W]; tpl["metadata"]["y_shape"] = [N_ANGLES, H]
...
def build_operators(gain=1.0):
    ...
    radon = get_primitive("ct_radon", {"n_angles": N_ANGLES, "H": H, "W": W})
    bl = get_primitive("beer_lambert", {"I_0": 8000.0})
    sens = get_primitive("photon_sensor", {"quantum_efficiency": 0.85, "gain": gain})
    def fwd(x): return sens.forward(bl.forward(radon.forward(src.forward(x))))
    def adj(y): return src.adjoint(radon.adjoint(bl.adjoint(sens.adjoint(y))))
    return fwd, adj
...
def main():
    ...
    # Adjoint backprojection reconstruction
    radon_prim = get_primitive("ct_radon", {"n_angles": N_ANGLES, "H": H, "W": W})
    sens_prim = get_primitive("photon_sensor", {"quantum_efficiency": 0.85, "gain": 1.0})
    x_hat_w1 = radon_prim.adjoint(sens_prim.adjoint(y))
    ...
    a_sha256 = hashlib.sha256(f"cbct_gain".encode()).hexdigest()[:16]
