#!/usr/bin/env python3
"""Ultrasound — Phase B Experiment Script.

Ultrasound: y = Receive(Beamform(AcousticProp(x))) + noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_ultrasound_experiment.py
"""
...
SEED = 42
MODALITY = "ultrasound"
TEMPLATE_KEY = "ultrasound_graph_v2"
H, W = 64, 64
NOISE_SIGMA = 0.01
...
def compile_graph(key):
    ...
    tpl["metadata"]["x_shape"] = [H, W]; tpl["metadata"]["y_shape"] = [H]
...
def build_operators(sensitivity=1.0):
    ...
    def fwd(x): return sens.forward(bf.forward(prop.forward(src.forward(x))))
    def adj(y):
        try:
            return src.adjoint(prop.adjoint(bf.adjoint(sens.adjoint(y))))
        except Exception:
...
def main():
    ...
    a_sha256 = hashlib.sha256(f"ultrasound_sens".encode()).hexdigest()[:16]
