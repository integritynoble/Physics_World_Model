#!/usr/bin/env python3
"""Angiography — Phase B Experiment Script.

Angiography: y = XRayDetector(FluoroTemporalIntegrator(BeerLambert(XRaySource(x)))) + Poisson+Gaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_angiography_experiment.py
"""
...
TEMPLATE_KEY = "angiography_graph_v2"
H, W = 64, 64
...
def build_operators(gain=1.0):
    ...
    bl = get_primitive("beer_lambert", {"I_0": 6000.0})
    fti = get_primitive("fluoro_temporal_integrator", {"n_frames": 4, "decay": 0.85})
    sens = get_primitive("xray_detector_sensor", {"scintillator_efficiency": 0.75, "gain": gain})
    def fwd(x): return sens.forward(fti.forward(bl.forward(src.forward(x))))
    def adj(y):
        r = src.adjoint(bl.adjoint(fti.adjoint(sens.adjoint(y))))
        # fti.adjoint returns (n_frames, H, W); sum over temporal axis
        if r.ndim == 3:
            r = r.sum(axis=0)
        return r
    return fwd, adj
...
def main():
    ...
    x_hat_w1 = -np.log(np.clip(y / (0.75 * 6000.0), 1e-8, None))
    ...
    a_sha256 = hashlib.sha256(f"angiography_gain".encode()).hexdigest()[:16]
