#!/usr/bin/env python3
"""Elastography -- Phase B Experiment Script.

Elastography: y = AcousticReceive(AcousticPropagation(ElasticWaveModel(AcousticSource(x)))) + Gaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_elastography_experiment.py
"""
...
TEMPLATE_KEY = "elastography_graph_v2"
H, W = 64, 64
NOISE_SIGMA = 0.015
...
def build_operators(sensitivity=1.0):
    ...
    ewm = get_primitive("elastic_wave_model", {"shear_speed_m_per_s": 3.0, "push_duration_us": 100.0})
    ...
    def fwd(x): return sens.forward(prop.forward(ewm.forward(src.forward(x))))
    def adj(y): return src.adjoint(ewm.adjoint(prop.adjoint(sens.adjoint(y))))
    return fwd, adj
...
def main():
    ...
    a_sha256 = hashlib.sha256(f"elastography_sensitivity".encode()).hexdigest()[:16]
