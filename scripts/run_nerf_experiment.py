#!/usr/bin/env python3
"""NeRF (Neural Radiance Field) — Phase B Experiment Script.

NeRF: y = Sensor(VolumeRendering(Source(x))) + Gaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_nerf_experiment.py
"""
...
def build_operators(gain=1.0):
    ...
    vol = get_primitive("volume_rendering_stub", {"n_views": 1, "render_mode": "mip"})
    sens = get_primitive("generic_sensor", {"gain": gain})
    def fwd(x): return sens.forward(vol.forward(src.forward(x)))
    def pseudo_inv(y):
...
def main():
    ...
    save_operator_meta(rb_dir, {"a_definition": "callable", "a_extraction_method": "graph_stripped",
        "a_sha256": a_sha256, "linearity": "nonlinear", "linearization_notes": "MIP projection",
