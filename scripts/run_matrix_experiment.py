#!/usr/bin/env python3
"""Matrix (Generic Random Measurement) -- Phase B Experiment Script.

Matrix: y = GenericSensor(RandomMask(GenericSource(x))) + PoissonGaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_matrix_experiment.py
"""
...
def compile_graph(key):
    ...
    tpl["metadata"]["x_shape"] = [H, W]; tpl["metadata"]["y_shape"] = [1024]
...
def build_operators(gain=1.0):
    ...
    rmask = get_primitive("random_mask", {"seed": 42, "H": H, "W": W, "sampling_rate": 0.25})
    sens = get_primitive("generic_sensor", {"gain": gain})
    def fwd(x): return sens.forward(rmask.forward(src.forward(x)))
    def adj(y): return src.adjoint(rmask.adjoint(sens.adjoint(y)))
    return fwd, adj
...
def main():
    ...
    # Adjoint maps from compressed measurements back to image space
    ...
    a_sha256 = hashlib.sha256(f"matrix_gain".encode()).hexdigest()[:16]
    ...
    if x_hat_uncorrected.ndim == 1:
        x_hat_uncorrected = x_hat_uncorrected.reshape(H, W)
    ...
    if x_hat_corrected.ndim == 1:
        x_hat_corrected = x_hat_corrected.reshape(H, W)
