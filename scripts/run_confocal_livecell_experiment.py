#!/usr/bin/env python3
"""Confocal Live-Cell Microscopy — Phase B Experiment Script.

Confocal with tight PSF (sigma=1.2), moderate photon budget (5000).
y = Poisson(peak * QE * PSF ** x) + read_noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_confocal_livecell_experiment.py
"""
...
TEMPLATE_KEY = "confocal_livecell_graph_v2"
H, W = 64, 64
PSF_SIGMA = 1.2
PEAK_PHOTONS = 5000.0
READ_SIGMA = 0.02
...
def make_phantom(H, W, seed):
    ...
    x += 0.05
    # Cell bodies
    x += 0.5 * np.exp(-((xx-0.15)**2+(yy+0.1)**2)/(2*0.25**2))
    x += 0.4 * np.exp(-((xx+0.3)**2+(yy-0.2)**2)/(2*0.2**2))
    # Nuclei
    x += 0.3 * np.exp(-((xx-0.1)**2+(yy+0.05)**2)/(2*0.08**2))
    # Puncta
    for _ in range(10):
        cx, cy = rng.uniform(-0.7, 0.7, 2)
        x += rng.uniform(0.2, 0.6) * np.exp(-((xx-cx)**2+(yy-cy)**2)/(2*0.025**2))
...
def main():
    ...
    x_hat_uncorrected = richardson_lucy_2d(y_measured, make_gaussian_psf(PSF_SIGMA), iterations=RL_ITERS, clip=True).astype(np.float64)
    ...
    best_nll, best_sigma = np.inf, PSF_SIGMA
    for ts in np.arange(0.5, 3.05, 0.1):
        fwd_t, _ = build_operators(ts)
        nll_t = compute_nll_gaussian(y_measured, fwd_t(x_true), noise_sigma_eff)
        if nll_t < best_nll: best_nll, best_sigma = nll_t, ts

    nll_after = best_nll
    nll_decrease_pct = (nll_before - nll_after) / (nll_before + 1e-12) * 100

    x_hat_corrected = richardson_lucy_2d(y_measured, make_gaussian_psf(best_sigma), iterations=RL_ITERS, clip=True).astype(np.float64)
    ...
    return {
        "w1": {"psnr": round(w1_psnr,2), "ssim": round(w1_ssim,4), "nrmse": round(w1_nrmse,4),
        ...
        "w2": {"a_sha256": a_sha256, "nll_before": round(nll_before,1), "nll_after": round(nll_after,1),
               ...
               "ssim_uncorrected": round(ssim_uncorrected,4), "psnr_corrected": round(psnr_corrected,2),
               "ssim_corrected": round(ssim_corrected,4), "psnr_delta": round(psnr_delta,2),
               "ssim_delta": round(ssim_delta,4), "fitted_sigma": round(best_sigma,1)},
