#!/usr/bin/env python3
"""EELS (Electron Energy Loss Spectroscopy) — Phase B Experiment Script.

EELS: y = GenericSensor(EnergyResolvingDetector(GenericSource(x))) + Poisson-Gaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_eels_experiment.py
"""
...
def build_operators(gain=1.0):
    ...
    erd = get_primitive("energy_resolving_detector", {"n_channels": 1024, "energy_range_ev": [0, 3000], "qe": 0.7})
    sens = get_primitive("generic_sensor", {"gain": gain})
    def fwd(x): return sens.forward(erd.forward(src.forward(x)))
    def adj(y): return src.adjoint(erd.adjoint(sens.adjoint(y)))
    return fwd, adj
...
def main():
    ...
    a_sha256 = hashlib.sha256(f"eels_gain".encode()).hexdigest()[:16]
