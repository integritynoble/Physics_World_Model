#!/usr/bin/env python3
"""Photoacoustic — Phase B Experiment Script.

Photoacoustic: y = Transducer(AcousticProp(OpticalAbsorption(x))) + noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_photoacoustic_experiment.py
"""
...
TEMPLATE_KEY = "photoacoustic_graph_v2"
H, W = 64, 64
NOISE_SIGMA = 0.01
...
def build_operators(sensitivity=1.0, grueneisen=0.8):
    src = get_primitive("photon_source", {"strength": 1.0})
    absorb = get_primitive("optical_absorption", {"grueneisen": grueneisen, "mu_a": 1.0})
    prop = get_primitive("acoustic_propagation", {"speed_of_sound": 1500.0, "n_sensors": 64, "x_shape": [H, W]})
    sens = get_primitive("transducer_sensor", {"sensitivity": sensitivity})
    def fwd(x): return sens.forward(prop.forward(absorb.forward(src.forward(x))))
    def adj(y):
        try:
            return src.adjoint(absorb.adjoint(prop.adjoint(sens.adjoint(y))))
        except Exception:
...
def main():
    ...
    a_sha256 = hashlib.sha256(f"photoacoustic_sens".encode()).hexdigest()[:16]
    ...
    noise_sigma_eff = max(float(np.std(y_measured - y_clean_pert)), NOISE_SIGMA)
