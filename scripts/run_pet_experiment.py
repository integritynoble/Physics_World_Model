#!/usr/bin/env python3
"""PET (Positron Emission Tomography) — Phase B Experiment Script.

PET: y = Sensor(Scatter(EmissionProjection(x))) + Poisson noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_pet_experiment.py
"""
...
def compile_graph(key):
    ...
    tpl["metadata"]["x_shape"] = [H, W]; tpl["metadata"]["y_shape"] = [32, H]
...
def build_operators(gain=1.0):
    ...
    proj = get_primitive("emission_projection", {"n_angles": 32, "x_shape": [H, W]})
    scat = get_primitive("scatter_model", {"scatter_fraction": 0.15, "kernel_sigma": 3.0})
    sens = get_primitive("photon_sensor", {"quantum_efficiency": 0.85, "gain": gain})
    def fwd(x): return sens.forward(scat.forward(proj.forward(src.forward(x))))
    def adj(y):
        try:
            return src.adjoint(proj.adjoint(scat.adjoint(sens.adjoint(y))))
        except Exception:
            return src.adjoint(proj.adjoint(sens.adjoint(y)))
    return fwd, adj
...
def main():
    ...
    a_sha256 = hashlib.sha256(f"pet_gain".encode()).hexdigest()[:16]
