#!/usr/bin/env python3
"""Neutron Tomography \u2014 Phase B Experiment Script.

Neutron Tomo: y = GenericSensor(ParticleAttenuation(GenericSource(x))) + noise
ParticleAttenuation: y = I_0 * exp(-cross_section * x)

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_neutron_tomo_experiment.py
"""
...
TEMPLATE_KEY = "neutron_tomo_graph_v2"
H, W = 64, 64
...
def build_operators(gain=1.0):
    ...
    att = get_primitive("particle_attenuation", {"I_0": I_0, "cross_section": CROSS_SECTION})
    sens = get_primitive("generic_sensor", {"gain": gain})
    def fwd(x): return sens.forward(att.forward(src.forward(x)))
    def inv(y):
        # Beer-Lambert inversion: x = -log(y / (gain * I_0)) / cross_section
        y_safe = np.clip(np.real(y) if np.iscomplexobj(y) else y, 1e-8, None)
        x_hat = -np.log(y_safe / (gain * I_0)) / CROSS_SECTION
...
def main():
    ...
    _, inv_fn = build_operators(gain=1.0)
    x_hat_w1 = inv_fn(y)
    if x_hat_w1.max() > 0:
    ...
    _, inv_uncorr = build_operators(gain=1.0)
    x_hat_uncorrected = inv_uncorr(y_measured)
    psnr_uncorrected = psnr_fn(x_hat_uncorrected.astype(np.float64), x_true, max_val=float(x_true.max()))

    _, inv_corr = build_operators(gain=best_gain)
    x_hat_corrected = inv_corr(y_measured)
    ...
    save_operator_meta(rb_dir, {"a_definition": "callable", "a_extraction_method": "graph_stripped",
        "a_sha256": a_sha256, "linearity": "nonlinear", "linearization_notes": "Beer-Lambert exponential",
