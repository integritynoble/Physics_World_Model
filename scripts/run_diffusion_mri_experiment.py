#!/usr/bin/env python3
"""Diffusion MRI -- Phase B Experiment Script.

Diffusion MRI: y = CoilSensor(MRIkSpace(SpinSource(x))) + ComplexGaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_diffusion_mri_experiment.py
"""
...
TEMPLATE_KEY = "diffusion_mri_graph_v2"
H, W = 64, 64
NOISE_SIGMA = 0.015
...
def build_operators(sensitivity=1.0):
    ...
    ksp = get_primitive("mri_kspace", {"H": H, "W": W, "sampling_rate": 0.30, "seed": 42})
    coil = get_primitive("coil_sensor", {"sensitivity": sensitivity})
    def fwd(x): return coil.forward(ksp.forward(src.forward(x)))
    def adj(y): return src.adjoint(ksp.adjoint(coil.adjoint(y)))
    return fwd, adj
...
def main():
    ...
    a_sha256 = hashlib.sha256(f"diffusion_mri_sensitivity".encode()).hexdigest()[:16]
