#!/usr/bin/env python3
"""fMRI (functional Magnetic Resonance Imaging) -- Phase B Experiment Script.

fMRI: y = CoilSensor(SequenceBlock(MRIkSpace(SpinSource(x)))) + ComplexGaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_fmri_experiment.py
"""
...
def build_operators(sensitivity=1.0):
    src = get_primitive("spin_source", {"strength": 1.0})
    ksp = get_primitive("mri_kspace", {"H": H, "W": W, "sampling_rate": 0.50, "seed": 42})
    seq = get_primitive("sequence_block", {"TR_ms": 2000.0, "TE_ms": 30.0, "flip_angle_deg": 90.0})
    coil = get_primitive("coil_sensor", {"sensitivity": sensitivity})
    def fwd(x): return coil.forward(seq.forward(ksp.forward(src.forward(x))))
    def adj(y): return src.adjoint(ksp.adjoint(seq.adjoint(coil.adjoint(y))))
    return fwd, adj
...
def main():
    ...
    a_sha256 = hashlib.sha256(f"fmri_sensitivity".encode()).hexdigest()[:16]
