#!/usr/bin/env python3
"""Fluorescence Lifetime Imaging (FLIM) — Phase B Experiment Script.

FLIM: y = Poisson(peak * QE * gate * PSF ** x) + noise
Temporal gating with PSF blur and low photon count.

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_flim_experiment.py
"""
...
def build_operators(sigma, gain=1.0):
    ...
    gate = get_primitive("temporal_mask", {"H": H, "W": W, "T": 1, "seed": SEED})
    sens = get_primitive("photon_sensor", {"quantum_efficiency": 0.9, "gain": gain})
    def fwd(x): return sens.forward(gate.forward(blur.forward(src.forward(x))))
    def adj(y): return src.adjoint(blur.adjoint(gate.adjoint(sens.adjoint(y))))
    return fwd, adj
...
def main():
    ...
    # W2 - gain mismatch (1.0 → 1.4) — gain mismatch works better for low-photon FLIM
    perturbed_gain = 1.4
    ...
    # Reconstruct with corrected gain
    fwd_corr, adj_corr = build_operators(PSF_SIGMA, gain=best_gain)
    # Scale measurement by gain correction factor, then RL with PSF
