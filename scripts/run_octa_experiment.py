#!/usr/bin/env python3
"""OCTA (OCT Angiography) — Phase B Experiment Script.

OCTA: y = PhotonSensor(VesselFlowContrast(AngularSpectrum(PhotonSource(x)))) + Poisson+Gaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_octa_experiment.py
"""
...
def build_operators(gain=1.0):
    ...
    prop = get_primitive("angular_spectrum", {"wavelength": 0.84e-6, "distance": 1.0e-3, "pixel_size": 5.0e-6})
    vfc = get_primitive("vessel_flow_contrast", {"n_frames": 4, "threshold": 0.1})
    sens = get_primitive("photon_sensor", {"quantum_efficiency": 0.85, "gain": gain})
    def fwd(x): return sens.forward(vfc.forward(prop.forward(src.forward(x))))
    def adj(y):
        # Use angular_spectrum adjoint (backpropagation), skip vessel_flow_contrast (no adjoint)
        y_scaled = sens.adjoint(y)
        x_hat = prop.adjoint(y_scaled)
        return np.abs(x_hat) if np.iscomplexobj(x_hat) else x_hat
    return fwd, adj
...
def main():
    ...
    # Adjoint backpropagation reconstruction
    ...
    a_sha256 = hashlib.sha256(f"octa_gain".encode()).hexdigest()[:16]
