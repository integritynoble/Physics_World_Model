#!/usr/bin/env python3
"""Quick test of the 6 new MRI solvers on 1 scene."""
import sys, os
...
NEW_SOLVERS = ["pnp_dncnn", "score_mri", "cascade_net", "kt_sparse_sense", "smash", "kiki_net"]
...
def main():
    ...
    for key in NEW_SOLVERS:
        ...
        try:
            ...
            elapsed = time.time() - t0
            print(f"  [FAIL] {key:20s} {spec['name']:25s} Error: {str(e)[:80]}  ({elapsed:.1f}s)")
