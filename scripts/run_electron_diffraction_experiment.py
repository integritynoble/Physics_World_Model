#!/usr/bin/env python3
"""Electron Diffraction (4D-STEM) — Phase B Experiment Script.

Electron Diffraction: y = EnergyResolvingDetector(DiffractionCamera(GenericSource(x))) + Poisson-Gaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_electron_diffraction_experiment.py
"""
...
TEMPLATE_KEY = "electron_diffraction_graph_v2"
H, W = 64, 64
...
def build_operators(gain=1.0):
    ...
    diff = get_primitive("diffraction_camera", {"detector_psf_sigma": 0.5})
    erd = get_primitive("energy_resolving_detector", {"n_channels": 256, "energy_range_ev": [0, 2000], "qe": 0.8})
    def fwd(x):
        return gain * erd.forward(diff.forward(src.forward(x)))
    def adj(y):
        # diffraction_camera is non-linear (|FFT|^2), so pseudo-inverse via clip+normalize
...
def main():
    ...
    a_sha256 = hashlib.sha256(f"electron_diffraction_gain".encode()).hexdigest()[:16]
    ...
    save_operator_meta(rb_dir, {"a_definition": "callable", "a_extraction_method": "graph_stripped",
        "a_sha256": a_sha256, "linearity": "nonlinear", "linearization_notes": "diffraction_camera is |FFT|^2",
