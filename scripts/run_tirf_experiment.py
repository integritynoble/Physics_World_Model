#!/usr/bin/env python3
"""TIRF Microscopy — Phase B Experiment Script.

TIRF: y = Sensor(Conv2D(EvanescentDecay(x))) + noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_tirf_experiment.py
"""
...
def build_operators(gain=1.0):
    ...
    decay = get_primitive("evanescent_decay", {"penetration_depth": 100.0})
    blur = get_primitive("conv2d", {"sigma": PSF_SIGMA, "mode": "reflect"})
    sens = get_primitive("photon_sensor", {"quantum_efficiency": 0.90, "gain": gain})
    def fwd(x): return sens.forward(blur.forward(decay.forward(src.forward(x))))
    return fwd
