#!/usr/bin/env python3
"""LiDAR \u2014 Phase B Experiment Script.

LiDAR: y = SPADToFSensor(TimeOfFlightGate(ScanTrajectory(PhotonSource(x)))) + noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_lidar_experiment.py
"""
...
def build_operators(gain=1.0):
    ...
    scan = get_primitive("scan_trajectory", {"scan_type": "raster", "n_points": 64, "dwell_time": 1.0})
    tof = get_primitive("tof_gate", {"n_bins": 64, "bin_width_ns": 0.5, "timing_jitter_ns": 0.05})
    sens = get_primitive("spad_tof_sensor", {"n_bins": 64, "dead_time_ns": 20.0, "qe": 0.25})
    def base_fwd(x): return sens.forward(tof.forward(scan.forward(src.forward(x))))
    def base_adj(y): return src.adjoint(scan.adjoint(tof.adjoint(sens.adjoint(y))))
    # Wrap with gain multiplier for W2 mismatch
    def fwd(x): return gain * base_fwd(x)
    def adj(y): return base_adj(y / gain) if gain != 0 else base_adj(y)
    return fwd, adj
...
def main():
    ...
    x_hat_w1 = adj_fn(y)
    x_hat_w1 = np.clip(np.real(x_hat_w1) if np.iscomplexobj(x_hat_w1) else x_hat_w1, 0, None)
    if x_hat_w1.max() > 0:
        x_hat_w1 = x_hat_w1 / x_hat_w1.max() * x_true.max()
    t_w1 = time.time() - t0

    # LiDAR scan produces 1D output; reshape for PSNR/SSIM against 2D phantom
    if x_hat_w1.shape != x_true.shape:
        x_hat_2d = np.zeros_like(x_true)
        flat_hat = x_hat_w1.ravel()
        flat_2d = x_hat_2d.ravel()
        n = min(len(flat_hat), len(flat_2d))
        flat_2d[:n] = flat_hat[:n]
        x_hat_w1 = flat_2d.reshape(x_true.shape)
    ...
    if x_hat_uncorrected.shape != x_true.shape:
        tmp = np.zeros_like(x_true)
        flat = tmp.ravel(); src_flat = x_hat_uncorrected.ravel()
        n = min(len(flat), len(src_flat)); flat[:n] = src_flat[:n]
        x_hat_uncorrected = flat.reshape(x_true.shape)
    ...
    if x_hat_corrected.shape != x_true.shape:
        tmp = np.zeros_like(x_true)
        flat = tmp.ravel(); src_flat = x_hat_corrected.ravel()
        n = min(len(flat), len(src_flat)); flat[:n] = src_flat[:n]
        x_hat_corrected = flat.reshape(x_true.shape)
