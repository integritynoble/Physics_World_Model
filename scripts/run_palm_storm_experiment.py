#!/usr/bin/env python3
"""PALM/STORM Localization Microscopy — Phase B Experiment Script.

PALM/STORM: y = Sensor(BlinkingEmitter(x)) + noise — no conv2d, pseudo-inverse recon.

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_palm_storm_experiment.py
"""
...
SEED = 42
MODALITY = "palm_storm"
TEMPLATE_KEY = "palm_storm_graph_v2"
H, W = 64, 64
PEAK_PHOTONS = 1000.0
READ_SIGMA = 0.05
...
def build_operators(gain=1.0):
    ...
    blink = get_primitive("blinking_emitter", {"density": 0.1, "photons_per_emitter": 1000, "seed": 42})
    sens = get_primitive("photon_sensor", {"quantum_efficiency": 0.90, "gain": gain})
    def fwd(x): return sens.forward(blink.forward(src.forward(x)))
    return fwd
...
def main():
    ...
    # Pseudo-inverse recon (no conv2d in this pipeline)
    x_hat_w1 = np.clip(y, 0, None)
    xmax = x_hat_w1.max()
    if xmax > 0:
        x_hat_w1 = x_hat_w1 / xmax
    t_w1 = time.time() - t0
    ...
    xmax_uc = x_hat_uncorrected.max()
    if xmax_uc > 0:
        x_hat_uncorrected = x_hat_uncorrected / xmax_uc
    ...
    x_hat_corrected = np.clip(y_measured / best_gain, 0, None)
    xmax_c = x_hat_corrected.max()
    if xmax_c > 0:
        x_hat_corrected = x_hat_corrected / xmax_c
