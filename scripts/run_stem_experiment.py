#!/usr/bin/env python3
"""STEM (Scanning Transmission Electron Microscopy) — Phase B Experiment Script.

STEM: y = ElectronDetector(ThinObjectPhase(ElectronBeamSource(x))) + Gaussian noise
Uses tem_graph_v2 as base template with overridden graph_id and metadata.modality.

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_stem_experiment.py
"""
...
SEED = 42
MODALITY = "stem"
TEMPLATE_KEY = "tem_graph_v2"  # Use TEM template as base
H, W = 64, 64
NOISE_SIGMA = 0.015
...
def compile_graph(key):
    """Use tem_graph_v2 template but override graph_id and metadata.modality for STEM."""
    tpl = dict(load_template(key)); tpl.pop("description", None)
    tpl["metadata"]["x_shape"] = [H, W]; tpl["metadata"]["y_shape"] = [H, W]
    tpl["metadata"]["modality"] = "stem"  # Override modality
    return GraphCompiler().compile(OperatorGraphSpec.model_validate({"graph_id": "stem_graph_v2", **tpl}))

def build_operators(gain=1.0):
    src = get_primitive("electron_beam_source", {"accelerating_voltage_kv": 200.0, "beam_current_na": 0.5})
    top = get_primitive("thin_object_phase", {"sigma": 0.00729, "thickness_nm": 20.0})
    sens = get_primitive("electron_detector_sensor", {"detector_type": "BF", "collection_efficiency": 0.7, "gain": gain})
    def fwd(x): return sens.forward(top.forward(src.forward(x)))
    def adj(y): return src.adjoint(top.adjoint(sens.adjoint(y)))
    return fwd, adj
...
def main():
    ...
    a_sha256 = hashlib.sha256(f"stem_gain".encode()).hexdigest()[:16]
