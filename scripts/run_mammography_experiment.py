#!/usr/bin/env python3
"""Mammography — Phase B Experiment Script.

Mammography: y = PhotonSensor(BeerLambert(XRaySource(x))) + Poisson+Gaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_mammography_experiment.py
"""
...
TEMPLATE_KEY = "mammography_graph_v2"
H, W = 64, 64
...
def build_operators(gain=1.0):
    ...
    def fwd(x): return sens.forward(bl.forward(src.forward(x)))
    def adj(y): return src.adjoint(bl.adjoint(sens.adjoint(y)))
    return fwd, adj
...
def main():
    ...
    x_hat_w1 = -np.log(np.clip(y / (0.85 * 8000.0), 1e-8, None))
    ...
    a_sha256 = hashlib.sha256(f"mammography_gain".encode()).hexdigest()[:16]
