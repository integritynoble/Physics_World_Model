#!/usr/bin/env python3
"""Electron Tomography — Phase B Experiment Script.

Electron Tomography: y = ElectronDetector(ThinObjectPhase(ElectronBeamSource(x))) + Gaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_electron_tomography_experiment.py
"""
...
TEMPLATE_KEY = "electron_tomography_graph_v2"
H, W = 64, 64
NOISE_SIGMA = 0.02
...
def build_operators(gain=1.0):
    src = get_primitive("electron_beam_source", {"accelerating_voltage_kv": 300.0, "beam_current_na": 0.1})
    top = get_primitive("thin_object_phase", {"sigma": 0.00729, "thickness_nm": 50.0})
...
def main():
    ...
    a_sha256 = hashlib.sha256(f"electron_tomography_gain".encode()).hexdigest()[:16]
