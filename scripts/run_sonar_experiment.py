#!/usr/bin/env python3
"""Sonar -- Phase B Experiment Script.

Sonar: y = TransducerSensor(BeamformDelay(AcousticSource(x))) + PoissonGaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_sonar_experiment.py
"""
...
def compile_graph(key):
    ...
    tpl["metadata"]["x_shape"] = [H, W]; tpl["metadata"]["y_shape"] = [64]
    return GraphCompiler().compile(OperatorGraphSpec.model_validate({"graph_id": key, **tpl}))

def build_operators(sensitivity=1.0):
    src = get_primitive("acoustic_source", {"strength": 1.0})
    bfd = get_primitive("beamform_delay", {"n_elements": 32})
    sens = get_primitive("transducer_sensor", {"sensitivity": sensitivity})
    def fwd(x): return sens.forward(bfd.forward(src.forward(x)))
    def adj(y):
        out = src.adjoint(bfd.adjoint(sens.adjoint(y)))
        out = np.real(out) if np.iscomplexobj(out) else out
        # Resize to (H, W) if shape doesn't match
        if out.shape != (H, W):
            import scipy.ndimage
            out2d = out.reshape(out.shape[0], -1) if out.ndim == 2 else out.reshape(-1, 1)
            out = scipy.ndimage.zoom(out2d, (H / out2d.shape[0], W / out2d.shape[1]), order=1)
        return out
    return fwd, adj

def add_poisson_gaussian_noise(y_clean, peak_photons, read_sigma, rng):
    scaled = np.maximum(np.abs(y_clean) * peak_photons, 0.0)
    poisson_part = rng.poisson(scaled).astype(np.float64) / peak_photons
    read_noise = rng.randn(*y_clean.shape) * read_sigma
    return poisson_part + read_noise
...
def main():
    ...
    rng = np.random.RandomState(SEED)
    y = add_poisson_gaussian_noise(y_clean, PEAK_PHOTONS, READ_SIGMA, rng)
    ...
    # Adjoint maps from measurement space back to image space (64,64)
    _, adj_fn = build_operators(sensitivity=1.0)
    x_hat_w1 = adj_fn(y)
    ...
    # W2 - sensitivity mismatch (1.0 -> 1.3)
    a_sha256 = hashlib.sha256(f"sonar_sensitivity".encode()).hexdigest()[:16]

    rng2 = np.random.RandomState(SEED + 10)
    fwd_pert, _ = build_operators(sensitivity=1.3)
    y_clean_pert = fwd_pert(x_true)
    y_measured = add_poisson_gaussian_noise(y_clean_pert, PEAK_PHOTONS, READ_SIGMA, rng2)
    noise_sigma_eff = max(float(np.std(y_measured - y_clean_pert)), 0.001)

    fwd_nom, _ = build_operators(sensitivity=1.0)
    nll_before = compute_nll_gaussian(y_measured, fwd_nom(x_true), noise_sigma_eff)

    best_nll, best_sens = np.inf, 1.0
    for ts in np.arange(0.5, 2.05, 0.05):
        fwd_t, _ = build_operators(sensitivity=ts)
        nll_t = compute_nll_gaussian(y_measured, fwd_t(x_true), noise_sigma_eff)
        if nll_t < best_nll: best_nll, best_sens = nll_t, ts

    nll_after = best_nll
    nll_decrease_pct = (nll_before - nll_after) / (nll_before + 1e-12) * 100

    _, adj_uncorr = build_operators(sensitivity=1.0)
    ...
    _, adj_corr = build_operators(sensitivity=best_sens)
