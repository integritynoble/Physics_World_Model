#!/usr/bin/env python3
"""Structured Light \u2014 Phase B Experiment Script.

Structured Light: y = PhotonSensor(DepthOptics(StructuredLightProjector(PhotonSource(x)))) + noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_structured_light_experiment.py
"""
...
TEMPLATE_KEY = "structured_light_graph_v2"
H, W = 64, 64
...
def build_operators(gain=1.0):
    ...
    proj = get_primitive("structured_light_projector", {"n_patterns": 4, "fringe_freq": 8.0})
    optics = get_primitive("depth_optics", {"focal_length": 50.0, "aperture": 2.8})
    sens = get_primitive("photon_sensor", {"quantum_efficiency": 0.80, "gain": gain})
    def fwd(x): return sens.forward(optics.forward(proj.forward(src.forward(x))))
    def adj(y): return src.adjoint(proj.adjoint(sens.adjoint(y)))
    return fwd, adj
