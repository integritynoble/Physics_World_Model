#!/usr/bin/env python3
"""ToF Camera \u2014 Phase B Experiment Script.

ToF Camera: y = SPADToFSensor(TimeOfFlightGate(PhotonSource(x))) + noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_tof_camera_experiment.py
"""
...
SEED = 42
MODALITY = "tof_camera"
TEMPLATE_KEY = "tof_camera_graph_v2"
H, W = 64, 64
PEAK_PHOTONS = 5000.0
...
def build_operators(gain=1.0):
    ...
    tof = get_primitive("tof_gate", {"n_bins": 64, "bin_width_ns": 1.0, "timing_jitter_ns": 0.1})
    sens = get_primitive("spad_tof_sensor", {"n_bins": 64, "dead_time_ns": 50.0, "qe": 0.3})
    def base_fwd(x): return sens.forward(tof.forward(src.forward(x)))
    def base_adj(y): return src.adjoint(tof.adjoint(sens.adjoint(y)))
