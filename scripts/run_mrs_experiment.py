#!/usr/bin/env python3
"""MRS (Magnetic Resonance Spectroscopy) -- Phase B Experiment Script.

MRS: y = CoilSensor(SequenceBlock(MRIkSpace(SpinSource(x)))) + ComplexGaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_mrs_experiment.py
"""
...
def build_operators(sensitivity=1.0):
    ...
    ksp = get_primitive("mri_kspace", {"H": H, "W": W, "sampling_rate": 1.0, "seed": 42})
    seq = get_primitive("sequence_block", {"TR_ms": 3000.0, "TE_ms": 135.0, "flip_angle_deg": 90.0})
...
def main():
    ...
    a_sha256 = hashlib.sha256(f"mrs_sensitivity".encode()).hexdigest()[:16]
