#!/usr/bin/env python3
"""Proton Radiography \u2014 Phase B Experiment Script.

Proton Radiography: y = GenericSensor(MultipleScattering(ParticleAttenuation(GenericSource(x)))) + noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_proton_radiography_experiment.py
"""
...
TEMPLATE_KEY = "proton_radiography_graph_v2"
H, W = 64, 64
...
def build_operators(gain=1.0):
    ...
    att = get_primitive("particle_attenuation", {"I_0": 10000.0, "cross_section": 0.03})
    scat = get_primitive("multiple_scattering", {"sigma": 1.5})
    sens = get_primitive("generic_sensor", {"gain": gain})
    def fwd(x): return sens.forward(scat.forward(att.forward(src.forward(x))))
    def adj(y):
        # Pseudo-inverse: clip + normalize (scattering makes exact inversion hard)
...
def main():
    ...
    save_operator_meta(rb_dir, {"a_definition": "callable", "a_extraction_method": "graph_stripped",
        "a_sha256": a_sha256, "linearity": "nonlinear", "linearization_notes": "Attenuation + scattering",
