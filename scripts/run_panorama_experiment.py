#!/usr/bin/env python3
"""Panorama Imaging — Phase B Experiment Script.

Panorama: y = Sensor(FrameIntegration(Conv2D(x))) + noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_panorama_experiment.py
"""
...
SEED = 42
MODALITY = "panorama"
TEMPLATE_KEY = "panorama_graph_v2"
H, W = 64, 64
...
def build_operators(gain=1.0):
    ...
    integ = get_primitive("frame_integration", {"axis": 0, "T": 1})
    sens = get_primitive("photon_sensor", {"quantum_efficiency": 0.9, "gain": gain})
    def fwd(x): return sens.forward(integ.forward(blur.forward(src.forward(x))))
    return fwd
...
def main():
    ...
    # y is 1D (frame_integration sums axis 0), use pseudo-inverse: tile back to 2D
    x_hat_w1 = np.tile(np.clip(y, 0, None), (H, 1))
    if x_hat_w1.max() > 0:
    ...
    x_hat_uncorrected = np.tile(np.clip(y_measured, 0, None), (H, 1))
    ...
    x_hat_corrected = np.tile(np.clip(y_measured / best_gain, 0, None), (H, 1))
