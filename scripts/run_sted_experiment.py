#!/usr/bin/env python3
"""STED Microscopy — Phase B Experiment Script.

STED: y = Sensor(Conv2D(SaturationDepletion(x))) + noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_sted_experiment.py
"""
...
def build_operators(gain=1.0):
    ...
    depl = get_primitive("saturation_depletion", {"depletion_factor": 0.5, "psf_sigma": 1.0})
    blur = get_primitive("conv2d", {"sigma": PSF_SIGMA, "mode": "reflect"})
    sens = get_primitive("photon_sensor", {"quantum_efficiency": 0.45, "gain": gain})
    def fwd(x): return sens.forward(blur.forward(depl.forward(src.forward(x))))
    return fwd
