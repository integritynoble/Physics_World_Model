#!/usr/bin/env python3
"""Doppler Ultrasound -- Phase B Experiment Script.

Doppler US: y = AcousticReceive(DopplerEstimator(AcousticPropagation(AcousticSource(x)))) + Gaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_doppler_ultrasound_experiment.py
"""
...
TEMPLATE_KEY = "doppler_ultrasound_graph_v2"
H, W = 64, 64
NOISE_SIGMA = 0.01
...
def build_operators(sensitivity=1.0):
    ...
    prop = get_primitive("acoustic_propagation", {"speed_of_sound": 1540.0, "n_sensors": 32, "x_shape": [H, W]})
    dopp = get_primitive("doppler_estimator", {"prf_hz": 10000.0, "wall_filter_cutoff": 50.0})
    sens = get_primitive("acoustic_receive_sensor", {"sensitivity": sensitivity})
    def fwd(x): return sens.forward(dopp.forward(prop.forward(src.forward(x))))
    def adj(y):
        # doppler_estimator has no adjoint; skip it and use propagation adjoint
        out = src.adjoint(prop.adjoint(sens.adjoint(y)))
...
def main():
    ...
    rng = np.random.RandomState(SEED)
    y = y_clean + rng.randn(*y_clean.shape) * NOISE_SIGMA
    ...
    # PSNR/SSIM compare reconstructed x_hat (64,64) to x_true (64,64)
    ...
    a_sha256 = hashlib.sha256(f"doppler_ultrasound_sensitivity".encode()).hexdigest()[:16]

    rng2 = np.random.RandomState(SEED + 10)
    fwd_pert, _ = build_operators(sensitivity=1.3)
    y_clean_pert = fwd_pert(x_true)
    y_measured = y_clean_pert + rng2.randn(*y_clean_pert.shape) * NOISE_SIGMA
