#!/usr/bin/env python3
"""Ptychography — Phase B Experiment Script.

Ptychography: y = |Fresnel(coded_mask * x)|² + noise
Scanning coherent diffraction with coded probe mask.

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_ptychography_experiment.py
"""
...
TEMPLATE_KEY = "ptychography_graph_v2"
H, W = 64, 64
...
def build_operators(distance):
    ...
    mask = get_primitive("coded_mask", {"seed": 42, "H": H, "W": W})
    prop = get_primitive("fresnel_prop", {"wavelength": 0.5e-6, "distance": distance, "pixel_size": 1.0e-6})
    det = get_primitive("magnitude_sq", {})
    sens = get_primitive("photon_sensor", {"quantum_efficiency": 0.9, "gain": 1.0})
    def fwd(x):
        out = sens.forward(det.forward(prop.forward(mask.forward(src.forward(x)))))
        return np.real(out) if np.iscomplexobj(out) else out
    def recon(y):
        # sqrt(y) → backpropagate → take abs as estimate
        amp = np.sqrt(np.maximum(y / 0.9, 0))  # undo QE
...
def main():
    ...
    # W2 - propagation distance mismatch (1.0e-3 → 1.5e-3)
    nom_dist = 1.0e-3
    pert_dist = 1.5e-3
    a_sha256 = hashlib.sha256(f"ptychography_{nom_dist}".encode()).hexdigest()[:16]
    ...
    ssim_uncorrected = ssim_fn(x_hat_uncorrected.astype(np.float64), x_true)

    best_nll, best_dist = np.inf, nom_dist
    for td in np.linspace(0.5e-3, 2.0e-3, 31):
    ...
    _, recon_corr = build_operators(best_dist)
    x_hat_corrected = recon_corr(y_measured)
    ...
    ssim_corrected = ssim_fn(x_hat_corrected.astype(np.float64), x_true)
