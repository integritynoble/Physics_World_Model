#!/usr/bin/env python3
"""SEM (Scanning Electron Microscopy) — Phase B Experiment Script.

SEM: y = ElectronDetector(YieldModel(ElectronBeamSource(x))) + Gaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_sem_experiment.py
"""
...
def build_operators(gain=100.0):
    src = get_primitive("electron_beam_source", {"accelerating_voltage_kv": 15.0, "beam_current_na": 1.0})
    ym = get_primitive("yield_model", {"yield_coeff": 0.1, "detector_type": "SE"})
    sens = get_primitive("electron_detector_sensor", {"detector_type": "SE", "collection_efficiency": 0.5, "gain": gain})
    def fwd(x): return sens.forward(ym.forward(src.forward(x)))
    def adj(y): return src.adjoint(ym.adjoint(sens.adjoint(y)))
    return fwd, adj
...
def main():
    ...
    # Pseudo-inverse recon: clip + normalize (yield_model may not have full adjoint)
    _, adj_fn = build_operators(gain=100.0)
    x_hat_w1 = adj_fn(y)
    ...
    # W2 - gain mismatch (100.0 -> 130.0)
    a_sha256 = hashlib.sha256(f"sem_gain".encode()).hexdigest()[:16]

    rng2 = np.random.RandomState(SEED + 10)
    fwd_pert, _ = build_operators(gain=130.0)
    ...
    fwd_nom, _ = build_operators(gain=100.0)
    nll_before = compute_nll_gaussian(y_measured, fwd_nom(x_true), noise_sigma_eff)

    best_nll, best_gain = np.inf, 100.0
    for tg in np.arange(50.0, 200.0 + 5.0, 5.0):
    ...
    _, adj_uncorr = build_operators(gain=100.0)
