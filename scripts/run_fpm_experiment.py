#!/usr/bin/env python3
"""Fourier Ptychographic Microscopy (FPM) — Phase B Experiment Script.

FPM: y = |AS(x)|² + noise — angular spectrum with intensity detection.

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_fpm_experiment.py
"""
...
def main():
    ...
    nom_dist = 1.0e-3
    a_sha256 = hashlib.sha256(f"fpm_{nom_dist}".encode()).hexdigest()[:16]
