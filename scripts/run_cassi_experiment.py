#!/usr/bin/env python3
"""CASSI Modality — Phase B Experiment Script.
...
Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_cassi_experiment.py
"""
...
SEED = 42
MODALITY = "cassi"
TEMPLATE_KEY = "cassi_graph_v2"
H, W, L = 64, 64, 8  # Spatial height, width, spectral bands
NOISE_SIGMA = 0.01
GAP_TV_ITERS = 50
GAP_TV_LAM = 0.001
...
def make_hyperspectral_phantom(H: int, W: int, L: int, seed: int) -> np.ndarray:
    """Create a 3D smooth hyperspectral phantom (H, W, L).

    Uses Gaussian blob spatial abundances multiplied by spectral endmembers
    to create a realistic hyperspectral test cube.
    """
    rng = np.random.RandomState(seed)

    # 4 spectral endmembers (smooth spectra)
    n_endmembers = 4
    wavelengths = np.linspace(0, 1, L)
    endmembers = np.zeros((n_endmembers, L))
    for i in range(n_endmembers):
        center = (i + 0.5) / n_endmembers
        endmembers[i] = np.exp(-0.5 * ((wavelengths - center) / 0.15) ** 2)

    # Gaussian blob abundances in spatial domain
    yy, xx = np.meshgrid(np.linspace(0, 1, H), np.linspace(0, 1, W), indexing='ij')
    abundances = np.zeros((H, W, n_endmembers))
    for i in range(n_endmembers):
        cy, cx = rng.rand(2) * 0.6 + 0.2  # centers in [0.2, 0.8]
        sigma_s = rng.rand() * 0.1 + 0.1  # spatial sigma [0.1, 0.2]
        abundances[:, :, i] = np.exp(-0.5 * ((yy - cy) ** 2 + (xx - cx) ** 2) / sigma_s ** 2)

    # Normalize abundances to sum to 1 at each pixel
    abundances /= (abundances.sum(axis=2, keepdims=True) + 1e-12)

    # Mix: x(h,w,l) = sum_i abundances(h,w,i) * endmembers(i,l)
    x = np.einsum('hwi,il->hwl', abundances, endmembers)

    # Scale to [0, 1]
    x = x / (x.max() + 1e-12) * 0.9 + 0.05  # range ~ [0.05, 0.95]
...
def build_cassi_operators(seed: int, H: int, W: int, L: int, disp_step: float = 1.0):
    """Build CASSI forward/adjoint callables from individual primitives.

    Returns (forward_fn, adjoint_fn, primitives_dict).
    """
    source = PhotonSource(params={"strength": 1.0})
    mask = CodedMask(params={"seed": seed, "H": H, "W": W})
    disp = SpectralDispersion(params={"disp_step": disp_step})
    integ = FrameIntegration(params={"axis": -1, "T": L})
    sensor = PhotonSensor(params={"quantum_efficiency": 0.9, "gain": 1.0})

    def forward_fn(x):
        out = source.forward(x)
        out = mask.forward(out)
        out = disp.forward(out)
        out = integ.forward(out)
        out = sensor.forward(out)
        return out

    def adjoint_fn(y):
        out = sensor.adjoint(y)
        out = integ.adjoint(out)
        out = disp.adjoint(out)
        out = mask.adjoint(out)
    ...
    return forward_fn, adjoint_fn, prims
...
def main():
    ...
    print("=" * 70)
    print("CASSI Phase B Experiment")
    print("=" * 70)

    # ── 1. Compile graph ──────────────────────────────────────────────────
    print("\n[1] Compiling cassi_graph_v2 ...")
    ...
    # ── 2. Create phantom ─────────────────────────────────────────────────
    x_true = make_hyperspectral_phantom(H, W, L, SEED)
    print(f"\n[2] Phantom: shape={x_true.shape}, range=[{x_true.min():.4f}, {x_true.max():.4f}]")

    # ── 3. Create RunBundle ───────────────────────────────────────────────
    os.makedirs(RUNS_DIR, exist_ok=True)
    rb_dir = write_runbundle_skeleton(RUNS_DIR, "cassi_exp")
    ...
    fwd_fn, adj_fn, prims = build_cassi_operators(SEED, H, W, L, disp_step=1.0)

    # ── Mode I: reconstruct with GAP-TV ──────────────────────────────────
    t0 = time.time()
    x_hat_w1 = gap_tv_operator(
        y, fwd_fn, adj_fn, (H, W, L),
        iterations=GAP_TV_ITERS, lam=GAP_TV_LAM,
    ).astype(np.float64)
    t_inv = time.time() - t0
    solver_used = "gap_tv_operator"
    ...
    # Hash the mask as the operator fingerprint
    mask_arr = prims["mask"]._mask.copy()
    a_sha256 = compute_operator_hash(mask_arr)
    print(f"    Mask shape: {mask_arr.shape}, dtype: {mask_arr.dtype}")
    print(f"    A_sha256 (mask): {a_sha256[:16]}...")

    # ── Inject synthetic mismatch: dispersion step drift ──────────────────
    # Nominal disp_step=1.0, perturbed to 1.15
    perturbed_disp_step = 1.15
    fwd_perturbed, adj_perturbed, _ = build_cassi_operators(
        SEED, H, W, L, disp_step=perturbed_disp_step,
    )

    # Generate "measured" y using perturbed operator + noise
    rng2 = np.random.RandomState(SEED + 10)
    y_measured = fwd_perturbed(x_true) + rng2.randn(*y.shape) * NOISE_SIGMA

    print(f"    Mismatch: disp_step drift 1.0 → {perturbed_disp_step}")
    ...
    t0 = time.time()
    x_hat_uncorrected = gap_tv_operator(
        y_measured, fwd_fn, adj_fn, (H, W, L),
        iterations=GAP_TV_ITERS, lam=GAP_TV_LAM,
    ).astype(np.float64)
    ...
    # ── Fit correction: 1D grid search over disp_step ─────────────────────
    print("    Grid search over disp_step [0.8, 1.3] ...")
    search_grid = np.linspace(0.8, 1.3, 26)  # 26 points
    best_nll = np.inf
    best_disp = 1.0
    for ds in search_grid:
        fwd_trial, _, _ = build_cassi_operators(SEED, H, W, L, disp_step=ds)
    ...
    print(f"    Best disp_step: {best_disp:.3f} (NLL={best_nll:.1f})")

    # Build corrected operator
    fwd_corrected, adj_corrected, prims_corrected = build_cassi_operators(
        SEED, H, W, L, disp_step=best_disp,
    )
    a_corrected_sha256 = compute_operator_hash(prims_corrected["mask"]._mask)
    ...
    t0 = time.time()
    x_hat_corrected = gap_tv_operator(
        y_measured, fwd_corrected, adj_corrected, (H, W, L),
        iterations=GAP_TV_ITERS, lam=GAP_TV_LAM,
    ).astype(np.float64)
    ...
    results["w2"] = {
        ...
        "mismatch_params": {"disp_step_nominal": 1.0, "disp_step_perturbed": perturbed_disp_step},
        "mismatch_description": f"Dispersion step drift 1.0 → {perturbed_disp_step}",
        "correction_family": "Pre",
        "fitted_disp_step": round(best_disp, 3),
    ...
    w2_meta = {
        ...
        "mismatch_params": {"disp_step_nominal": 1.0, "disp_step_perturbed": perturbed_disp_step},
        "correction_family": "Pre",
        "fitted_params": {"disp_step": round(best_disp, 3)},
        "nll_before": round(nll_before, 1),
        "nll_after": round(nll_after, 1),
        "nll_decrease_pct": round(nll_decrease_pct, 1),
    ...
    # Save results JSON
    results_path = os.path.join(rb_dir, "cassi_experiment_results.json")
    ...
    print(f"  W2 fitted disp:    {best_disp:.3f}")
