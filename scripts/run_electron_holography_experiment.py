#!/usr/bin/env python3
"""Electron Holography — Phase B Experiment Script.

Electron Holography: y = PhotonSensor(FresnelProp(GenericSource(x))) + Poisson-Gaussian noise
Uses adjoint reconstruction (fresnel_prop has adjoint).

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_electron_holography_experiment.py
"""
...
TEMPLATE_KEY = "electron_holography_graph_v2"
H, W = 64, 64
...
def build_operators(gain=1.0):
    ...
    prop = get_primitive("fresnel_prop", {"wavelength": 2.5e-12, "distance": 1.0e-3, "pixel_size": 1.0e-10})
    sens = get_primitive("photon_sensor", {"quantum_efficiency": 0.8, "gain": gain})
    def fwd(x): return sens.forward(prop.forward(src.forward(x)))
    def adj(y):
        """Adjoint reconstruction: prop.adjoint(sens.adjoint(y)), take np.abs()."""
        return np.abs(src.adjoint(prop.adjoint(sens.adjoint(y))))
    return fwd, adj
...
def main():
    ...
    # Fresnel prop output may be complex; take real part for intensity
    ...
    # ADJOINT recon (fresnel_prop has adjoint)
    ...
    a_sha256 = hashlib.sha256(f"electron_holography_gain".encode()).hexdigest()[:16]
    ...
    save_operator_meta(rb_dir, {"a_definition": "callable", "a_extraction_method": "graph_stripped",
        "a_sha256": a_sha256, "linearity": "linear", "linearization_notes": "fresnel_prop is linear with adjoint",
