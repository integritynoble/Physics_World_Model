#!/usr/bin/env python3
"""Fluoroscopy — Phase B Experiment Script.

Fluoroscopy: y = XRayDetector(FluoroTemporalIntegrator(BeerLambert(XRaySource(x)))) + Poisson+Gaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_fluoroscopy_experiment.py
"""
...
TEMPLATE_KEY = "fluoroscopy_graph_v2"
H, W = 64, 64
...
def build_operators(gain=1.0):
    ...
    bl = get_primitive("beer_lambert", {"I_0": 5000.0})
    fti = get_primitive("fluoro_temporal_integrator", {"n_frames": 8, "decay": 0.9})
    sens = get_primitive("xray_detector_sensor", {"scintillator_efficiency": 0.7, "gain": gain})
...
def main():
    ...
    x_hat_w1 = -np.log(np.clip(y / (0.7 * 5000.0), 1e-8, None))
    ...
    a_sha256 = hashlib.sha256(f"fluoroscopy_gain".encode()).hexdigest()[:16]
