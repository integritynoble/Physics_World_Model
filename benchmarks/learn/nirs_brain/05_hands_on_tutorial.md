# 05 — Hands-On Tutorial: Functional Near-Infrared Spectroscopy (fNIRS)

This tutorial walks through running the PWM benchmark for Functional Near-Infrared Spectroscopy (fNIRS),
...
config_path = ROOT / "benchmarks" / "configs" / "nirs_brain.yaml"
...
try:
    from pwm_core.recon.fbp import run_fbp
    print("Solver loaded: FBP")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality nirs_brain
...
solvers_to_test = {
    "FBP": ("pwm_core.recon.fbp", "run_fbp"),
    "DL-Recon": ("pwm_core.recon.dl_recon", "dl_reconstruct"),
}
...
# axes[2].set_title("DL-Recon")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("nirs_brain_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/nirs_brain.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/nirs_brain_expanded.yaml`
