# 05 — Hands-On Tutorial: Electron Holography

This tutorial walks through running the PWM benchmark for Electron Holography,
...
config_path = ROOT / "benchmarks" / "configs" / "electron_holography.yaml"
...
try:
    from pwm_core.recon.phase_retrieval_solver import run_phase_retrieval
    print("Solver loaded: Phase Retrieval (HIO)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality electron_holography
...
solvers_to_test = {
    "Phase Retrieval (HIO)": ("pwm_core.recon.phase_retrieval_solver", "run_phase_retrieval"),
    "Best Quality": ("pwm_core.recon", "reconstruct"),
}
...
# axes[1].set_title("Phase Retrieval (HIO)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("electron_holography_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/electron_holography.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/electron_holography_expanded.yaml`
