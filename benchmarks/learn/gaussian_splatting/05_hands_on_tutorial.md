# 05 — Hands-On Tutorial: 3D Gaussian Splatting (3DGS)

This tutorial walks through running the PWM benchmark for 3D Gaussian Splatting (3DGS),
...
config_path = ROOT / "benchmarks" / "configs" / "gaussian_splatting.yaml"
...
try:
    ...
    print("Solver loaded: EWA Splatting")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality gaussian_splatting
...
solvers_to_test = {
    "EWA Splatting": ("pwm_core.recon.gaussian_splatting_solver", "run_gaussian_splatting"),
    "3DGS (full)": ("pwm_core.recon.gaussian_splatting_solver", "run_gaussian_splatting"),
}
...
# axes[1].set_title("EWA Splatting")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("3DGS (full)")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("gaussian_splatting_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/gaussian_splatting.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/gaussian_splatting_expanded.yaml`
