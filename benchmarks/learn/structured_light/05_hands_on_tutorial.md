# 05 — Hands-On Tutorial: Structured-Light Depth Camera

This tutorial walks through running the PWM benchmark for Structured-Light Depth Camera,
...
config_path = ROOT / "benchmarks" / "configs" / "structured_light.yaml"
...
try:
    ...
    print("Solver loaded: FISTA-L2 (phase unwrap)")
except ImportError:
...
solvers_to_test = {
    "FISTA-L2 (phase unwrap)": ("pwm_core.recon.classical", "run_fista_l2"),
...
# axes[1].set_title("FISTA-L2 (phase unwrap)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("structured_light_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/structured_light.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/structured_light_expanded.yaml`
