# Benchmark QA Check — bioluminescence_tomo

**URL:** https://pwm.platformai.org/benchmark/bioluminescence_tomo
**Check Date:** 2026-03-03 14:35 UTC
**Status:** PASS

## Summary

| Severity | Count |
|----------|-------|
| ERROR | 0 |
| WARNING | 0 |
| INFO | 2 |
| PASSED | 23 |

## Info

- Page size: 56,150 bytes
- Methods: Sci-Former + gradient, PnP-BM3D + gradient, ResUNet + gradient, Tikhonov + gradient

## Passed Checks

- [x] Main page loads (HTTP 200)
- [x] Title: Bioluminescence Tomography (BLT) — Physics World Model
- [x] Description: Bioluminescence Tomography (BLT)
- [x] Spec notation: Src → R → P → D
- [x] Challenge Leaderboard section present
- [x] Leaderboard: 4 entries
- [x] Data Preview / Gallery section present
- [x] Challenge public page loads (HTTP 200)
- [x] Challenge public has HDF5 reference
- [x] Challenge dev page loads (HTTP 200)
- [x] Challenge dev has HDF5 reference
- [x] Challenge public HDF5 on GCS OK
- [x] Challenge dev HDF5 on GCS OK
- [x] Compete page loads (HTTP 200)
- [x] Contribute page loads (HTTP 200)
- [x] Forward model reference found
- [x] Learning materials directory exists
- [x] Learn: README.md (1,492 B)
- [x] Learn: 01_physics_fundamentals.md (2,124 B)
- [x] Learn: 02_forward_model.md (2,698 B)
- [x] Learn: 03_reconstruction_algorithms.md (2,054 B)
- [x] Learn: 04_pwm_benchmark.md (2,406 B)
- [x] Learn: 05_hands_on_tutorial.md (3,578 B)

---
*Generated by `scripts/check_modality.py` v2*