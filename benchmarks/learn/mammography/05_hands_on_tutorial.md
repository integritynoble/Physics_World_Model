# 05 — Hands-On Tutorial: Mammography

This tutorial walks through running the PWM benchmark for Mammography,
...
config_path = ROOT / "benchmarks" / "configs" / "mammography.yaml"
...
try:
    ...
    print("Solver loaded: FBP (mammography)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality mammography
...
solvers_to_test = {
    "FBP (mammography)": ("pwm_core.recon.ct_solvers", "run_fbp"),
...
# axes[1].set_title("FBP (mammography)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("mammography_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/mammography.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/mammography_expanded.yaml`
