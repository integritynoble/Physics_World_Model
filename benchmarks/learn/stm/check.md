# Benchmark QA Check — stm

**URL:** https://pwm.platformai.org/benchmark/stm
**HTTP Status:** 200
**Page Size:** 56,600 bytes
**Check Date:** 2026-03-03 14:22 UTC

## Summary

| Severity | Count |
|----------|-------|
| WARNING | 2 |
| INFO | 4 |

## Issues by Category

### Forward Model

- 🟡 **WARNING**: No explicit forward model equation (y = ...) found on page
- 🟡 **WARNING**: Notation inconsistency: Forward operator denoted as both H and A

### Images & Links

- 🔵 **INFO**: GCS image reference: /gcs/challenge-data/v1.0/stm_challenge_public.h5 — verify it loads
- 🔵 **INFO**: GCS image reference: /gcs/challenge-data/v1.0/stm_challenge_dev.h5 — verify it loads

### Algorithm Names

- 🔵 **INFO**: Solver 'CARE' from registry not found on page
- 🔵 **INFO**: Solver 'Richardson-Lucy' from registry not found on page

---
*Auto-generated by `benchmarks/learn/check_all_modalities.py`*
