# 05 — Hands-On Tutorial: Transmission Electron Microscopy (TEM)

This tutorial walks through running the PWM benchmark for Transmission Electron Microscopy (TEM),
...
config_path = ROOT / "benchmarks" / "configs" / "tem.yaml"
...
try:
    ...
    print("Solver loaded: FISTA-L2 (CTF correction)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality tem
...
solvers_to_test = {
    "FISTA-L2 (CTF correction)": ("pwm_core.recon.classical", "run_fista_l2"),
...
# axes[1].set_title("FISTA-L2 (CTF correction)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("tem_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/tem.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/tem_expanded.yaml`
