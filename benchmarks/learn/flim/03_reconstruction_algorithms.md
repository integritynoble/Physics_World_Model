# 03 — Reconstruction Algorithms: Fluorescence Lifetime Imaging (FLIM)
...
The default solver for Fluorescence Lifetime Imaging (FLIM) is **`phasor`**.
...
| best_quality | MLE Fit | `pwm_core.recon.flim_solver` | `mle_fit_recon` | No | Becker 2012, J. Microscopy |
| famous_dl | FLIMNet | `pwm_core.recon.flim_solver` | `flimnet_recon` | No | Smith et al. 2019, Biomed. Optics Express |
| small_gpu | FLIMNet | `pwm_core.recon.flim_solver` | `flimnet_recon` | No |  |
...
### Traditional Cpu: Phasor Analysis
...
3. Add the solver tier to the modality config in `benchmarks/configs/flim.yaml`
