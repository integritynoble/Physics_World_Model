# 05 — Hands-On Tutorial: Fluorescence Lifetime Imaging (FLIM)

This tutorial walks through running the PWM benchmark for Fluorescence Lifetime Imaging (FLIM),
...
config_path = ROOT / "benchmarks" / "configs" / "flim.yaml"
...
try:
    from pwm_core.recon.flim_solver import run_flim
    print("Solver loaded: Phasor Analysis")
except ImportError:
...
solvers_to_test = {
    "Phasor Analysis": ("pwm_core.recon.flim_solver", "run_flim"),
    "MLE Fit": ("pwm_core.recon.flim_solver", "mle_fit_recon"),
}
...
# axes[1].set_title("Phasor Analysis")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("MLE Fit")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("flim_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/flim.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/flim_expanded.yaml`
