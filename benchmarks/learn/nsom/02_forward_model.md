# 02 — Forward Model: Near-field Scanning Optical Microscopy (NSOM)
...
This modality uses the **`scanning_probe`** category module:
Near-field / tip-sample interaction.
...
The PWM benchmark for Near-field Scanning Optical Microscopy (NSOM) uses:

- **Forward model**: `scanning_probe` with `nonlinear_operator` operator
- **Default solver**: `near_field_recon`
- **Metrics**: ['psnr', 'ssim']
- **Primary metric**: psnr
