# 05 — Hands-On Tutorial: Electron Energy Loss Spectroscopy (EELS)

This tutorial walks through running the PWM benchmark for Electron Energy Loss Spectroscopy (EELS),
...
config_path = ROOT / "benchmarks" / "configs" / "eels.yaml"
...
try:
    ...
    print("Solver loaded: FISTA-L2 (Fourier ratio)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality eels
...
solvers_to_test = {
    "FISTA-L2 (Fourier ratio)": ("pwm_core.recon.classical", "run_fista_l2"),
...
# axes[1].set_title("FISTA-L2 (Fourier ratio)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("eels_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/eels.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/eels_expanded.yaml`
