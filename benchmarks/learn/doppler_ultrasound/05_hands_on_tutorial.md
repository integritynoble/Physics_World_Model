# 05 — Hands-On Tutorial: Doppler Ultrasound

This tutorial walks through running the PWM benchmark for Doppler Ultrasound,
...
config_path = ROOT / "benchmarks" / "configs" / "doppler_ultrasound.yaml"
...
try:
    from pwm_core.recon.photoacoustic_solver import run_photoacoustic
    print("Solver loaded: Back-Projection (Doppler)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality doppler_ultrasound
...
solvers_to_test = {
    "Back-Projection (Doppler)": ("pwm_core.recon.photoacoustic_solver", "run_photoacoustic"),
...
# axes[1].set_title("Back-Projection (Doppler)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("doppler_ultrasound_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/doppler_ultrasound.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/doppler_ultrasound_expanded.yaml`
