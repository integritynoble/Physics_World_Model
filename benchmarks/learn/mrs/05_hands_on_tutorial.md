# 05 — Hands-On Tutorial: MR Spectroscopy (MRS)

This tutorial walks through running the PWM benchmark for MR Spectroscopy (MRS),
...
config_path = ROOT / "benchmarks" / "configs" / "mrs.yaml"
...
try:
    ...
    print("Solver loaded: SENSE (spectroscopy)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality mrs
...
solvers_to_test = {
    "SENSE (spectroscopy)": ("pwm_core.recon.mri_solvers", "run_sense"),
...
# axes[1].set_title("SENSE (spectroscopy)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("mrs_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/mrs.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/mrs_expanded.yaml`
