# 05 — Hands-On Tutorial: Structured Illumination Microscopy (SIM)

This tutorial walks through running the PWM benchmark for Structured Illumination Microscopy (SIM),
...
config_path = ROOT / "benchmarks" / "configs" / "sim.yaml"
...
try:
    ...
    print("Solver loaded: Wiener-SIM")
except ImportError:
...
solvers_to_test = {
    "Wiener-SIM": ("pwm_core.recon.sim_solver", "run_sim_reconstruction"),
    "HiFi-SIM": ("pwm_core.recon.sim_solver", "hifi_sim_2d"),
}
...
# axes[1].set_title("Wiener-SIM")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("HiFi-SIM")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
...
- Read the full config: `benchmarks/configs/sim.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/sim_expanded.yaml`
