# 05 — Hands-On Tutorial: Dual-Energy X-ray Absorptiometry (DEXA)

This tutorial walks through running the PWM benchmark for Dual-Energy X-ray Absorptiometry (DEXA),
...
config_path = ROOT / "benchmarks" / "configs" / "dexa.yaml"
...
try:
    ...
    print("Solver loaded: FISTA-L2 (dual-energy)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality dexa
...
solvers_to_test = {
    "FISTA-L2 (dual-energy)": ("pwm_core.recon.classical", "run_fista_l2"),
...
# axes[1].set_title("FISTA-L2 (dual-energy)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("dexa_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/dexa.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/dexa_expanded.yaml`
