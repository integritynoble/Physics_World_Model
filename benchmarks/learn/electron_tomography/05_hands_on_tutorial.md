# 05 — Hands-On Tutorial: Electron Tomography

This tutorial walks through running the PWM benchmark for Electron Tomography,
...
config_path = ROOT / "benchmarks" / "configs" / "electron_tomography.yaml"
...
try:
    ...
    print("Solver loaded: FBP (SIRT baseline)")
except ImportError:
...
solvers_to_test = {
    "FBP (SIRT baseline)": ("pwm_core.recon.ct_solvers", "run_fbp"),
...
# axes[1].set_title("FBP (SIRT baseline)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("electron_tomography_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/electron_tomography.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/electron_tomography_expanded.yaml`
