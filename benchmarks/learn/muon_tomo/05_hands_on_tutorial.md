# 05 — Hands-On Tutorial: Muon Tomography

This tutorial walks through running the PWM benchmark for Muon Tomography,
...
config_path = ROOT / "benchmarks" / "configs" / "muon_tomo.yaml"
...
try:
    ...
    print("Solver loaded: FBP (muon tomography)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality muon_tomo
...
solvers_to_test = {
    "FBP (muon tomography)": ("pwm_core.recon.ct_solvers", "run_fbp"),
...
# axes[1].set_title("FBP (muon tomography)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("muon_tomo_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/muon_tomo.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/muon_tomo_expanded.yaml`
