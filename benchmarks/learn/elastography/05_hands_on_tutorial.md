# 05 — Hands-On Tutorial: Shear-Wave Elastography

This tutorial walks through running the PWM benchmark for Shear-Wave Elastography,
...
config_path = ROOT / "benchmarks" / "configs" / "elastography.yaml"
...
try:
    from pwm_core.recon.mri_solvers import run_sense
    print("Solver loaded: SENSE (displacement field)")
except ImportError:
...
solvers_to_test = {
    "SENSE (displacement field)": ("pwm_core.recon.mri_solvers", "run_sense"),
...
# axes[1].set_title("SENSE (displacement field)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("elastography_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/elastography.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/elastography_expanded.yaml`
