# 03 — Reconstruction Algorithms: Diffusion MRI (DTI)
...
The default solver for Diffusion MRI (DTI) is **`weighted_least_squares`**.
...
|------|------|--------|----------|-----|-----------|
| traditional_cpu | SENSE (WLS tensor fit) | `pwm_core.recon.mri_solvers` | `run_sense` | No |  |
...
### Traditional Cpu: SENSE (WLS tensor fit)
...
3. Add the solver tier to the modality config in `benchmarks/configs/diffusion_mri.yaml`
