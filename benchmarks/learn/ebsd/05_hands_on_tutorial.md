# 05 — Hands-On Tutorial: Electron Backscatter Diffraction (EBSD)

This tutorial walks through running the PWM benchmark for Electron Backscatter Diffraction (EBSD),
...
config_path = ROOT / "benchmarks" / "configs" / "ebsd.yaml"
...
try:
    ...
    print("Solver loaded: FISTA-L2 (Hough baseline)")
except ImportError:
...
solvers_to_test = {
    "FISTA-L2 (Hough baseline)": ("pwm_core.recon.classical", "run_fista_l2"),
...
# axes[1].set_title("FISTA-L2 (Hough baseline)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("ebsd_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/ebsd.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/ebsd_expanded.yaml`
