# 05 — Hands-On Tutorial: Confocal 3D Z-Stack

This tutorial walks through running the PWM benchmark for Confocal 3D Z-Stack,
...
config_path = ROOT / "benchmarks" / "configs" / "confocal_3d.yaml"
...
try:
    ...
    print("Solver loaded: 3D Richardson-Lucy")
except ImportError:
...
solvers_to_test = {
    "3D Richardson-Lucy": ("pwm_core.recon.richardson_lucy", "run_richardson_lucy"),
    "3D CARE": ("pwm_core.recon.care_unet", "care_restore_3d"),
}
...
# axes[1].set_title("3D Richardson-Lucy")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("3D CARE")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("confocal_3d_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/confocal_3d.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/confocal_3d_expanded.yaml`
