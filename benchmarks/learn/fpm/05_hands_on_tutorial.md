# 05 — Hands-On Tutorial: Fourier Ptychographic Microscopy (FPM)

This tutorial walks through running the PWM benchmark for Fourier Ptychographic Microscopy (FPM),
...
config_path = ROOT / "benchmarks" / "configs" / "fpm.yaml"
...
try:
    from pwm_core.recon.fpm_solver import run_fpm
    print("Solver loaded: Sequential Phase Retrieval")
except ImportError:
...
solvers_to_test = {
    "Sequential Phase Retrieval": ("pwm_core.recon.fpm_solver", "run_fpm"),
    "Gradient Descent FPM": ("pwm_core.recon.fpm_solver", "gradient_descent_fpm_recon"),
}
...
# axes[1].set_title("Sequential Phase Retrieval")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Gradient Descent FPM")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("fpm_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/fpm.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/fpm_expanded.yaml`
