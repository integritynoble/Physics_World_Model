# 05 — Hands-On Tutorial: Scanning Electron Microscopy (SEM)

This tutorial walks through running the PWM benchmark for Scanning Electron Microscopy (SEM),
...
config_path = ROOT / "benchmarks" / "configs" / "sem.yaml"
...
try:
    ...
    print("Solver loaded: Richardson-Lucy (SEM)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality sem
...
solvers_to_test = {
    "Richardson-Lucy (SEM)": ("pwm_core.recon.richardson_lucy", "run_richardson_lucy"),
...
# axes[1].set_title("Richardson-Lucy (SEM)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("sem_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/sem.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/sem_expanded.yaml`
