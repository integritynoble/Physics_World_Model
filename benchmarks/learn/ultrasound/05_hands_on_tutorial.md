# 05 — Hands-On Tutorial: Ultrasound B-mode Imaging

This tutorial walks through running the PWM benchmark for Ultrasound B-mode Imaging,
...
config_path = ROOT / "benchmarks" / "configs" / "ultrasound.yaml"
...
try:
    ...
    print("Solver loaded: FISTA-L2 (ultrasound)")
except ImportError:
...
solvers_to_test = {
    "FISTA-L2 (ultrasound)": ("pwm_core.recon.classical", "run_fista_l2"),
...
# axes[1].set_title("FISTA-L2 (ultrasound)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("ultrasound_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/ultrasound.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/ultrasound_expanded.yaml`
