# 03 — Reconstruction Algorithms: Functional MRI (BOLD fMRI)
...
The default solver for Functional MRI (BOLD fMRI) is **`sense`**.
...
|------|------|--------|----------|-----|-----------|
| traditional_cpu | SENSE (fMRI) | `pwm_core.recon.mri_solvers` | `run_sense` | No |  |
...
3. Add the solver tier to the modality config in `benchmarks/configs/fmri.yaml`
