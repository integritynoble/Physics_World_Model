# 05 — Hands-On Tutorial: Functional MRI (BOLD fMRI)

This tutorial walks through running the PWM benchmark for Functional MRI (BOLD fMRI),
...
config_path = ROOT / "benchmarks" / "configs" / "fmri.yaml"
...
try:
    ...
    print("Solver loaded: SENSE (fMRI)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality fmri
...
solvers_to_test = {
    "SENSE (fMRI)": ("pwm_core.recon.mri_solvers", "run_sense"),
...
# axes[1].set_title("SENSE (fMRI)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("fmri_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/fmri.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/fmri_expanded.yaml`
