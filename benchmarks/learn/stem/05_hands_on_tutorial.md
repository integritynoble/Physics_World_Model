# 05 — Hands-On Tutorial: Scanning Transmission Electron Microscopy (STEM)

This tutorial walks through running the PWM benchmark for Scanning Transmission Electron Microscopy (STEM),
...
config_path = ROOT / "benchmarks" / "configs" / "stem.yaml"
...
try:
    ...
    print("Solver loaded: Richardson-Lucy (STEM)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality stem
...
solvers_to_test = {
    "Richardson-Lucy (STEM)": ("pwm_core.recon.richardson_lucy", "run_richardson_lucy"),
...
# axes[1].set_title("Richardson-Lucy (STEM)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("stem_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/stem.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/stem_expanded.yaml`
