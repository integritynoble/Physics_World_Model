# 05 — Hands-On Tutorial: Fluoroscopy

This tutorial walks through running the PWM benchmark for Fluoroscopy,
...
config_path = ROOT / "benchmarks" / "configs" / "fluoroscopy.yaml"
...
try:
    ...
    print("Solver loaded: FBP (fluoroscopy)")
except ImportError:
...
solvers_to_test = {
    "FBP (fluoroscopy)": ("pwm_core.recon.ct_solvers", "run_fbp"),
...
# axes[1].set_title("FBP (fluoroscopy)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("fluoroscopy_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/fluoroscopy.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/fluoroscopy_expanded.yaml`
