...
def gap_denoise(y, Phi, A, At, _lambda=1, accelerate=True, 
    ...
    model = net()
    model.load_state_dict(torch.load(r'/home/spiritai/PnP-CASSI-main/check_points/deep_denoiser.pth'))
    model.eval()
...
def admm_denoise(y, Phi, A, At, _lambda=1, gamma=0.01,
    ...
    model.load_state_dict(
        torch.load(r'/home/spiritai/PnP-CASSI-main/check_points/deep_denoiser.pth'))
    model.eval()
    for q, v in model.named_parameters():
        v.requires_grad = False
    model = model.to(device)
