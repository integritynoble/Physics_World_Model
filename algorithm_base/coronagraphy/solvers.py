"""Solvers for Stellar Coronagraphy (coronagraphy).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.coronagraphy.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class CoronagraphyOperator:
    """Generic PSF-convolution operator for Stellar Coronagraphy."""
    ...
    def info(self):
        return {"modality": "coronagraphy", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return CoronagraphyOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for coronagraphy."""
    ...
    try:
        if spec["module"] == "algorithm_base.coronagraphy.solvers":
...
def list_solvers():
    """List all available solvers for coronagraphy."""
