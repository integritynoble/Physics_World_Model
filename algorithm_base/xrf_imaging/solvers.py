"""Solvers for X-ray Fluorescence (XRF) Imaging (xrf_imaging).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.xrf_imaging.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class XrfImagingOperator:
    """Generic PSF-convolution operator for X-ray Fluorescence (XRF) Imaging."""
    ...
    def info(self):
        return {"modality": "xrf_imaging", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return XrfImagingOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for xrf_imaging."""
    ...
    try:
        if spec["module"] == "algorithm_base.xrf_imaging.solvers":
...
def list_solvers():
    """List all available solvers for xrf_imaging."""
