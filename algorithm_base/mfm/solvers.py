"""Solvers for Magnetic Force Microscopy (MFM) (mfm).
...
SOLVERS = {
    ...
    "mfm_dl": {
        "name": "MFM-UNet",
        "module": "pwm_core.recon.mfm_solvers",
        "function": "mfm_dl_recon",
        "gpu": True,
        "reference": "Kim, M. et al. (2021) DL for magnetic force microscopy, npj Comput. Mater. 7:87",
    },
    # ── Classical (1949-1974) ──
    "wiener": {
        "name": "Wiener Deconvolution",
        "module": "algorithm_base.mfm.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class MfmOperator:
    """Generic PSF-convolution operator for Magnetic Force Microscopy (MFM)."""
    ...
    def info(self):
        return {"modality": "mfm", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return MfmOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.mfm.solvers":
...
def list_solvers():
    """List all available solvers for mfm."""
