"""Solvers for Functional Near-Infrared Spectroscopy (fNIRS) (nirs_brain).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.nirs_brain.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class NirsBrainOperator:
    """Generic PSF-convolution operator for Functional Near-Infrared Spectroscopy (fNIRS)."""
    ...
    def info(self):
        return {"modality": "nirs_brain", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return NirsBrainOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for nirs_brain."""
    ...
    try:
        if spec["module"] == "algorithm_base.nirs_brain.solvers":
...
def list_solvers():
    """List all available solvers for nirs_brain."""
