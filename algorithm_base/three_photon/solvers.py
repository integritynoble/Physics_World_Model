"""Solvers for Three-Photon Microscopy (three_photon).
...
SOLVERS = {
    ...
    "3p_dl": {
        "name": "3P-Net (CARE)",
        "module": "pwm_core.recon.three_photon_solvers",
        "function": "three_photon_care_recon",
        "gpu": True,
        "reference": "Weigert, M. et al. (2018) CARE for 3P deep tissue imaging, Nature Methods 15:1090",
    },
    # ── Classical (1949-1974) ──
    "wiener": {
        "name": "Wiener Deconvolution",
        "module": "algorithm_base.three_photon.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class ThreePhotonOperator:
    """Generic PSF-convolution operator for Three-Photon Microscopy."""
    ...
    def info(self):
        return {"modality": "three_photon", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return ThreePhotonOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for three_photon."""
    ...
    try:
        if spec["module"] == "algorithm_base.three_photon.solvers":
...
def list_solvers():
    """List all available solvers for three_photon."""
