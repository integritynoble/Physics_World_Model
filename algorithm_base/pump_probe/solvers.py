"""Solvers for Pump-Probe Microscopy (pump_probe).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.pump_probe.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class PumpProbeOperator:
    """Generic PSF-convolution operator for Pump-Probe Microscopy."""
    ...
    def info(self):
        return {"modality": "pump_probe", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return PumpProbeOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for pump_probe."""
    ...
    try:
        if spec["module"] == "algorithm_base.pump_probe.solvers":
...
def list_solvers():
    """List all available solvers for pump_probe."""
