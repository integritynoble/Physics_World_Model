"""Solvers for Stimulated Raman Scattering (SRS) Microscopy (srs).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.srs.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class SrsOperator:
    """Generic PSF-convolution operator for Stimulated Raman Scattering (SRS) Microscopy."""
    ...
    def info(self):
        return {"modality": "srs", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return SrsOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.srs.solvers":
...
def list_solvers():
    """List all available solvers for srs."""
