...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.electron_diffraction.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class ElectronDiffractionOperator:
    """Generic PSF-convolution operator for 4D-STEM Electron Diffraction."""
    ...
    def info(self):
        return {"modality": "electron_diffraction", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return ElectronDiffractionOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for electron_diffraction."""
    ...
    try:
        if spec["module"] == "algorithm_base.electron_diffraction.solvers":
