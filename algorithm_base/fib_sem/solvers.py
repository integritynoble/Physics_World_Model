"""Solvers for Focused Ion Beam SEM (FIB-SEM) (fib_sem).
...
SOLVERS = {
    ...
    "fibsem_dl": {
        "name": "FIB-SEM-Net",
        "module": "pwm_core.recon.fibsem_solvers",
        "function": "fibsem_dl_recon",
        "gpu": True,
        "reference": "Heinrich, L. et al. (2021) Whole-cell organelle segmentation in volume EM, Nature 599:141",
    },
    # ── Classical (1949-1974) ──
    "wiener": {
        "name": "Wiener Deconvolution",
        "module": "algorithm_base.fib_sem.solvers",
        "function": "run_wiener",
        "gpu": False,
    ...
    "dl_3dcnn": {
        "name": "3D-CNN",
        "module": "algorithm_base.fib_sem.solvers",
        "function": "run_dl_3dcnn",
        "gpu": True,
        "reference": "3D CNN reconstruction, 2018",
    },
    "dl_nerf_dl": {
        "name": "NeRF-DL",
        "module": "algorithm_base.fib_sem.solvers",
        "function": "run_dl_nerf_dl",
        "gpu": True,
        "reference": "Neural rendering, 2020",
    },
    "dl_transformer": {
        "name": "3D-Transformer",
        "module": "algorithm_base.fib_sem.solvers",
        ...
        "gpu": True,
        "reference": "Transformer for 3D, 2023",
    },
    "dl_diffusion": {
        "name": "3D-Diffusion",
        "module": "algorithm_base.fib_sem.solvers",
        ...
        "gpu": True,
        "reference": "Diffusion for 3D reconstruction, 2025",
    },
}
...
class FibSemOperator:
    """Generic PSF-convolution operator for Focused Ion Beam SEM (FIB-SEM)."""
    ...
    def info(self):
        return {"modality": "fib_sem", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return FibSemOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for fib_sem."""
    ...
    try:
        if spec["module"] == "algorithm_base.fib_sem.solvers":
...
def run_dl_3dcnn(y, physics, cfg=None):
    """3D-CNN (3D CNN reconstruction, 2018). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_3dcnn")

def run_dl_nerf_dl(y, physics, cfg=None):
    """NeRF-DL (Neural rendering, 2020). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_nerf_dl")

def run_dl_transformer(y, physics, cfg=None):
    """3D-Transformer (Transformer for 3D, 2023). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_transformer")

def run_dl_diffusion(y, physics, cfg=None):
    """3D-Diffusion (Diffusion for 3D reconstruction, 2025). Fallback: Wiener + NLM."""
...
def list_solvers():
    """List all available solvers for fib_sem."""
