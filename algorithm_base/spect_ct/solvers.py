"""Solvers for SPECT/CT Fusion (spect_ct).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.spect_ct.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class SpectCtOperator:
    """Generic PSF-convolution operator for SPECT/CT Fusion."""
    ...
    def info(self):
        return {"modality": "spect_ct", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return SpectCtOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for spect_ct."""
    ...
    try:
        if spec["module"] == "algorithm_base.spect_ct.solvers":
...
def list_solvers():
    """List all available solvers for spect_ct."""
