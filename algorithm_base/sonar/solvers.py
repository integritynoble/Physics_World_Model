"""Solvers for Sonar Imaging (sonar).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.sonar.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class SonarOperator:
    """Generic PSF-convolution operator for Sonar Imaging."""
    ...
    def info(self):
        return {"modality": "sonar", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return SonarOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.sonar.solvers":
...
def list_solvers():
    """List all available solvers for sonar."""
