"""Solvers for Streak Camera Imaging (streak_camera).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.streak_camera.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class StreakCameraOperator:
    """Generic PSF-convolution operator for Streak Camera Imaging."""
    ...
    def info(self):
        return {"modality": "streak_camera", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return StreakCameraOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for streak_camera."""
    ...
    try:
        if spec["module"] == "algorithm_base.streak_camera.solvers":
...
def list_solvers():
    """List all available solvers for streak_camera."""
