"""Solvers for Neutron Radiography / Tomography (neutron_tomo).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.neutron_tomo.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class NeutronTomoOperator:
    """Generic PSF-convolution operator for Neutron Radiography / Tomography."""
    ...
    def info(self):
        return {"modality": "neutron_tomo", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return NeutronTomoOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for neutron_tomo."""
    ...
    try:
        if spec["module"] == "algorithm_base.neutron_tomo.solvers":
...
def list_solvers():
    """List all available solvers for neutron_tomo."""
