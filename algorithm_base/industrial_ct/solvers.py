"""Solvers for Industrial X-ray CT (industrial_ct).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.industrial_ct.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class IndustrialCtOperator:
    """Generic PSF-convolution operator for Industrial X-ray CT."""
    ...
    def info(self):
        return {"modality": "industrial_ct", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return IndustrialCtOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for industrial_ct."""
    ...
    try:
        if spec["module"] == "algorithm_base.industrial_ct.solvers":
...
def list_solvers():
    """List all available solvers for industrial_ct."""
