"""Solvers for Secondary Ion Mass Spectrometry (SIMS) Imaging (sims).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.sims.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class SimsOperator:
    """Generic PSF-convolution operator for Secondary Ion Mass Spectrometry (SIMS) Imaging."""
    ...
    def info(self):
        return {"modality": "sims", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return SimsOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.sims.solvers":
...
def list_solvers():
    """List all available solvers for sims."""
