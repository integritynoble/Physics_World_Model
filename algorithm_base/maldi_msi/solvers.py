"""Solvers for MALDI Mass Spectrometry Imaging (maldi_msi).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.maldi_msi.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class MaldiMsiOperator:
    """Generic PSF-convolution operator for MALDI Mass Spectrometry Imaging."""
    ...
    def info(self):
        return {"modality": "maldi_msi", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return MaldiMsiOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for maldi_msi."""
    ...
    try:
        if spec["module"] == "algorithm_base.maldi_msi.solvers":
...
def list_solvers():
    """List all available solvers for maldi_msi."""
