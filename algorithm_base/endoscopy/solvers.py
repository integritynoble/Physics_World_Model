"""Solvers for Fiber Bundle Endoscopy (endoscopy).
...
SOLVERS = {
    ...
    "best_quality": {
        "name": "EndoMapper-Net",
        "module": "pwm_core.recon.endoscopy_solvers",
        "function": "endomapper_recon",
        "gpu": True,
        "reference": "Ozyoruk, K.B. et al. (2021) EndoMapper, Nat. Mach. Intel. 3",
    },
    # Original
    "famous_dl": {
        "name": "AF-SfMLearner",
        "module": "pwm_core.recon.endoscopy_solvers",
        "function": "af_sfm_learner_recon",
        "gpu": True,
        "reference": "Shao, S. et al. (2022) Self-supervised depth estimation in endoscopy, MICCAI 2022",
    },
    # ── Classical (1949-1974) ──
    "wiener": {
        "name": "Wiener Deconvolution",
        "module": "algorithm_base.endoscopy.solvers",
        "function": "run_wiener",
        "gpu": False,
    ...
    "dl_swinir": {
        "name": "SwinIR-Med",
        "module": "algorithm_base.endoscopy.solvers",
        "function": "run_dl_swinir",
        "gpu": True,
        "reference": "Liang et al., ICCV 2021",
    },
    "dl_diffusion": {
        "name": "DiffusionMed",
        "module": "algorithm_base.endoscopy.solvers",
        ...
        "gpu": True,
        "reference": "Diffusion model for medical imaging, 2024",
    },
    "dl_mamba": {
        "name": "MedMamba",
        "module": "algorithm_base.endoscopy.solvers",
        ...
        "gpu": True,
        "reference": "SSM for medical imaging, 2026",
    },
}
...
class EndoscopyOperator:
    """Generic PSF-convolution operator for Fiber Bundle Endoscopy."""
    ...
    def info(self):
        return {"modality": "endoscopy", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return EndoscopyOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for endoscopy."""
    ...
    try:
        if spec["module"] == "algorithm_base.endoscopy.solvers":
...
def run_dl_unet(y, physics, cfg=None):
    """Med-UNet (Ronneberger et al., MICCAI 2015). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_unet")

def run_dl_swinir(y, physics, cfg=None):
    """SwinIR-Med (Liang et al., ICCV 2021). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_swinir")

def run_dl_diffusion(y, physics, cfg=None):
    """DiffusionMed (Diffusion model for medical imaging, 2024). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_diffusion")

def run_dl_mamba(y, physics, cfg=None):
    """MedMamba (SSM for medical imaging, 2026). Fallback: Wiener + NLM."""
...
def list_solvers():
    """List all available solvers for endoscopy."""
