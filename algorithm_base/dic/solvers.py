"""Solvers for Differential Interference Contrast (DIC) (dic).
...
SOLVERS = {
    ...
    "dic_dl": {
        "name": "DIC-Net",
        "module": "pwm_core.recon.dic_solvers",
        "function": "dic_dl_recon",
        "gpu": True,
        "reference": "Mir, A. et al. (2015) Automated DIC microscopy, J. Microsc. 257(2)",
    },
    # ── Classical (1949-1974) ──
    "wiener": {
        "name": "Wiener Deconvolution",
        "module": "algorithm_base.dic.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class DicOperator:
    """Generic PSF-convolution operator for Differential Interference Contrast (DIC)."""
    ...
    def info(self):
        return {"modality": "dic", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return DicOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.dic.solvers":
...
def list_solvers():
    """List all available solvers for dic."""
