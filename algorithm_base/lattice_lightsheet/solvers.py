"""Solvers for Lattice Light-Sheet Microscopy (lattice_lightsheet).
...
SOLVERS = {
    ...
    "llsm_dl": {
        "name": "LLSM-CARE",
        "module": "pwm_core.recon.llsm_solvers",
        "function": "llsm_care_recon",
        "gpu": True,
        "reference": "Weigert, M. et al. (2018) Content-aware restoration for lattice light-sheet, Nature Methods 15:1090",
    },
    # ── Classical (1949-1974) ──
    "wiener": {
        "name": "Wiener Deconvolution",
        "module": "algorithm_base.lattice_lightsheet.solvers",
        "function": "run_wiener",
        "gpu": False,
    ...
    "dl_care": {
        ...
        "gpu": True,
        "reference": "Weigert et al., Nat Methods 2018",
    },
    "dl_n2v": {
        "name": "Noise2Void",
        "module": "algorithm_base.lattice_lightsheet.solvers",
    ...
    "dl_restormer": {
        "name": "Restormer",
        "module": "algorithm_base.lattice_lightsheet.solvers",
        "function": "run_dl_restormer",
        "gpu": True,
        "reference": "Zamir et al., CVPR 2022",
    },
    "dl_diffusion": {
        "name": "DiffusionMicro",
        "module": "algorithm_base.lattice_lightsheet.solvers",
        ...
        "gpu": True,
        "reference": "Diffusion-based microscopy, 2025",
    },
}
...
class LatticeLightsheetOperator:
    """Generic PSF-convolution operator for Lattice Light-Sheet Microscopy."""
    ...
    def info(self):
        return {"modality": "lattice_lightsheet", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return LatticeLightsheetOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for lattice_lightsheet."""
    ...
    try:
        if spec["module"] == "algorithm_base.lattice_lightsheet.solvers":
...
def run_dl_care(y, physics, cfg=None):
    """CARE (Weigert et al., Nat Methods 2018). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_care")

def run_dl_n2v(y, physics, cfg=None):
    """Noise2Void (Krull et al., CVPR 2019). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_n2v")

def run_dl_restormer(y, physics, cfg=None):
    """Restormer (Zamir et al., CVPR 2022). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_restormer")

def run_dl_diffusion(y, physics, cfg=None):
    """DiffusionMicro (Diffusion-based microscopy, 2025). Fallback: Wiener + NLM."""
...
def list_solvers():
    """List all available solvers for lattice_lightsheet."""
