"""Solvers for Passive Microwave Radiometry (passive_microwave).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.passive_microwave.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class PassiveMicrowaveOperator:
    """Generic PSF-convolution operator for Passive Microwave Radiometry."""
    ...
    def info(self):
        return {"modality": "passive_microwave", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return PassiveMicrowaveOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for passive_microwave."""
    ...
    try:
        if spec["module"] == "algorithm_base.passive_microwave.solvers":
...
def list_solvers():
    """List all available solvers for passive_microwave."""
