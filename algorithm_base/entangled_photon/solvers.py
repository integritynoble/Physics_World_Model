"""Solvers for Entangled Photon Microscopy (entangled_photon).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.entangled_photon.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class EntangledPhotonOperator:
    """Generic PSF-convolution operator for Entangled Photon Microscopy."""
    ...
    def info(self):
        return {"modality": "entangled_photon", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return EntangledPhotonOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for entangled_photon."""
    ...
    try:
        if spec["module"] == "algorithm_base.entangled_photon.solvers":
...
def list_solvers():
    """List all available solvers for entangled_photon."""
