"""Solvers for Intravascular Ultrasound (IVUS) (ivus).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.ivus.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class IvusOperator:
    """Generic PSF-convolution operator for Intravascular Ultrasound (IVUS)."""
    ...
    def info(self):
        return {"modality": "ivus", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return IvusOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.ivus.solvers":
...
def list_solvers():
    """List all available solvers for ivus."""
