"""Solvers for Interferometric SAR (InSAR) (insar).
...
class InsarOperator:
    """Generic PSF-convolution operator for Interferometric SAR (InSAR)."""
    ...
    def info(self):
        return {"modality": "insar", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return InsarOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.insar.solvers":
...
def list_solvers():
    """List all available solvers for insar."""
