"""Solvers for LiDAR Scanner (lidar).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.lidar.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class LidarOperator:
    """Generic PSF-convolution operator for LiDAR Scanner."""
    ...
    def info(self):
        return {"modality": "lidar", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return LidarOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.lidar.solvers":
...
def list_solvers():
    """List all available solvers for lidar."""
