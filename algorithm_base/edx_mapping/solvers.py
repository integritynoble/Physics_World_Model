...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.edx_mapping.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class EdxMappingOperator:
    """Generic PSF-convolution operator for STEM-EDX Elemental Mapping."""
    ...
    def info(self):
        return {"modality": "edx_mapping", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return EdxMappingOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for edx_mapping."""
    ...
    try:
        if spec["module"] == "algorithm_base.edx_mapping.solvers":
