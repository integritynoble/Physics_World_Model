"""Solvers for Portal Imaging (EPID) (portal_imaging).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.portal_imaging.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class PortalImagingOperator:
    """Generic PSF-convolution operator for Portal Imaging (EPID)."""
    ...
    def info(self):
        return {"modality": "portal_imaging", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return PortalImagingOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for portal_imaging."""
    ...
    try:
        if spec["module"] == "algorithm_base.portal_imaging.solvers":
...
def list_solvers():
    """List all available solvers for portal_imaging."""
