"""Solvers for Electron Holography (electron_holography).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.electron_holography.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class ElectronHolographyOperator:
    """Generic PSF-convolution operator for Electron Holography."""
    ...
    def info(self):
        return {"modality": "electron_holography", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return ElectronHolographyOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for electron_holography."""
    ...
    try:
        if spec["module"] == "algorithm_base.electron_holography.solvers":
...
def list_solvers():
    """List all available solvers for electron_holography."""
