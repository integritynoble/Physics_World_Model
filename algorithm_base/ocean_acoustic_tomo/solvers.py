"""Solvers for Ocean Acoustic Tomography (ocean_acoustic_tomo).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.ocean_acoustic_tomo.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class OceanAcousticTomoOperator:
    """Generic PSF-convolution operator for Ocean Acoustic Tomography."""
    ...
    def info(self):
        return {"modality": "ocean_acoustic_tomo", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return OceanAcousticTomoOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for ocean_acoustic_tomo."""
    ...
    try:
        if spec["module"] == "algorithm_base.ocean_acoustic_tomo.solvers":
...
def list_solvers():
    """List all available solvers for ocean_acoustic_tomo."""
