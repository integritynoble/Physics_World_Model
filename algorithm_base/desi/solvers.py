"""Solvers for DESI Mass Spectrometry Imaging (desi).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.desi.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class DesiOperator:
    """Generic PSF-convolution operator for DESI Mass Spectrometry Imaging."""
    ...
    def info(self):
        return {"modality": "desi", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return DesiOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.desi.solvers":
...
def list_solvers():
    """List all available solvers for desi."""
