"""Solvers for X-ray Angiography (angiography).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.angiography.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class AngiographyOperator:
    """Generic PSF-convolution operator for X-ray Angiography."""
    ...
    def info(self):
        return {"modality": "angiography", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return AngiographyOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for angiography."""
    ...
    try:
        if spec["module"] == "algorithm_base.angiography.solvers":
...
def list_solvers():
    """List all available solvers for angiography."""
