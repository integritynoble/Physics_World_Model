...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.two_photon.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class TwoPhotonOperator:
    """Generic PSF-convolution operator for Two-Photon / Multiphoton Microscopy."""
    ...
    def info(self):
        return {"modality": "two_photon", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return TwoPhotonOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for two_photon."""
    ...
    try:
        if spec["module"] == "algorithm_base.two_photon.solvers":
