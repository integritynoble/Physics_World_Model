"""Solvers for Confocal Laser Endomicroscopy (CLE) (confocal_endomicroscopy).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.confocal_endomicroscopy.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class ConfocalEndomicroscopyOperator:
    """Generic PSF-convolution operator for Confocal Laser Endomicroscopy (CLE)."""
    ...
    def info(self):
        return {"modality": "confocal_endomicroscopy", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return ConfocalEndomicroscopyOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for confocal_endomicroscopy."""
    ...
    try:
        if spec["module"] == "algorithm_base.confocal_endomicroscopy.solvers":
...
def list_solvers():
    """List all available solvers for confocal_endomicroscopy."""
