"""Solvers for X-ray Fluorescence Tomography (xrf_tomo).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.xrf_tomo.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class XrfTomoOperator:
    """Generic PSF-convolution operator for X-ray Fluorescence Tomography."""
    ...
    def info(self):
        return {"modality": "xrf_tomo", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return XrfTomoOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for xrf_tomo."""
    ...
    try:
        if spec["module"] == "algorithm_base.xrf_tomo.solvers":
...
def list_solvers():
    """List all available solvers for xrf_tomo."""
