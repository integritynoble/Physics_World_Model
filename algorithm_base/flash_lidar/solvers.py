"""Solvers for Flash LiDAR (flash_lidar).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.flash_lidar.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class FlashLidarOperator:
    """Generic PSF-convolution operator for Flash LiDAR."""
    ...
    def info(self):
        return {"modality": "flash_lidar", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return FlashLidarOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for flash_lidar."""
    ...
    try:
        if spec["module"] == "algorithm_base.flash_lidar.solvers":
...
def list_solvers():
    """List all available solvers for flash_lidar."""
