...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.cryo_et.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class CryoEtOperator:
    """Generic PSF-convolution operator for Cryo-Electron Tomography (Cryo-ET)."""
    ...
    def info(self):
        return {"modality": "cryo_et", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return CryoEtOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for cryo_et."""
    ...
    try:
        if spec["module"] == "algorithm_base.cryo_et.solvers":
