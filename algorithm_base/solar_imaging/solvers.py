"""Solvers for Solar EUV/X-ray Imaging (solar_imaging).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.solar_imaging.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class SolarImagingOperator:
    """Generic PSF-convolution operator for Solar EUV/X-ray Imaging."""
    ...
    def info(self):
        return {"modality": "solar_imaging", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return SolarImagingOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for solar_imaging."""
    ...
    try:
        if spec["module"] == "algorithm_base.solar_imaging.solvers":
...
def list_solvers():
    """List all available solvers for solar_imaging."""
