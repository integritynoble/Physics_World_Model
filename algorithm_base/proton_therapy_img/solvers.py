"""Solvers for Proton Therapy Imaging (proton_therapy_img).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.proton_therapy_img.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class ProtonTherapyImgOperator:
    """Generic PSF-convolution operator for Proton Therapy Imaging."""
    ...
    def info(self):
        return {"modality": "proton_therapy_img", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return ProtonTherapyImgOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for proton_therapy_img."""
    ...
    try:
        if spec["module"] == "algorithm_base.proton_therapy_img.solvers":
...
def list_solvers():
    """List all available solvers for proton_therapy_img."""
