"""Solvers for Diffusion MRI (DTI) (diffusion_mri).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.diffusion_mri.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class DiffusionMriOperator:
    """Generic PSF-convolution operator for Diffusion MRI (DTI)."""
    ...
    def info(self):
        return {"modality": "diffusion_mri", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return DiffusionMriOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for diffusion_mri."""
    ...
    try:
        if spec["module"] == "algorithm_base.diffusion_mri.solvers":
...
def list_solvers():
    """List all available solvers for diffusion_mri."""
