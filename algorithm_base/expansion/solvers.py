...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.expansion.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class ExpansionOperator:
    """Generic PSF-convolution operator for Expansion Microscopy (ExM)."""
    ...
    def info(self):
        return {"modality": "expansion", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return ExpansionOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for expansion."""
    ...
    try:
        if spec["module"] == "algorithm_base.expansion.solvers":
