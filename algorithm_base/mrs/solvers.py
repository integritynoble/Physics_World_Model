"""Solvers for MR Spectroscopy (MRS) (mrs).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.mrs.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class MrsOperator:
    """Generic PSF-convolution operator for MR Spectroscopy (MRS)."""
    ...
    def info(self):
        return {"modality": "mrs", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return MrsOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.mrs.solvers":
...
def list_solvers():
    """List all available solvers for mrs."""
