"""Solvers for Electrical Impedance Tomography (EIT) (impedance_tomo).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.impedance_tomo.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class ImpedanceTomoOperator:
    """Generic PSF-convolution operator for Electrical Impedance Tomography (EIT)."""
    ...
    def info(self):
        return {"modality": "impedance_tomo", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return ImpedanceTomoOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for impedance_tomo."""
    ...
    try:
        if spec["module"] == "algorithm_base.impedance_tomo.solvers":
...
def list_solvers():
    """List all available solvers for impedance_tomo."""
