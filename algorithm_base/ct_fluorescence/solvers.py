"""Solvers for CT + Fluorescence (FLIT) (ct_fluorescence).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.ct_fluorescence.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class CtFluorescenceOperator:
    """Generic PSF-convolution operator for CT + Fluorescence (FLIT)."""
    ...
    def info(self):
        return {"modality": "ct_fluorescence", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return CtFluorescenceOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for ct_fluorescence."""
    ...
    try:
        if spec["module"] == "algorithm_base.ct_fluorescence.solvers":
...
def list_solvers():
    """List all available solvers for ct_fluorescence."""
