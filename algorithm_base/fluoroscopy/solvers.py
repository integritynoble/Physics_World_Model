"""Solvers for Fluoroscopy (fluoroscopy).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.fluoroscopy.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class FluoroscopyOperator:
    """Generic PSF-convolution operator for Fluoroscopy."""
    ...
    def info(self):
        return {"modality": "fluoroscopy", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return FluoroscopyOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for fluoroscopy."""
    ...
    try:
        if spec["module"] == "algorithm_base.fluoroscopy.solvers":
...
def list_solvers():
    """List all available solvers for fluoroscopy."""
