"""Solvers for Polarization Microscopy (polarization).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.polarization.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class PolarizationOperator:
    """Generic PSF-convolution operator for Polarization Microscopy."""
    ...
    def info(self):
        return {"modality": "polarization", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return PolarizationOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for polarization."""
    ...
    try:
        if spec["module"] == "algorithm_base.polarization.solvers":
...
def list_solvers():
    """List all available solvers for polarization."""
