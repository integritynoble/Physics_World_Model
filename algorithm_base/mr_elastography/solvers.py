"""Solvers for MR Elastography (MRE) (mr_elastography).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.mr_elastography.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class MrElastographyOperator:
    """Generic PSF-convolution operator for MR Elastography (MRE)."""
    ...
    def info(self):
        return {"modality": "mr_elastography", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return MrElastographyOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for mr_elastography."""
    ...
    try:
        if spec["module"] == "algorithm_base.mr_elastography.solvers":
...
def list_solvers():
    """List all available solvers for mr_elastography."""
