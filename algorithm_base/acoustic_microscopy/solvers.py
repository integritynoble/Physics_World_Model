"""Solvers for Scanning Acoustic Microscopy (SAM) (acoustic_microscopy).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.acoustic_microscopy.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class AcousticMicroscopyOperator:
    """Generic PSF-convolution operator for Scanning Acoustic Microscopy (SAM)."""
    ...
    def info(self):
        return {"modality": "acoustic_microscopy", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return AcousticMicroscopyOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for acoustic_microscopy."""
    ...
    try:
        if spec["module"] == "algorithm_base.acoustic_microscopy.solvers":
...
def list_solvers():
    """List all available solvers for acoustic_microscopy."""
