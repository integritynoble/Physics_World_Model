"""Solvers for Particle Calorimetry (particle_calorimetry).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.particle_calorimetry.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class ParticleCalorimetryOperator:
    """Generic PSF-convolution operator for Particle Calorimetry."""
    ...
    def info(self):
        return {"modality": "particle_calorimetry", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return ParticleCalorimetryOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for particle_calorimetry."""
    ...
    try:
        if spec["module"] == "algorithm_base.particle_calorimetry.solvers":
...
def list_solvers():
    """List all available solvers for particle_calorimetry."""
