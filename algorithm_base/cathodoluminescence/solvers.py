"""Solvers for Cathodoluminescence (CL) Imaging (cathodoluminescence).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.cathodoluminescence.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class CathodoluminescenceOperator:
    """Generic PSF-convolution operator for Cathodoluminescence (CL) Imaging."""
    ...
    def info(self):
        return {"modality": "cathodoluminescence", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return CathodoluminescenceOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for cathodoluminescence."""
    ...
    try:
        if spec["module"] == "algorithm_base.cathodoluminescence.solvers":
...
def list_solvers():
    """List all available solvers for cathodoluminescence."""
