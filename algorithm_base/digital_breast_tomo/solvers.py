"""Solvers for Digital Breast Tomosynthesis (DBT) (digital_breast_tomo).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.digital_breast_tomo.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class DigitalBreastTomoOperator:
    """Generic PSF-convolution operator for Digital Breast Tomosynthesis (DBT)."""
    ...
    def info(self):
        return {"modality": "digital_breast_tomo", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return DigitalBreastTomoOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for digital_breast_tomo."""
    ...
    try:
        if spec["module"] == "algorithm_base.digital_breast_tomo.solvers":
...
def list_solvers():
    """List all available solvers for digital_breast_tomo."""
