"""Solvers for PET/CT Fusion (pet_ct).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.pet_ct.solvers",
        "function": "run_wiener",
        "gpu": False,
...
# ---------------------------------------------------------------------------
# CT forward/adjoint operator for PET/CT (radon / iradon based)
# ---------------------------------------------------------------------------

class PetCtOperator:
    """CT forward/adjoint operator for PET/CT using radon/iradon."""
    ...
    def info(self):
        return {"modality": "pet_ct", "x_shape": self.x_shape,
...
def _make_operator(y, cfg):
    ...
    return PetCtOperator(n_angles, n_det, output_size)
...
def _do_fbp_recon(y, physics=None, cfg=None):
    """FBP reconstruction from sinogram for PET/CT data."""
    from skimage.transform import iradon
    cfg = cfg or {}
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for pet_ct.
    ...
    try:
        if spec["module"] == "algorithm_base.pet_ct.solvers":
...
def run_dl_transformer(y, physics, cfg=None):
    """TransFuse-PET/CT (Wang et al., IEEE TMI 2023). Fallback: FBP + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_transformer")

def run_dl_diffusion(y, physics, cfg=None):
    """DiffusionFusion-PET/CT (Song et al., 2024). Fallback: FBP + NLM."""
...
def list_solvers():
    """List all available solvers for pet_ct."""
...
def run_petct_dl(y, operator=None, cfg=None):
    """PET-CT-Fusion-Net [proxy]. CPU only."""
    return run_solver("petct_dl", y, operator, cfg)
