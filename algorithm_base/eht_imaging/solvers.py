"""Solvers for Event Horizon Telescope (EHT) Imaging (eht_imaging).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.eht_imaging.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class EhtImagingOperator:
    """Generic PSF-convolution operator for Event Horizon Telescope (EHT) Imaging."""
    ...
    def info(self):
        return {"modality": "eht_imaging", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return EhtImagingOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for eht_imaging."""
    ...
    try:
        if spec["module"] == "algorithm_base.eht_imaging.solvers":
...
def list_solvers():
    """List all available solvers for eht_imaging."""
