"""Solvers for Magnetic Particle Imaging (MPI) (magnetic_particle).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.magnetic_particle.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class MagneticParticleOperator:
    """Generic PSF-convolution operator for Magnetic Particle Imaging (MPI)."""
    ...
    def info(self):
        return {"modality": "magnetic_particle", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return MagneticParticleOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for magnetic_particle."""
    ...
    try:
        if spec["module"] == "algorithm_base.magnetic_particle.solvers":
...
def list_solvers():
    """List all available solvers for magnetic_particle."""
