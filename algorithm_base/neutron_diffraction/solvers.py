"""Solvers for Neutron Diffraction (neutron_diffraction).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.neutron_diffraction.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class NeutronDiffractionOperator:
    """Generic PSF-convolution operator for Neutron Diffraction."""
    ...
    def info(self):
        return {"modality": "neutron_diffraction", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return NeutronDiffractionOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for neutron_diffraction."""
    ...
    try:
        if spec["module"] == "algorithm_base.neutron_diffraction.solvers":
...
def list_solvers():
    """List all available solvers for neutron_diffraction."""
