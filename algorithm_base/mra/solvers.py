"""Solvers for MR Angiography (MRA) (mra).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.mra.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class MraOperator:
    """Generic PSF-convolution operator for MR Angiography (MRA)."""
    ...
    def info(self):
        return {"modality": "mra", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return MraOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.mra.solvers":
...
def list_solvers():
    """List all available solvers for mra."""
