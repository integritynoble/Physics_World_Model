"""Solvers for Transmission Electron Microscopy (TEM) (tem).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.tem.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class TemOperator:
    """Generic PSF-convolution operator for Transmission Electron Microscopy (TEM)."""
    ...
    def info(self):
        return {"modality": "tem", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return TemOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.tem.solvers":
...
def list_solvers():
    """List all available solvers for tem."""
