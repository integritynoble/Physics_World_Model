...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    # NeRF TTO requires camera pose metadata. Without it, fall back to RL proxy.
    nerf_keys = {"traditional_cpu", "best_quality", "famous_dl", "small_gpu"}
    if solver_key in nerf_keys and operator is None:
        spec = SOLVERS["rl_proxy"]
        mod = importlib.import_module(spec["module"])
