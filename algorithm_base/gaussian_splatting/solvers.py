...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.gaussian_splatting.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class GaussianSplattingOperator:
    """Generic PSF-convolution operator for 3D Gaussian Splatting (3DGS)."""
    ...
    def info(self):
        return {"modality": "gaussian_splatting", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return GaussianSplattingOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for gaussian_splatting."""
    ...
    try:
        if spec["module"] == "algorithm_base.gaussian_splatting.solvers":
