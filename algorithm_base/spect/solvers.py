...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.spect.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class SpectOperator:
    """Generic PSF-convolution operator for Single Photon Emission CT (SPECT)."""
    ...
    def info(self):
        return {"modality": "spect", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return SpectOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.spect.solvers":
