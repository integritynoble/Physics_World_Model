"""Solvers for TIRF Microscopy (tirf).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.tirf.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class TirfOperator:
    """Generic PSF-convolution operator for TIRF Microscopy."""
    ...
    def info(self):
        return {"modality": "tirf", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return TirfOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.tirf.solvers":
...
def list_solvers():
    """List all available solvers for tirf."""
