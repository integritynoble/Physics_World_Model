"""Solvers for Dual-Energy X-ray Absorptiometry (DEXA) (dexa).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.dexa.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class DexaOperator:
    """Generic PSF-convolution operator for Dual-Energy X-ray Absorptiometry (DEXA)."""
    ...
    def info(self):
        return {"modality": "dexa", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return DexaOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.dexa.solvers":
...
def list_solvers():
    """List all available solvers for dexa."""
