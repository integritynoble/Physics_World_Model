"""Solvers for Scanning Tunneling Microscopy (STM) (stm).
...
SOLVERS = {
    ...
    "stm_dl": {
        "name": "STM-Net",
        "module": "pwm_core.recon.stm_solvers",
        "function": "stm_dl_recon",
        "gpu": True,
        "reference": "Ziatdinov, M. et al. (2021) DL for atomic-level STM, Nat. Mach. Intell. 3:269",
    },
    # ── Classical (1949-1974) ──
    "wiener": {
        "name": "Wiener Deconvolution",
        "module": "algorithm_base.stm.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class StmOperator:
    """Generic PSF-convolution operator for Scanning Tunneling Microscopy (STM)."""
    ...
    def info(self):
        return {"modality": "stm", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return StmOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.stm.solvers":
...
def list_solvers():
    """List all available solvers for stm."""
