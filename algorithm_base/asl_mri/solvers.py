"""Solvers for Arterial Spin Labeling (ASL) MRI (asl_mri).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.asl_mri.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class AslMriOperator:
    """Generic PSF-convolution operator for Arterial Spin Labeling (ASL) MRI."""
    ...
    def info(self):
        return {"modality": "asl_mri", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return AslMriOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for asl_mri."""
    ...
    try:
        if spec["module"] == "algorithm_base.asl_mri.solvers":
...
def list_solvers():
    """List all available solvers for asl_mri."""
