...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.cars.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class CarsOperator:
    """Generic PSF-convolution operator for Coherent Anti-Stokes Raman (CARS) Microscopy."""
    ...
    def info(self):
        return {"modality": "cars", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return CarsOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.cars.solvers":
