...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.cest_mri.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class CestMriOperator:
    """Generic PSF-convolution operator for CEST MRI."""
    ...
    def info(self):
        return {"modality": "cest_mri", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return CestMriOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for cest_mri."""
    ...
    try:
        if spec["module"] == "algorithm_base.cest_mri.solvers":
...
def list_solvers():
    """List all available solvers for cest_mri."""
