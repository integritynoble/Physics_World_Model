"""Solvers for MR Fingerprinting (MRF) (mr_fingerprinting).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.mr_fingerprinting.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class MrFingerprintingOperator:
    """Generic PSF-convolution operator for MR Fingerprinting (MRF)."""
    ...
    def info(self):
        return {"modality": "mr_fingerprinting", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return MrFingerprintingOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for mr_fingerprinting."""
    ...
    try:
        if spec["module"] == "algorithm_base.mr_fingerprinting.solvers":
...
def list_solvers():
    """List all available solvers for mr_fingerprinting."""
