"""Solvers for Ghost Imaging (ghost_imaging).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.ghost_imaging.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class GhostImagingOperator:
    """Generic PSF-convolution operator for Ghost Imaging."""
    ...
    def info(self):
        return {"modality": "ghost_imaging", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return GhostImagingOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for ghost_imaging."""
    ...
    try:
        if spec["module"] == "algorithm_base.ghost_imaging.solvers":
...
def list_solvers():
    """List all available solvers for ghost_imaging."""
