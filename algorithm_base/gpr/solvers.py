"""Solvers for Ground-Penetrating Radar (GPR) (gpr).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.gpr.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class GprOperator:
    """Generic PSF-convolution operator for Ground-Penetrating Radar (GPR)."""
    ...
    def info(self):
        return {"modality": "gpr", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return GprOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.gpr.solvers":
...
def list_solvers():
    """List all available solvers for gpr."""
