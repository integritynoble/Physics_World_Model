"""Solvers for Functional MRI (BOLD fMRI) (fmri).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.fmri.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class FmriOperator:
    """Generic PSF-convolution operator for Functional MRI (BOLD fMRI)."""
    ...
    def info(self):
        return {"modality": "fmri", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return FmriOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.fmri.solvers":
...
def list_solvers():
    """List all available solvers for fmri."""
