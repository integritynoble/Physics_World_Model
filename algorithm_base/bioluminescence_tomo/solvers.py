"""Solvers for Bioluminescence Tomography (BLT) (bioluminescence_tomo).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.bioluminescence_tomo.solvers",
        "function": "run_wiener",
        "gpu": False,
    ...
    "dl_mamba": {
        ...
        "gpu": True,
        "reference": "SSM for inverse problems, 2026",
    },
}
...
class BioluminescenceTomoOperator:
    """Generic PSF-convolution operator for Bioluminescence Tomography (BLT)."""
    ...
    def info(self):
        return {"modality": "bioluminescence_tomo", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return BioluminescenceTomoOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for bioluminescence_tomo."""
    ...
    try:
        if spec["module"] == "algorithm_base.bioluminescence_tomo.solvers":
...
def list_solvers():
    """List all available solvers for bioluminescence_tomo."""
