"""Solvers for Atom Probe Tomography (APT) (atom_probe).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.atom_probe.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class AtomProbeOperator:
    """Generic PSF-convolution operator for Atom Probe Tomography (APT)."""
    ...
    def info(self):
        return {"modality": "atom_probe", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return AtomProbeOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for atom_probe."""
    ...
    try:
        if spec["module"] == "algorithm_base.atom_probe.solvers":
...
def list_solvers():
    """List all available solvers for atom_probe."""
