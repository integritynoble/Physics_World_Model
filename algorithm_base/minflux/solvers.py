"""Solvers for MINFLUX Nanoscopy (minflux).
...
SOLVERS = {
    ...
    "minflux_dl": {
        "name": "MINFLUX-Net",
        "module": "pwm_core.recon.minflux_solvers",
        "function": "minflux_dl_recon",
        "gpu": True,
        "reference": "Gwosch, K.C. et al. (2020) MINFLUX nanoscopy 3D, Nature Methods 17:217",
    },
    # ── Classical (1949-1974) ──
    "wiener": {
        "name": "Wiener Deconvolution",
        "module": "algorithm_base.minflux.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class MinfluxOperator:
    """Generic PSF-convolution operator for MINFLUX Nanoscopy."""
    ...
    def info(self):
        return {"modality": "minflux", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return MinfluxOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for minflux."""
    ...
    try:
        if spec["module"] == "algorithm_base.minflux.solvers":
...
def list_solvers():
    """List all available solvers for minflux."""
