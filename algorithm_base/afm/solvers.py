"""Solvers for Atomic Force Microscopy (AFM) (afm).
...
SOLVERS = {
    ...
    "afm_dl": {
        "name": "AFM-UNet",
        "module": "pwm_core.recon.afm_solvers",
        "function": "afm_unet_recon",
        "gpu": True,
        "reference": "Cherukara, M.J. et al. (2020) AI-enabled high-res, real-time imaging, npj Comput. Mater. 6:203",
    },
    # ── Classical (1949-1974) ──
    "wiener": {
        "name": "Wiener Deconvolution",
        "module": "algorithm_base.afm.solvers",
        "function": "run_wiener",
        "gpu": False,
    ...
    "dl_cnn": {
        ...
        "function": "run_dl_cnn",
        "gpu": True,
        "reference": "CNN for scanning probe, 2019",
    },
    "dl_gan": {
        "name": "Probe-GAN",
        "module": "algorithm_base.afm.solvers",
        "function": "run_dl_gan",
        "gpu": True,
        "reference": "GAN super-resolution, 2020",
    },
    "dl_transformer": {
        ...
        "gpu": True,
        "reference": "Transformer for probe imaging, 2023",
    },
    "dl_diffusion": {
        "name": "Probe-Diffusion",
        "module": "algorithm_base.afm.solvers",
...
class AfmOperator:
    """Generic PSF-convolution operator for Atomic Force Microscopy (AFM)."""
    ...
    def info(self):
        return {"modality": "afm", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return AfmOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.afm.solvers":
...
def run_dl_cnn(y, physics, cfg=None):
    """Probe-CNN (CNN for scanning probe, 2019). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_cnn")

def run_dl_gan(y, physics, cfg=None):
    """Probe-GAN (GAN super-resolution, 2020). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_gan")

def run_dl_transformer(y, physics, cfg=None):
    """Probe-Transformer (Transformer for probe imaging, 2023). Fallback: Wiener + NLM."""
...
def list_solvers():
    """List all available solvers for afm."""
