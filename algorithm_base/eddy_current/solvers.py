"""Solvers for Eddy Current Imaging (eddy_current).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.eddy_current.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class EddyCurrentOperator:
    """Generic PSF-convolution operator for Eddy Current Imaging."""
    ...
    def info(self):
        return {"modality": "eddy_current", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return EddyCurrentOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for eddy_current."""
    ...
    try:
        if spec["module"] == "algorithm_base.eddy_current.solvers":
...
def list_solvers():
    """List all available solvers for eddy_current."""
