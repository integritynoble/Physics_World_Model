"""Solvers for Terahertz Imaging (THz) (terahertz).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.terahertz.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class TerahertzOperator:
    """Generic PSF-convolution operator for Terahertz Imaging (THz)."""
    ...
    def info(self):
        return {"modality": "terahertz", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return TerahertzOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for terahertz."""
    ...
    try:
        if spec["module"] == "algorithm_base.terahertz.solvers":
...
def list_solvers():
    """List all available solvers for terahertz."""
