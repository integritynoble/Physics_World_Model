"""Solvers for Electron Tomography (electron_tomography).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.electron_tomography.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class ElectronTomographyOperator:
    """Generic PSF-convolution operator for Electron Tomography."""
    ...
    def info(self):
        return {"modality": "electron_tomography", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return ElectronTomographyOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for electron_tomography."""
    ...
    try:
        if spec["module"] == "algorithm_base.electron_tomography.solvers":
...
def list_solvers():
    """List all available solvers for electron_tomography."""
