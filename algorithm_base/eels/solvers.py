...
class EelsOperator:
    """Generic PSF-convolution operator for Electron Energy Loss Spectroscopy (EELS)."""
    ...
    def info(self):
        return {"modality": "eels", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return EelsOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.eels.solvers":
