...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.spinning_disk.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class SpinningDiskOperator:
    """Generic PSF-convolution operator for Spinning Disk Confocal Microscopy."""
    ...
    def info(self):
        return {"modality": "spinning_disk", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return SpinningDiskOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for spinning_disk."""
    ...
    try:
        if spec["module"] == "algorithm_base.spinning_disk.solvers":
