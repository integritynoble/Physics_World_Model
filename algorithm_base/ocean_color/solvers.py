"""Solvers for Ocean Color Remote Sensing (ocean_color).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.ocean_color.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class OceanColorOperator:
    """Generic PSF-convolution operator for Ocean Color Remote Sensing."""
    ...
    def info(self):
        return {"modality": "ocean_color", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return OceanColorOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for ocean_color."""
    ...
    try:
        if spec["module"] == "algorithm_base.ocean_color.solvers":
...
def list_solvers():
    """List all available solvers for ocean_color."""
