"""Solvers for Generic Matrix Sensing (matrix).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.matrix.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class MatrixOperator:
    """Generic PSF-convolution operator for Generic Matrix Sensing."""
    ...
    def info(self):
        return {"modality": "matrix", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return MatrixOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for matrix."""
    ...
    try:
        if spec["module"] == "algorithm_base.matrix.solvers":
...
def list_solvers():
    """List all available solvers for matrix."""
