"""Solvers for Raman Imaging / Microscopy (raman_imaging).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.raman_imaging.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class RamanImagingOperator:
    """Generic PSF-convolution operator for Raman Imaging / Microscopy."""
    ...
    def info(self):
        return {"modality": "raman_imaging", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return RamanImagingOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for raman_imaging."""
    ...
    try:
        if spec["module"] == "algorithm_base.raman_imaging.solvers":
...
def list_solvers():
    """List all available solvers for raman_imaging."""
