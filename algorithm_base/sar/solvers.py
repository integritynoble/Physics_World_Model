"""Solvers for Synthetic Aperture Radar (SAR) (sar).
...
class SarOperator:
    """Generic PSF-convolution operator for Synthetic Aperture Radar (SAR)."""
    ...
    def info(self):
        return {"modality": "sar", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return SarOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.sar.solvers":
...
def list_solvers():
    """List all available solvers for sar."""
