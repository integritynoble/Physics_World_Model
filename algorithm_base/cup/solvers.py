"""Solvers for Compressed Ultrafast Photography (CUP) (cup).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.cup.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class CupOperator:
    """Generic PSF-convolution operator for Compressed Ultrafast Photography (CUP)."""
    ...
    def info(self):
        return {"modality": "cup", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return CupOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.cup.solvers":
...
def list_solvers():
    """List all available solvers for cup."""
