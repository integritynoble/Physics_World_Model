...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.dot.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class DotOperator:
    """Generic PSF-convolution operator for Diffuse Optical Tomography (DOT)."""
    ...
    def info(self):
        return {"modality": "dot", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return DotOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.dot.solvers":
