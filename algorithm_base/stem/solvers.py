"""Solvers for Scanning Transmission Electron Microscopy (STEM) (stem).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.stem.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class StemOperator:
    """Generic PSF-convolution operator for Scanning Transmission Electron Microscopy (STEM)."""
    ...
    def info(self):
        return {"modality": "stem", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return StemOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.stem.solvers":
...
def list_solvers():
    """List all available solvers for stem."""
