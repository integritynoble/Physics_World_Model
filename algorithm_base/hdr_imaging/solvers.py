"""Solvers for High Dynamic Range (HDR) Imaging (hdr_imaging).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.hdr_imaging.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class HdrImagingOperator:
    """Generic PSF-convolution operator for High Dynamic Range (HDR) Imaging."""
    ...
    def info(self):
        return {"modality": "hdr_imaging", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return HdrImagingOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for hdr_imaging."""
    ...
    try:
        if spec["module"] == "algorithm_base.hdr_imaging.solvers":
...
def list_solvers():
    """List all available solvers for hdr_imaging."""
