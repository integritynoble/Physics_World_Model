"""Solvers for Weather / Doppler Radar (weather_radar).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.weather_radar.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class WeatherRadarOperator:
    """Generic PSF-convolution operator for Weather / Doppler Radar."""
    ...
    def info(self):
        return {"modality": "weather_radar", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return WeatherRadarOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for weather_radar."""
    ...
    try:
        if spec["module"] == "algorithm_base.weather_radar.solvers":
...
def list_solvers():
    """List all available solvers for weather_radar."""
