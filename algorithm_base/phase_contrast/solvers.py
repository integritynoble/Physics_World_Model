"""Solvers for Phase Contrast Microscopy (phase_contrast).
...
SOLVERS = {
    ...
    "pc_dl": {
        "name": "PhaseNet",
        "module": "pwm_core.recon.phase_contrast_solvers",
        "function": "phase_net_recon",
        "gpu": True,
        "reference": "Rivenson, Y. et al. (2018) Phase recovery with DL, Light: Sci. & Appl. 7:17141",
    },
    # ── Classical (1949-1974) ──
    "wiener": {
        "name": "Wiener Deconvolution",
        "module": "algorithm_base.phase_contrast.solvers",
        "function": "run_wiener",
        "gpu": False,
    ...
    "dl_phasenet": {
        "name": "PhaseNet",
        "module": "algorithm_base.phase_contrast.solvers",
        "function": "run_dl_phasenet",
        "gpu": True,
        "reference": "DL phase retrieval, 2018",
    },
    "dl_prdeep": {
        "name": "prDeep",
        "module": "algorithm_base.phase_contrast.solvers",
        "function": "run_dl_prdeep",
        "gpu": True,
        "reference": "Deep phase retrieval, 2020",
    },
    "dl_transformer": {
        "name": "Phase-Transformer",
        "module": "algorithm_base.phase_contrast.solvers",
        "function": "run_dl_transformer",
        "gpu": True,
        "reference": "Transformer for phase, 2023",
    },
    "dl_diffusion": {
        ...
        "gpu": True,
        "reference": "Diffusion for phase retrieval, 2025",
    },
}
...
class PhaseContrastOperator:
    """Generic PSF-convolution operator for Phase Contrast Microscopy."""
    ...
    def info(self):
        return {"modality": "phase_contrast", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return PhaseContrastOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for phase_contrast."""
    ...
    try:
        if spec["module"] == "algorithm_base.phase_contrast.solvers":
...
def run_dl_phasenet(y, physics, cfg=None):
    """PhaseNet (DL phase retrieval, 2018). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_phasenet")

def run_dl_prdeep(y, physics, cfg=None):
    """prDeep (Deep phase retrieval, 2020). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_prdeep")

def run_dl_transformer(y, physics, cfg=None):
    """Phase-Transformer (Transformer for phase, 2023). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_transformer")

def run_dl_diffusion(y, physics, cfg=None):
    """Phase-Diffusion (Diffusion for phase retrieval, 2025). Fallback: Wiener + NLM."""
...
def list_solvers():
    """List all available solvers for phase_contrast."""
