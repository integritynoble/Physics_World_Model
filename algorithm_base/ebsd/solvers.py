"""Solvers for Electron Backscatter Diffraction (EBSD) (ebsd).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.ebsd.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class EbsdOperator:
    """Generic PSF-convolution operator for Electron Backscatter Diffraction (EBSD)."""
    ...
    def info(self):
        return {"modality": "ebsd", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return EbsdOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.ebsd.solvers":
...
def list_solvers():
    """List all available solvers for ebsd."""
