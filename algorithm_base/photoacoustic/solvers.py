"""Solvers for Photoacoustic Imaging (photoacoustic).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.photoacoustic.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class PhotoacousticOperator:
    """Generic PSF-convolution operator for Photoacoustic Imaging."""
    ...
    def info(self):
        return {"modality": "photoacoustic", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return PhotoacousticOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for photoacoustic."""
    ...
    try:
        if spec["module"] == "algorithm_base.photoacoustic.solvers":
...
def list_solvers():
    """List all available solvers for photoacoustic."""
