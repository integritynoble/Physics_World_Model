"""Solvers for Light Field Imaging (light_field).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.light_field.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class LightFieldOperator:
    """Generic PSF-convolution operator for Light Field Imaging."""
    ...
    def info(self):
        return {"modality": "light_field", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return LightFieldOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for light_field."""
    ...
    try:
        if spec["module"] == "algorithm_base.light_field.solvers":
...
def list_solvers():
    """List all available solvers for light_field."""
