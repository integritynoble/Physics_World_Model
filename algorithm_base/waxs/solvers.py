"""Solvers for Wide-Angle X-ray Scattering (WAXS) (waxs).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.waxs.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class WaxsOperator:
    """Generic PSF-convolution operator for Wide-Angle X-ray Scattering (WAXS)."""
    ...
    def info(self):
        return {"modality": "waxs", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return WaxsOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.waxs.solvers":
...
def list_solvers():
    """List all available solvers for waxs."""
