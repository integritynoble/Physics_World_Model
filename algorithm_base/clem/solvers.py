"""Solvers for Correlative Light-Electron Microscopy (CLEM) (clem).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.clem.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class ClemOperator:
    """Generic PSF-convolution operator for Correlative Light-Electron Microscopy (CLEM)."""
    ...
    def info(self):
        return {"modality": "clem", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return ClemOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.clem.solvers":
...
def list_solvers():
    """List all available solvers for clem."""
