"""Solvers for DNA-PAINT Super-Resolution (dna_paint).
...
SOLVERS = {
    ...
    "dna_paint_dl": {
        ...
        "gpu": True,
        "reference": "Speiser, A. et al. (2021) DL for dense SMLM, Nature Methods 18:1090",
    },
    # ── Classical (1949-1974) ──
    "wiener": {
        "name": "Wiener Deconvolution",
        "module": "algorithm_base.dna_paint.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class DnaPaintOperator:
    """Generic PSF-convolution operator for DNA-PAINT Super-Resolution."""
    ...
    def info(self):
        return {"modality": "dna_paint", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return DnaPaintOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for dna_paint."""
    ...
    try:
        if spec["module"] == "algorithm_base.dna_paint.solvers":
...
def list_solvers():
    """List all available solvers for dna_paint."""
