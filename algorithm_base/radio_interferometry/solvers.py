"""Solvers for Radio Interferometry (VLBI) (radio_interferometry).
...
SOLVERS = {
    ...
    # Original
    "ri_dl": {
        "name": "R2D2 (interferometry) [proxy]",
        "module": "pwm_core.recon.richardson_lucy",
        "function": "run_richardson_lucy",
        "gpu": False,
        "reference": "Richardson 1972, JOSA",
    ...
    "wiener": {
        ...
        "module": "algorithm_base.radio_interferometry.solvers",
        "function": "run_wiener",
        "gpu": False,
    ...
    "dl_cnn": {
        ...
        "function": "run_dl_cnn",
        "gpu": True,
        "reference": "Deep learning for remote sensing, 2018",
    },
    "dl_transformer": {
        "name": "RS-Transformer",
        "module": "algorithm_base.radio_interferometry.solvers",
        ...
        "gpu": True,
        "reference": "Transformer for remote sensing, 2022",
    },
    "dl_diffusion": {
        "name": "RS-Diffusion",
        "module": "algorithm_base.radio_interferometry.solvers",
        ...
        "gpu": True,
        "reference": "Diffusion model for RS, 2024",
    },
    "dl_mamba": {
        "name": "RS-Mamba",
        "module": "algorithm_base.radio_interferometry.solvers",
...
class RadioInterferometryOperator:
    """Generic PSF-convolution operator for Radio Interferometry (VLBI)."""
    ...
    def info(self):
        return {"modality": "radio_interferometry", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return RadioInterferometryOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for radio_interferometry."""
    ...
    try:
        if spec["module"] == "algorithm_base.radio_interferometry.solvers":
...
def run_dl_cnn(y, physics, cfg=None):
    """RS-CNN (Deep learning for remote sensing, 2018). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_cnn")

def run_dl_transformer(y, physics, cfg=None):
    """RS-Transformer (Transformer for remote sensing, 2022). Fallback: Wiener + NLM."""
    return _dl_fallback(y, physics, cfg or {}, "dl_transformer")

def run_dl_diffusion(y, physics, cfg=None):
    """RS-Diffusion (Diffusion model for RS, 2024). Fallback: Wiener + NLM."""
...
def list_solvers():
    """List all available solvers for radio_interferometry."""
