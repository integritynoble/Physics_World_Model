"""Solvers for Active Thermography (IR) (active_thermography).
...
class ActiveThermographyOperator:
    """Generic PSF-convolution operator for Active Thermography (IR)."""
    ...
    def info(self):
        return {"modality": "active_thermography", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return ActiveThermographyOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for active_thermography."""
    ...
    try:
        if spec["module"] == "algorithm_base.active_thermography.solvers":
...
def list_solvers():
    """List all available solvers for active_thermography."""
