"""Solvers for Confocal Live-Cell Microscopy (confocal_livecell).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.confocal_livecell.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class ConfocalLivecellOperator:
    """Generic PSF-convolution operator for Confocal Live-Cell Microscopy."""
    ...
    def info(self):
        return {"modality": "confocal_livecell", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return ConfocalLivecellOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for confocal_livecell."""
    ...
    try:
        if spec["module"] == "algorithm_base.confocal_livecell.solvers":
...
def list_solvers():
    """List all available solvers for confocal_livecell."""
