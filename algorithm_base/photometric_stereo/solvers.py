"""Solvers for Photometric Stereo (photometric_stereo).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.photometric_stereo.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class PhotometricStereoOperator:
    """Generic PSF-convolution operator for Photometric Stereo."""
    ...
    def info(self):
        return {"modality": "photometric_stereo", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return PhotometricStereoOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for photometric_stereo."""
    ...
    try:
        if spec["module"] == "algorithm_base.photometric_stereo.solvers":
...
def list_solvers():
    """List all available solvers for photometric_stereo."""
