"""Solvers for Time-of-Flight Depth Camera (tof_camera).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.tof_camera.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class TofCameraOperator:
    """Generic PSF-convolution operator for Time-of-Flight Depth Camera."""
    ...
    def info(self):
        return {"modality": "tof_camera", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return TofCameraOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for tof_camera."""
    ...
    try:
        if spec["module"] == "algorithm_base.tof_camera.solvers":
...
def list_solvers():
    """List all available solvers for tof_camera."""
