"""Solvers for OCT Angiography (OCTA) (octa).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.octa.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class OctaOperator:
    """Generic PSF-convolution operator for OCT Angiography (OCTA)."""
    ...
    def info(self):
        return {"modality": "octa", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return OctaOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.octa.solvers":
...
def list_solvers():
    """List all available solvers for octa."""
