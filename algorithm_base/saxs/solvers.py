"""Solvers for Small-Angle X-ray Scattering (SAXS) (saxs).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.saxs.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class SaxsOperator:
    """Generic PSF-convolution operator for Small-Angle X-ray Scattering (SAXS)."""
    ...
    def info(self):
        return {"modality": "saxs", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return SaxsOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.saxs.solvers":
...
def list_solvers():
    """List all available solvers for saxs."""
