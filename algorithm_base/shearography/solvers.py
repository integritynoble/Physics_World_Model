"""Solvers for Shearography (shearography).
...
class ShearographyOperator:
    """Generic PSF-convolution operator for Shearography."""
    ...
    def info(self):
        return {"modality": "shearography", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return ShearographyOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for shearography."""
    ...
    try:
        if spec["module"] == "algorithm_base.shearography.solvers":
...
def list_solvers():
    """List all available solvers for shearography."""
