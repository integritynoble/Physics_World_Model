"""Solvers for Talbot-Lau X-ray Grating Interferometry (talbot_lau).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.talbot_lau.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class TalbotLauOperator:
    """Generic PSF-convolution operator for Talbot-Lau X-ray Grating Interferometry."""
    ...
    def info(self):
        return {"modality": "talbot_lau", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return TalbotLauOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for talbot_lau."""
    ...
    try:
        if spec["module"] == "algorithm_base.talbot_lau.solvers":
...
def list_solvers():
    """List all available solvers for talbot_lau."""
