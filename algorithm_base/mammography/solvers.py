"""Solvers for Mammography (mammography).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.mammography.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class MammographyOperator:
    """Generic PSF-convolution operator for Mammography."""
    ...
    def info(self):
        return {"modality": "mammography", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return MammographyOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for mammography."""
    ...
    try:
        if spec["module"] == "algorithm_base.mammography.solvers":
...
def list_solvers():
    """List all available solvers for mammography."""
