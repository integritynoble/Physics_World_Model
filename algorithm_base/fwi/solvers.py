"""Solvers for Full-Waveform Inversion (FWI) (fwi).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.fwi.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class FwiOperator:
    """Generic PSF-convolution operator for Full-Waveform Inversion (FWI)."""
    ...
    def info(self):
        return {"modality": "fwi", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return FwiOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.fwi.solvers":
...
def list_solvers():
    """List all available solvers for fwi."""
