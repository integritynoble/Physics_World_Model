"""Solvers for X-ray NDT (Radiography) (xray_ndt).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.xray_ndt.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class XrayNdtOperator:
    """Generic PSF-convolution operator for X-ray NDT (Radiography)."""
    ...
    def info(self):
        return {"modality": "xray_ndt", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return XrayNdtOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for xray_ndt."""
    ...
    try:
        if spec["module"] == "algorithm_base.xray_ndt.solvers":
...
def list_solvers():
    """List all available solvers for xray_ndt."""
