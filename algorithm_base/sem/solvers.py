"""Solvers for Scanning Electron Microscopy (SEM) (sem).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.sem.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class SemOperator:
    """Generic PSF-convolution operator for Scanning Electron Microscopy (SEM)."""
    ...
    def info(self):
        return {"modality": "sem", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return SemOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.sem.solvers":
...
def list_solvers():
    """List all available solvers for sem."""
