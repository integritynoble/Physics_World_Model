"""Solvers for Polarimetric SAR (PolSAR) (polsar).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.polsar.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class PolsarOperator:
    """Generic PSF-convolution operator for Polarimetric SAR (PolSAR)."""
    ...
    def info(self):
        return {"modality": "polsar", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return PolsarOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for polsar."""
    ...
    try:
        if spec["module"] == "algorithm_base.polsar.solvers":
...
def list_solvers():
    """List all available solvers for polsar."""
