"""Solvers for Structured-Light Depth Camera (structured_light).
...
SOLVERS = {
    # Original
    "traditional_cpu": {
        "name": "FISTA-L2 (phase unwrap) [proxy]",
    ...
    "wiener": {
        ...
        "module": "algorithm_base.structured_light.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class StructuredLightOperator:
    """Generic PSF-convolution operator for Structured-Light Depth Camera."""
    ...
    def info(self):
        return {"modality": "structured_light", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return StructuredLightOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for structured_light."""
    ...
    try:
        if spec["module"] == "algorithm_base.structured_light.solvers":
...
def list_solvers():
    """List all available solvers for structured_light."""
