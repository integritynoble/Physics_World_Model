"""Solvers for Event Camera / Dynamic Vision Sensor (DVS) (event_camera).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.event_camera.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class EventCameraOperator:
    """Generic PSF-convolution operator for Event Camera / Dynamic Vision Sensor (DVS)."""
    ...
    def info(self):
        return {"modality": "event_camera", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return EventCameraOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    """Run a solver by key for event_camera."""
    ...
    try:
        if spec["module"] == "algorithm_base.event_camera.solvers":
...
def list_solvers():
    """List all available solvers for event_camera."""
