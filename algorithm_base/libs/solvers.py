"""Solvers for Laser-Induced Breakdown Spectroscopy (LIBS) Imaging (libs).
...
class LibsOperator:
    """Generic PSF-convolution operator for Laser-Induced Breakdown Spectroscopy (LIBS) Imaging."""
    ...
    def info(self):
        return {"modality": "libs", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return LibsOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.libs.solvers":
...
def list_solvers():
    """List all available solvers for libs."""
