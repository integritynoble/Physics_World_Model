"""Solvers for Contrast-Enhanced Ultrasound (CEUS) (ceus).
...
SOLVERS = {
    ...
    "wiener": {
        ...
        "module": "algorithm_base.ceus.solvers",
        "function": "run_wiener",
        "gpu": False,
...
class CeusOperator:
    """Generic PSF-convolution operator for Contrast-Enhanced Ultrasound (CEUS)."""
    ...
    def info(self):
        return {"modality": "ceus", "x_shape": self.x_shape, "psf_sigma": self.psf_sigma}


def _make_operator(y, cfg):
    cfg = cfg or {}
    sigma = cfg.get("psf_sigma", 2.0)
    return CeusOperator(y.shape[:2], sigma)
...
def run_solver(solver_key: str, y: np.ndarray, operator: Any = None,
    ...
    try:
        if spec["module"] == "algorithm_base.ceus.solvers":
...
def list_solvers():
    """List all available solvers for ceus."""
