...
(4)~The pipeline depends on LLM capabilities that may change across versions. The backbone (Claude Sonnet 4.6, Anthropic, 2025) is the primary model. Testing with two additional backbones (Claude Opus 4.6, GPT-5.4) shows moderate robustness. Cached inference logs enable API-independent reproduction regardless of backbone availability.
...
Python. LLM backbone: Claude Sonnet 4.6 (Anthropic, 2025), temperature 0. Primitives: NumPy, SciPy, PyTorch. All code, \texttt{spec.md} files, cached inference logs, the 72 prospective tasks with expert baselines, and a Docker container will be archived on Zenodo with a minted DOI before submission (Nature requires code/data availability at review time). The GitHub repository is at \url{https://github.com/integritynoble/Physics_World_Model}. Exact reproduction commands in \texttt{REPRODUCE.md}. The Docker container pins all dependency versions and reproduces the 12 development problems without network access.
