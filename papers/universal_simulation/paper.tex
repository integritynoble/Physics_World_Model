...
Large language models can generate scientific simulation code from natural-language descriptions, but the generated code silently fails on most non-textbook problems because it lacks mathematical validation. We formalize the class of certifiably solvable problems (the \emph{simulability class} $\Sclass$) through four conditions---finite specifiability, Hadamard stability, approximability, and certifiability---and show that a Judge Agent can automate verification of these conditions on AI-generated code. In controlled ablation across 134 test cases, removing the Judge increases the silent-failure rate from 1.5\% to 42\%. On clinical CT imaging (200 cases), the validated pipeline reaches 99\% of expert quality. A prospective benchmark of 72 blinded tasks from 12 external scientists confirms generalization: 89\% success with automated error bounds versus 53\% without the Judge. The residual 1.5\% failure rate maps to the theoretical boundary of $\Sclass$---bifurcation-sensitive regimes where certifiability breaks down---for which we propose continuation-based detection heuristics. We introduce \texttt{spec.md}, a universal specification format for scientific computation problems analogous to FASTA for sequences, and conjecture that four mathematical obstructions (undecidability, uncomputability, quantum irreducibility, infinite-precision barriers) exhaustively characterize the boundary of $\Sclass$. Code, data, \texttt{spec.md} files for all 72 benchmark tasks, and cached inference logs are archived at Zenodo.

% =====================================================================
% SUMMARY PARAGRAPH (Nature bold-print standfirst, ~100 words)
% =====================================================================

\textbf{Classical mathematical validation can be fully automated to make AI-generated scientific simulation reliable---and the right formalization of ``what is simulable'' shows why this works and where it must fail. We define the simulability class $\Sclass$ through four conditions (finite specifiability, Hadamard stability, approximability, certifiability) and show that a Judge Agent, automating verification of these conditions, reduces the silent-failure rate from 42\% to 1.5\% across 134 test problems spanning 12 domains. The residual 1.5\% maps exactly to the theoretical boundary of $\Sclass$---bifurcation points where certifiability breaks down. We introduce \texttt{spec.md} as a universal specification format (the canonical encoding of problems in $\Sclass$), and conjecture that four mathematical obstructions exhaustively characterize its boundary.}
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}
\node[draw=black!40, rounded corners=4pt, fill=gray!3, inner sep=0pt, text width=13cm] (code) {%
\begin{minipage}{13cm}
\small
\begin{lstlisting}[
...
# Specification: CT Reconstruction (LoDoPaB)  (*@\color{red!60!black}\textnormal{\scriptsize\bfseries $\leftarrow$ \texttt{spec.md}}@*)

## Domain (*@$\Domain$@*)                      (*@\color{blue!70!black}\textnormal{\scriptsize\bfseries $\leftarrow$ S1.1}@*)
domain: 2D image grid, 128 x 128 pixels
pixel_size: 0.7 mm

## Equations (*@$\mathcal{E}$@*)               (*@\color{blue!70!black}\textnormal{\scriptsize\bfseries $\leftarrow$ S1.2}@*)
forward: y = Radon(x) + eta
inverse: min_x ||Radon(x)-y||^2 + lam*TV(x)
parameters: {n_angles: 128, noise: Poisson}

## Boundary Conditions (*@$\mathcal{B}$@*)     (*@\color{blue!70!black}\textnormal{\scriptsize\bfseries $\leftarrow$ S1.3}@*)
non_negativity: x(i,j) >= 0 for all pixels

## Initial Conditions (*@$\mathcal{I}$@*)      (*@\color{blue!70!black}\textnormal{\scriptsize\bfseries $\leftarrow$ S1.4}@*)
x_0 = FBP result (warm start)

## Observables (*@$\mathcal{O}$@*)             (*@\color{blue!70!black}\textnormal{\scriptsize\bfseries $\leftarrow$ S1.5}@*)
metrics: [PSNR (dB), SSIM, relative_error]

## Tolerance (*@$\varepsilon$@*)               (*@\color{blue!70!black}\textnormal{\scriptsize\bfseries $\leftarrow$ S1.6}@*)
PSNR_minimum: 30.0 dB, SSIM_minimum: 0.85
\end{lstlisting}
\end{minipage}%
};
% Right-side annotation bracket
\draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick, blue!60!black]
  ([xshift=3pt]code.north east) -- ([xshift=3pt]code.south east)
  node[midway, right=6pt, align=left, font=\small\itshape, text=blue!60!black]
  {$\Spec = (\Domain, \mathcal{E}, \mathcal{B}, \mathcal{I}, \mathcal{O}, \varepsilon)$\\[2pt]
   \textnormal{\scriptsize The S1 six-tuple}};
\end{tikzpicture}%
}
\caption{A \texttt{spec.md} file for CT reconstruction---the canonical finite representation required by S1. Each section maps to one component of the six-tuple $\Spec = (\Domain, \mathcal{E}, \mathcal{B}, \mathcal{I}, \mathcal{O}, \varepsilon)$. This format is human-readable, machine-parseable, and solver-independent: any system accepting the same six-tuple can consume a \texttt{spec.md} file. We propose \texttt{spec.md} as a universal exchange format for scientific computation, analogous to FASTA for sequences or CIF for crystal structures. All 72 prospective benchmark tasks are archived as \texttt{spec.md} files.}
\label{fig:specmd}
\end{figure}
