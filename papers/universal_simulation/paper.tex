...
Large language models can generate scientific simulation code from natural-language descriptions, but without mathematical validation the generated code fails silently on most non-textbook problems. Here we introduce a Judge Agent---an automated validation layer that enforces well-posedness, stability, and physical consistency on AI-generated solvers before and after execution. We ground the pipeline in the \emph{simulability class} $\Sclass$, defined by four implementation-independent conditions---finite specifiability, Hadamard stability, approximability, and certifiability---which formally delineate problems amenable to automated bounded-error solution from those at the \emph{scientific event horizon} where guarantees break down. We propose \texttt{spec.md}, a structured specification format that realizes the finite-specifiability condition, as a universal exchange format for scientific computation problems, analogous to FASTA for sequences or CIF for crystal structures. In controlled ablation, removing the Judge increases the silent-failure rate from 1.5\% to 42\% across 134 test cases. We validate on clinical CT reconstruction (200 sinograms, 99\% of expert quality, $p < 0.001$) and on 72 blinded tasks from 12 external scientists at 9 institutions, achieving a median 500$\times$ setup-time reduction ($\rho$) over expert workflows. The residual 1.5\% failure rate is confined to bifurcation-sensitive regimes at the boundary of $\Sclass$, for which we propose continuation-based rejection heuristics. Code, data, and cached inference logs are archived at Zenodo.

% =====================================================================
% SUMMARY PARAGRAPH (Nature bold-print standfirst, ~100 words)
% =====================================================================

\textbf{AI code generators produce scientific simulations that compile and converge but silently return wrong answers, because the generated code is not mathematically validated. We define a simulability class $\Sclass$ via four implementation-independent conditions (finite specifiability, Hadamard stability, approximability, certifiability) and show that a Judge Agent, verifying membership in $\Sclass$, reduces the silent-failure rate from 42\% to 1.5\% across 134 test problems. We propose \texttt{spec.md}---the canonical serialization of problems in $\Sclass$---as a universal exchange format for scientific computation. On clinical CT imaging ($n = 200$), the validated pipeline reaches 99\% of expert quality with a 600$\times$ setup-time advantage. The residual failures cluster at the scientific event horizon---bifurcation points at the boundary of $\Sclass$---for which we propose continuation-based rejection criteria.}
...
We address both gaps. First, we define the \emph{simulability class} $\Sclass$ (Definition~\ref{def:simulability}): the set of scientific computation problems satisfying four implementation-independent conditions---finite specifiability, Hadamard stability, approximability, and certifiability. Problems in $\Sclass$ can be solved with a user-specified error tolerance; problems outside $\Sclass$---at what we call the \emph{scientific event horizon}---cannot be guaranteed by any automated pipeline. Condition S1 (finite specifiability) is realized concretely by \texttt{spec.md}, a structured specification format that we propose as a universal exchange format for scientific computation, analogous to FASTA for sequences or CIF for crystal structures. Second, we present a three-agent pipeline (Fig.~\ref{fig:pipeline}) in which a Plan Agent ($\Agent_P$) translates natural language into a \texttt{spec.md} file, a Judge Agent ($\Agent_J$) validates membership in $\Sclass$ against the four conditions, and an Execute Agent ($\Agent_E$) generates and runs the solver code. After execution, $\Agent_J$ audits the solution for physical consistency, realizing the certifiability condition S4. The underlying mathematics is entirely classical; the contribution is empirical evidence that automated $\Sclass$-verification substantially reduces silent failures---from 42\% to 1.5\%---across 134 test problems spanning 12 scientific domains, with a median setup-time efficiency gain of $\rho \approx 500\times$ relative to expert workflows.

\textbf{What this paper does not claim.} The pipeline does not outperform domain-optimized tools: on any single well-defined PDE, an expert using COMSOL~\cite{comsol} or FEniCS will produce a more accurate solution (Extended Data Table~\ref{tab:tool_comparison}). The class $\Sclass$ is verified for 25 numerical method families (Proposition~\ref{prop:realizability}) but not proven to cover all methods. The error bound is conditional on a correct \texttt{spec.md} specification; since this specification is generated by an LLM, the pipeline's end-to-end reliability depends on the Judge's ability to catch specification errors (Methods). The completeness conjecture (Conjecture~\ref{conj:obstruction}) is stated as a research direction, not a proven result. The pipeline retains a 1.5\% qualitative failure rate at the boundary of $\Sclass$ (Section~\ref{sec:failure}).
...
Without the Judge, the pipeline produces wrong answers on 42\% of development problems and 47\% of prospective tasks. The failures concentrate on non-textbook problems: stiff ODEs, inverse problems, turbulent flows, and near-critical parameter regimes---precisely the problems near the scientific event horizon where S2 or S4 are close to failing. With the Judge verifying S1--S4 membership in $\Sclass$, the success rate is 100\% on the 12 development problems (which were used to design the Judge's checks) and 89\% (64/72) on the held-out tasks, with automated error bounds (S4) on every successful solution. No other condition produces any error bound.
...
\textbf{Characterizing the residual 1.5\%.} Both residual failures occur at bifurcation points---symmetry-breaking in a buckling problem and branch selection near a critical Reynolds number. These problems lie at the scientific event horizon: they formally satisfy S1--S3 locally, but S4 (certifiability) fails because the error bound diverges at the bifurcation point---the Lipschitz constant grows as $L \sim |\theta - \theta_c|^{-\alpha}$, making the computable bound vacuous. The Judge's current checks verify S1--S4 pointwise at the given parameters; they cannot detect proximity to $\partial \Sclass$ without global landscape information. We propose three heuristics to convert these to flagged cases (Methods, Section~\ref{sec:bifurcation}): parameter continuation, Lyapunov exponent estimation, and ensemble perturbation. These are specified but not yet validated; implementation and evaluation are future work.

\textbf{Per-gate ablation.} Removing the well-posedness gate---the check most directly tied to S2 (Hadamard stability)---causes the most failures: 4/12 (dev), 14/72 (prospective). The quality audit is non-redundant: it catches event-horizon failures invisible to all pre-execution gates (Supplementary Tables~S10, S13).
...
\textbf{$\Sclass$ and \texttt{spec.md}: two sides of one coin.} The redesigned simulability class (Definition~\ref{def:simulability}) separates two concerns. $\Sclass$ defines what is simulable \emph{in principle}---through four implementation-independent conditions. The \texttt{spec.md} format defines what is simulable \emph{in practice}---as a concrete, machine-parseable serialization of S1's six-tuple $(\Domain, \mathcal{E}, \mathcal{B}, \mathcal{I}, \mathcal{O}, \varepsilon)$. Making this connection explicit transforms \texttt{spec.md} from an implementation detail of our pipeline into a specification language for scientific computation: just as FASTA encodes any nucleotide sequence and CIF encodes any crystal structure, \texttt{spec.md} can encode any problem in $\Sclass$. A shared specification format directly addresses the reproducibility crisis~\cite{baker2016}: researchers can share the \emph{intent} of a simulation (the problem to be solved) without the baggage of specific software, library versions, or legacy code. The 72 prospective tasks in our benchmark are archived as \texttt{spec.md} files, and we invite the community to extend the format.

\textbf{Toward a completeness theory.} The four conditions of $\Sclass$ characterize the boundary of certifiably solvable scientific computation. We conjecture (Conjecture~\ref{conj:obstruction}) that four mathematical obstructions---Turing undecidability, BSS uncomputability, quantum measurement irreducibility, and infinite-precision barriers---are exhaustive: every problem outside $\Sclass$ fails due to one of these four. Proving this conjecture, even for restricted problem classes such as linear elliptic PDEs, would connect AI-assisted simulation to the foundations of computable analysis and constitute a significant advance in the theory of scientific computation. The \texttt{spec.md} format provides a practical entry point: if every solved problem in a discipline can be expressed as a valid \texttt{spec.md}, that discipline's computational practice lies within $\Sclass$. We invite the community to test this by expressing benchmark problems from their fields in \texttt{spec.md} format.
...
(2)~$\Sclass$ is realized for 25 numerical method families (Proposition~\ref{prop:realizability}) but not proven to cover all methods. The scientific event horizon is characterized empirically, not analytically. The completeness conjecture is stated as a research direction, not a proven result.
...
(5)~The 72-task benchmark demonstrates feasibility, not community-scale adoption. Whether \texttt{spec.md} can serve as a community standard is an empirical question.
...
The ``equivalently'' direction is true by definition. The non-trivial direction---that these four are the \emph{only} reasons a problem can fail to be in $\Sclass$---is where the research program lives. Full discussion of each obstruction in Supplementary Section~S2.
