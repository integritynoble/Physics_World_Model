...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}
\node[draw=black!40, rounded corners=4pt, fill=gray!3, inner sep=0pt, text width=14cm] (code) {%
\begin{minipage}{14cm}
\small
\begin{lstlisting}[
...
# Specification: CT Reconstruction (LoDoPaB)     (*@\color{red!60!black}\textnormal{\scriptsize\bfseries $\leftarrow$ \texttt{spec.md} header}@*)

## Domain                                         (*@\color{blue!70!black}\textnormal{\scriptsize\bfseries $\leftarrow$ S1.1: $\Domain$}@*)
domain: 2D image grid
geometry: 128 x 128 pixels
pixel_size: 0.7 mm                   # from LoDoPaB-CT
dimension: 2

## Equations                                      (*@\color{blue!70!black}\textnormal{\scriptsize\bfseries $\leftarrow$ S1.2: $\mathcal{E}$}@*)
equations: |
  forward: y = Radon(x) + eta
  inverse: min_x 0.5*||Radon(x)-y||^2 + lam_TV*TV(x) + lam_L2*||x||^2
  constraint: x >= 0

parameters:
  n_angles: 128                      # projection angles (0 to pi)
  n_detectors: 183                   # detector elements per angle
  geometry: parallel_beam
  noise_model: Poisson               # realistic clinical noise
  lambda_TV: 1.0e-3                  # auto-tuned via Morozov
  lambda_L2: 1.0e-5                  # Tikhonov weight
  max_iterations: 500                # ADMM iterations
  image_size: [128, 128]

## Boundary Conditions                            (*@\color{blue!70!black}\textnormal{\scriptsize\bfseries $\leftarrow$ S1.3: $\mathcal{B}$}@*)
boundary: |
  non_negativity: x(i,j) >= 0 for all pixels
  support: x = 0 outside patient body contour (if known)

## Initial Conditions                             (*@\color{blue!70!black}\textnormal{\scriptsize\bfseries $\leftarrow$ S1.4: $\mathcal{I}$}@*)
initial: |
  x_0 = filtered back-projection (FBP) result (warm start)

## Observables                                    (*@\color{blue!70!black}\textnormal{\scriptsize\bfseries $\leftarrow$ S1.5: $\mathcal{O}$}@*)
observables:
...
## Tolerance                                      (*@\color{blue!70!black}\textnormal{\scriptsize\bfseries $\leftarrow$ S1.6: $\varepsilon$}@*)
tolerance:
...
\draw[decorate, decoration={brace, amplitude=6pt, mirror}, thick, blue!60!black]
  ([xshift=4pt]code.north east) -- ([xshift=4pt]code.south east)
  node[midway, right=8pt, align=left, font=\small\itshape, text=blue!60!black]
  {$\Spec = (\Domain, \mathcal{E}, \mathcal{B}, \mathcal{I}, \mathcal{O}, \varepsilon)$\\[2pt]
   \textnormal{\scriptsize The S1 six-tuple}\\[4pt]
   \textnormal{\scriptsize YAML-compatible}\\
   \textnormal{\scriptsize key-value syntax}};
\end{tikzpicture}%
}
\caption{A real \texttt{spec.md} file for CT reconstruction (from the benchmark archive), showing the full YAML-compatible format. Each \texttt{\#\#} section maps to one component of the S1 six-tuple $\Spec = (\Domain, \mathcal{E}, \mathcal{B}, \mathcal{I}, \mathcal{O}, \varepsilon)$. Parameters use typed key-value pairs with inline comments; multi-line blocks use YAML \texttt{|} syntax. The format is human-readable, machine-parseable, and solver-independent. The optional \texttt{Primitives Required} section links to the engineering layer (Proposition~\ref{prop:realizability}). All 72 prospective benchmark tasks and 12 development problems are archived as \texttt{spec.md} files. Formal grammar in Supplementary Section~S8.}
\label{fig:specmd}
\end{figure}
