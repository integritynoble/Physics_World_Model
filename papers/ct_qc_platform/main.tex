...
\definecolor{lightblue}{HTML}{E3EEF9}
\definecolor{lightgreen}{HTML}{E3F5E3}
\definecolor{lightred}{HTML}{FDE8E8}
\definecolor{lightyellow}{HTML}{FFF8E1}
\definecolor{lightpurple}{HTML}{F0E6F6}
...
manual, vendor-specific, and poorly reproducible~\cite{samei2019performance}.
...
that lack integrity protection~\cite{droege1982ct_qa}.
...
Storage uses atomic writes (temporary file $+$ \texttt{os.replace})
to prevent corruption.
The \texttt{compare\_to\_baseline} method computes per-metric deltas
classified as STABLE ($<5\%$), DRIFTED ($5$--$15\%$), or ALERT
($\geq 15\%$), with a minimum denominator of 10 to avoid inflated
percentages for near-zero metrics.
...
All metrics follow a metric-first design: they are computed directly
from reconstructed pixel data, with no forward model required.
...
6 & Noise (Std Dev) & HU &
  vs.\ baseline &
  $\sigma$ of uniform water ROI (Bessel-corrected) \\
7 & Low-Contrast Detect. & targets &
  $\geq 4$ &
  Count with CNR~$\geq 1.0$ (Rose criterion) \\
...
Center detection thresholds at $-500$~HU and computes the binary
centroid.
...
where $\sigma_{\text{bg}}$ is Bessel-corrected.
...
% UCL 3-sigma
\addplot[pwmRed, dashed, domain=0:21] {7.0};
\addlegendentry{UCL ($3\sigma$)}
% 2-sigma warning
\addplot[pwmOrange, dotted, domain=0:21] {6.2};
\addlegendentry{Warning ($2\sigma$)}
% Center line
\addplot[pwmBlue, domain=0:21] {4.6};
\addlegendentry{Baseline $\bar{x}$}
% 2-sigma warning lower
\addplot[pwmOrange, dotted, domain=0:21] {3.0};
% LCL
\addplot[pwmRed, dashed, domain=0:21] {2.2};
\addlegendentry{LCL ($3\sigma$)}
% Data -- stable period then drift
\addplot[mark=*, mark size=1.5pt, pwmBlue!80!black]
  coordinates {
    (1,4.5) (2,4.7) (3,4.4) (4,4.8) (5,4.3)
    (6,4.6) (7,4.9) (8,4.5) (9,4.7) (10,4.6)
    (11,5.0) (12,5.1) (13,5.3) (14,5.2) (15,5.5)
    (16,5.7) (17,5.9) (18,6.1) (19,6.4) (20,6.8)
  };
\addlegendentry{Measurements}

% WE Rule 5 annotation
\draw[pwmRed, thick, decorate, decoration={brace, amplitude=4pt}]
  (axis cs:14,6.9) -- (axis cs:20,6.9)
...
\end{tikzpicture}
\caption{%
  Example Shewhart control chart for noise standard deviation.
  ...
  points, measurements 14--20) triggers a WARNING alert before the
  UCL is crossed, demonstrating the early-warning benefit of
  trending-based QC.
}
...
\begin{equation}
  ...
  \label{eq:score}
\end{equation}
Confidence: high (top $> 2\times$ second), moderate ($> 1.2\times$),
low (otherwise).
Disambiguation test recommended when the top two hypotheses are
within 20\%.
...
\text{intercept}$);
(4)~canonical resampling via trilinear interpolation
(\texttt{scipy.ndimage.zoom})~\cite{scipy};
...
The \texttt{ReportGenerator} produces three artifacts:
(1)~\texttt{physicist\_report.json}---versioned, SHA-256-hashed
machine-readable record with per-metric entries, drift alerts,
diagnosis, and baseline reference;
(2)~\texttt{physicist\_report.pdf}---human-readable document via
fpdf2~\cite{fpdf2} with PASS/FAIL summary, metrics table, drift
alerts, diagnosis, and signature block;
(3)~\texttt{evidence/} folder with ROI overlays, trend plots
...
This creates an \emph{abundance flywheel}: each scanner model added
lowers the marginal cost of the next, because infrastructure is
...
\begin{axis}[
  ...
  bar width=8pt,
  xlabel={QA Metric},
  ylabel={Deviation from reference},
  symbolic x coords={
...
% Deviations (platform value - nominal, in respective units)
...
% ACR tolerance band
\draw[pwmRed, dashed, line width=0.6pt] (axis cs:Water,2) -- (axis cs:Noise,2);
\draw[pwmRed, dashed, line width=0.6pt] (axis cs:Water,-2) -- (axis cs:Noise,-2);
\node[font=\tiny\color{pwmRed}, anchor=west] at (axis cs:Noise,2.2) {ACR tolerance};

\end{axis}
\end{tikzpicture}
\caption{%
  Metric deviations from nominal values on an ACR CT~464 phantom
  scan (GE Revolution, 120~kVp, 5~mm axial).
  All nine metrics fall within ACR tolerance bands (dashed red
  lines at $\pm 2$ normalized units).
  Water, Bone, Air, Acrylic, Poly: deviation in HU; Geom: mm;
  Slice: mm; Unif: HU; Noise: HU.
}
...
kernel).
\Cref{fig:validation} shows the deviation of each metric from its
nominal value.
All nine metrics fell within ACR tolerance bands:
CT number of water deviated by 0.3~HU ($< 5$~HU criterion),
geometric accuracy by $-0.15$~mm ($< 2$~mm), slice thickness by
0.08~mm ($< 1.5$~mm), and uniformity by 0.2~HU ($< 5$~HU).
...
Gear~5 (Data Trusts)---immutable baselines and QC time series;
Gear~6 (Decision Logs)---full audit trail per QC run;
Gear~7 (Two-Source Rule)---diagnosis requires multiple converging
features;
Gear~9 (Fairness Targets)---consistent QC across vendors.
...
cross-platform comparison~\cite{samei2019performance}.
...
The CasePack abstraction fills this gap: it is the first open,
version-controlled QA workflow specification for CT QC.
...
(1)~Validation used a single scanner model; multi-vendor validation
is planned.
(2)~The mismatch library is expert-curated; data-driven enrichment
from multi-site QC databases could improve diagnostic coverage.
(3)~The platform is for research/internal QA use; SaMD deployment
would require FDA 510(k) and IEC 62304 compliance~\cite{iec62304,samd_fda}.
(4)~Low-contrast detectability uses a simplified CNR criterion;
more sophisticated model-observer approaches could improve
correlation with human observer performance~\cite{solomon2016lcd}.
...
All analyses use ACR CT~464 phantom data only; no patient data
were used.
