...
Every imaging failure, regardless of modality, can be traced to exactly
three root causes.  We formalize this claim as the \emph{Triad Law} and
...
by its binding gate before any reconstruction is attempted.  The output
...
real-world systems}.  Consider CASSI~\cite{wagadarikar2008cassi} spectral imaging: a single-pixel
mask shift degrades MST-L~\cite{cai2022mst} from 34.81~dB (Scenario~I, ideal operator) to
18.09~dB (Scenario~II, mismatched operator) --- a loss of
\textbf{16.72~dB}.  By contrast, upgrading from the classical solver
GAP-TV~\cite{yuan2016gaptv} (24.22~dB) to the state-of-the-art MST-L (34.81~dB) under ideal
conditions yields a gain of 10.59~dB.  The mismatch penalty dwarfs the
...
We define the \emph{recovery ratio} $\rho$ to quantify how much of the
mismatch-induced degradation can be recovered through operator
correction:
%
\begin{equation}
    \rho \;=\; \frac{\text{PSNR}_{\text{III}} - \text{PSNR}_{\text{II}}}
                    {\text{PSNR}_{\text{I}}   - \text{PSNR}_{\text{II}}}
    \label{eq:recovery_ratio}
\end{equation}
%
where Scenario~I uses the ideal (true) operator, Scenario~II uses the
mismatched (nominal) operator, and Scenario~III applies the \pwm{}
operator correction pipeline.  A value of $\rho = 1$ indicates full
recovery; $\rho = 0$ indicates that correction had no effect.
...
\textbf{Modality}  & $\Delta_{\text{mismatch}}$ (dB)
                    ...
                    & \textbf{Binding Gate} \\
\midrule
CASSI               & 16.72 & 10.59 & 1.58$\times$ & Gate 3 \\
CACTI               & 20.85 & ---   & ---           & Gate 3 \\
SPC                  & 17.72 & ---   & ---           & Gate 3 \\
MRI                  & 38.03 & ---   & ---           & Gate 3 \\
...
\cref{tab:gate_binding}, Gate~3 is binding.  This motivates the \pwm{}
