...
We define the \emph{recovery ratio} $\rho$ (Eq.~\ref{eq:recovery} in \cref{sec:scenarios}) to quantify how much of the mismatch-induced degradation can be recovered through operator correction.  A value of $\rho = 1$ indicates full recovery; $\rho = 0$ indicates that correction had no effect.  Values $\rho > 1$ are possible when the corrected operator provides implicit regularization that yields reconstruction quality exceeding even the ideal-condition baseline, as observed for CACTI (\cref{sec:evidence}).
