...
\begin{itemize}
    ...
    \item \textbf{Scenario~III (Oracle Mask)} --- true operator applied to
...
\end{itemize}
%
\noindent Scenario~IV (Oracle sanity check) reproduces Scenario~I by design for CASSI and is omitted for brevity.  Scenario~III values are InverseNet-validated~\cite{yang2026inversenet}.

\begin{table}[t]
\centering
\caption{CASSI deep dive: 5~solvers $\times$ 3~scenarios + projected fine-tuned estimate.  The mismatch
cost (I$\to$II) dominates the solver upgrade gain under ideal conditions.  Scenario~III reports Oracle Mask reconstruction (true operator on mismatched data); Scenario~IV reproduces Scenario~I by design and is omitted for brevity.}
\label{tab:cassi}
\smallskip
\begin{tabular}{@{}lcccc@{}}
\toprule
...
Oracle Mask (Scenario~III) substantially recovers MST-L to 27.33~dB
($+6.50$~dB), while HDNet sees zero recovery (21.88~dB in both Scenarios
II and III), confirming that HDNet is mask-oblivious.
