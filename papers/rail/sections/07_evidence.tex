...
CASSI (Alg~1)        & mask\_geo+dispersion   & 20.96 & 21.50 & \worst{$+$0.54}  \\
CASSI (Alg~2)        & mask\_geo+dispersion   & 20.96 & 21.72 & $+$0.76  \\
...
\end{itemize}
%
\noindent Scenario~IV (Oracle sanity check) reproduces Scenario~I by design for CASSI and is omitted for brevity.  Oracle Mask values are InverseNet-validated~\cite{yang2026inversenet}.

\begin{table}[t]
\centering
\caption{CASSI deep dive: 5~solvers $\times$ 3~scenarios + projected fine-tuned estimate.  The mismatch
cost (I$\to$II) dominates the solver upgrade gain under ideal conditions.  Oracle Mask$^\S$ reports reconstruction with the true operator on mismatched data; Scenario~IV reproduces Scenario~I by design and is omitted for brevity.}
\label{tab:cassi}
\smallskip
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\smallskip
{\footnotesize $\S$~Oracle Mask values represent reconstruction with the true operator applied to mismatched data (InverseNet Scenario~III), providing an upper bound on what any correction algorithm can achieve.  These correspond to the formal Scenario~IV definition in the 4-Scenario Protocol.}
