...
All 26/26 modalities pass.  Average PSNR $\approx 35.7$~dB (excluding Phase Retrieval identity test); range 25.46--61.35~dB over physically meaningful modalities.}
\label{tab:26mod}
\smallskip
...
(100.00~dB, a numerical ceiling rather than a measured value).  The average of ${\approx}\,35.7$~dB (excluding the Phase Retrieval identity test) confirms that the
...
In addition to the 9-layer Industrial Intelligence Stack (\cref{sec:stack}), the SolveEverything framework defines 10 operational gears---concrete mechanisms that drive each layer.  \Cref{tab:gears} reports the implementation status of each gear within PWM.
