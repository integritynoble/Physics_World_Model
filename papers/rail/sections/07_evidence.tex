...
operator) except for CASSI rows, which report Scenario~IV (Oracle Mask)
...
CASSI (Alg~1, GAP-TV)$^\S$ & mask\_geo+dispersion  & 20.96 & 21.50 & \worst{$+$0.54}  \\
...
\smallskip
{\footnotesize $\S$~CASSI ``After'' values are Scenario~IV (Oracle Mask), not Scenario~III; see note below.}
...
Both \cref{tab:correction} and \cref{tab:sensitivity} now use the same InverseNet 5-parameter mismatch (sub-pixel mask shift, rotation, dispersion drift, spectral axis tilt).  The Scenario~II baseline of 20.96\,dB in \cref{tab:correction} corresponds to GAP-TV reconstruction under this mismatch; the sensitivity table reports per-solver Scenario~II values.  CASSI ``After'' values in \cref{tab:correction} are oracle upper bounds (true operator applied to mismatched data); actual calibration gains may be lower.
