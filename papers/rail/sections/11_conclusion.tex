...
that achieve 35+\,dB on simulated benchmarks collapse by 5--12\,dB when
confronted with physical measurements.  The gap is not a solver problem.
It is an infrastructure problem.

For two decades, the field has been optimizing the wrong layer.  It has
...
\begin{itemize}
  ...
  \item \textbf{The four-scenario evaluation protocol}
        (Ideal / Baseline / Oracle / Full-Pipeline), which for the first
  ...
  \item \textbf{LIP-Arena and the Triad Law}, providing real-time
        ...
        reconstruction failure into sensing, inversion, and prior
        components.
\end{itemize}

The evidence is unambiguous.  Across the 26-modality benchmark,
operator correction alone improved reconstruction quality by +0.8 to
+48.25\,dB on 16 modalities---gains that no solver upgrade can
replicate.  The CASSI deep-dive was particularly revealing: correcting
the operator produced a 7$\times$ larger quality improvement than
upgrading the solver architecture.  The bottleneck was never the
