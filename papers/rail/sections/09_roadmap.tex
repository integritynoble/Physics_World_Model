...
similar to protein-structure prediction before CASP and the Protein Data
...
of shared infrastructure that enabled the AlphaFold revolution:
...
\begin{itemize}
  ...
  \item \textbf{No universal evaluation protocol.}  PSNR on a simulated
        ...
        (Ideal/Baseline/Oracle/Full-Pipeline) introduced in this work is,
...
\noindent
The analogy is not superficial.  AlphaFold succeeded not because of a
single architectural innovation, but because CASP provided a legible,
...
\caption{%
  ...
  L3 and L5 refer to standardization maturity levels
  (analogous to autonomous-driving levels), where L3 denotes
  ``conditionally automated'' and L5 denotes ``fully autonomous.''
}
\label{tab:targets}
\small
\begin{tabular}{@{}lccc@{}}
...
\noindent
\textbf{Recovery ratio} measures the fraction of theoretical information
recovered by the full pipeline (operator + solver) relative to the ideal
operator bound.  \textbf{Oracle gap} is the PSNR difference between the
Oracle scenario (Scenario~III) and the Full-Pipeline scenario
(Scenario~IV).  \textbf{Zero-shot generalization} measures reconstruction
