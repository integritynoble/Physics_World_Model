...
represents years of sustained algorithmic progress in
hyperspectral image reconstruction.
Yet when the coded aperture mask is subjected to a 5-parameter
perturbation (two-axis sub-pixel shift, rotation, dispersion drift, and spectral
axis tilt; see \cref{sec:sensitivity} for details)---well
within the manufacturing and alignment tolerances of any real optical
system---the same method collapses to 20.83\,dB, a loss of
\textbf{13.98\,dB}.
Under this sub-pixel mismatch model, deep-learning solvers (HDNet, MST-S, MST-L) lose
12.78--13.98\,dB (\cref{tab:sensitivity}).
To put this in perspective, the entire history of
CASSI reconstruction---from GAP-TV~\cite{yuan2016gaptv} at 24.34\,dB to MST-L~\cite{cai2022mst} at 34.81\,dB---spans only 10.47\,dB.
A sub-pixel operator mismatch erases years of
algorithmic improvement.

This is not an isolated failure.
Across modalities---CASSI,
CACTI~\cite{llull2013cacti}, single-pixel
cameras (SPC)~\cite{duarte2008spc}---the pattern repeats: the most powerful neural solvers
exhibit the \emph{largest} sensitivity to forward-model error.
Classical methods such as GAP-TV lose only 3.38\,dB under the same
...
that maps measurements to images, whether via deep learning~\cite{ongie2020deep}
or algorithm unrolling~\cite{monga2021unrolling}---but the \emph{infrastructure} around
...
Our design follows the SolveEverything.org framework~\cite{wissner2026solveeverything},
...
The system comprises four interlocking components (mapped to the
Industrial Intelligence Stack in \cref{tab:stack_mapping}):
...
\begin{enumerate}
  \item \textbf{OperatorGraph IR}---a universal directed acyclic graph
        ...
        electrons, spins, acoustic waves, particles) using 90 validated
        templates.
  \item \textbf{4-Scenario Evaluation Protocol}---a standardized
        framework that measures every solver under Ideal, Mismatch,
        Corrected (calibrated), and Oracle Mask conditions,
...
\begin{enumerate}
  ...
  \item \textbf{OperatorGraph IR as universal physics representation.}
        ...
        formalism, with 90 validated templates supporting automatic
  ...
  \item \textbf{26-modality benchmark evidence.}
        ...
        operator mismatch, with degradation ranging from 3.38\,dB
        (GAP-TV) to 13.98\,dB (MST-L) on CASSI alone
        (\cref{sec:problem}).

  \item \textbf{Operator correction across 9 correction configurations
        spanning 7 distinct modalities (16 registered).}
        We show that calibrating the forward operator---even with
        classical methods---recovers a substantial fraction of mismatch
        loss across 9 correction configurations spanning 7 distinct modalities (with 7 additional
