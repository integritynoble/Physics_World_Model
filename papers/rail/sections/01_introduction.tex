...
of \textbf{16.72\,dB}.
Even under a conservative 1-pixel-shift-plus-rotation model, the
degradation exceeds 14.5\,dB (\cref{tab:sensitivity}).
...
Our design follows the SolveEverything.org framework~\cite{solveeverything},
...
\begin{enumerate}
  ...
  \item \textbf{16-modality operator correction results.}
        We show that calibrating the forward operator---even with
        classical methods---recovers a substantial fraction of mismatch
        loss across 16 modalities, confirming that the dominant failure
        mode in deployed imaging systems is not solver quality but
        operator fidelity.
...
Subsequent sections describe experiments, ablations, and implications
for the field.
