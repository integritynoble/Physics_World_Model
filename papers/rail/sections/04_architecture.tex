...
IV  & Oracle Mask & $\mathbf{H}$ & $\mathbf{H}$
    & Pipeline sanity check: reproduces Scenario~I through the full
      pipeline, verifying no artifacts are introduced ($\pm 0.01$\,dB). \\
\bottomrule
\end{tabular}
\end{table}
...
A partial oracle condition in which the true operator is applied to
mismatched data, verifying pipeline integrity and providing an upper
bound on what any correction algorithm can achieve.
This scenario enables diagnostic analysis of which mismatch axis
matters most.
