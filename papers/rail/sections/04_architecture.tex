...
\begin{enumerate}
  \item \textbf{Photons}: optical imaging (CASSI, SPC, lensless,
        coded diffraction, \etc{}).
  \item \textbf{Electrons}: electron microscopy (cryo-EM, STEM, SEM).
  \item \textbf{Spins}: magnetic resonance imaging (MRI,
        diffusion MRI, spectroscopic MRI).
...
\Cref{fig:operatorgraph} illustrates the OperatorGraph for a standard
single-disperser CASSI system:
\begin{equation}
...
I   & Ideal      & $\mathbf{H}$    & $\mathbf{H}$
    & Oracle upper bound: what is achievable with perfect operator
      knowledge. \\
II  & Assumed    & $\mathbf{H}$    & $\mathbf{H}_{\mathrm{nom}}$
...
\begin{align}
  ...
  \text{Recovery Ratio}  &= \text{PSNR}_\mathrm{III} \;/\; \text{PSNR}_\mathrm{I}, \label{eq:recovery} \\
...
(\cref{tab:stack_mapping}): a recovery ratio $\geq 0.80$ and an oracle
...
To address this, PWM includes the \textbf{LIP-Arena} (Live Imaging
Performance Arena), a prospective evaluation competition in which test
...
\Cref{fig:cms} illustrates the four-phase Commit-Measure-Score protocol:
