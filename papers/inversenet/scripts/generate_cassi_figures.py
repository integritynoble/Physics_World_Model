...
def plot_scenario_comparison(summary):
    ...
    logger.info("  Saved scenario_comparison.png")
    plt.close()
...
def plot_method_comparison_heatmap(summary):
    ...
    logger.info("  Saved method_comparison_heatmap.png")
    plt.close()
...
def plot_gap_comparison(summary):
    ...
    bars1 = ax1.bar(
        x, gap_i_ii, color=[METHOD_COLORS[m] for m in METHODS],
    ...
    bars2 = ax2.bar(
        x, gap_ii_iii, color=[METHOD_COLORS[m] for m in METHODS],
    ...
    logger.info("  Saved gap_comparison.png")
    plt.close()
...
def plot_psnr_distribution(detailed):
    ...
    for idx, sc in enumerate(SCENARIOS):
        ...
        bp = ax.boxplot(
            data, tick_labels=[METHOD_LABELS[m] for m in METHODS],
    ...
    logger.info("  Saved psnr_distribution.png")
    plt.close()
...
def plot_per_scene_psnr(detailed):
    ...
    for ax_idx, method in enumerate(METHODS):
        ax = axes[ax_idx]
        for sc, ls, marker in zip(SCENARIOS, ["-", "--", "-."], ["o", "s", "^"]):
            vals = [r[sc][method]["psnr"] for r in detailed]
            ax.plot(
                x, vals, ls, marker=marker, markersize=5, linewidth=1.5,
    ...
    logger.info("  Saved per_scene_psnr.png")
    plt.close()
...
def plot_oracle_gain_per_scene(detailed):
    ...
    for i, method in enumerate(METHODS):
        ...
        offset = (i - (n_methods - 1) / 2) * width
        ax.bar(
            x + offset, gains, width, label=METHOD_LABELS[method],
            color=METHOD_COLORS[method], alpha=0.85,
            edgecolor="black", linewidth=0.5,
        )
    ...
    logger.info("  Saved oracle_gain_per_scene.png")
    plt.close()


def create_summary_table(summary):
    """Create CSV table with results (LaTeX-ready)."""
    logger.info("Creating summary table...")

    output_file = TABLES_DIR / "cassi_results_table.csv"

    with open(output_file, "w") as f:
        f.write(
            "Method,"
            "Scenario I (PSNR),Scenario II (PSNR),Scenario III (PSNR),"
        ...
        for method in METHODS:
            ...
            piii_m = get_psnr_mean(summary, "scenario_iii", method)
            piii_s = get_psnr_std(summary, "scenario_iii", method)
            ...
            siii_m = get_ssim_mean(summary, "scenario_iii", method)
            siii_s = get_ssim_std(summary, "scenario_iii", method)
            g_i_ii = get_gap_mean(summary, method, "gap_i_ii")
            g_ii_iii = get_gap_mean(summary, method, "gap_ii_iii")

            f.write(
                f"{METHOD_LABELS[method]},"
                f"{pi_m:.2f}+/-{pi_s:.2f},"
                f"{pii_m:.2f}+/-{pii_s:.2f},"
                f"{piii_m:.2f}+/-{piii_s:.2f},"
                f"{si_m:.4f}+/-{si_s:.4f},"
                f"{sii_m:.4f}+/-{sii_s:.4f},"
                f"{siii_m:.4f}+/-{siii_s:.4f},"
                f"{g_i_ii:.2f},{g_ii_iii:.2f}\n"
            )
...
def main():
    logger.info("=" * 70)
    logger.info("CASSI InverseNet Figure Generation")
    logger.info("=" * 70)
    ...
    logger.info(f"Table:  {TABLES_DIR / 'cassi_results_table.csv'}")
    logger.info("=" * 70)
