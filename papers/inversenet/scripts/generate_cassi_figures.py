...
3. Per-scene PSNR boxplot (distribution across scenes)
...
def plot_scenario_comparison(summary: Dict) -> None:
    ...
    ax.set_title('CASSI Reconstruction: Scenario Comparison', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
...
def plot_method_comparison_heatmap(summary: Dict) -> None:
    ...
    ax.set_title('PSNR Heatmap: Methods × Scenarios', fontsize=14, fontweight='bold')
...
def plot_psnr_distribution(detailed_results: List[Dict]) -> None:
    """
    Create boxplot showing PSNR distribution across 10 scenes for each method.
