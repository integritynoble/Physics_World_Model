...
\midrule
\textbf{Mean} & 28.06 & 18.51 & 26.21 & 80.7\% & 31.85 & 19.02 & 27.45 & 65.7\% & 30.98 & 19.40 & 29.78 & \best{89.7\%} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\caption{Per-scene/video recovery ratio ($\rho$) heatmaps for CASSI (top, 10 scenes $\times$ 4 methods), CACTI (middle, 6 videos $\times$ 4 methods), and SPC (bottom, 11 images $\times$ 3 methods). Red indicates low recovery; green indicates high recovery. An em-dash (---) indicates $\rho$ is undefined because degradation is below 0.5\,dB (the method is naturally robust to mismatch); this applies to CASSI GAP-TV, whose iterative optimization adapts to the actual measurement operator at inference time. Scene-dependent patterns are evident: CACTI's \textit{drop} video has notably lower recovery due to its high-frequency content.}
\label{fig:heatmaps}
\end{figure}
