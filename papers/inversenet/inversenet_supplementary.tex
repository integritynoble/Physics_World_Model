...
\begin{itemize}
    ...
    \item \textbf{Reference reconstructions}: \texttt{Recon\_scene1.mat} through \texttt{Recon\_scene5.mat}, each a $660 \times 660 \times 28$ hyperspectral cube reconstructed by the dataset authors using their best algorithm. These serve as the \emph{evaluation reference}---see below.
\end{itemize}

\subsubsection{Why reference reconstructions, not ground truth?}
...
Therefore, \textbf{no ground truth exists} for real CASSI data.

The published reference reconstructions are the best available approximation: they were produced by the dataset authors using their strongest algorithm with the calibrated hardware mask.
We use these as an evaluation reference to compute PSNR, with two important caveats:
\begin{enumerate}
    \item \textbf{Absolute PSNR values are not directly comparable to simulation.} In simulation (Table~1 of the main paper), PSNR is computed against the true spectral cube; on real data, PSNR is computed against an imperfect algorithmic reconstruction. This explains why real-data PSNR values (17--26\,dB) are lower than might be expected.
    \item \textbf{The diagnostic quantity is the \emph{difference} $\Delta$, not the absolute value.} Since both calibrated and mismatched reconstructions are compared against the \emph{same} reference, the difference $\Delta = \text{PSNR}_{\text{cal}} - \text{PSNR}_{\text{mis}}$ isolates the effect of mask mismatch, independent of reference quality.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{HDNet}: deep network operating on the full $660 \times 714$ measurement. Mask-oblivious: does not take the mask as input, only the initial spectral estimate.
    \item \textbf{MST-S, MST-L}: mask-guided spectral transformers. Due to a hardcoded $256 \div \text{height}$ operation in the architecture's \texttt{shift\_back()} function, MST cannot process $660 \times 660$ inputs directly. We therefore \textbf{center-crop} both measurement ($660 \times 714 \to 256 \times 310$) and mask ($660 \times 660 \to 256 \times 256$) before running MST. PSNR is computed against the corresponding crop of the reference reconstruction. This is noted with $\dagger$ in Table~5 of the main paper.
\end{itemize}

\subsubsection{Per-scene results.}
\Cref{tab:cassi_real_perscene} reports per-scene PSNR.

\begin{table}[h]
\centering
\caption{CASSI real data per-scene PSNR~(dB) against published reference reconstructions. $\Delta$: calibrated $-$ mismatched. Mismatch: spatial shift only ($dx{=}0.5$, $dy{=}0.3$\,px, no dispersion perturbation).}
\label{tab:cassi_real_perscene}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}l|cc|c|cc|c|cc|c|cc|c@{}}
\toprule
& \multicolumn{3}{c|}{\textbf{GAP-TV}} & \multicolumn{3}{c|}{\textbf{HDNet}} & \multicolumn{3}{c|}{\textbf{MST-S$^\dagger$}} & \multicolumn{3}{c}{\textbf{MST-L$^\dagger$}} \\
\textbf{Scene} & \textbf{Cal.} & \textbf{Mis.} & $\boldsymbol{\Delta}$ & \textbf{Cal.} & \textbf{Mis.} & $\boldsymbol{\Delta}$ & \textbf{Cal.} & \textbf{Mis.} & $\boldsymbol{\Delta}$ & \textbf{Cal.} & \textbf{Mis.} & $\boldsymbol{\Delta}$ \\
\midrule
Scene 1 & 23.83 & 24.18 & $-$0.35 & 25.92 & 25.92 & 0.00 & 19.39 & 20.27 & $-$0.88 & 18.74 & 18.82 & $-$0.08 \\
Scene 2 & 21.40 & 21.52 & $-$0.12 & 26.27 & 26.27 & 0.00 & 20.85 & 20.82 & 0.03 & 20.40 & 20.37 & 0.03 \\
Scene 3 & 20.48 & 20.61 & $-$0.13 & 24.93 & 24.93 & 0.00 & 18.23 & 18.27 & $-$0.04 & 18.09 & 18.15 & $-$0.06 \\
Scene 4 & 21.11 & 21.20 & $-$0.09 & 26.89 & 26.89 & 0.00 & 16.05 & 16.13 & $-$0.08 & 15.76 & 15.70 & 0.06 \\
Scene 5 & 21.61 & 21.71 & $-$0.10 & 25.35 & 25.35 & 0.00 & 16.78 & 16.93 & $-$0.15 & 16.00 & 15.97 & 0.03 \\
\midrule
\textbf{Mean} & 21.69 & 21.84 & $-$0.16 & 25.87 & 25.87 & 0.00 & 18.26 & 18.48 & $-$0.22 & 17.80 & 17.80 & $-$0.00 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{enumerate}
    \item \textbf{Negligible $\Delta$}: All methods show $|\Delta| \leq 0.88$\,dB (mean $|\Delta| \leq 0.22$\,dB). Spatial mask shift of 0.5\,px does not significantly degrade real CASSI reconstruction.
    \item \textbf{HDNet $\Delta = 0$ everywhere}: HDNet does not take the mask as input, so changing the mask has zero effect on its output. This perfectly matches the ``mask-oblivious'' classification from simulation.
    \item \textbf{Negative $\Delta$ for some methods}: Some methods actually produce slightly \emph{higher} PSNR under the shifted mask. This is possible because the reference reconstructions themselves were produced with the calibrated mask and contain their own reconstruction artefacts; a small mask shift can coincidentally reduce the distance to this imperfect reference.
    \item \textbf{Comparison with simulation}: The simulation experiments (Table~1 of the main paper) show 3.38--13.98\,dB degradation under a \emph{5-parameter} mismatch model that includes dispersion perturbation. The near-zero $\Delta$ on real data (with spatial-only mismatch) confirms that \textbf{dispersion mismatch is the dominant degradation mechanism for CASSI}, consistent with the architectural analysis in the main paper.
\end{enumerate}
...
\end{enumerate}
Methods: \textbf{GAP-TV} (50 iterations, $\lambda_{\text{TV}}{=}0.1$) and \textbf{EfficientSCI} (pretrained, loaded with \texttt{strict=False} since the checkpoint was trained for $\text{cr}{=}8$ while we use $\text{cr}{=}10$).
...
\begin{enumerate}
    ...
    \item \textbf{EfficientSCI residual increases only $\sim$1.3$\times$}: EfficientSCI is an end-to-end learned method whose reconstruction is less directly coupled to the mask. The small residual increase is consistent with the observation from simulation that learned methods produce outputs with moderate data fidelity regardless of the mask.
    \item \textbf{Absolute residual levels}: GAP-TV achieves orders-of-magnitude lower calibrated residuals ($\sim$10$^{-5}$) than EfficientSCI ($\sim$10$^{-2}$), because GAP-TV explicitly minimises data fidelity while EfficientSCI optimises a learned objective. This does not necessarily mean GAP-TV produces better visual quality---only that its reconstruction is more consistent with the assumed forward model.
\end{enumerate}
...
\begin{table}[h]
\centering
\caption{Comparison of simulation and real hardware findings for CASSI and CACTI. Simulation has ground truth; real data uses proxy metrics.}
...
\midrule
\textbf{CASSI} & & \\
\quad Ground truth & True hyperspectral cube & None (use reference recon.) \\
\quad Metric & PSNR against ground truth & PSNR against reference \\
\quad Mismatch & 5 params (spatial + dispersion) & 2 params (spatial only) \\
\quad Degradation & 3.38--13.98\,dB & $\leq$0.22\,dB \\
...
\quad Degradation & 10.94--20.58\,dB & Residual increases 10$\times$ (GAP-TV) \\
\quad Key finding & All methods collapse & Spatial shift alone causes large data infidelity \\
\bottomrule
\end{tabular}
\end{table}

The real hardware experiments confirm two simulation findings: (1)~mask-oblivious methods (HDNet) are completely insensitive to mask mismatch, and (2)~the severity of spatial mismatch varies by modality---negligible for CASSI (where dispersion dominates) but severe for CACTI (where the mask directly modulates each frame).
