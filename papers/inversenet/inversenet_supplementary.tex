...
\midrule
Scene 1 & 0.0015 & 0.0030 & 2.0$\times$ & 0.185 & 0.185 & 1.0$\times$ & 0.100 & 0.107 & 1.1$\times$ & 0.101 & 0.095 & 0.9$\times$ \\
Scene 2 & 0.0021 & 0.0033 & 1.6$\times$ & 0.149 & 0.149 & 1.0$\times$ & 0.254 & 0.238 & 0.9$\times$ & 0.254 & 0.228 & 0.9$\times$ \\
Scene 3 & 0.0020 & 0.0032 & 1.6$\times$ & 0.183 & 0.183 & 1.0$\times$ & 0.094 & 0.092 & 1.0$\times$ & 0.101 & 0.091 & 0.9$\times$ \\
Scene 4 & 0.0015 & 0.0030 & 2.0$\times$ & 0.144 & 0.144 & 1.0$\times$ & 0.156 & 0.147 & 0.9$\times$ & 0.179 & 0.159 & 0.9$\times$ \\
Scene 5 & 0.0023 & 0.0042 & 1.8$\times$ & 0.113 & 0.113 & 1.0$\times$ & 0.120 & 0.120 & 1.0$\times$ & 0.130 & 0.122 & 0.9$\times$ \\
\midrule
\textbf{Mean} & 0.0019 & 0.0033 & 1.8$\times$ & 0.155 & 0.155 & 1.0$\times$ & 0.145 & 0.141 & 1.0$\times$ & 0.153 & 0.139 & 0.9$\times$ \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{HDNet ratio $= 1.0\times$ everywhere}: HDNet does not take the mask as input, so it produces the \emph{identical} reconstruction regardless of the mask condition. The residual is therefore exactly the same for both conditions, confirming the ``mask-oblivious'' classification from simulation.
    \item \textbf{MST-S/L ratios $\approx 0.9$--$1.0\times$}: The mask-guided transformers show near-unity ratios, indicating their learned spectral priors are largely insensitive to small spatial shifts (0.5\,px). The reconstruction changes minimally between calibrated and shifted masks.
...
The real hardware experiments confirm two simulation findings: (1)~mask-oblivious methods (HDNet) are completely insensitive to mask mismatch (residual ratio exactly 1.0$\times$), and (2)~the severity of spatial mismatch varies by modality---moderate for CASSI ($1.8\times$ for GAP-TV, where dispersion dominates over spatial shift) but severe for CACTI ($10.4\times$ for GAP-TV, where the mask directly modulates each frame).
