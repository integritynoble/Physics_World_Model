...
\end{equation}
This metric requires \emph{only} the real measurement $\mathbf{y}$ and the assumed mask $M$---no ground truth.
It measures how well the reconstruction, when re-measured through the assumed operator, reproduces the original detector image.
...
\begin{itemize}
    ...
    \item \textbf{MST-S, MST-L}: mask-guided spectral transformers. Due to a hardcoded $256 \div \text{height}$ operation in the architecture's \texttt{shift\_back()} function, MST cannot process $660 \times 660$ inputs directly. We therefore \textbf{center-crop} both measurement ($660 \times 714 \to 256 \times 310$) and mask ($660 \times 660 \to 256 \times 256$) before running MST. The measurement residual is computed using the cropped measurement and mask. This is noted with $\dagger$ in Table~5 of the main paper.
\end{itemize}
...
\begin{table}[h]
\centering
\caption{CASSI real data per-scene normalised measurement residual $r = \|\mathbf{y} - \mathbf{\Phi}\hat{\mathbf{x}}\|^2 / \|\mathbf{y}\|^2$ (lower is better). Ratio: mismatched\,/\,calibrated. Mismatch: spatial shift only ($dx{=}0.5$, $dy{=}0.3$\,px, no dispersion perturbation). $^\dagger$MST evaluated on center $256 \times 256$ crop.}
...
\textbf{Scene} & \textbf{Cal.} & \textbf{Mis.} & \textbf{Ratio} & \textbf{Cal.} & \textbf{Mis.} & \textbf{Ratio} & \textbf{Cal.} & \textbf{Mis.} & \textbf{Ratio} & \textbf{Cal.} & \textbf{Mis.} & \textbf{Ratio} \\
\midrule
Scene 1 & 0.0015 & 0.0009 & 0.6$\times$ & 0.185 & 0.187 & 1.0$\times$ & 0.100 & 0.109 & 1.1$\times$ & 0.101 & 0.097 & 1.0$\times$ \\
Scene 2 & 0.0021 & 0.0013 & 0.6$\times$ & 0.149 & 0.150 & 1.0$\times$ & 0.254 & 0.225 & 0.9$\times$ & 0.254 & 0.214 & 0.8$\times$ \\
Scene 3 & 0.0020 & 0.0008 & 0.4$\times$ & 0.183 & 0.185 & 1.0$\times$ & 0.094 & 0.082 & 0.9$\times$ & 0.101 & 0.082 & 0.8$\times$ \\
Scene 4 & 0.0015 & 0.0009 & 0.6$\times$ & 0.144 & 0.146 & 1.0$\times$ & 0.156 & 0.145 & 0.9$\times$ & 0.179 & 0.156 & 0.9$\times$ \\
Scene 5 & 0.0023 & 0.0016 & 0.7$\times$ & 0.113 & 0.115 & 1.0$\times$ & 0.120 & 0.120 & 1.0$\times$ & 0.130 & 0.121 & 0.9$\times$ \\
\midrule
\textbf{Mean} & 0.0019 & 0.0011 & 0.6$\times$ & 0.155 & 0.157 & 1.0$\times$ & 0.145 & 0.136 & 1.0$\times$ & 0.153 & 0.134 & 0.9$\times$ \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{enumerate}
    \item \textbf{Residual ratios $\leq 1$}: All methods show mismatched-to-calibrated residual ratios between 0.4$\times$ and 1.1$\times$ (mean $\leq 1.0\times$). Spatial mask shift of 0.5\,px does not increase the measurement residual on real CASSI data, confirming that spatial shift alone is not a significant degradation source.
    \item \textbf{HDNet ratio $= 1.0\times$ everywhere}: HDNet does not take the mask as input, so changing the mask has zero effect on its output. The residual ratio of exactly 1.0$\times$ across all scenes perfectly matches the ``mask-oblivious'' classification from simulation.
    \item \textbf{GAP-TV ratio $< 1$}: GAP-TV actually achieves \emph{lower} residual under the shifted mask (mean ratio 0.6$\times$). This is an artefact of iterative optimisation: since GAP-TV explicitly minimises the data fidelity term, a small mask shift changes the optimisation landscape and the algorithm may converge to a different minimum that happens to have lower residual but not necessarily better spectral reconstruction.
    \item \textbf{Large absolute residual differences across methods}: GAP-TV achieves residuals $\sim$10$^{-3}$ while deep learning methods show residuals $\sim$10$^{-1}$. This reflects their fundamentally different objectives: GAP-TV directly minimises data fidelity, while HDNet/MST use learned priors that prioritise perceptual quality over measurement consistency.
    \item \textbf{Comparison with simulation}: The simulation experiments (Table~1 of the main paper) show 3.38--13.98\,dB degradation under a \emph{5-parameter} mismatch model that includes dispersion perturbation. The near-unity residual ratios on real data (with spatial-only mismatch) confirm that \textbf{dispersion mismatch is the dominant degradation mechanism for CASSI}, consistent with the architectural analysis in the main paper.
\end{enumerate}
...
\quad Degradation & 3.38--13.98\,dB loss & Residual ratio $\leq 1.0\times$ \\
...
The real hardware experiments confirm two simulation findings: (1)~mask-oblivious methods (HDNet) are completely insensitive to mask mismatch (residual ratio exactly 1.0$\times$), and (2)~the severity of spatial mismatch varies by modality---negligible for CASSI (residual ratios $\leq 1.0\times$, where dispersion dominates) but severe for CACTI (residual increases $10\times$, where the mask directly modulates each frame).
