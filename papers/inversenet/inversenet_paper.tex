...
Scenario~IV evaluates blind calibration via grid search over mismatch parameters using the measurement residual $\|\mathbf{y} - \mathbf{\Phi}'(\hat{\mathbf{x}})\|^2$ as objective, with a classical solver as inner loop.
\Cref{tab:scenario_iv} shows the results.
CACTI achieves near-perfect calibration ($26.99$\,dB, matching Scenario~III).
CASSI recovers $78\%$ of the gap ($+1.36$\,dB of $+1.74$\,dB), with dispersion parameters not covered by the spatial-only search.
For SPC, the measurement-residual objective cannot distinguish the true gain drift from $\alpha{=}0$, suggesting that more sophisticated objectives may be needed for radiometric mismatch.
