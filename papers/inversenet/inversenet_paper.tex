...
Since \textbf{no ground truth} exists for real CASSI captures (the true spectral cube cannot be independently measured), we use the same ground-truth-free metric as for CACTI: the normalised measurement residual $r = \|\mathbf{y} - \mathbf{\Phi}\hat{\mathbf{x}}\|^2 / \|\mathbf{y}\|^2$.
...
\begin{table}[t]
\centering
\caption{CASSI real data (5 scenes, $660 \times 660 \times 28$). Normalised measurement residual $r = \|\mathbf{y} - \mathbf{\Phi}\hat{\mathbf{x}}\|^2 / \|\mathbf{y}\|^2$ (lower is better; no ground truth required). Ratio: mismatched\,/\,calibrated. $^\dagger$MST evaluated on center $256 \times 256$ crop.}
\label{tab:cassi_real}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Method} & \textbf{Calibrated} & \textbf{Mismatched} & \textbf{Ratio} \\
\midrule
GAP-TV            & 0.0019 & 0.0011 & 0.6$\times$ \\
HDNet             & 0.155  & 0.157  & 1.0$\times$ \\
MST-S$^\dagger$   & 0.145  & 0.136  & 1.0$\times$ \\
MST-L$^\dagger$   & 0.153  & 0.134  & 0.9$\times$ \\
\bottomrule
\end{tabular}
\end{table}

Spatial mask shift produces residual ratios of $0.6$--$1.0\times$---the measurement residual does \emph{not} increase under mismatch.
...
The difference arises because CASSI's dispersive forward model creates spectral overlap in each detector pixel: a small mask shift redistributes which spectral information is multiplexed, but does not break measurement consistency.
GAP-TV, which explicitly minimises the residual during reconstruction, achieves very low absolute residuals ($\sim$0.002) and its residual actually \emph{decreases} under the shifted mask ($0.6\times$).
HDNet shows ratio $1.0\times$, consistent with its mask-oblivious architecture.
...
This validates that \textbf{dispersion mismatch, not spatial shift alone, is the dominant CASSI degradation source} (\cref{fig:cassi_real}).

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/cassi_real_comparison.pdf}
\caption{CASSI real data (Scene~1, band~14, central $256 \times 256$ patch): calibrated (top) vs.\ mismatched (bottom) reconstructions for four methods. The visual differences are subtle, consistent with the near-unity residual ratios in \cref{tab:cassi_real}---spatial mask shift alone is not the dominant degradation source for CASSI.}
