...
Across 27 test scenes and 11 methods, we find that operator mismatch degrades deep learning methods by 10--21\,dB, with mask-aware architectures both the most sensitive and the most recoverable through calibration (up to 47\% recovery for CASSI, 90\% for SPC).
...
The residual gap $\Delta_{\text{res}} = \text{PSNR}_{\text{I}} - \text{PSNR}_{\text{III}}$ represents unrecoverable losses due to measurement corruption and architectural limitations in the oracle correction.
