...
HDNet             & 0.155  & 0.155  & 1.0$\times$ \\
MST-S$^\dagger$   & 0.145  & 0.141  & 1.0$\times$ \\
MST-L$^\dagger$   & 0.153  & 0.139  & 0.9$\times$ \\
\bottomrule
\end{tabular}
\end{table}

GAP-TV shows the largest residual increase ($1.8\times$), because it explicitly fits the forward model during reconstruction: when reconstructed with a wrong mask, the result is inconsistent with the true measurement.
HDNet shows ratio exactly $1.0\times$, consistent with its mask-oblivious architecture (identical output regardless of mask).
MST-S/L show ratios $\approx 1\times$, indicating their learned spectral priors are largely insensitive to small spatial shifts.
