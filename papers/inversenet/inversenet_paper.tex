...
\caption{CASSI mismatch degradation (left, $\Delta_{\text{deg}}$) and oracle recovery (right, $\Delta_{\text{rec}}$) per method under 5-parameter mismatch. HDNet shows zero recovery due to its mask-oblivious architecture. MST-L achieves the highest recovery (+6.50\,dB), though the moderate recovery ratio (46.5\%) reflects the difficulty of correcting dispersion mismatch with current fixed-step architectures.}
\label{fig:cassi_gap}
\end{figure}
