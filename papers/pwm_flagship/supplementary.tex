...
Scenario~I (ideal) PSNR values and recovery ratios ($\rho$) for all configurations are reported in the companion Rail paper's evidence section and in the RunBundle manifests in the code repository.}
...
Sc.~II = mismatched (baseline); Sc.~III-A1 = correction Algorithm~1; Sc.~IV (Oracle) = ground-truth operator through full pipeline.
...
\textbf{Scene} & \textbf{Method} &
  \textbf{Sc.~I} & \textbf{Sc.~II} & \textbf{Sc.~III-A1} & \textbf{Sc.~IV} \\
\midrule
\endfirsthead
\toprule
\textbf{Scene} & \textbf{Method} &
