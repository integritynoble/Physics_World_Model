...
GAP-TV values for Sc.~I, Sc.~II, and Sc.~III-A1 are from InverseNet validation~\cite{yang2026inversenet}. Sc.~III-A1 entries marked $^*$ use InverseNet Oracle Mask values (true operator on mismatched data) as upper bounds; Algorithm~1 re-validation under the 5-parameter mismatch is pending. The Sc.~IV column in this table reports a pipeline sanity check (true operator through the full pipeline), verifying no artifacts are introduced ($\pm 0.01$~dB from Sc.~I). Note: the main text defines Scenario~IV as the Oracle Mask (correction ceiling); the oracle correction values for GAP-TV are reported in the Sc.~III-A1 column.
