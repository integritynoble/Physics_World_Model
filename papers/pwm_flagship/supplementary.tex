...
five representative methods under four scenarios.
...
Table~\ref{tab:yaml} summarises the nine YAML registry files that define the
PWM framework's modular architecture.
...
\midrule
1 & \texttt{modalities.yaml}         & 312 & Defines all 26 modalities with physical carrier, compression ratio, default solver, and citation keys. \\
2 & \texttt{solver\_registry.yaml}   & 187 & Maps solver names to implementations (ADMM, FISTA, GAP-TV, PnP, learned). \\
3 & \texttt{operator\_registry.yaml} & 245 & Registers forward/adjoint operator classes with shape contracts. \\
4 & \texttt{dataset\_registry.yaml}  & 156 & Links modalities to benchmark datasets (KAIST, fastMRI, etc.). \\
5 & \texttt{metric\_registry.yaml}   &  78 & Defines quality metrics (PSNR, SSIM, LPIPS) with normalization. \\
6 & \texttt{correction\_params.yaml} & 134 & Lists all 16 correction parameters with bounds and learning rates. \\
7 & \texttt{triad\_config.yaml}      &  92 & Triad Law gate thresholds and reporting format. \\
8 & \texttt{runbundle\_schema.yaml}  & 168 & RunBundle v0.3.0 JSON schema definition. \\
9 & \texttt{hardware\_profiles.yaml} &  64 & GPU/CPU profiles for runtime estimation and RoIC computation. \\
\bottomrule
\end{tabular}
\end{table}
