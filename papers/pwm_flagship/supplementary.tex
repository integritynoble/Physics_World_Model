...
Algorithm~2 results are reported in the correction parameter table (Table~\ref{tab:correction_params}).\footnote{Per-scene values in this table were obtained from an earlier experimental run; the correction parameter table (Table~\ref{tab:correction_params}) reports values from the final calibration run. Minor differences ($< 0.2$~dB) arise from different random seeds used in the mismatch simulation.}}
