...
The Triad Law formalises three successive gates that every computational-imaging
reconstruction must pass.  We derive rigorous bounds for each gate and show how
the \emph{recovery ratio} $\rho$ summarises the overall reconstruction quality.
...
favourable operator conditioning.
...
denote the parameterised forward operator.
...
regularisation benefits from the corrected operator), $\rho$ may exceed~1.

\begin{proposition}
Under convex reconstruction loss and convex constraint on
...
%% ============================================================
\section{Supplementary Note 3: All 16 Modality Correction Parameters}
\label{sec:params}

Table~\ref{tab:correction_params} reports the correction results for all 16
calibration parameters across the validated modalities.  The first 9 modalities
...
Table~\ref{tab:yaml} summarises the nine YAML registry files (totalling
...
3 & \texttt{photon\_db.yaml}           & 624 & Photon models parameterised by source power, quantum efficiency, exposure, and detector. \\
...
The \texttt{RunBundle} is PWM's reproducibility artefact.  Every experiment
...
\texttt{operator\_state}& object   & Serialised OperatorGraph including all node parameters and edge metadata. \\
...
\texttt{solver\_config} & object   & Complete solver hyperparameters (algorithm, iterations, step size, regularisation weight, denoiser checkpoint). \\
\texttt{modality}       & string   & Modality identifier matching \texttt{modalities.yaml}. \\
\texttt{scenario}       & string   & One of \texttt{I} (ideal), \texttt{II} (mismatch), \texttt{III} (corrected). \\
...
\begin{itemize}
  ...
  \item \textbf{CASSI Algorithm~2} has the lowest RoIC (6.5\,dB/GPU-hr)
        because it jointly optimises mask geometry and dispersion over 2000
...
\begin{itemize}
  ...
  \item The OperatorGraph compilation overhead is amortised: once compiled,
