...
The $+48.25$\,dB correction gain reported for MRI in the main text reflects a single-coil scenario with 5\% multiplicative sensitivity error---a pathological case chosen to stress-test the framework. Under clinically realistic multi-coil conditions (8 coils, $256 \times 256$, $4\times$ acceleration, CG-SENSE reconstruction) with spatially smooth sensitivity errors simulating patient repositioning, the expected correction gains are substantially more modest: $+3$ to $+8$\,dB for 2--3\% mismatch (the clinically realistic range). These gains remain clinically meaningful, as even $+3$\,dB corresponds to a ${\sim}30\%$ reduction in mean-squared error, which can improve diagnostic confidence in parallel imaging reconstructions. Full quantitative results will be reported upon completion of the multi-coil validation experiments using the \texttt{run\_mri\_realistic.py} pipeline.
