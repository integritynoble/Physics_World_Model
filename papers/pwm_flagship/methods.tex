...
reconstruction regularization, produces slightly higher PSNR than the
ideal oracle---a rare phenomenon attributable to beneficial regularization
bias.
...
\begin{enumerate}
  ...
  \item \textbf{3D beam search.}  The mask affine subspace
    ...
    centerd on the 1D optima.  The top-$k$ ($k = 5$) candidates by
...
\begin{description}
  ...
  \item[Scenario~IV (Oracle):]
    $\yobs = \Htrue\, \xgt$; reconstruct with $\Htrue$ applied to
    the mismatched data.
    This provides an upper bound on what model correction alone can
    achieve, isolating the operator-mismatch contribution from solver
...
is ISTA~\cite{beck2009fista} (iterative shrinkage-thresholding algorithm) with a wavelet
sparsifying transform.

\paragraph{MRI configuration.}
Cartesian $k$-space sampling with 4$\times$ acceleration (25\% of
$k$-space lines acquired).  Mismatch is parameterized as errors in
