...
The \texttt{graph\_templates.yaml} registry contains 90 templates
spanning all five carrier families. Of these, 26 modalities carry formal
validation status (7~with full end-to-end correction validation, 1~with
Scenario~I baseline, 18~with template-level validation; Supplementary
Table~S3). Representative modalities by carrier family include:

\begin{itemize}
  \item \textbf{Photons (optical and X-ray):}  CASSI, SPC, CACTI, structured
  ...
  \item \textbf{Particles:}  Neutron tomography, proton radiography,
    and muon tomography.
\end{itemize}
...
The default solver is SENSE~\cite{pruessmann1999sense} with $\ell_1$-wavelet regularization.

\paragraph{ESPIRiT comparison protocol.}
To quantitatively compare PWM with ESPIRiT~\cite{uecker2014espirit}, we evaluate
four conditions on the same multi-coil MRI configuration: (1)~Scenario~I (true maps),
(2)~Scenario~II (5\% mismatched maps), (3)~ESPIRiT auto-calibrated maps estimated
from the 24-line ACS region via eigenvalue decomposition of the calibration matrix,
and (4)~PWM beam-search corrected maps (grid search over per-coil amplitude and phase
scaling). All four conditions use the same CG-SENSE solver ($\ell = 10^{-3}$, 30~iterations).
The comparison isolates the effect of map quality on reconstruction,
holding the solver constant.
...
\paragraph{CASSI real-data configuration.}
...
\paragraph{CACTI real-data configuration.}
...
error present in any production system.

\paragraph{Clinical CT phantom configuration.}
For clinical translation, \pwm{} is evaluated on CT quality assurance
using the ACR CT accreditation phantom (Gammex 464).
The phantom contains inserts of known attenuation (bone ${\sim}955$\,HU,
air ${\sim}{-}1000$\,HU, acrylic ${\sim}121$\,HU, polyethylene
${\sim}{-}96$\,HU) and geometric targets for measuring spatial
resolution, slice thickness, and low-contrast detectability.  Mismatch
is parameterized as center-of-rotation offset ($\Delta r$, mm), beam
hardening coefficient drift ($\Delta \mu$, \%), and detector gain
variation ($\Delta g$, \%).  Ten ACR-aligned metrics are computed
automatically: CT number accuracy for five materials (water, bone, air,
acrylic, polyethylene), geometric accuracy ($\pm 2$\,mm tolerance),
slice thickness ($\pm 1.5$\,mm), uniformity ($\leq 5$\,HU), noise
standard deviation, and spatial resolution ($\geq 5$\,lp/cm).

\paragraph{Clinical MRI validation configuration.}
For MRI clinical validation, \pwm{} processes multi-coil $k$-space data
from public datasets (fastMRI~\cite{zbontar2018fastmri}).  Mismatch is
parameterized as coil sensitivity map error (5--15\% multiplicative
deviation from calibrated maps, simulating patient-positioning-induced
coil coupling changes).  The default solver is CG-SENSE with $\ell_1$-wavelet
regularization at $4\times$ acceleration.  Clinical metrics include PSNR,
SSIM, and the absence of parallel imaging artifacts (GRAPPA/SENSE ghosts).

\paragraph{Ultrasound configuration.}
Pulse-echo B-mode imaging is simulated with a 64-element linear array,
5\,MHz center frequency, element pitch $0.3$\,mm, and speed of sound (SoS)
$c = 1540$\,m/s.  The tissue phantom is a $64 \times 64$ reflectivity map
with Rayleigh-distributed speckle, point scatterers, and anechoic cyst
regions.  Frequency-dependent attenuation follows
$\alpha(f) = 0.5\,\text{dB\,cm}^{-1}\text{MHz}^{-1}$.  Mismatch is
parameterized as SoS error $\Delta c \in \{10, 25, 50, 100\}$\,m/s,
...
21 SoS candidates in $[1400, 1600]$\,m/s, selecting the value that
minimizes the RF channel residual $\|y - A_{\hat{c}} \hat{x}\|^2 / \|y\|^2$.

\paragraph{Cryo-EM configuration.}
Cryo-EM imaging is modeled as contrast transfer function (CTF) filtering
of a 2D projected potential ($64 \times 64$).  The CTF is parameterized by
defocus $\Delta f = -500$\,nm, spherical aberration $C_s = 2.0$\,mm,
electron wavelength $\lambda = 0.00197$\,nm (300\,kV), with a B-factor
envelope ($B = 50$\,\AA$^2$) and ice-thickness attenuation (mean free
path $350$\,nm, thickness $80$\,nm).  Mismatch is parameterized as defocus
error $\Delta(\Delta f) \in \{25, 50, 100, 200\}$\,nm.  The default solver
is Wiener filtering with SNR\,$= 100$.  Calibration uses grid search over
21 defocus values in $[-800, -200]$\,nm, selecting the value that minimizes
the Fourier-domain measurement residual.

\paragraph{Cone-beam CT configuration.}
Fan-beam CBCT is simulated with source-to-origin distance $D_{\mathrm{so}} = 100$\,cm,
source-to-detector distance $D_{\mathrm{sd}} = 150$\,cm, 180 projection
angles over $[0, \pi)$, and 91 detector elements.  The object is a
$64 \times 64$ Shepp--Logan phantom.  Mismatch is parameterized as detector
offset $\Delta s \in \{1, 2, 5, 10\}$\,pixels.  The default solver is
FDK reconstruction (Ram-Lak ramp filtering followed by distance-weighted
backprojection).  Calibration uses grid search over 25 offset candidates in
$[-12, 12]$\,px, selecting the value that maximizes reconstruction PSNR.
...
four depth planes ($z \in \{500, 1000, 1500, 2000\}$\,\textmu m) with
wavelength $\lambda = 632.8$\,nm and pixel pitch $6.45$\,\textmu m.
Mismatch is parameterized as propagation distance error
$\Delta z \in \{10, 50, 100, 200\}$\,\textmu m applied uniformly to all
depth planes.  The default solver is FISTA-TV (200 iterations,
$\lambda_{\mathrm{TV}} = 0.01$).  Calibration uses a sharpness-based
autofocus: Brenner gradient is maximized over a coarse grid of 51
candidates in $[-250, 250]$\,\textmu m, followed by local refinement.
