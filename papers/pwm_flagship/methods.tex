...
The remaining three agents handle asset management (\texttt{AssetManager}), system discernment (\texttt{SystemDiscern}), and pre-flight validation (\texttt{PreflightChecker}).
...
\begin{description}
  ...
  \item[Scenario~IV (Oracle):]
    $\yobs = \Htrue\, \xgt$; reconstruct with $\Htrue$.
    A sanity-check scenario that verifies pipeline consistency by
