...
\begin{description}
  ...
  \item[Scenario~IV (Oracle sanity check):]
    $\yobs = \Htrue\, \xgt$; reconstruct with $\Htrue$ through the
    full correction pipeline (bypassing the correction step but
    executing all pre- and post-processing stages).  Verifies that
    the pipeline introduces no artifacts and reproduces Scenario~I
    results to within $\pm 0.01$\,dB.
\end{description}
