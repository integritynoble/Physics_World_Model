...
or optimised during calibration, as well as read-only metadata flags
...
\begin{itemize}
  ...
  \item \textbf{Nonlinear operators.}  Squared magnitude
    ...
    (\texttt{phase\_abs}), and detector quantisation
    (\texttt{quantize}).
...
Correctness of every linear primitive is verified by a randomised
...
the primitive passes if $\delta_{\max} < 10^{-5}$.  At the graph level,
...
\end{enumerate}
The compiled \texttt{GraphOperator} is serialisable to JSON and hashable
...
organised across 64 modalities, grouped by physical carrier:
...
\begin{itemize}
  \item \textbf{Photons (optical):}  CASSI, SPC, CACTI, structured
    ...
    photoacoustic imaging, and phase retrieval.
...
Each template is parameterised by a fidelity tier that controls the
degree of physical realism in the simulated forward model:

\begin{description}
  \item[Tier~1 (Ideal):]  Noiseless measurement with perfectly known
    operator parameters.  Establishes the upper-bound reconstruction
    quality limited only by the sensing geometry.
  \item[Tier~2 (Noisy):]  Adds a modality-appropriate noise model
    (Poisson shot noise plus Gaussian read noise for photon-counting
    modalities, Rician noise for MRI, Poisson noise for CT).
    The noise model parameters are drawn from the \texttt{photon\_db.yaml}
    registry.
  \item[Tier~3 (Mismatched):]  Perturbs operator parameters
    $\boldsymbol{\theta}_{\text{true}} \neq \boldsymbol{\theta}_{\text{nom}}$
    while adding noise.  Perturbation families and ranges are specified in
    \texttt{perturbation\_families.yaml} and \texttt{mismatch\_db.yaml}.
  \item[Tier~4 (Full-wave):]  Complete physical simulation including
    wave-optical propagation, spatially varying aberrations, detector
    nonlinearities, and environmental drift.  Currently implemented for
    holography and ptychography; other modalities degrade gracefully to
    Tier~3.
\end{description}


%% =========================================================================
\subsection*{Triad Law Formalisation}
...
via randomised SVD for large operators), the dimension of the null space,
...
a deterministic photon model parameterised by source power, quantum
...
which catalogues, for each modality, the set of mismatch parameters
(spatial shifts, rotational offsets, dispersion errors, PSF deviations,
coil sensitivity errors, centre-of-rotation offsets, \etc{}), their
typical ranges, and available correction methods.  The mismatch severity
score $s \in [0, 1]$ is computed as the normalised $\ell_2$ distance
...
(Section~\ref{sec:methods:eval}), PWM identifies the dominant gate by
...
\end{equation}
which is bounded $\recoveryratio \in [0, 1]$ in the typical case.
...
calibration fully closes the mismatch gap.  Values $\recoveryratio > 1$
are occasionally observed when the corrected operator, combined with
reconstruction regularisation, produces slightly higher PSNR than the
ideal oracle---a rare phenomenon attributable to beneficial regularisation
bias.
...
The PWM agent system comprises 9 specialist agents and 8 support classes
totalling 10,545 lines of Python.  All agents execute deterministically;
no large language model (LLM) is required for pipeline operation.
An LLM may optionally be invoked via the \texttt{HybridAgent} wrapper for
natural-language narrative generation or edge-case modality mapping, but
all quantitative decisions are made by the deterministic code path.
...
authorised, the negotiator inspects all three upstream reports and applies
...
intermediate visualisations at each pipeline stage), and
...
JSON serialisation.  This design ensures that pipeline failures surface
...
angle $\varphi_d$), plus an optional PSF width $\sigma_{\text{psf}}$.
...
\begin{enumerate}
  ...
  \item \textbf{3D beam search.}  The mask affine subspace
    ...
    centred on the 1D optima.  The top-$k$ ($k = 5$) candidates by
    reconstruction PSNR are retained.
  \item \textbf{2D beam search.}  For each retained mask candidate,
    the dispersion subspace $(a_1, \varphi_d)$ is searched over a
...
optimise all mismatch parameters via gradient descent.  The key components
are:
...
\begin{enumerate}
  ...
  \item \textbf{GPU grid initialisation.}  A full-range 3D grid search
  ...
  \item \textbf{Staged gradient refinement.}  Each of the 9 candidates is
    refined using Adam optimisation (learning rate $10^{-2}$, decaying to
    $10^{-3}$) for 200 steps.  For each candidate, 4 random restarts with
    jittered initialisation guard against local minima.  The loss function
...
We evaluate every modality under four standardised scenarios that
...
\begin{description}
  ...
  \item[Scenario~II (Assumed/Baseline):]
  ...
  \item[Scenario~IV (Oracle Mask):]
    $\yobs = \Htrue\, \xgt$; reconstruct with $\Htrue$ but using nominal
    dispersion parameters.
    This partial oracle isolates the contribution of mask mismatch
    versus dispersion mismatch in CASSI, and is modality-specific.
\end{description}
...
\begin{itemize}
  \item \textbf{PSNR} (peak signal-to-noise ratio, in dB): the primary
    metric, computed per scene and averaged.  For signals normalised to
    $[0, 1]$, $\psnr = 10 \log_{10}(1 / \text{MSE})$.  For SPC data
    normalised to $[0, 255]$, the peak value is 255.
...
\paragraph{Datasets.}
\begin{itemize}
  \item \textbf{CASSI:} 10 scenes from the KAIST dataset~\cite{kaist},
...
dispersed along the spatial dimension.  The six-parameter mismatch model
$\mismatchparam = (dx, dy, \theta, a_1, \varphi_d, \sigma_{\text{psf}})$
describes: mask spatial shift in $x$ ($dx$, pixels), mask spatial shift in
$y$ ($dy$, pixels), mask rotation angle ($\theta$, degrees), dispersion
slope ($a_1$, pixels per band), dispersion axis angle ($\varphi_d$,
degrees), and PSF blur width ($\sigma_{\text{psf}}$, pixels).
For the primary mismatch experiment, the true mismatch parameters are
$\mismatchparam_{\text{true}} = (dx = 1.094,\; dy = -2.677,\;
\theta = -0.559^\circ,\; a_1 = \text{default},\;
\varphi_d = -0.316^\circ)$.
Solvers evaluated include GAP-TV, MST-L, and HDNet, all of which receive
the same operator and differ only in their reconstruction algorithm.
...
into a single snapshot measurement.  Mismatch is parameterised as a
temporal mask timing offset (sub-frame shift).  The default solver is
GAP-TV with total-variation regularisation.
...
($r = m / n \in \{0.10, 0.25, 0.50\}$).  Mismatch is modelled as a
multiplicative gain bias on the measurement matrix.  The default solver
is ISTA (iterative shrinkage-thresholding algorithm) with a wavelet
sparsifying transform.

\paragraph{MRI configuration.}
Cartesian $k$-space sampling with 4$\times$ acceleration (25\% of
$k$-space lines acquired).  Mismatch is parameterised as errors in
the coil sensitivity maps used for parallel imaging reconstruction.
The default solver is SENSE with $\ell_1$-wavelet regularisation.

\paragraph{CT configuration.}
Fan-beam geometry with 180 projections over $180^\circ$.  Mismatch is
modelled as a centre-of-rotation (CoR) offset, which produces
characteristic arc artefacts in the reconstruction.  The default solver
...
is released as open-source software under the MIT licence at
\url{https://github.com/integritynoble/Physics_World_Model}.  The
codebase is organised into two Python packages:
...
for loading.  Data ranges are standardised: CASSI and CACTI
reconstructions are normalised to $[0, 1]$; SPC reconstructions are in
$[0, 255]$.
...
hashes of all input data and output artefacts, metric values, and
