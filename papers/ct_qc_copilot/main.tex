...
degradation 3--8~months before threshold exceedance in 4 of 30
...
systems~\cite{samei2019performance}.
...
\caption{%
  ...
  $\sim$70~minutes for manual analysis.
}
\label{fig:workflow}
\end{figure}
...
gradual drift in noise, uniformity, or CT number over the final
6~months.
Synthetic data were generated by perturbing nominal metric values
with Gaussian noise (scanner-model-specific $\sigma$) and applying
linear drift ramps to the 4 degrading scanners.
...
Western Electric rules triggered 3--8~months before the drifting
...
CT\# drift     & 1 & 1 & 0 & 8 months    & WE 3,4 \\
Stable         & 26 & --- & 0 & --- & --- \\
\midrule
\textbf{Total} & \textbf{30} & \textbf{4/4} & \textbf{0} & \textbf{3--8 months} & \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{axis}[
  ...
  yticklabels={Total,Sign-off,Report writing,Trending,Threshold check,Metric computation,DICOM handling},
  ytick=data,
...
\addplot[fill=pwmRed!60, draw=pwmRed] coordinates {
  (5,DICOM) (20,Metrics) (5,Threshold)
  (10,Trending) (15,Report) (2,Signoff) (25,Total)
};
...
\addplot[fill=pwmGreen!60, draw=pwmGreen] coordinates {
  ...
  (0.01,Trending) (0.02,Report) (3.0,Signoff) (4.2,Total)
};
...
\caption{%
  ...
  Note: the ``Total'' for manual is the estimated sum across tasks
  with some parallelism; values shown are representative medians.
}
...
and metric computation (20~min), threshold checking (5~min),
trend maintenance (10~min), report writing (15~min), and review/sign-off
(2~min).
The Copilot reduced this to $4.2 \pm 0.8$~minutes, with computation
completing in under 5~seconds and physicist review averaging 3~minutes
...
accuracy within ACR tolerances, drift detection 3--8~months before
