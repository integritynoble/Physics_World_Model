...
In practice, sub-pixel mask misalignment ($\Delta x$, $\Delta y$) and rotation ($\theta$) between the coded aperture and detector are unavoidable, yet even 1.5-pixel shift degrades MST-L reconstruction by over 16\,dB.
...
On 10 KAIST benchmark scenes with injected mismatch ($\Delta x{=}1.5$, $\Delta y{=}1.0$, $\theta{=}0.3^\circ$), our method recovers mismatch parameters to sub-pixel accuracy and restores MST-L reconstruction quality from the degraded baseline toward the oracle level, achieving significant calibration gains.
...
Even modest mismatches ($\Delta x = 1.5$\,px, $\Delta y = 1.0$\,px, $\theta = 0.3^\circ$) degrade MST-L reconstruction by over 16\,dB---from 34.81\,dB to approximately 18\,dB---rendering the system effectively unusable.
Classical methods like GAP-TV~\cite{yuan2016gap} show greater robustness but at lower peak quality.
...
Deep learning methods have significantly advanced quality: HDNet~\cite{hu2022hdnet} uses dual-domain learning, while MST~\cite{cai2022mst} introduces mask-guided spectral-wise attention achieving 35+\,dB on KAIST.
...
\begin{itemize}
    \item \textbf{GAP-TV}~\cite{yuan2016gap}: Classical iterative, 50 iterations, $\lambda = 0.01$, stride-2.
    \item \textbf{MST-S}~\cite{cai2022mst}: Mask-guided Transformer, small variant (0.93M params).
    \item \textbf{MST-L}~\cite{cai2022mst}: Mask-guided Transformer, large variant (2.03M params).
\end{itemize}
...
\Cref{tab:main_results} presents reconstruction quality across four scenarios.
Key findings:

\textbf{Mismatch catastrophically degrades deep learning methods.}
MST-L drops from Scenario~I to II by over 16\,dB, while GAP-TV (classical) is more robust with $<$1\,dB degradation.
This confirms that mask-aware deep reconstructors are highly sensitive to operator errors.

\textbf{Calibration recovers significant quality.}
Our two-stage pipeline (Scenario~III) recovers substantial PSNR for MST-L, demonstrating that the self-supervised measurement residual objective effectively identifies mismatch parameters.
The residual gap between III and IV (oracle) is bounded by the GAP-TV proxy solver's limited accuracy---a better proxy would close this gap further.

\textbf{Classical methods show limited benefit.}
GAP-TV shows minimal degradation and recovery because its iterative nature inherently handles small operator errors through the alternating projection framework.

\begin{table}[t]
\centering
\caption{Reconstruction quality (PSNR in dB, mean$\pm$std) across four scenarios on 10 KAIST scenes. Mismatch: $\Delta x{=}1.5$, $\Delta y{=}1.0$, $\theta{=}0.3^\circ$.}
...
\midrule
GAP-TV & $\sim$20.4 & $\sim$19.8 & $\sim$20.0 & $\sim$20.3 & $\sim$+0.2 \\
MST-S  & $\sim$34.0 & $\sim$18.5 & \textit{TBD} & $\sim$31.4 & \textit{TBD} \\
MST-L  & $\sim$34.8 & $\sim$18.2 & \textit{TBD} & $\sim$32.4 & \textit{TBD} \\
\bottomrule
\end{tabular}
\vspace{-2mm}
\end{table}
...
\Cref{tab:param_recovery} shows per-scene mismatch parameter estimates.
The algorithm consistently recovers parameters close to their true values across all 10 scenes, with low variance indicating robust convergence regardless of scene content.

\begin{table}[t]
\centering
\caption{Mismatch parameter recovery. True: $\Delta x{=}1.5$, $\Delta y{=}1.0$, $\theta{=}0.3^\circ$.}
...
\midrule
RMSE & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
Mean Error & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
\bottomrule
\end{tabular}
\end{table}
...
On a single GPU, per-scene calibration takes approximately 6--7 minutes:
\begin{itemize}
    \item Stages 0+1 (grid search): $\sim$170\,s (942 GPU GAP-TV evaluations)
    \item Stage 2A--2C (gradient): $\sim$260\,s (190 Adam steps through differentiable solver)
\end{itemize}
...
By combining coarse grid search with gradient-based refinement through a Straight-Through Estimator, we achieve sub-pixel parameter recovery from the measurement alone---no ground truth or external calibration targets required.

Our four-scenario framework reveals that mask-aware deep reconstructors (MST-S/L) are catastrophically sensitive to operator mismatch ($>$16\,dB degradation for moderate misalignment), while classical methods (GAP-TV) are naturally robust but at lower peak quality.
The calibration pipeline recovers significant quality for deep learning methods, approaching oracle performance.
