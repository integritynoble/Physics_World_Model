...
In practice, sub-pixel mask misalignment ($\Delta x$, $\Delta y$) and rotation ($\theta$) between the coded aperture and detector are unavoidable, yet even 1.5-pixel shift degrades MST-L reconstruction by 16.40\,dB (from 34.81 to 18.41\,dB).
...
On 10 KAIST benchmark scenes with injected mismatch ($\Delta x{=}1.5$, $\Delta y{=}1.0$, $\theta{=}0.3^\circ$), our method recovers +3.01\,dB for MST-L and +2.99\,dB for PnP-HSICNN through self-supervised calibration.
We evaluate five reconstruction methods (GAP-TV, MST-S, MST-L, HDNet, PnP-HSICNN) across four scenarios, revealing a mask-sensitivity spectrum: mask-guided transformers suffer catastrophic degradation ($-$16\,dB) but gain most from calibration (+3\,dB), while deep prior methods (HDNet) show inherent robustness ($-$8.8\,dB) with smaller calibration benefit.
...
Specifically, three parameters characterize the dominant mask-detector misalignment: horizontal shift $\Delta x$, vertical shift $\Delta y$, and rotation angle $\theta$.
Even modest mismatches ($\Delta x = 1.5$\,px, $\Delta y = 1.0$\,px, $\theta = 0.3^\circ$) degrade MST-L reconstruction by 16.40\,dB---from 34.81\,dB to 18.41\,dB---rendering the system effectively unusable.
In contrast, deep prior methods like HDNet~\cite{hu2022hdnet} suffer only 8.79\,dB degradation (34.79 to 26.00\,dB) due to their learned spectral priors, while classical methods like GAP-TV~\cite{yuan2016gap} show only 0.40\,dB degradation but at lower peak quality (20.37\,dB).

\textbf{Challenges in CASSI calibration.}
Correcting mask mismatch through the reconstruction pipeline presents unique challenges:
\begin{enumerate}
    ...
    \item \textbf{Coupled parameters}: Translation and rotation interact through the mask pattern, making sequential optimization suboptimal.
...
\begin{enumerate}
    ...
    \item A \textbf{two-stage calibration pipeline}: coarse grid search (Stage~0--1) followed by gradient refinement (Stage~2A--2C) for robust, sub-pixel parameter recovery.
...
\begin{equation}
\label{eq:mismatch}
    \tilde{\mathbf{m}} = \mathcal{W}(\mathbf{m}; \Delta x, \Delta y, \theta),
\end{equation}
where $\mathcal{W}$ applies bilinear-interpolated translation $(\Delta x, \Delta y)$ and rotation $\theta$ about the mask center.
The true measurement is generated with the misaligned mask $\tilde{\mathbf{m}}$, while reconstruction assumes the nominal mask $\mathbf{m}$.
...
\textbf{Mismatch injection.} Fixed parameters: $\Delta x = 1.5$\,px, $\Delta y = 1.0$\,px, $\theta = 0.3^\circ$.
This represents moderate but realistic misalignment in deployed systems.
...
MST-L drops 16.40\,dB from Scenario~I (34.81) to II (18.41), and MST-S drops 15.49\,dB.
In contrast, HDNet degrades by only 8.79\,dB (34.79$\to$26.00) because its learned spectral prior provides inherent robustness to mask mismatch.
GAP-TV (classical) shows only 0.40\,dB degradation.
...
Our two-stage pipeline (Scenario~III) recovers +3.15\,dB for MST-S, +3.01\,dB for MST-L, and +2.99\,dB for PnP-HSICNN.
The residual gap between III and IV (oracle) is 9.78--11.03\,dB for MST-S/L, bounded by the GAP-TV proxy solver's limited accuracy.

\textbf{Deep prior methods show robustness but limited calibration benefit.}
HDNet achieves the best Scenario~II performance (26.00\,dB) despite using only lightweight data-consistency refinement with the mask.
Its small calibration gain (+0.20\,dB) confirms that the learned prior dominates the mask-based update, making it naturally robust but unable to leverage improved mask estimates.

\textbf{PnP-HSICNN benefits substantially from calibration.}
The ADMM-based plug-and-play method recovers +2.99\,dB (14.54$\to$17.53), with its oracle gain of +7.57\,dB demonstrating strong mask-dependence.
...
\begin{table}[t]
\centering
\caption{Reconstruction quality (PSNR in dB, mean$\pm$std) across four scenarios on 10 KAIST scenes. Mismatch: $\Delta x{=}1.5$, $\Delta y{=}1.0$, $\theta{=}0.3^\circ$. Degradation = I$-$II. Gain = III$-$II.}
...
\midrule
GAP-TV & 20.37$\pm$1.84 & 19.97$\pm$1.79 & 20.16$\pm$1.84 & 20.35$\pm$1.84 & $-$0.40 & \textbf{+0.20} \\
MST-S  & 33.98$\pm$2.50 & 18.49$\pm$2.08 & 21.64$\pm$2.08 & 31.42$\pm$2.45 & $-$15.49 & \textbf{+3.15} \\
MST-L  & \best{34.81$\pm$2.11} & 18.41$\pm$1.97 & 21.42$\pm$2.02 & \best{32.45$\pm$2.41} & $-$16.40 & \textbf{+3.01} \\
HDNet  & 34.79$\pm$2.65 & \best{26.00$\pm$2.48} & \best{26.20$\pm$2.44} & 26.45$\pm$2.66 & $-$8.79 & \textbf{+0.20} \\
PnP-HSICNN & 21.70$\pm$6.16 & 14.54$\pm$2.21 & 17.53$\pm$2.98 & 22.11$\pm$5.49 & $-$7.15 & \textbf{+2.99} \\
\bottomrule
\end{tabular}
\vspace{-2mm}
\end{table}
...
\Cref{tab:param_recovery} shows aggregated mismatch parameter recovery statistics.
The $\Delta y$ parameter is recovered most accurately (RMSE\,=\,0.523\,px), while $\Delta x$ has higher error (RMSE\,=\,1.022\,px) due to coupling with the dispersion axis.
The $\theta$ RMSE of 0.650$^\circ$ reflects the difficulty of rotation estimation at sub-degree scales.

\begin{table}[t]
\centering
\caption{Mismatch parameter recovery across 10 KAIST scenes. True: $\Delta x{=}1.5$, $\Delta y{=}1.0$, $\theta{=}0.3^\circ$.}
\label{tab:param_recovery}
\small
\begin{tabular}{l ccc}
\toprule
Metric & $\Delta x$ (px) & $\Delta y$ (px) & $\theta$ ($^\circ$) \\
\midrule
...
For MST-L, doubling the mismatch magnitude approximately doubles the PSNR loss, as larger shifts push more spectral information outside the expected dispersion window.

\textbf{Calibration benefit increases with mismatch.}
The gain from calibration (II$\to$III) grows with mismatch severity, as larger mismatches provide stronger gradient signals for the optimizer.
At scale 3.0$\times$, the benefit is maximized.
...
The gradient refinement stages provide additional gain beyond grid search alone, confirming that the differentiable pipeline extracts information not accessible through discrete search.
The improvement is especially pronounced for the coupled parameters ($\Delta y$, $\theta$).

\subsection{Computational Cost}

On a single GPU, per-scene calibration takes approximately 3 minutes, with full 5-method evaluation at $\sim$10.6 minutes:
\begin{itemize}
    \item Stages 0+1 (grid search): $\sim$60\,s (942 GPU GAP-TV evaluations)
    \item Stage 2A--2C (gradient): $\sim$121\,s (190 Adam steps through differentiable solver)
    \item Reconstruction (5 methods): $\sim$453\,s (GAP-TV, MST-S/L, HDNet, PnP-HSICNN)
\end{itemize}
Total calibration averages 180.7$\pm$0.6\,s per scene, indicating stable convergence regardless of scene content.
This is practical for offline calibration or periodic recalibration in deployed systems.
...
Our four-scenario framework with five reconstruction methods reveals a \emph{mask-sensitivity spectrum}: mask-guided transformers (MST-S/L) suffer catastrophic degradation ($-$16\,dB) but gain most from calibration (+3\,dB); plug-and-play methods (PnP-HSICNN) show moderate degradation ($-$7\,dB) and benefit (+3\,dB); deep prior methods (HDNet) are inherently robust ($-$9\,dB) but gain little (+0.2\,dB); and classical methods (GAP-TV) are most robust ($-$0.4\,dB) at lower peak quality.
