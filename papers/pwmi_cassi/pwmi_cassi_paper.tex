...
Evaluating five reconstruction methods across four scenarios on 10 KAIST scenes, we uncover a \emph{mask-sensitivity spectrum}: mask-guided transformers (MST-S/L) suffer catastrophic degradation ($>$15\,dB) but recover ${\sim}48$\% of the oracle gap ($\rho \approx 48$\%), deep prior methods (HDNet) show inherent robustness ($\rho \approx 0$\%), and iterative methods show intermediate sensitivity ($\rho \approx 13$--$23$\%).
This spectrum provides a practical design guideline: deployed systems with limited calibration infrastructure should prefer robust reconstructors, while well-calibrated systems benefit most from mask-guided architectures.
...
The CASSI forward model maps a hyperspectral cube $\mathbf{x} \in \mathbb{R}^{H \times W \times \Lambda}$ to a 2D measurement $\mathbf{y} \in \mathbb{R}^{H \times W'}$:
\begin{equation}
...
In practice, five parameters characterize the dominant misalignment: mask translation $(\Delta x, \Delta y)$, rotation $\theta$, dispersion slope $a_1$, and axis angle $\alpha$.
...
End-to-end deep networks have significantly advanced quality: HDNet~\cite{hu2022hdnet} uses dual-domain unfolding, MST~\cite{cai2022mst,cai2022mst_pp} introduces mask-guided spectral-wise attention, CST~\cite{cai2022cst} applies coarse-to-fine sparse transformers, and DAUHST~\cite{cai2022dauhst} combines degradation-aware unfolding with half-shuffle attention; see Yuan et al.~\cite{yuan2021snapshot} for a survey.
...
HDNet shows negligible calibration benefit ($\rho \approx 0$\%, +0.05\,dB), confirming that the mask plays a marginal role in its reconstruction.
...
Calibration gain is negligible ($\rho \approx 0$\%), confirming the prior's independence from mask accuracy.
...
Mask-guided transformers (MST-S/L, $\rho \approx 48$\%) and mask-refined iterative methods (GAP-TV/PnP-HSICNN, $\rho \approx 13$--$23$\%) represent two distinct operating regimes, while deep prior methods (HDNet, $\rho \approx 0$\%) achieve inherent robustness at the cost of calibration-agnostic reconstruction.
This taxonomy provides a design guideline: system builders should match reconstructor choice to available calibration quality.
