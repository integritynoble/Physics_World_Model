...
\item \textbf{Specification-dominance finding}: an algorithm-comparison study using the top-5 published algorithms per modality (sourced from the literature) reveals that forward-model specification, not algorithm choice, is the primary quality determinant for well-conditioned systems (CoV $\approx$3--6\%), while ill-conditioned compressive systems enter an algorithm-sensitive regime (CoV $>$40\%).
\end{enumerate}
...
\caption{\textbf{Specification dominance across 5 established modalities.}  \textbf{(a)}~Inter-algorithm PSNR Coefficient of Variation (CoV) for the top-5 published reconstruction algorithms per modality.  Well-conditioned systems (blue, CoV $\approx$3.5--6.2\%) show that the top algorithms converge tightly---the forward-model specification constrains the quality ceiling.  SIM shows moderate CoV (15.0\%).  Ill-conditioned compressive systems (red, CoV $>$40\%) require careful algorithm selection.  \textbf{(b)}~Individual algorithm PSNR values (dots) with mean (horizontal bar) for each modality.  Tight clusters for CT/MRI/CASSI confirm specification dominance; wide spread for Lensless shows algorithm sensitivity.  Data from Extended Data Table~\ref{tab:expert_study}.}
...
The top-5 published reconstruction algorithms per modality---spanning deep learning, transformer, diffusion, and deep unrolling methods---on 5 established modalities (Extended Data Table~\ref{tab:expert_study}, Figure~\ref{fig:spec_dominance}): inter-method PSNR CoV ranges from 3.5\% (MRI) to 40.8\% (lensless), revealing a clear gradient.  For well-conditioned systems (CT, MRI, CASSI), CoV remains low ($\approx$3.5--6.2\%), indicating that the top algorithms converge tightly---the forward-model specification constrains the quality ceiling.  SIM shows moderate CoV (15.0\%), while for ill-conditioned compressive systems (lensless), CoV rises sharply ($>$40\%), delineating an algorithm-sensitive regime where reconstruction method selection becomes critical.  This finding---that getting the \emph{physics right} constrains the quality envelope more than the \emph{numerics}---is the central empirical insight of this work.
...
The central finding of this work is that \emph{forward-model specification---the choice and arrangement of physical operators---is the primary determinant of reconstruction quality}, not the reconstruction algorithm.  The algorithm-comparison study quantifies this: for well-conditioned systems (CT, MRI, CASSI), the top-5 published algorithms converge tightly with CoV $\approx$3.5--6.2\%, meaning the forward-model specification constrains the quality ceiling even among state-of-the-art methods spanning a 6\,dB range.  SIM shows moderate CoV (15.0\%), while for ill-conditioned compressive systems (lensless), CoV exceeds 40\%, delineating a regime where algorithm selection becomes critical.  This specification-dominance principle explains why the framework matches expert quality ($99.8\pm3.2$\%) with order-of-magnitude reductions in development time (Table~\ref{tab:expert}): the agents automate the \emph{specification} step that matters most.
