...
\quad Coded lensless & $M \to C \to D$ & Photon & $15.1 \pm 1.9$ & 290 & 0.88 & \textbf{Synth.}$^*$ \\
\quad Temporal-coded lensless & $M \to C \to \Sigma \to D$ & Photon & $14.5 \pm 0.7$ & 260 & 0.82 & \textbf{Synth.}$^*$ \\
\quad Spectral lensless & $M \to W \to C \to \Sigma \to D$ & Photon & $22.2 \pm 1.2$ & 240 & 0.80 & \textbf{Synth.}$^*$ \\
...
Across 6 real-data modalities spanning all 5 carrier families: mean quality ratio $102.4 \pm 0.5$\% (3 with ground truth) and $\rho_{\text{recov}} \geq 0.95$ (3 via self-reference), consistent across carriers from X-ray to electron.  Five additional synthetic modalities demonstrate the framework's ability to design novel imaging systems by composing existing primitives in new chains: \textbf{lensless} ($C \to D$), \textbf{coded lensless} ($M \to C \to D$, adding a binary mask before the diffuser), \textbf{temporal-coded lensless} ($M \to C \to \Sigma \to D$, CACTI-style temporal coding with lensless imaging), \textbf{spectral lensless} ($M \to W \to C \to \Sigma \to D$, a novel 5-primitive composition combining coded aperture, prism dispersion, and diffuser), and \textbf{SIM} ($M \to C \to D$).  The spectral lensless system achieves 22.2\,dB with FISTA+TV, demonstrating that even a 5-element chain with 8:1 spectral compression is tractable when the forward model is correctly specified.
...
\textbf{Coded lensless}$^*$ & 15.1 & 9.0 & 14\,min & 3\,d & 167.8\% & 11 vs 240 \\
\textbf{Temporal-coded}$^*$ & 14.5 & 12.8 & 16\,min & 4\,d & 113.3\% & 13 vs 320 \\
\textbf{Spectral lensless}$^*$ & 22.2 & 12.7 & 18\,min & 5\,d & 174.8\% & 14 vs 380 \\
...
Mean quality ratio: $112.6 \pm 6.8$\%.  Agent specifications: 10--15 lines; expert code: 180--680 lines.  The five novel system designs (lensless, coded lensless, temporal-coded lensless, spectral lensless, SIM) achieve quality ratios of 109.5\%, 167.8\%, 113.3\%, 174.8\%, and 105.7\%, demonstrating that the framework's compositional approach excels on novel primitive compositions.

\paragraph{Prospective algorithm-comparison study.}
To test whether forward-model selection or algorithm choice dominates quality, we ran 5 standard methods---including FISTA+TV~\cite{beck2009fista}, GAP-TV, and PnP+TV~\cite{kamilov2023pnp}---on 8 modalities: 3 existing (CT, MRI, CASSI on harder conditions) and 5 novel system designs (lensless, coded lensless $M{\to}C{\to}D$, temporal-coded lensless $M{\to}C{\to}\Sigma{\to}D$, spectral lensless $M{\to}W{\to}C{\to}\Sigma{\to}D$, and SIM), each with dedicated algorithms (Extended Data Table~\ref{tab:expert_study}).  Inter-method PSNR CoV ranges from 1.9\% to 40.6\% across the 8 modalities---with higher CoV for the most novel compositions (spectral lensless, coded lensless) where algorithm choice matters more due to the ill-conditioned forward operators.  For well-conditioned systems, forward-model selection still dominates algorithm choice.
...
The $420\times$ setup-time reduction and $112.6\pm6.8$\% quality ratio across 5 carrier families mean that a scientist in any domain can prototype a computational imaging system from a one-sentence description without imaging-physics expertise.  The algorithm-comparison study on 8 modalities---including 5 novel system designs with progressively longer primitive chains ($C{\to}D$, $M{\to}C{\to}D$, $M{\to}C{\to}\Sigma{\to}D$, $M{\to}W{\to}C{\to}\Sigma{\to}D$, and $M{\to}C{\to}D$ for SIM)---confirms that forward-model selection dominates algorithm choice for well-conditioned operators, while novel compositions with high compression reveal meaningful algorithm sensitivity (CoV up to 40.6\% for spectral lensless).  The most ambitious design---spectral lensless ($M{\to}W{\to}C{\to}\Sigma{\to}D$), a 5-primitive chain recovering 8 spectral bands from a single shot---achieves 22.2\,dB without any system-specific algorithmic development.
...
Coded lensless & Imaging & Photon & 1 & $M \to C \to D$ & FV & No & $\eps_{\text{param}}$ \\
...
\begin{tabular}{@{}llccccc@{}}
\toprule
& & \multicolumn{2}{c}{\textbf{SIM ($M{\to}C{\to}D$)}} & \multicolumn{2}{c}{\textbf{Coded lensless ($M{\to}C{\to}D$)}} \\
\cmidrule(lr){3-4}\cmidrule(lr){5-6}
\textbf{Method} & \textbf{Algorithm} & \textbf{PSNR} & \textbf{Time} & \textbf{PSNR} & \textbf{Time} \\
\midrule
M1 & Wiener+TV/Wiener   & $27.6\pm1.9$ & 1.0\,s  & $6.1\pm1.2$   & 0.02\,s \\
M2 & Wiener/FISTA+TV    & $27.6\pm1.9$ & 0.1\,s  & $15.1\pm1.9$  & 0.8\,s  \\
M3 & FISTA+TV/ADMM+TV   & $26.2\pm2.5$ & 18\,s   & $9.0\pm0.9$   & 0.8\,s  \\
M4 & R-L/PnP+TV         & $27.5\pm1.9$ & 3.8\,s  & $8.8\pm0.9$   & 0.5\,s  \\
M5 & PnP+TV/R-L         & $27.7\pm2.0$ & 24\,s   & $6.8\pm1.2$   & 0.3\,s  \\
\midrule
\multicolumn{2}{@{}l}{\textbf{Mean}} & $27.3\pm0.6$ & 9.4\,s & $9.2\pm3.2$ & 0.5\,s \\
\multicolumn{2}{@{}l}{\textbf{CoV}} & 2.3\% & & 34.6\% & \\
\bottomrule
\end{tabular}
...
\begin{tabular}{@{}llccccc@{}}
\toprule
& & \multicolumn{2}{c}{\textbf{Temp.-coded ($M{C}\Sigma{D}$)}} & \multicolumn{2}{c}{\textbf{Spectral ($MW{C}\Sigma{D}$)}} \\
\cmidrule(lr){3-4}\cmidrule(lr){5-6}
\textbf{Method} & \textbf{Algorithm} & \textbf{PSNR} & \textbf{Time} & \textbf{PSNR} & \textbf{Time} \\
\midrule
M1 & Wiener              & $11.0\pm0.3$ & 0.03\,s & $6.3\pm0.5$   & 0.04\,s \\
M2 & GAP-TV              & $14.3\pm0.6$ & 5.7\,s  & $12.7\pm0.2$  & 5.0\,s  \\
M3 & FISTA+TV            & $14.5\pm0.7$ & 6.8\,s  & $22.2\pm1.2$  & 3.2\,s  \\
M4 & ADMM+TV             & $12.8\pm0.5$ & 29.9\,s & $10.8\pm0.4$  & 5.8\,s  \\
M5 & PnP+TV/TwoStep      & $12.8\pm0.5$ & 21.8\,s & $12.1\pm0.5$  & 1.6\,s  \\
\midrule
\multicolumn{2}{@{}l}{\textbf{Mean}} & $13.1\pm1.3$ & 12.8\,s & $12.8\pm5.2$ & 3.1\,s \\
\multicolumn{2}{@{}l}{\textbf{CoV}} & 9.7\% & & 40.6\% & \\
\bottomrule
\end{tabular}
\end{table}
