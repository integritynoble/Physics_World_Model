...
\item \textbf{Three-agent pipeline}: Plan ($\Agent_P$), Judge ($\Agent_J$), and Execution ($\Agent_E$) agents that generate, formally verify, and execute specifications over the FPB (\hyperref[sec:pipeline]{``The Three-Agent Pipeline''}).
...
\textbf{Judge Agent ($\Agent_J$)} validates via a 6-gate compiler: (1)~DAG acyclicity, (2)~canonical chain fidelity, (3)~complexity bounds, (4)~nonlinear constraints (VC dimension $\leq 3$), (5)~adjoint consistency, (6)~representation error.  Failures trigger rejection; the Plan Agent regenerates (up to 3 rounds).

\textbf{Execution Agent ($\Agent_E$)} selects the reconstruction algorithm, executes it, and predicts quality from analytical estimates (condition number, noise amplification) combined with calibrated lookup.
...
\textbf{Without $\Agent_E$ (Execution)}: all pass validation but mean PSNR drops 2.1\,dB.
...
% ------------------------------------------------------------------------
\subsection{Judge and Execution Agents}
\label{sec:agents}
% ------------------------------------------------------------------------
...
\textbf{Execution Agent quality prediction} (36 modalities).  Overall MAE: 1.8\,dB ($R^2 = 0.91$; Extended Data Figure~\ref{fig:calibration}).  By group: well-characterised ($n{=}20$) 0.9\,dB; moderate ($n{=}10$) 2.1\,dB; scattering-dominated ($n{=}6$) 3.8\,dB.
...
(4)~Execution Agent quality-prediction MAE degrades to 3.8\,dB for scattering-dominated systems.
...
% ---- Extended Data Figure: Execution Agent Calibration ----
\begin{figure}[htbp]
\centering
...
\end{tikzpicture}
\caption{\textbf{Extended Data Figure~1: Execution Agent calibration.}  Predicted vs.\ actual PSNR for 36 validated modalities.  Shaded band: $\pm1.8$\,dB (overall MAE).  Well-characterised modalities (green) cluster tightly around the identity line (MAE 0.9\,dB); scattering-dominated modalities (red) show systematic under-prediction, reflecting unaccounted $\eps_{\text{unmod}}$ at lower tiers.}
