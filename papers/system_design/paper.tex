...
Designing a computational imaging system requires weeks of specialist effort per modality~\cite{baker2016}.  Building on a companion representation theorem (11 primitives, 170+ modalities)~\cite{paperII}, we introduce \texttt{spec.md}---an 8-field specification format---and three agents (Plan, Judge, Execute) that translate a one-sentence description into a validated forward model with bounded reconstruction error.  Our central result (Theorem~\ref{thm:design}) decomposes the design-to-real error into five independently bounded terms, each linked to a diagnostic gate and corrective action.  On 6 real-data modalities spanning all 5 carrier families, the framework matches expert quality ($99.6 \pm 3.1$\%) with up to $420\times$ setup-time reduction.  Ten novel system designs---composing primitives into chains recovering depth, spectra, and video from single shots (3D to 5D)---demonstrate compositional reach beyond the training registry.  Of 173 compilable modalities, 39 have quantitative validation and 6 are validated on real data.
...
Five standard methods on 8 modalities (3 existing + 5 novel; Extended Data Table~\ref{tab:expert_study}): inter-method PSNR CoV ranges from 0.2\% (CASSI) to 47.8\% (spectral lensless).  For well-conditioned systems ($\leq$1:1 compression), forward-model selection dominates algorithm choice; for compressive systems ($\geq$8:1), iterative regularisation is essential.
...
The $420\times$ setup-time reduction and $99.6\pm3.1$\% quality ratio on established modalities across 5 carrier families mean that a scientist can prototype a computational imaging system from a one-sentence description without imaging-physics expertise.  The algorithm-comparison study confirms that forward-model selection dominates algorithm choice for well-conditioned operators (CoV $<$6\% for CT, MRI, CASSI, lensless), while compressive systems show strong algorithm sensitivity (CoV 42--48\% for temporal-coded and spectral lensless).
...
AI for scientific discovery~\cite{jumper2021alphafold,merchant2023gnome,wang2023scientific,boiko2023autonomous}, operator libraries~\cite{ong2019sigpy,adler2017odl,tachella2023deepinverse,kamilov2023pnp}, and automated experiment design~\cite{tian2023ci,arridge2019solving} address related problems but none provide a formal error decomposition from natural-language intent to reconstruction quality.

\textbf{Limitations.}
(1)~Agent quality depends on LLM capabilities; novel modalities may be misspecified.
(2)~Quantitative reconstruction validated for 39/173 modalities; real data for 6.
...
Systems combining depth encoding with dispersion but no spatial modulation compile through all gates but are flagged by the Judge as severely ill-conditioned (condition number $>10^6$), confirming that spatial coding is a prerequisite for dispersion-based compressive imaging.
