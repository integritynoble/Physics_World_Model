...
% Triad gates (primary, below judge) — increased vertical gap
\node[triad, below=0.7cm of judge, xshift=-1.5cm] (t1) {$G_1$\\Recover.};
...
\node[sysgate, right=0.25cm of t3] (cost) {Cost};
\node[sysgate, right=0.25cm of cost] (feas) {Feasib.};

% Structural compiler (prerequisite, below Triad) — more vertical space
\node[gate, fill=gray!10, draw=gray!50!black, below=1.0cm of t2, minimum width=3.0cm] (comp) {Structural Compiler\\{\tiny (DAG, Chain, Bnd, NL, Adj, Err)}};
...
% Reject — moved further down to avoid overlap
\node[box, fill=red!8, draw=red!50!black, dashed, below=0.5cm of comp] (rej) {\scriptsize Reject};
...
% Triad bracket — more space below for label
...
\draw[arr, gray] (judge.south) -- ++(0,-0.2) -| (t2.north);
\end{tikzpicture}
