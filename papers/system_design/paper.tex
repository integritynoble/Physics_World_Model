...
Designing a computational imaging system requires weeks of specialist effort per modality~\cite{baker2016}.  Building on a companion representation theorem~\cite{paperII}, we introduce \texttt{spec.md}---a 7-field specification format---and three agents (Plan, Judge, Performance) that translate a one-sentence description into a validated forward model with bounded reconstruction error.  Our central result (Theorem~\ref{thm:design}) decomposes the design-to-real error into five independently measurable terms, each linked to a compiler gate and corrective action.  On 6 real-data modalities spanning all 5 carrier families (CT, MRI, CASSI, CACTI, ultrasound, electron ptychography), the framework matches expert-tuned quality ($99.6 \pm 3.1$\%) with up to $420\times$ reduction in setup time.  The Judge rejects ill-posed designs with $F_1 = 0.96$; a prospective algorithm-comparison study confirms that forward-model selection dominates reconstruction algorithm choice (inter-method PSNR CoV $< 5.7$\%).  Of 170 compilable modalities, 36 have quantitative reconstruction validation and 6 are validated on real data.
...
The framework's central design choice is that \texttt{spec.md}---not code---is the canonical scientific artifact.  A specification separates the imaging problem (\emph{what}) from the numerical method (\emph{how}), enabling sharing, version control, and peer review independently of implementation~\cite{wilkinson2016fair}.  A reviewer can verify---by running the 6-gate compiler---that a forward model is physically consistent \emph{without reading code}.  A \texttt{spec.md} has seven mandatory fields (Figure~\ref{fig:specmd}).
...
\textbf{Performance Agent ($\Agent_\Pi$)} predicts reconstruction quality from analytical estimates (condition number, noise amplification) combined with calibrated lookup.
...
\begin{table}[htbp]
\centering
\caption{Cross-modality validation (14 of 36 modalities), grouped by carrier family.  $\rho_{\text{recov}}$: 4-scenario recovery ratio~\cite{paperII}.  95\% CIs via bootstrap ($n{=}1000$).  Bold = real measured data (challenging conditions: 60-view CT, 4-coil MRI); $\dagger$ = self-reference metric (no ground truth).}
\label{tab:cross}
\small
...
Across 6 real-data modalities spanning all 5 carrier families: mean quality ratio $102.4 \pm 0.5$\% (3 with ground truth) and $\rho_{\text{recov}} \geq 0.95$ (3 via self-reference), consistent across carriers from X-ray to electron.
...
\textbf{Without $\Agent_\Pi$}: all pass validation but mean PSNR drops 2.1\,dB.
...
\textbf{Performance prediction} (36 modalities).  Overall MAE: 1.8\,dB ($R^2 = 0.91$; Extended Data Figure~\ref{fig:calibration}).  By group: well-characterised ($n{=}20$) 0.9\,dB; moderate ($n{=}10$) 2.1\,dB; scattering-dominated ($n{=}6$) 3.8\,dB.
...
\begin{table}[htbp]
\centering
\caption{Agent vs.\ established reconstruction methods.  ``Agent'': best-performing automated algorithm; ``Expert'': established variational method (FISTA+TV~\cite{beck2009fista} for real data; ASTRA~\cite{aarle2016}, SigPy~\cite{ong2019sigpy}, GAP-TV for synthetic).  Bold = real measured data.  Quality $>100$\% indicates Agent slightly exceeds Expert.  Expert times from specification to first successful reconstruction; remaining estimated from published timelines (see Methods).}
\label{tab:expert}
\small
...
Mean quality ratio: $99.6 \pm 3.1$\%.  Agent specifications: 10--15 lines; expert code: 210--680 lines.

\paragraph{Prospective algorithm-comparison study.}
To test whether forward-model selection or algorithm choice dominates quality, we ran 5 standard methods~\cite{boyd2011admm,beck2009fista,rudin1992nonlinear,kamilov2023pnp} on deliberately harder conditions (60-view fan-beam CT, 4-coil MRI) to amplify inter-method differences (Extended Data Table~\ref{tab:expert_study}).  Inter-method PSNR CoV is 3.7\%, 5.7\%, and 1.9\% across the 3 modalities---much smaller than sample-to-sample variation ($\sim$15\%), confirming that \emph{forward-model selection dominates algorithm choice}.
...
The $420\times$ setup-time reduction and $99.6\pm3.1$\% quality ratio across 5 carrier families mean that a scientist in any domain can prototype a computational imaging system from a one-sentence description without imaging-physics expertise.  The algorithm-comparison study (Extended Data Table~\ref{tab:expert_study}) confirms that forward-model selection dominates algorithm choice (inter-method PSNR CoV $< 5.7$\%), validating the specification-centric philosophy.

\texttt{spec.md} is designed to play for imaging an analogous standardisation role to that which FASTA played for genomics: decoupling the physics (\emph{what}) from the numerics (\emph{how}), enabling sharing and peer review at the physics level~\cite{wilkinson2016fair}.  An independence test confirmed reproducibility: three \texttt{spec.md} files reproduced PSNR within $\pm 0.3$\,dB across independent agent instances (see Methods).  We propose that the community test this hypothesis by piloting \texttt{spec.md} as a journal supplement for computational imaging papers.
...
(4)~Performance Agent MAE degrades to 3.8\,dB for scattering-dominated systems.
...
% ---- Extended Data Figure: Performance Agent Calibration ----
\begin{figure}[htbp]
\centering
...
\end{tikzpicture}
\caption{\textbf{Extended Data Figure~1: Performance Agent calibration.}  Predicted vs.\ actual PSNR for 36 validated modalities.  Shaded band: $\pm1.8$\,dB (overall MAE).  Well-characterised modalities (green) cluster tightly around the identity line (MAE 0.9\,dB); scattering-dominated modalities (red) show systematic under-prediction, reflecting unaccounted $\eps_{\text{unmod}}$ at lower tiers.}
