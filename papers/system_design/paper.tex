...
\node[gate, below=0.5cm of judge, xshift=-1.8cm] (g1) {DAG};
\node[gate, right=0.18cm of g1] (g2) {Chain};
\node[gate, right=0.18cm of g2] (g3) {Bnd.};
\node[gate, right=0.18cm of g3] (g4) {NL};
\node[gate, right=0.18cm of g4] (g5) {Adj.};
\node[gate, right=0.18cm of g5] (g6) {Err.};
% Triad gates (below compiler gates)
\node[gate, fill=red!8, draw=red!40!black, below=0.35cm of g1, xshift=0.6cm] (t1) {G1};
\node[gate, fill=red!8, draw=red!40!black, right=0.18cm of t1] (t2) {G2};
\node[gate, fill=red!8, draw=red!40!black, right=0.18cm of t2] (t3) {G3};
...
% Reject
\node[box, fill=red!8, draw=red!50!black, dashed, below=2.6cm of judge, xshift=-1.5cm] (rej) {\scriptsize Reject};
...
$\Agent_P$ generates a \texttt{spec.md}; $\Agent_J$ validates in two stages: a 6-gate compiler
(DAG${\to}\eps_{\text{spec}}$, Chain${\to}\eps_{\text{trans}}$, Bounds${\to}$complexity, NL${\to}\eps_{\text{param}}$, Adj.${\to}$consistency, Err.${\to}\eps_{\text{FPB}}$) plus 3 Triad gates~\cite{paperII} (G1: recoverability, G2: carrier budget, G3: operator mismatch); rejected designs trigger
...
\textbf{Judge Agent ($\Agent_J$)} performs two-stage validation.
\emph{Stage~1}: a 6-gate compiler checks DAG acyclicity, canonical chain fidelity, complexity bounds, nonlinear constraints (VC $\leq 3$), adjoint consistency, and representation error.
\emph{Stage~2}: the 3 Triad gates~\cite{paperII} check recoverability (G1---is the sampling sufficient?), carrier budget (G2---is the SNR above the noise floor?), and operator mismatch (G3---does the model match the intended physics?).
Failures at either stage trigger rejection; the Plan Agent regenerates (up to 3 rounds).
...
The Judge executes \emph{deterministic verification}, not prediction.  The 6-gate compiler certifies specification correctness (every $\eps$ term in Theorem~\ref{thm:design} is bounded); the 3 Triad gates certify design quality (the measurement is recoverable, the carrier budget is sufficient, and the forward model is faithful).  Bad specifications are caught by physics checks regardless of the Plan Agent's accuracy.
...
% ------------------------------------------------------------------------
\subsection{Two-Stage Judge Validation}
% ------------------------------------------------------------------------

The Judge Agent performs two-stage validation: a 6-gate compiler for specification correctness, followed by 3 Triad gates~\cite{paperII} for design quality.

\subsubsection{6-Gate Compiler}
...
\subsubsection{Triad Gates}

After compilation, the Judge evaluates the 3 Triad gates from~\cite{paperII}---the same diagnostic law that governs existing instruments now applied prospectively to agent-designed specifications:

\textbf{Triad Gate~1 (Recoverability).}  Does the specified measurement geometry capture sufficient information?  The Judge checks that the null-space dimension of the forward operator is compatible with the target reconstruction quality: $\dim(\mathcal{N}(A)) / n < \tau_1$ (threshold $\tau_1$ depends on regularization strength).  \emph{Design action if violated}: increase sampling density or add complementary views.

\textbf{Triad Gate~2 (Carrier Budget).}  Is the specified SNR above the noise floor?  The Judge estimates measurement SNR from the noise model in \texttt{spec.md}: $\text{SNR}_{\text{spec}} > \text{SNR}_{\min}$ (typically 5\,dB).  \emph{Design action if violated}: increase source power, integration time, or detector quantum efficiency.

\textbf{Triad Gate~3 (Operator Mismatch).}  Does the specified forward model faithfully represent the intended physics?  The Judge cross-checks the \texttt{spec.md} mismatch sources against the canonical mismatch catalog for the modality, and verifies that calibration strategies are specified for all high-severity sources.  \emph{Design action if violated}: add missing mismatch correction or increase fidelity tier.
