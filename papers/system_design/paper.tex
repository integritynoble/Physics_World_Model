...
%
We previously established that 11 canonical primitives suffice to represent any imaging forward model across 170 modalities and 22 application domains~\cite{paperII}, but this Finite Primitive Basis (FPB) provides no design automation, no formal validation, and no link from natural-language intent to reconstruction error.
%
Here we introduce \texttt{spec.md}---a structured 7-field specification format that does for imaging what FASTA did for biological sequences: it decouples the imaging problem from its numerical implementation, making system designs shareable, version-controlled, and peer-reviewable as standalone scientific artifacts---together with three agents (Plan, Judge, Performance) that translate a one-sentence natural-language description into a validated, executable forward model with formal error guarantees.
%
Unlike black-box AI design tools, our framework provides a \emph{formal contract}: we prove that the design-to-real reconstruction error decomposes into five independently bounded, independently measurable terms (Theorem~\ref{thm:design}), each linked to a specific compiler gate, a specific corrective action, and a specific physical origin---making the system's reliability transparent and verifiable from the 6-gate compiler output alone.
%
On 6 real-data modalities spanning all 5 carrier families---clinical CT~\cite{leuschner2021}, multi-coil MRI, coded-aperture spectral imaging (CASSI), snapshot compressive video (CACTI), in-vivo ultrasound (PICMUS), and 4D-STEM electron ptychography---the framework achieves $98.1 \pm 0.6$\% of expert-tuned quality with up to $420\times$ reduction in setup time, consistent across all 5 carrier families (X-ray, photon, spin, acoustic, electron); across 36 validated modalities, the Judge rejects ill-posed designs with 96\% precision and the Performance Agent predicts PSNR within $\pm 1.8$\,dB ($R^2 = 0.91$); and a prospective head-to-head study with 5 independent expert agents confirms that physics model selection dominates algorithm choice (inter-expert PSNR CoV $<$ 5.7\%), validating the framework's specification-centric design philosophy.
%
A tier-lifting protocol demonstrated on diffuse-optical and coherent-imaging cross-tier combinations reduces unmodeled-physics error from 7.8\% to machine precision ($<10^{-12}$), demonstrating that the framework can handle scattering-dominated systems that require full-physics simulation.
...
The parallel with genomics is instructive.  Before FASTA, biological sequence data was communicated through idiosyncratic formats that varied between laboratories.  The adoption of a universal exchange format---human-readable, machine-parseable, and decoupled from any specific analysis tool---transformed genomics from a cottage industry into a global collaborative enterprise.  Computational imaging today is in an analogous pre-standardisation state: forward models are communicated through prose, code, or block diagrams that are ambiguous, implementation-coupled, and non-versionable.

The mathematical foundation for a similar transformation now exists.  We previously showed~\cite{paperII} that 11 canonical primitives---Propagate ($P$), Modulate ($M$), Project ($\Pi$), Encode ($F$), Convolve ($C$), Accumulate ($\Sigma$), Detect ($D$), Sample ($S$), Disperse ($W$), Scatter ($R$), and Transform ($\Lambda$)---suffice to express any imaging forward model across 170 modalities, 22 application domains, and 5 carrier families.  Each primitive is implemented across a four-level \emph{fidelity hierarchy}: Tier-0 (geometric optics, ray transforms), Tier-1 (Fourier/convolution, paraxial models), Tier-2 (shift-variant linear operators: Radon, k-space, coded aperture), and Tier-3 (full-physics: FDTD, Monte Carlo, learned surrogates).  This \emph{Finite Primitive Basis} (FPB) is the representation language---but it does not tell a user which primitives to compose, what parameters to set, whether the result is physically valid, or how close the designed system will be to reality.
...
Expert PSNR spread is narrow within each modality ($\sigma < 1.2$\,dB), confirming that algorithm choice contributes modestly relative to forward-model accuracy.  The fastest expert (E2, FBP/zero-fill) is consistently fast but sacrifices SSIM; iterative methods (E1, E3--E5) trade compute time for structural fidelity.  Across all 3 modalities, the inter-expert PSNR coefficient of variation is 3.7\%, 5.7\%, and 1.9\% respectively---much smaller than the sample-to-sample variation (std/mean $\approx$ 15\%), indicating that \emph{physics model selection dominates algorithm choice}.  This finding has a direct architectural implication: the framework's focus on automating forward-model specification (via \texttt{spec.md}) rather than reconstruction algorithm selection targets the right bottleneck.  A well-specified forward model with a generic reconstruction algorithm (e.g., FISTA+TV) achieves nearly the same quality as the best expert-tuned algorithm---the specification \emph{is} the science; the algorithm is engineering.
