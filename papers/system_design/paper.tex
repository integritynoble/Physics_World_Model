...
\textbf{CT} (LoDoPaB~\cite{leuschner2021}, $n{=}200$): 128 parallel-beam projections, Poisson noise.  Agent PSNR $31.7 \pm 1.2$\,dB; expert ASTRA~\cite{aarle2016} $32.1 \pm 1.1$\,dB (quality ratio 98.8\%, $\rho_{\text{time}} \approx 420$).
\textbf{MRI} (M4Raw~\cite{lyu2023m4raw}, $n{=}50$): $4\times$ Cartesian undersampling.  Agent $31.7 \pm 1.4$\,dB; expert SigPy~\cite{ong2019sigpy} $32.4 \pm 1.3$\,dB (97.8\%).
\textbf{CASSI} (KAIST~\cite{meng2020gap}, $n{=}10$): 28-band coded aperture.  Agent $29.5 \pm 1.0$\,dB; expert GAP-TV~\cite{monga2021unrolling} $30.2 \pm 0.9$\,dB (97.7\%).
...
Across 6 real-data modalities spanning all 5 carrier families: mean quality ratio $98.1 \pm 0.6$\% (3 with ground truth) and $\rho_{\text{recov}} \geq 0.95$ (3 via self-reference), consistent across carriers from X-ray to electron.
...
\begin{table}[htbp]
\centering
\caption{Agent vs.\ established reconstruction libraries.  ``Expert'' denotes production-quality, community-maintained tools (ASTRA~\cite{aarle2016} for CT, SigPy~\cite{ong2019sigpy} for MRI, GAP-TV for CASSI).  Bold = real measured data.  Expert times from specification to first successful reconstruction, cross-validated against published reports; remaining estimated from published timelines (see Methods).}
\label{tab:expert}
\small
...
Mean quality ratio: $97.6 \pm 1.1$\%.  Agent specifications: 10--15 lines; expert code: 210--680 lines.
