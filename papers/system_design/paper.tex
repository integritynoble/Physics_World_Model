...
%
We previously established that 11 canonical primitives suffice to represent any imaging forward model across 168+ modalities~\cite{paperII}, but this Finite Primitive Basis (FPB) provides no design automation, no formal validation, and no link from natural-language intent to reconstruction error.
%
...
%
On real clinical CT (LoDoPaB, 200 cases~\cite{leuschner2021}), the framework achieves 98\% of expert-tuned quality with $420\times$ reduction in setup time; across 36 validated modalities spanning 5 carrier families, the Judge rejects ill-posed designs with 96\% precision and the Performance Agent predicts PSNR within $\pm 1.8$\,dB ($R^2 = 0.91$).
%
...
This definition encompasses 168+ modalities across 22 domains (Extended Data Table~\ref{tab:ext_registry}).  We build the \emph{design layer} on top of the FPB.  Our contributions are:
\begin{enumerate}[nosep]
...
Accepts a natural-language description and generates a \texttt{spec.md}, selecting FPB primitives, carrier type, geometry, noise model, and parameters from the 168+-modality registry and 36 validated canonical decompositions.
...
We validated the framework on all 168+ modalities.  For each, the Plan Agent generated a \texttt{spec.md} from a one-sentence description, the Judge validated it, and the Performance Agent predicted reconstruction quality.  Table~\ref{tab:cross} reports 12 representative modalities spanning all 5 carrier families, grouped by carrier; full results for all 36 validated modalities appear in Methods.

\begin{table}[htbp]
\centering
\caption{Cross-modality validation (12 of 36 modalities), grouped by carrier family.  $\rho_{\text{recov}}$: 4-scenario recovery ratio~\cite{paperII}.  95\% CIs via bootstrap ($n{=}1000$).  Bold = real measured data.}
\label{tab:cross}
\small
...
\quad Ultrasound & $P \to R \to P \to D$ & Acoustic & $27.9 \pm 1.4$ & 250 & 0.81 & Synth. \\
\addlinespace[2pt]
...
\textbf{Compilation.}  All 168+ modalities: 100\% compilation pass rate (Gates~1--4).  Among the 36 fully validated: canonical chain fidelity 31/36 (86.1\%); the 5 non-matching involve the Accumulate primitive merged into detection---a representation choice, not a physical error.  Mean compilation time: 0.35\,ms.
...
Across 3 real-data modalities: mean quality ratio $98.1 \pm 0.6$\%.  Extending real-data validation to additional modalities (ultrasound, OCT, PET) is an immediate next step.
...
(2)~Of 168+ modalities that compile, quantitative reconstruction is validated for 36; real-data validation covers 3 modalities.  Compilation alone does not guarantee reconstruction quality.
...
Ultrasound & Medical & Acoustic & 2 & $P \to D$ & T & No & $\eps_{\text{unmod}}$ \\
...
E-ptychography & Material & Electron & 2 & $M \to P \to D$ & HO & No & $\eps_{\text{param}}$ \\
\addlinespace
...
The LoDoPaB-CT dataset is publicly available~\cite{leuschner2021}.  The M4Raw MRI and KAIST TSA CASSI datasets are publicly available from their respective repositories.  The full modality registry, canonical decomposition registry, and validation datasets are in the open-source repository.
