...
What determines reconstruction quality in computational imaging---the algorithm or the physical measurement design?  We show that forward-model specification is the primary quality determinant: inter-algorithm PSNR variation is below 6\% for well-conditioned systems but reaches 42--48\% for compressive ones, establishing a quantitative boundary between specification-dominated and algorithm-dominated regimes.  Building on a companion representation theorem (11 primitives, 170+ modalities)~\cite{paperII}, we introduce \texttt{spec.md} and three agents (Plan, Judge, Execute) that translate a one-sentence description into a validated forward model with bounded reconstruction error (Theorem~\ref{thm:design}).  On 6 real-data modalities spanning all 5 carrier families, the framework matches expert quality ($99.6 \pm 3.1$\%).  Ten novel system designs---composing primitives into chains from 3D to 5D---demonstrate compositional reach beyond the training registry.  Of 173 compilable modalities, 39 have quantitative validation and 6 are validated on real data.
...
Five standard reconstruction methods on 8 modalities (3 existing + 5 novel; Extended Data Table~\ref{tab:expert_study}): inter-method PSNR CoV ranges from 0.2\% (CASSI) to 47.8\% (spectral lensless), revealing a sharp transition.  For well-conditioned systems ($\leq$1:1 compression), forward-model specification dominates algorithm choice (CoV $<$6\%); for compressive systems ($\geq$8:1), iterative regularisation becomes essential (CoV 42--48\%).  This finding---that getting the \emph{physics right} matters more than the \emph{numerics}---is the central empirical insight of this work.
