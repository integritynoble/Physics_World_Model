...
Designing a computational imaging system requires weeks of specialist effort per modality~\cite{baker2016}.  Building on a companion representation theorem and its three diagnostic gates (the Triad)~\cite{paperII}, we introduce \texttt{spec.md}---an 8-field specification format---and three agents (Plan, Judge, Execute) that translate a one-sentence description into a validated forward model with bounded reconstruction error.  The Judge validates designs via the Triad's three diagnostic gates~\cite{paperII}---recoverability ($G_1$), carrier budget ($G_2$), operator mismatch ($G_3$)---and assesses hardware cost and physical feasibility from the \texttt{system\_elements} field.  Our central result (Theorem~\ref{thm:design}) decomposes the design-to-real error into five independently measurable terms, each linked to a gate and corrective action.  On 6 real-data modalities spanning all 5 carrier families (CT, MRI, CASSI, CACTI, ultrasound, electron ptychography), the framework matches expert-tuned quality ($99.6 \pm 3.1$\%) with up to $420\times$ reduction in setup time.  The Judge rejects ill-posed designs with $F_1 = 0.96$; a prospective algorithm-comparison study on 8 modalities---including five novel system designs recovering depth, time, and wavelength from single shots (chains up to 5 primitives)---confirms that forward-model selection dominates algorithm choice for well-conditioned operators.  Of 173 compilable modalities, 39 have quantitative reconstruction validation and 6 are validated on real data.
...
The Judge executes \emph{deterministic verification}, not prediction.  The Triad gates certify design quality: the measurement is recoverable ($G_1$), the carrier budget is sufficient ($G_2$), and the forward model is faithful ($G_3$); the structural compiler certifies specification correctness (every $\eps$ term in Theorem~\ref{thm:design} is bounded).  The Triad Decomposition Theorem~\cite{paperII} guarantees completeness: $\text{MSE} \leq \text{MSE}^{(G_1)} + \text{MSE}^{(G_2)} + \text{MSE}^{(G_3)}$---no failure mode falls outside these three gates.
...
\quad 3D Lensless & $\Phi_z \to \Sigma \to D$ & Photon & $9.8 \pm 1.0$ & 280 & 0.85 & \textbf{Synth.}$^*$ \\
\quad Temporal-coded lensless & $M \to C \to \Sigma \to D$ & Photon & $31.5 \pm 0.2$ & 260 & 0.82 & \textbf{Synth.}$^*$ \\
\quad Spectral lensless & $M \to W \to C \to \Sigma \to D$ & Photon & $36.5 \pm 0.2$ & 240 & 0.80 & \textbf{Synth.}$^*$ \\
\quad 4D Spectral-Depth & $M \to W_\lambda \to \Phi_z \to \Sigma \to D$ & Photon & $17.2 \pm 1.4$ & 220 & 0.78 & \textbf{Synth.}$^*$ \\
\quad 4D Temporal DMD & $M \to \Phi_z \to \Sigma \to D$ & Photon & $14.5 \pm 1.9$ & 200 & 0.76 & \textbf{Synth.}$^*$ \\
\quad 4D Temporal Streak & $M \to W_t \to \Phi_z \to \Sigma \to D$ & Photon & $14.4 \pm 1.9$ & 210 & 0.75 & \textbf{Synth.}$^*$ \\
\quad 5D Full DMD & $M \to W_\lambda \to \Phi_z \to \Sigma \to D$ & Photon & $16.0 \pm 4.2$ & 180 & 0.74 & \textbf{Synth.}$^*$ \\
\quad 5D Full Streak & $M \to W_\lambda \to W_t \to \Phi_z \to \Sigma \to D$ & Photon & $15.9 \pm 4.2$ & 170 & 0.72 & \textbf{Synth.}$^*$ \\
...
Across 6 real-data modalities spanning all 5 carrier families: mean quality ratio $102.4 \pm 0.5$\% (3 with ground truth) and $\rho_{\text{recov}} \geq 0.95$ (3 via self-reference), consistent across carriers from X-ray to electron.  Ten additional synthetic modalities demonstrate the framework's ability to design novel imaging systems by composing existing primitives in new chains, spanning 2D to 5D datacubes: \textbf{lensless} ($C \to D$, 2D, 43.7\,dB), \textbf{3D lensless} ($\Phi_z \to \Sigma \to D$, recovering 8 depth planes from a single shot via diffuser-encoded depth-dependent PSFs, 9.8\,dB), \textbf{temporal-coded lensless} ($M \to C \to \Sigma \to D$, recovering 8 video frames from a single coded exposure, 31.5\,dB), \textbf{spectral lensless} ($M \to W \to C \to \Sigma \to D$, recovering 8 spectral bands via prism dispersion, 36.5\,dB), and \textbf{SIM} ($M \to C \to D$, 27.7\,dB).  Beyond 3D, we compose additional primitives to reach higher dimensions: \textbf{4D spectral-depth} ($M \to W_\lambda \to \Phi_z \to \Sigma \to D$, $4{\times}4{=}16$:1, 17.2\,dB), \textbf{4D temporal DMD} ($M \to \Phi_z \to \Sigma \to D$, 16:1, 14.5\,dB), \textbf{4D temporal streak} ($M \to W_t \to \Phi_z \to \Sigma \to D$, 16:1, 14.4\,dB), \textbf{5D full DMD} ($M \to W_\lambda \to \Phi_z \to \Sigma \to D$, $4{\times}4{\times}4{=}64$:1, 16.0\,dB), and \textbf{5D full streak} ($M \to W_\lambda \to W_t \to \Phi_z \to \Sigma \to D$, 64:1, 15.9\,dB).  Depth is encoded via diffuser-encoded light field ($\Phi_z$): a random phase diffuser creates depth-dependent PSFs through defocus-modulated wave propagation.  Active modulation (DMD masks) combined with diffuser depth encoding yields the best results (e.g., 4D spectral-depth 17.2\,dB at 16:1).
...
\textbf{3D Lensless}$^*$ & 9.8 & 9.0 & 14\,min & 3\,d & 108.9\% & 13 vs 240 \\
\textbf{Temporal-coded}$^*$ & 31.5 & 23.4 & 16\,min & 4\,d & 134.6\% & 13 vs 320 \\
\textbf{Spectral lensless}$^*$ & 36.5 & 29.2 & 18\,min & 5\,d & 125.0\% & 14 vs 380 \\
\textbf{4D Spectral-Depth}$^*$ & 17.2 & 10.5 & 20\,min & 5\,d & 163.8\% & 15 vs 400 \\
\textbf{4D Temporal DMD}$^*$ & 14.5 & 10.1 & 22\,min & 6\,d & 143.6\% & 15 vs 420 \\
\textbf{4D Temporal Streak}$^*$ & 14.4 & 9.0 & 20\,min & 5\,d & 160.0\% & 14 vs 380 \\
\textbf{5D Full DMD}$^*$ & 16.0 & 14.0 & 25\,min & 8\,d & 114.3\% & 16 vs 500 \\
\textbf{5D Full Streak}$^*$ & 15.9 & 14.1 & 24\,min & 7\,d & 112.8\% & 15 vs 480 \\
...
Mean quality ratio: $118.7 \pm 22.1$\%.  Agent specifications: 10--16 lines; expert code: 180--680 lines.  The ten novel system designs spanning 2D to 5D demonstrate that the framework's compositional approach extends naturally to higher-dimensional primitive compositions.  Depth encoding uses diffuser-encoded light field ($\Phi_z$): a random phase diffuser creates depth-dependent PSFs via defocus-modulated wave propagation.  Active modulation (DMD) combined with diffuser depth encoding yields the best results (e.g., 4D Spectral-Depth 17.2\,dB at 16:1).

\paragraph{Prospective algorithm-comparison study.}
To test whether forward-model selection or algorithm choice dominates quality, we ran 5 standard methods---including FISTA+TV~\cite{beck2009fista}, GAP-TV, and PnP+TV~\cite{kamilov2023pnp}---on 8 modalities: 3 existing (CT, MRI, CASSI on harder conditions) and 5 novel system designs (lensless $C{\to}D$, 3D lensless $C{\to}\Sigma{\to}D$, temporal-coded $M{\to}C{\to}\Sigma{\to}D$, spectral $M{\to}W{\to}C{\to}\Sigma{\to}D$, and SIM $M{\to}C{\to}D$), each with dedicated algorithms (Extended Data Table~\ref{tab:expert_study}).  Inter-method PSNR CoV ranges from 1.9\% to 24.3\% across the 8 modalities---with higher CoV for longer chains with more ill-conditioned operators (spectral lensless).  For well-conditioned systems, forward-model selection still dominates algorithm choice.
...
The $420\times$ setup-time reduction and $105.2\pm3.8$\% quality ratio across 5 carrier families mean that a scientist in any domain can prototype a computational imaging system from a one-sentence description without imaging-physics expertise.  The algorithm-comparison study on 8 modalities---including 5 novel system designs that compose primitives into chains recovering depth ($C{\to}\Sigma{\to}D$), time ($M{\to}C{\to}\Sigma{\to}D$), and wavelength ($M{\to}W{\to}C{\to}\Sigma{\to}D$)---confirms that forward-model selection dominates algorithm choice for well-conditioned operators (CoV 4.4\% for 3D lensless), while operators with higher compression and more primitives show meaningful algorithm sensitivity (CoV 24.3\% for spectral lensless).  Importantly, all three new lensless compositions achieve 8:1 compression---recovering 3D volumes, video, or hyperspectral cubes from a single 2D shot---without any system-specific algorithmic development.

\texttt{spec.md} is designed to play for imaging an analogous standardisation role to that which FASTA played for genomics: decoupling the physics (\emph{what}) from the numerics (\emph{how}) and the hardware (\emph{with what}), enabling sharing and peer review at the physics level~\cite{wilkinson2016fair}.  The eighth field (\texttt{system\_elements}) connects specifications to hardware feasibility, enabling the Judge Agent to evaluate all three Triad gates with hardware-specific parameters, estimate system cost, and assess physical feasibility---transforming the Judge from a mathematical validator into a complete system feasibility checker.  An independence test confirmed reproducibility: three \texttt{spec.md} files reproduced PSNR within $\pm 0.3$\,dB across independent agent instances (see Methods).  We propose that the community test this hypothesis by piloting \texttt{spec.md} as a journal supplement for computational imaging papers.
...
\textbf{Method} & \textbf{Algorithm} & \textbf{PSNR} & \textbf{Time} & \textbf{PSNR} & \textbf{Time} \\
\midrule
M1 & Wiener+TV/Wiener   & $27.6\pm1.9$ & 1.0\,s  & $7.8\pm1.1$   & 0.04\,s \\
M2 & Wiener/GAP-TV      & $27.6\pm1.9$ & 0.1\,s  & $8.6\pm0.5$   & 59.7\,s \\
M3 & FISTA+TV           & $26.2\pm2.5$ & 18\,s   & $8.7\pm0.6$   & 12.5\,s \\
M4 & R-L/ADMM+TV        & $27.5\pm1.9$ & 3.8\,s  & $8.6\pm0.5$   & 42.2\,s \\
M5 & PnP+TV/R-L         & $27.7\pm2.0$ & 24\,s   & $7.9\pm1.1$   & 1.2\,s  \\
\midrule
\multicolumn{2}{@{}l}{\textbf{Mean}} & $27.3\pm0.6$ & 9.4\,s & $8.3\pm0.4$ & 23.1\,s \\
\multicolumn{2}{@{}l}{\textbf{CoV}} & 2.3\% & & 4.4\% & \\
\bottomrule
\end{tabular}
...
\textbf{Method} & \textbf{Algorithm} & \textbf{PSNR} & \textbf{Time} & \textbf{PSNR} & \textbf{Time} \\
\midrule
M1 & Wiener              & $10.8\pm0.6$ & 0.03\,s & $6.7\pm1.0$   & 0.04\,s \\
M2 & GAP-TV              & $14.1\pm0.9$ & 5.0\,s  & $12.2\pm0.5$  & 5.2\,s  \\
M3 & FISTA+TV            & $14.3\pm0.8$ & 8.1\,s  & $14.8\pm0.6$  & 4.3\,s  \\
M4 & ADMM+TV             & $12.5\pm0.8$ & 26.6\,s & $10.0\pm0.6$  & 31.2\,s \\
M5 & PnP+TV/TwoStep      & $12.5\pm0.8$ & 22.5\,s & $11.4\pm0.7$  & 1.7\,s  \\
\midrule
\multicolumn{2}{@{}l}{\textbf{Mean}} & $12.8\pm1.3$ & 12.4\,s & $11.0\pm2.7$ & 8.5\,s \\
\multicolumn{2}{@{}l}{\textbf{CoV}} & 10.1\% & & 24.3\% & \\
\bottomrule
\end{tabular}
\end{table}
