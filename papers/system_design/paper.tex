...
Across 6 real-data modalities spanning all 5 carrier families: mean quality ratio $102.4 \pm 0.5$\% (3 with ground truth) and $\rho_{\text{recov}} \geq 0.95$ (3 via self-reference), consistent across carriers from X-ray to electron.  Five additional synthetic modalities demonstrate the framework's ability to design novel imaging systems by composing existing primitives in new chains: \textbf{lensless} ($C \to D$, 2D), \textbf{3D lensless} ($C \to \Sigma \to D$, recovering 8 depth planes from a single shot via depth-dependent PSFs), \textbf{temporal-coded lensless} ($M \to C \to \Sigma \to D$, recovering 8 video frames from a single coded exposure), \textbf{spectral lensless} ($M \to W \to C \to \Sigma \to D$, recovering 8 spectral bands via prism dispersion), and \textbf{SIM} ($M \to C \to D$).  Each of the three new lensless variants exploits a different physical dimension---depth, time, wavelength---achieving 8:1 compression with PSNR 9.8, 31.5, and 36.5\,dB respectively via FISTA+TV.
...
Mean quality ratio: $105.2 \pm 3.8$\%.  Agent specifications: 10--15 lines; expert code: 180--680 lines.  The five novel system designs (lensless $C{\to}D$, 3D lensless $C{\to}\Sigma{\to}D$, temporal-coded $M{\to}C{\to}\Sigma{\to}D$, spectral lensless $M{\to}W{\to}C{\to}\Sigma{\to}D$, SIM $M{\to}C{\to}D$) achieve quality ratios of 109.5\%, 101.2\%, 114.4\%, 121.3\%, and 105.7\%, demonstrating that the framework's compositional approach extends naturally to novel primitive compositions.
