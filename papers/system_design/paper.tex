...
Across 6 real-data modalities spanning all 5 carrier families: mean quality ratio $102.4 \pm 0.5$\% (3 with ground truth) and $\rho_{\text{recov}} \geq 0.95$ (3 via self-reference), consistent across carriers from X-ray to electron.  Ten additional synthetic modalities demonstrate the framework's ability to design novel imaging systems by composing existing primitives in new chains, spanning 2D to 5D datacubes: \textbf{lensless} ($C \to D$, 2D, 43.7\,dB), \textbf{3D lensless} ($C \to \Sigma \to D$, recovering 8 depth planes from a single shot via depth-dependent PSFs, 9.8\,dB), \textbf{temporal-coded lensless} ($M \to C \to \Sigma \to D$, recovering 8 video frames from a single coded exposure, 31.5\,dB), \textbf{spectral lensless} ($M \to W \to C \to \Sigma \to D$, recovering 8 spectral bands via prism dispersion, 36.5\,dB), and \textbf{SIM} ($M \to C \to D$, 27.7\,dB).  Beyond 3D, we compose additional primitives to reach higher dimensions: \textbf{4D spectral-depth} ($W_\lambda \to C \to \Sigma \to D$, $4{\times}4{=}16$:1, 11.6\,dB), \textbf{4D temporal DMD} ($M \to C \to \Sigma \to D$, 16:1, 15.6\,dB), \textbf{4D temporal streak} ($W_t \to C \to \Sigma \to D$, 16:1, 9.3\,dB), \textbf{5D full DMD} ($M \to W_\lambda \to C \to \Sigma \to D$, $2{\times}2{\times}2{=}8$:1, 15.4\,dB), and \textbf{5D full streak} ($W_\lambda \to W_t \to C \to \Sigma \to D$, 8:1, 14.5\,dB).  Active modulation (DMD masks) consistently outperforms passive dispersion (streak/prism) by 3--6\,dB at the same compression ratio, as binary masks provide higher measurement diversity.
