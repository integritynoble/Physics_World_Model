#!/usr/bin/env python3
"""Digital Holographic Microscopy — Phase B Experiment Script.

Holography: y = Sensor(Fresnel(x)) + noise — linear propagation.

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_holography_experiment.py
"""
...
SEED = 42
MODALITY = "holography"
TEMPLATE_KEY = "holography_graph_v2"
H, W = 64, 64
...
def build_operators(distance):
    ...
    prop = get_primitive("fresnel_prop", {"wavelength": 0.633e-6, "distance": distance, "pixel_size": 3.45e-6})
    sens = get_primitive("photon_sensor", {"quantum_efficiency": 0.9, "gain": 1.0})
    def fwd(x):
        out = sens.forward(prop.forward(src.forward(x)))
...
def main():
    ...
    rng = np.random.RandomState(SEED)
    y = y_clean + rng.normal(0, READ_SIGMA, size=y_clean.shape)
    ...
    _, adj_fn = build_operators(5.0e-3)
    x_hat_w1 = adj_fn(y)
    ...
    # W2 - propagation distance mismatch (5e-3 → 7e-3)
    nom_dist = 5.0e-3
    pert_dist = 7.0e-3
    a_sha256 = hashlib.sha256(f"holography_{nom_dist}".encode()).hexdigest()[:16]

    fwd_pert, _ = build_operators(pert_dist)
    rng2 = np.random.RandomState(SEED + 10)
    y_clean_pert = fwd_pert(x_true)
    y_measured = y_clean_pert + rng2.normal(0, READ_SIGMA, size=y_clean_pert.shape)
    ...
    _, adj_uncorr = build_operators(nom_dist)
    x_hat_uncorrected = np.clip(adj_uncorr(y_measured), 0, None)
    ...
    best_nll, best_dist = np.inf, nom_dist
    for td in np.linspace(3.0e-3, 10.0e-3, 29):
        fwd_t, _ = build_operators(td)
        nll_t = compute_nll_gaussian(y_measured, fwd_t(x_true), noise_sigma_eff)
        if nll_t < best_nll: best_nll, best_dist = nll_t, td

    nll_after = best_nll
    nll_decrease_pct = (nll_before - nll_after) / (nll_before + 1e-12) * 100

    _, adj_corr = build_operators(best_dist)
    x_hat_corrected = np.clip(adj_corr(y_measured), 0, None)
