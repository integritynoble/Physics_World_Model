#!/usr/bin/env python3
"""Optical Coherence Tomography (OCT) — Phase B Experiment Script.

OCT: y = Sensor(Angular_Spectrum(x)) + noise — linear interferometric imaging.

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_oct_experiment.py
"""
...
def build_operators(distance):
    ...
    prop = get_primitive("angular_spectrum", {"wavelength": 1.3e-6, "distance": distance, "pixel_size": 5.0e-6})
...
def main():
    ...
    _, adj_fn = build_operators(1.0e-3)
    x_hat_w1 = adj_fn(y)
    ...
    nom_dist = 1.0e-3
    pert_dist = 1.5e-3
    a_sha256 = hashlib.sha256(f"oct_{nom_dist}".encode()).hexdigest()[:16]
    ...
    for td in np.linspace(0.5e-3, 2.5e-3, 41):
