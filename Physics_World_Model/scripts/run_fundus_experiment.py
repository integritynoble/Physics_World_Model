#!/usr/bin/env python3
"""Fundus Camera — Phase B Experiment Script.

Fundus: y = PhotonSensor(Conv2D(PhotonSource(x))) + Poisson+Gaussian noise

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_fundus_experiment.py
"""
...
def build_operators(gain=1.0):
    ...
    conv = get_primitive("conv2d", {"sigma": 1.5, "mode": "reflect"})
    sens = get_primitive("photon_sensor", {"quantum_efficiency": 0.80, "gain": gain})
    def fwd(x): return sens.forward(conv.forward(src.forward(x)))
    def adj(y): return src.adjoint(conv.adjoint(sens.adjoint(y)))
    return fwd, adj
...
def main():
    ...
    ax = np.arange(-12, 13, dtype=np.float64)
    ...
    a_sha256 = hashlib.sha256(f"fundus_gain".encode()).hexdigest()[:16]
