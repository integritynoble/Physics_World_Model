#!/usr/bin/env python3
"""Diffuse Optical Tomography (DOT) — Phase B Experiment Script.

DOT: y = Sensor(Conv2D_diffusion(x)) + noise — diffusion PSF with photon detection.

Usage:
    PYTHONPATH="$PWD:$PWD/packages/pwm_core" python scripts/run_dot_experiment.py
"""
...
def build_operators(gain=1.0):
    ...
    blur = get_primitive("conv2d", {"sigma": PSF_SIGMA, "mode": "constant"})
...
def main():
    ...
    a_sha256 = hashlib.sha256(f"dot_{PSF_SIGMA}".encode()).hexdigest()[:16]
