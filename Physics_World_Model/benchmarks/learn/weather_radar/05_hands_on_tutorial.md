# 05 — Hands-On Tutorial: Weather / Doppler Radar

This tutorial walks through running the PWM benchmark for Weather / Doppler Radar,
...
config_path = ROOT / "benchmarks" / "configs" / "weather_radar.yaml"
...
try:
    from pwm_core.recon.sar_rda import range_doppler
    print("Solver loaded: RDA")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality weather_radar
...
solvers_to_test = {
    "RDA": ("pwm_core.recon.sar_rda", "range_doppler"),
    "SAR-DL": ("pwm_core.recon.sar_dl", "sar_dl_recon"),
}
...
# axes[1].set_title("RDA")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("SAR-DL")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("weather_radar_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/weather_radar.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/weather_radar_expanded.yaml`
