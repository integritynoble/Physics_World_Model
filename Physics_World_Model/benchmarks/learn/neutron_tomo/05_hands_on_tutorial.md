# 05 — Hands-On Tutorial: Neutron Radiography / Tomography

This tutorial walks through running the PWM benchmark for Neutron Radiography / Tomography,
...
config_path = ROOT / "benchmarks" / "configs" / "neutron_tomo.yaml"
...
try:
    ...
    print("Solver loaded: FBP (neutron tomography)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality neutron_tomo
...
solvers_to_test = {
    "FBP (neutron tomography)": ("pwm_core.recon.ct_solvers", "run_fbp"),
...
# axes[1].set_title("FBP (neutron tomography)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("neutron_tomo_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/neutron_tomo.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/neutron_tomo_expanded.yaml`
