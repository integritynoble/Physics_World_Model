# 05 — Hands-On Tutorial: Single Photon Emission CT (SPECT)

This tutorial walks through running the PWM benchmark for Single Photon Emission CT (SPECT),
...
config_path = ROOT / "benchmarks" / "configs" / "spect.yaml"
...
try:
    ...
    print("Solver loaded: FBP (emission tomography)")
except ImportError:
...
solvers_to_test = {
    "FBP (emission tomography)": ("pwm_core.recon.ct_solvers", "run_fbp"),
...
# axes[1].set_title("FBP (emission tomography)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("spect_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/spect.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/spect_expanded.yaml`
