# 03 — Reconstruction Algorithms: X-ray Computed Tomography (CT)
...
The default solver for X-ray Computed Tomography (CT) is **`fbp`**.
...
| best_quality | PnP-ADMM + RED-CNN | `pwm_core.recon.pnp` | `pnp_admm` | No |  |
| famous_dl | RED-CNN | `pwm_core.recon.redcnn` | `redcnn_denoise` | No | Chen et al. 2017, IEEE TMI |
| small_gpu | RED-CNN | `pwm_core.recon.redcnn` | `redcnn_denoise` | No |  |
...
3. Add the solver tier to the modality config in `benchmarks/configs/ct.yaml`
