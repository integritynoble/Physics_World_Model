# 05 — Hands-On Tutorial: Optical Coherence Tomography (OCT)

This tutorial walks through running the PWM benchmark for Optical Coherence Tomography (OCT),
...
config_path = ROOT / "benchmarks" / "configs" / "oct.yaml"
...
try:
    from pwm_core.recon.oct_solver import run_oct
    print("Solver loaded: FFT Recon")
except ImportError:
...
solvers_to_test = {
    "FFT Recon": ("pwm_core.recon.oct_solver", "run_oct"),
    "Spectral Estimation": ("pwm_core.recon.oct_solver", "spectral_estimation_recon"),
}
...
# axes[1].set_title("FFT Recon")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Spectral Estimation")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("oct_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/oct.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/oct_expanded.yaml`
