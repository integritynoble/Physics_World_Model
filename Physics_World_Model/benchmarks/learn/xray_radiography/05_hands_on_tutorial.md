# 05 — Hands-On Tutorial: X-ray Radiography

This tutorial walks through running the PWM benchmark for X-ray Radiography,
...
config_path = ROOT / "benchmarks" / "configs" / "xray_radiography.yaml"
...
try:
    ...
    print("Solver loaded: FBP (X-ray radiography)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality xray_radiography
...
solvers_to_test = {
    "FBP (X-ray radiography)": ("pwm_core.recon.ct_solvers", "run_fbp"),
...
# axes[1].set_title("FBP (X-ray radiography)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("xray_radiography_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/xray_radiography.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/xray_radiography_expanded.yaml`
