# 05 — Hands-On Tutorial: Diffusion MRI (DTI)

This tutorial walks through running the PWM benchmark for Diffusion MRI (DTI),
...
config_path = ROOT / "benchmarks" / "configs" / "diffusion_mri.yaml"
...
try:
    ...
    print("Solver loaded: SENSE (WLS tensor fit)")
except ImportError:
...
solvers_to_test = {
    "SENSE (WLS tensor fit)": ("pwm_core.recon.mri_solvers", "run_sense"),
...
# axes[1].set_title("SENSE (WLS tensor fit)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("diffusion_mri_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/diffusion_mri.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/diffusion_mri_expanded.yaml`
