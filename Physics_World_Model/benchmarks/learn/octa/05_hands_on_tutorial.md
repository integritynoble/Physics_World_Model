# 05 — Hands-On Tutorial: OCT Angiography (OCTA)

This tutorial walks through running the PWM benchmark for OCT Angiography (OCTA),
...
config_path = ROOT / "benchmarks" / "configs" / "octa.yaml"
...
try:
    ...
    print("Solver loaded: FFT Recon (OCTA)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality octa
...
solvers_to_test = {
    "FFT Recon (OCTA)": ("pwm_core.recon.oct_solver", "run_oct"),
...
# axes[1].set_title("FFT Recon (OCTA)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("octa_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/octa.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/octa_expanded.yaml`
