# 05 — Hands-On Tutorial: 4D-STEM Electron Diffraction

This tutorial walks through running the PWM benchmark for 4D-STEM Electron Diffraction,
...
config_path = ROOT / "benchmarks" / "configs" / "electron_diffraction.yaml"
...
try:
    ...
    print("Solver loaded: ePIE (electron ptychography)")
except ImportError:
...
solvers_to_test = {
    "ePIE (electron ptychography)": ("pwm_core.recon.ptychography_solver", "run_epie"),
...
# axes[1].set_title("ePIE (electron ptychography)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("electron_diffraction_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/electron_diffraction.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/electron_diffraction_expanded.yaml`
