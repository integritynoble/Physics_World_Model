# 05 — Hands-On Tutorial: LiDAR Scanner

This tutorial walks through running the PWM benchmark for LiDAR Scanner,
...
config_path = ROOT / "benchmarks" / "configs" / "lidar.yaml"
...
try:
    ...
    print("Solver loaded: FISTA-L2 (depth)")
except ImportError:
...
# Command-line usage:
# python benchmarks/runners/run_expanded.py --modality lidar
...
solvers_to_test = {
    "FISTA-L2 (depth)": ("pwm_core.recon.classical", "run_fista_l2"),
...
# axes[1].set_title("FISTA-L2 (depth)")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("Best Quality")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("lidar_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/lidar.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/lidar_expanded.yaml`
