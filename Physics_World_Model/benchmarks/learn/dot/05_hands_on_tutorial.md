# 05 — Hands-On Tutorial: Diffuse Optical Tomography (DOT)

This tutorial walks through running the PWM benchmark for Diffuse Optical Tomography (DOT),
...
config_path = ROOT / "benchmarks" / "configs" / "dot.yaml"
...
try:
    from pwm_core.recon.dot_solver import run_dot
    print("Solver loaded: Born Approximation")
except ImportError:
...
solvers_to_test = {
    "Born Approximation": ("pwm_core.recon.dot_solver", "run_dot"),
    "L-BFGS-TV": ("pwm_core.recon.dot_solver", "lbfgs_tv_recon"),
}
...
# axes[1].set_title("Born Approximation")
# axes[2].imshow(x_hat_best, cmap="gray")
# axes[2].set_title("L-BFGS-TV")
# for ax in axes:
#     ax.axis("off")
# plt.tight_layout()
# plt.savefig("dot_comparison.png", dpi=150)
# plt.show()
```
...
- Read the full config: `benchmarks/configs/dot.yaml`
- Explore the expanded config: `benchmarks/expanded_configs/dot_expanded.yaml`
