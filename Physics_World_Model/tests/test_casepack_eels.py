"""Tests for CasePack: EELS.

Template: eels_graph_v2
Chain: generic_source -> energy_resolving_detector -> generic_sensor -> poisson_gaussian_sensor
"""
import numpy as np
import pytest
import yaml
...
class TestCasePackEels:
    """CasePack acceptance tests for the eels modality."""

    def test_template_compiles(self):
        """eels_graph_v2 template compiles without error."""
        tpl = _load_template("eels_graph_v2")
        tpl_clean = dict(tpl)
        tpl_clean.pop("description", None)
        spec = OperatorGraphSpec.model_validate({"graph_id": "eels_graph_v2", **tpl_clean})
