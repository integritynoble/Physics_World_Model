...
\textbf{9~pipeline components} (including \textbf{8~agents} and the \textbf{RunBundle} packager) and \textbf{8~support classes}, totaling
...
$\mathbf{H}_{\mathrm{nom}}$ against the expected
$\mathbf{H}_{\mathrm{true}}$ using residual-norm heuristics and
...
Executes the end-to-end reconstruction pipeline: loads the
\texttt{ExperimentSpec}, compiles the \texttt{OperatorGraph}, applies the
selected correction strategy, invokes the solver, and collects all
metrics.  The runner enforces deterministic execution and resource-budget
compliance.

\paragraph{RunBundle.}
Packages every artifact produced by a pipeline execution---reconstructed
images, metric scores, \texttt{TriadReport}, logs, random seeds, and
timing data---into an immutable, content-addressed archive.  RunBundles
serve as the unit of reproducibility: any result can be verified by
replaying its RunBundle.
