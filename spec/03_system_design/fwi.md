...
```
[Source] → [Forward Model (Full-Waveform Inversion (FWI))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Full-Waveform Inversion (FWI) physics | operator model error |
...
**Dataset**: Full-Waveform Inversion (FWI)
**Input**: seismic waveforms (receivers × time, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/fwi.md`
...
plt.tight_layout(); plt.savefig('fwi_recon.png'); plt.show()
```
...
python3 main.py --modality fwi --period forward --prompt "your system description"
python3 main.py --modality fwi --period reconstruction --prompt "your algorithm"
```
