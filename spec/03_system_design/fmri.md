...
```
[Source] → [Forward Model (Functional MRI (BOLD fMRI))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Functional MRI (BOLD fMRI) physics | operator model error |
...
**Dataset**: Functional MRI (BOLD fMRI)
**Input**: BOLD volumes (T × H × W × D, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/fmri.md`
...
plt.tight_layout(); plt.savefig('fmri_recon.png'); plt.show()
```
...
python3 main.py --modality fmri --period forward --prompt "your system description"
python3 main.py --modality fmri --period reconstruction --prompt "your algorithm"
```
