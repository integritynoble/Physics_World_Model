...
```
[Source] → [Forward Model (Talbot-Lau X-ray Grating Interferometry)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Talbot-Lau X-ray Grating Interferometry physics | operator model error |
...
**Dataset**: Talbot-Lau X-ray Grating Interferometry
**Input**: stepping images (N_steps × H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/talbot_lau.md`
...
plt.tight_layout(); plt.savefig('talbot_lau_recon.png'); plt.show()
```
...
python3 main.py --modality talbot_lau --period forward --prompt "your system description"
python3 main.py --modality talbot_lau --period reconstruction --prompt "your algorithm"
```
