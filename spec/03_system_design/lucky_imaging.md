...
```
[Source] → [Forward Model (Lucky Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Lucky Imaging physics | operator model error |
...
**Dataset**: Lucky Imaging
**Input**: speckle frames (N × H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/lucky_imaging.md`
...
plt.tight_layout(); plt.savefig('lucky_imaging_recon.png'); plt.show()
```
...
python3 main.py --modality lucky_imaging --period forward --prompt "your system description"
python3 main.py --modality lucky_imaging --period reconstruction --prompt "your algorithm"
```
