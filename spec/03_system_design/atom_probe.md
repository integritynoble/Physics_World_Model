...
```
[Source] → [Forward Model (Atom Probe Tomography (APT))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Atom Probe Tomography (APT) physics | operator model error |
...
**Dataset**: Atom Probe Tomography (APT)
**Input**: hit positions (N × 3, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/atom_probe.md`
...
plt.tight_layout(); plt.savefig('atom_probe_recon.png'); plt.show()
```
...
python3 main.py --modality atom_probe --period forward --prompt "your system description"
python3 main.py --modality atom_probe --period reconstruction --prompt "your algorithm"
```
