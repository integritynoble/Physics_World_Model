...
```
[Source] → [Forward Model (Electron Holography)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Electron Holography physics | operator model error |
...
**Dataset**: Electron Holography
**Input**: hologram (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/electron_holography.md`
...
plt.tight_layout(); plt.savefig('electron_holography_recon.png'); plt.show()
```
...
python3 main.py --modality electron_holography --period forward --prompt "your system description"
python3 main.py --modality electron_holography --period reconstruction --prompt "your algorithm"
```
