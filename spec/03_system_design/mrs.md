...
```
[Source] → [Forward Model (MR Spectroscopy (MRS))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | MR Spectroscopy (MRS) physics | operator model error |
...
**Dataset**: MR Spectroscopy (MRS)
**Input**: FID (T, complex64)
**Algorithms**: 11 CPU, 4 GPU — see `spec/mrs.md`
...
plt.tight_layout(); plt.savefig('mrs_recon.png'); plt.show()
```
...
python3 main.py --modality mrs --period forward --prompt "your system description"
python3 main.py --modality mrs --period reconstruction --prompt "your algorithm"
```
