...
```
[Source] → [Forward Model (Coded Aperture Compressive Temporal Imaging (CACTI))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Coded Aperture Compressive Temporal Imaging (CACTI) physics | operator model error |
...
**Dataset**: Coded Aperture Compressive Temporal Imaging (CACTI)
**Input**: coded frames (B × H × W, float32)
**Algorithms**: 5 CPU, 2 GPU — see `spec/cacti.md`
...
plt.tight_layout(); plt.savefig('cacti_recon.png'); plt.show()
```
...
python3 main.py --modality cacti --period forward --prompt "your system description"
python3 main.py --modality cacti --period reconstruction --prompt "your algorithm"
```
