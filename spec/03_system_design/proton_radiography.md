...
```
[Source] → [Forward Model (Proton Radiography)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Proton Radiography physics | operator model error |
...
**Dataset**: Proton Radiography
**Input**: fluence map (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/proton_radiography.md`
...
plt.tight_layout(); plt.savefig('proton_radiography_recon.png'); plt.show()
```
...
python3 main.py --modality proton_radiography --period forward --prompt "your system description"
python3 main.py --modality proton_radiography --period reconstruction --prompt "your algorithm"
```
