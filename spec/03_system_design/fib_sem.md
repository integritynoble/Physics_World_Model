...
```
[Source] → [Forward Model (Focused Ion Beam SEM (FIB-SEM))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Focused Ion Beam SEM (FIB-SEM) physics | operator model error |
...
**Dataset**: Focused Ion Beam SEM (FIB-SEM)
**Input**: cross-sections (Z × H × W, uint8)
**Algorithms**: 9 CPU, 6 GPU — see `spec/fib_sem.md`
...
plt.tight_layout(); plt.savefig('fib_sem_recon.png'); plt.show()
```
...
python3 main.py --modality fib_sem --period forward --prompt "your system description"
python3 main.py --modality fib_sem --period reconstruction --prompt "your algorithm"
```
