...
```
[Source] → [Forward Model (Seismic Tomography)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Seismic Tomography physics | operator model error |
...
**Dataset**: Seismic Tomography
**Input**: travel times (src-recv, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/seismic_tomo.md`
...
plt.tight_layout(); plt.savefig('seismic_tomo_recon.png'); plt.show()
```
...
python3 main.py --modality seismic_tomo --period forward --prompt "your system description"
python3 main.py --modality seismic_tomo --period reconstruction --prompt "your algorithm"
```
