...
```
[Source] → [Forward Model (X-ray Radiography)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | X-ray Radiography physics | operator model error |
...
**Dataset**: X-ray Radiography
**Input**: attenuation image (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/xray_radiography.md`
...
plt.tight_layout(); plt.savefig('xray_radiography_recon.png'); plt.show()
```
...
python3 main.py --modality xray_radiography --period forward --prompt "your system description"
python3 main.py --modality xray_radiography --period reconstruction --prompt "your algorithm"
```
