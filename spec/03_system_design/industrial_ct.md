...
```
[Source] → [Forward Model (Industrial X-ray CT)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Industrial X-ray CT physics | center-of-rotation (CoR) offset |
| Detector | measurement | noise |

**Mismatch**: center-of-rotation (CoR) offset in range `[-10, +10] px`
**Correction**: sinogram symmetry calibration

## Reconstruction

**Dataset**: Industrial X-ray CT
**Input**: sinogram (angles × detectors, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/industrial_ct.md`
...
with h5py.File('industrial_ct_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/industrial_ct/public/
    y, x_true = f['y'][0], f['x_true'][0]
# Or: y = np.load('your_measurement.npy').astype('float32'); x_true = None

# Forward model + mismatch correction + reconstruction
x = run_solver('traditional_cpu', y, cfg={'cor_offset': None})  # None = auto-calibrate
...
plt.tight_layout(); plt.savefig('industrial_ct_recon.png'); plt.show()
```
...
python3 main.py --modality industrial_ct --period forward --prompt "your system description"
python3 main.py --modality industrial_ct --period reconstruction --prompt "your algorithm"
```
