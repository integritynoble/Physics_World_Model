...
```
[Source] → [Forward Model (Compressed Ultrafast Photography (CUP))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Compressed Ultrafast Photography (CUP) physics | operator model error |
...
**Dataset**: Compressed Ultrafast Photography (CUP)
**Input**: streak image (H × W_streak, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/cup.md`
...
plt.tight_layout(); plt.savefig('cup_recon.png'); plt.show()
```
...
python3 main.py --modality cup --period forward --prompt "your system description"
python3 main.py --modality cup --period reconstruction --prompt "your algorithm"
```
