...
```
[Source] → [Forward Model (Entangled Photon Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Entangled Photon Microscopy physics | operator model error |
...
**Dataset**: Entangled Photon Microscopy
**Input**: coincidence counts (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/entangled_photon.md`
...
plt.tight_layout(); plt.savefig('entangled_photon_recon.png'); plt.show()
```
...
python3 main.py --modality entangled_photon --period forward --prompt "your system description"
python3 main.py --modality entangled_photon --period reconstruction --prompt "your algorithm"
```
