...
```
[Source] → [Forward Model (4D-STEM Electron Diffraction)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | 4D-STEM Electron Diffraction physics | operator model error |
...
**Dataset**: 4D-STEM Electron Diffraction
**Input**: diffraction pattern (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/electron_diffraction.md`
...
plt.tight_layout(); plt.savefig('electron_diffraction_recon.png'); plt.show()
```
...
python3 main.py --modality electron_diffraction --period forward --prompt "your system description"
python3 main.py --modality electron_diffraction --period reconstruction --prompt "your algorithm"
```
