...
```
[Source] → [Forward Model (Muon Tomography)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Muon Tomography physics | operator model error |
...
**Dataset**: Muon Tomography
**Input**: muon tracks (N × 6, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/muon_tomo.md`
...
plt.tight_layout(); plt.savefig('muon_tomo_recon.png'); plt.show()
```
...
python3 main.py --modality muon_tomo --period forward --prompt "your system description"
python3 main.py --modality muon_tomo --period reconstruction --prompt "your algorithm"
```
