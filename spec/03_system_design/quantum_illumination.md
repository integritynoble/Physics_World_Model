...
```
[Source] → [Forward Model (Quantum Illumination)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Quantum Illumination physics | operator model error |
...
**Dataset**: Quantum Illumination
**Input**: coincidence image (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/quantum_illumination.md`
...
plt.tight_layout(); plt.savefig('quantum_illumination_recon.png'); plt.show()
```
...
python3 main.py --modality quantum_illumination --period forward --prompt "your system description"
python3 main.py --modality quantum_illumination --period reconstruction --prompt "your algorithm"
```
