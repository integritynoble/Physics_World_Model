...
```
[Source] → [Forward Model (Hyperspectral Remote Sensing)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Hyperspectral Remote Sensing physics | operator model error |
...
**Dataset**: Hyperspectral Remote Sensing
**Input**: radiance cube (H × W × bands, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/hyperspectral_remote.md`
...
plt.tight_layout(); plt.savefig('hyperspectral_remote_recon.png'); plt.show()
```
...
python3 main.py --modality hyperspectral_remote --period forward --prompt "your system description"
python3 main.py --modality hyperspectral_remote --period reconstruction --prompt "your algorithm"
```
