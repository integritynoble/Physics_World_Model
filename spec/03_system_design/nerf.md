...
```
[Source] → [Forward Model (Neural Radiance Fields (NeRF))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Neural Radiance Fields (NeRF) physics | operator model error |
...
**Dataset**: Neural Radiance Fields (NeRF)
**Input**: posed images (N × H × W × 3, float32)
**Algorithms**: 5 CPU, 1 GPU — see `spec/nerf.md`
...
plt.tight_layout(); plt.savefig('nerf_recon.png'); plt.show()
```
...
python3 main.py --modality nerf --period forward --prompt "your system description"
python3 main.py --modality nerf --period reconstruction --prompt "your algorithm"
```
