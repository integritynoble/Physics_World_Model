...
```
[Source] → [Forward Model (Spinning Disk Confocal Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Spinning Disk Confocal Microscopy physics | operator model error |
...
**Dataset**: Spinning Disk Confocal Microscopy
**Input**: Z-stack (Z × H × W, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/spinning_disk.md`
...
plt.tight_layout(); plt.savefig('spinning_disk_recon.png'); plt.show()
```
...
python3 main.py --modality spinning_disk --period forward --prompt "your system description"
python3 main.py --modality spinning_disk --period reconstruction --prompt "your algorithm"
```
