...
```
[Source] → [Forward Model (Coded Exposure / Flutter Shutter)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Coded Exposure / Flutter Shutter physics | operator model error |
...
**Dataset**: Coded Exposure / Flutter Shutter
**Input**: coded frames (N × H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/coded_exposure.md`
...
plt.tight_layout(); plt.savefig('coded_exposure_recon.png'); plt.show()
```
...
python3 main.py --modality coded_exposure --period forward --prompt "your system description"
python3 main.py --modality coded_exposure --period reconstruction --prompt "your algorithm"
```
