...
```
[Source] → [Forward Model (Wide-Angle X-ray Scattering (WAXS))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Wide-Angle X-ray Scattering (WAXS) physics | operator model error |
...
**Dataset**: Wide-Angle X-ray Scattering (WAXS)
**Input**: wide-angle pattern (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/waxs.md`
...
plt.tight_layout(); plt.savefig('waxs_recon.png'); plt.show()
```
...
python3 main.py --modality waxs --period forward --prompt "your system description"
python3 main.py --modality waxs --period reconstruction --prompt "your algorithm"
```
