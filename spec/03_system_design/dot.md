...
```
[Source] → [Forward Model (Diffuse Optical Tomography (DOT))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Diffuse Optical Tomography (DOT) physics | operator model error |
...
**Dataset**: Diffuse Optical Tomography (DOT)
**Input**: boundary flux (sources × detectors, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/dot.md`
...
plt.tight_layout(); plt.savefig('dot_recon.png'); plt.show()
```
...
python3 main.py --modality dot --period forward --prompt "your system description"
python3 main.py --modality dot --period reconstruction --prompt "your algorithm"
```
