...
```
[Source] → [Forward Model (Confocal Live-Cell Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Confocal Live-Cell Microscopy physics | operator model error |
...
**Dataset**: Confocal Live-Cell Microscopy
**Input**: time-lapse (T × H × W, float32)
**Algorithms**: 11 CPU, 5 GPU — see `spec/confocal_livecell.md`
...
plt.tight_layout(); plt.savefig('confocal_livecell_recon.png'); plt.show()
```
...
python3 main.py --modality confocal_livecell --period forward --prompt "your system description"
python3 main.py --modality confocal_livecell --period reconstruction --prompt "your algorithm"
```
