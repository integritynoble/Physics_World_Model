...
```
[Source] → [Forward Model (Machine Vision / AOI)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Machine Vision / AOI physics | operator model error |
...
**Dataset**: Machine Vision / AOI
**Input**: image (H × W × 3, uint8)
**Algorithms**: 11 CPU, 4 GPU — see `spec/machine_vision.md`
...
plt.tight_layout(); plt.savefig('machine_vision_recon.png'); plt.show()
```
...
python3 main.py --modality machine_vision --period forward --prompt "your system description"
python3 main.py --modality machine_vision --period reconstruction --prompt "your algorithm"
```
