...
```
[Source] → [Forward Model (Cryo-Electron Tomography (Cryo-ET))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Cryo-Electron Tomography (Cryo-ET) physics | operator model error |
...
**Dataset**: Cryo-Electron Tomography (Cryo-ET)
**Input**: tilt series (angles × H × W, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/cryo_et.md`
...
plt.tight_layout(); plt.savefig('cryo_et_recon.png'); plt.show()
```
...
python3 main.py --modality cryo_et --period forward --prompt "your system description"
python3 main.py --modality cryo_et --period reconstruction --prompt "your algorithm"
```
