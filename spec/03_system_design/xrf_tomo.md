...
```
[Source] → [Forward Model (X-ray Fluorescence Tomography)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | X-ray Fluorescence Tomography physics | operator model error |
...
**Dataset**: X-ray Fluorescence Tomography
**Input**: XRF sinograms (elem × angles × det, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/xrf_tomo.md`
...
plt.tight_layout(); plt.savefig('xrf_tomo_recon.png'); plt.show()
```
...
python3 main.py --modality xrf_tomo --period forward --prompt "your system description"
python3 main.py --modality xrf_tomo --period reconstruction --prompt "your algorithm"
```
