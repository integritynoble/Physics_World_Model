...
```
[Source] → [Forward Model (3D Gaussian Splatting (3DGS))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | 3D Gaussian Splatting (3DGS) physics | operator model error |
...
**Dataset**: 3D Gaussian Splatting (3DGS)
**Input**: posed images (N × H × W × 3, float32)
**Algorithms**: 11 CPU, 5 GPU — see `spec/gaussian_splatting.md`
...
plt.tight_layout(); plt.savefig('gaussian_splatting_recon.png'); plt.show()
```
...
python3 main.py --modality gaussian_splatting --period forward --prompt "your system description"
python3 main.py --modality gaussian_splatting --period reconstruction --prompt "your algorithm"
```
