...
```
[Source] → [Forward Model (Event Horizon Telescope (EHT) Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Event Horizon Telescope (EHT) Imaging physics | operator model error |
...
**Dataset**: Event Horizon Telescope (EHT) Imaging
**Input**: hologram (H × W, complex64)
**Algorithms**: 11 CPU, 4 GPU — see `spec/eht_imaging.md`
...
plt.tight_layout(); plt.savefig('eht_imaging_recon.png'); plt.show()
```
...
python3 main.py --modality eht_imaging --period forward --prompt "your system description"
python3 main.py --modality eht_imaging --period reconstruction --prompt "your algorithm"
```
