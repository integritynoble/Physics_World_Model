...
```
[Source] → [Forward Model (Digital Holographic Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Digital Holographic Microscopy physics | operator model error |
...
**Dataset**: Digital Holographic Microscopy
**Input**: hologram (H × W, float32)
**Algorithms**: 9 CPU, 8 GPU — see `spec/holography.md`
...
plt.tight_layout(); plt.savefig('holography_recon.png'); plt.show()
```
...
python3 main.py --modality holography --period forward --prompt "your system description"
python3 main.py --modality holography --period reconstruction --prompt "your algorithm"
```
