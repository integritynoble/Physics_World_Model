...
```
[Source] → [Forward Model (Magnetic Force Microscopy (MFM))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Magnetic Force Microscopy (MFM) physics | operator model error |
...
**Dataset**: Magnetic Force Microscopy (MFM)
**Input**: magnetic force map (H × W, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/mfm.md`
...
plt.tight_layout(); plt.savefig('mfm_recon.png'); plt.show()
```
...
python3 main.py --modality mfm --period forward --prompt "your system description"
python3 main.py --modality mfm --period reconstruction --prompt "your algorithm"
```
