...
```
[Source] → [Forward Model (Brachytherapy Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Brachytherapy Imaging physics | operator model error |
...
**Dataset**: Brachytherapy Imaging
**Input**: dose map (H × W × D, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/brachytherapy_img.md`
...
plt.tight_layout(); plt.savefig('brachytherapy_img_recon.png'); plt.show()
```
...
python3 main.py --modality brachytherapy_img --period forward --prompt "your system description"
python3 main.py --modality brachytherapy_img --period reconstruction --prompt "your algorithm"
```
