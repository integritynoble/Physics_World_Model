...
```
[Source] → [Forward Model (Low-Dose Widefield Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Low-Dose Widefield Microscopy physics | operator model error |
...
**Dataset**: Low-Dose Widefield Microscopy
**Input**: photon-limited image (H × W, float32)
**Algorithms**: 11 CPU, 5 GPU — see `spec/widefield_lowdose.md`
...
plt.tight_layout(); plt.savefig('widefield_lowdose_recon.png'); plt.show()
```
...
python3 main.py --modality widefield_lowdose --period forward --prompt "your system description"
python3 main.py --modality widefield_lowdose --period reconstruction --prompt "your algorithm"
```
