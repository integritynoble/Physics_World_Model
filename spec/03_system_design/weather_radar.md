...
```
[Source] → [Forward Model (Weather / Doppler Radar)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Weather / Doppler Radar physics | operator model error |
...
**Dataset**: Weather / Doppler Radar
**Input**: reflectivity (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/weather_radar.md`
...
plt.tight_layout(); plt.savefig('weather_radar_recon.png'); plt.show()
```
...
python3 main.py --modality weather_radar --period forward --prompt "your system description"
python3 main.py --modality weather_radar --period reconstruction --prompt "your algorithm"
```
