...
```
[Source] → [Forward Model (Generic Matrix Sensing)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Generic Matrix Sensing physics | operator model error |
...
**Dataset**: Generic Matrix Sensing
**Input**: partial matrix (M × N, float32, NaN=missing)
**Algorithms**: 12 CPU, 4 GPU — see `spec/matrix.md`
...
plt.tight_layout(); plt.savefig('matrix_recon.png'); plt.show()
```
...
python3 main.py --modality matrix --period forward --prompt "your system description"
python3 main.py --modality matrix --period reconstruction --prompt "your algorithm"
```
