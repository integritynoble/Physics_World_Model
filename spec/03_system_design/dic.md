...
```
[Source] → [Forward Model (Differential Interference Contrast (DIC))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Differential Interference Contrast (DIC) physics | operator model error |
...
**Dataset**: Differential Interference Contrast (DIC)
**Input**: DIC image (H × W, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/dic.md`
...
plt.tight_layout(); plt.savefig('dic_recon.png'); plt.show()
```
...
python3 main.py --modality dic --period forward --prompt "your system description"
python3 main.py --modality dic --period reconstruction --prompt "your algorithm"
```
