...
```
[Source] → [Forward Model (Ocean Color Remote Sensing)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Ocean Color Remote Sensing physics | operator model error |
...
**Dataset**: Ocean Color Remote Sensing
**Input**: radiance (H × W × bands, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/ocean_color.md`
...
plt.tight_layout(); plt.savefig('ocean_color_recon.png'); plt.show()
```
...
python3 main.py --modality ocean_color --period forward --prompt "your system description"
python3 main.py --modality ocean_color --period reconstruction --prompt "your algorithm"
```
