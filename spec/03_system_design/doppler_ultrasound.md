...
```
[Source] → [Forward Model (Doppler Ultrasound)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Doppler Ultrasound physics | operator model error |
...
**Dataset**: Doppler Ultrasound
**Input**: IQ data (H × W × T, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/doppler_ultrasound.md`
...
plt.tight_layout(); plt.savefig('doppler_ultrasound_recon.png'); plt.show()
```
...
python3 main.py --modality doppler_ultrasound --period forward --prompt "your system description"
python3 main.py --modality doppler_ultrasound --period reconstruction --prompt "your algorithm"
```
