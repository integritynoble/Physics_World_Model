...
```
[Source] → [Forward Model (CEST MRI)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | CEST MRI physics | operator model error |
...
**Algorithms**: 11 CPU, 4 GPU — see `spec/cest_mri.md`
...
plt.tight_layout(); plt.savefig('cest_mri_recon.png'); plt.show()
```
...
python3 main.py --modality cest_mri --period forward --prompt "your system description"
python3 main.py --modality cest_mri --period reconstruction --prompt "your algorithm"
```
