...
```
[Source] → [Forward Model (Second Harmonic Generation (SHG) Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Second Harmonic Generation (SHG) Microscopy physics | operator model error |
...
**Dataset**: Second Harmonic Generation (SHG) Microscopy
**Input**: Z-stack (Z × H × W, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/shg.md`
...
plt.tight_layout(); plt.savefig('shg_recon.png'); plt.show()
```
...
python3 main.py --modality shg --period forward --prompt "your system description"
python3 main.py --modality shg --period reconstruction --prompt "your algorithm"
```
