...
```
[Source] → [Forward Model (LiDAR Scanner)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | LiDAR Scanner physics | operator model error |
...
**Dataset**: LiDAR Scanner
**Input**: point cloud (N × 3, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/lidar.md`
...
plt.tight_layout(); plt.savefig('lidar_recon.png'); plt.show()
```
...
python3 main.py --modality lidar --period forward --prompt "your system description"
python3 main.py --modality lidar --period reconstruction --prompt "your algorithm"
```
