...
```
[Source] → [Forward Model (Streak Camera Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Streak Camera Imaging physics | operator model error |
...
**Dataset**: Streak Camera Imaging
**Input**: streak image (time × space, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/streak_camera.md`
...
plt.tight_layout(); plt.savefig('streak_camera_recon.png'); plt.show()
```
...
python3 main.py --modality streak_camera --period forward --prompt "your system description"
python3 main.py --modality streak_camera --period reconstruction --prompt "your algorithm"
```
