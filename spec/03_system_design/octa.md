...
```
[Source] → [Forward Model (OCT Angiography (OCTA))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | OCT Angiography (OCTA) physics | operator model error |
...
**Dataset**: OCT Angiography (OCTA)
**Input**: B-scans (T × depth × A-scans, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/octa.md`
...
plt.tight_layout(); plt.savefig('octa_recon.png'); plt.show()
```
...
python3 main.py --modality octa --period forward --prompt "your system description"
python3 main.py --modality octa --period reconstruction --prompt "your algorithm"
```
