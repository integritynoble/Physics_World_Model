...
```
[Source] → [Forward Model (DNA-PAINT Super-Resolution)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | DNA-PAINT Super-Resolution physics | operator model error |
...
**Dataset**: DNA-PAINT Super-Resolution
**Input**: localisation list (N × 2, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/dna_paint.md`
...
plt.tight_layout(); plt.savefig('dna_paint_recon.png'); plt.show()
```
...
python3 main.py --modality dna_paint --period forward --prompt "your system description"
python3 main.py --modality dna_paint --period reconstruction --prompt "your algorithm"
```
