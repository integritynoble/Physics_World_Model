...
| Forward model | Ptychographic Imaging physics | probe position error (px) |
| Detector | measurement | noise |

**Mismatch**: probe position error (px) in range `[-3, +3] px`
**Correction**: annealing position refinement

## Reconstruction

**Dataset**: Ptychographic Imaging
**Input**: diffraction patterns (N_pos × H × W, float32)
**Algorithms**: 11 CPU, 6 GPU — see `spec/ptychography.md`
...
x = run_solver('epie', y, cfg={'pos_error': None})  # None = auto-calibrate
...
plt.tight_layout(); plt.savefig('ptychography_recon.png'); plt.show()
```
...
python3 main.py --modality ptychography --period forward --prompt "your system description"
python3 main.py --modality ptychography --period reconstruction --prompt "your algorithm"
```
