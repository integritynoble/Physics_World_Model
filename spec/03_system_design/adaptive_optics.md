...
```
[Source] → [Forward Model (Adaptive Optics (AO) Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Adaptive Optics (AO) Imaging physics | operator model error |
...
**Dataset**: Adaptive Optics (AO) Imaging
**Input**: wavefront sensor (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/adaptive_optics.md`
...
plt.tight_layout(); plt.savefig('adaptive_optics_recon.png'); plt.show()
```
...
python3 main.py --modality adaptive_optics --period forward --prompt "your system description"
python3 main.py --modality adaptive_optics --period reconstruction --prompt "your algorithm"
```
