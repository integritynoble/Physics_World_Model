...
```
[Source] → [Forward Model (Panorama Multi-Focus Fusion)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Panorama Multi-Focus Fusion physics | operator model error |
...
**Dataset**: Panorama Multi-Focus Fusion
**Input**: images (N × H × W × 3, uint8)
**Algorithms**: 12 CPU, 4 GPU — see `spec/panorama.md`
...
plt.tight_layout(); plt.savefig('panorama_recon.png'); plt.show()
```
...
python3 main.py --modality panorama --period forward --prompt "your system description"
python3 main.py --modality panorama --period reconstruction --prompt "your algorithm"
```
