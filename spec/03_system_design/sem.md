...
```
[Source] → [Forward Model (Scanning Electron Microscopy (SEM))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Scanning Electron Microscopy (SEM) physics | operator model error |
...
**Dataset**: Scanning Electron Microscopy (SEM)
**Input**: SEM image (H × W, uint16)
**Algorithms**: 11 CPU, 4 GPU — see `spec/sem.md`
...
plt.tight_layout(); plt.savefig('sem_recon.png'); plt.show()
```
...
python3 main.py --modality sem --period forward --prompt "your system description"
python3 main.py --modality sem --period reconstruction --prompt "your algorithm"
```
