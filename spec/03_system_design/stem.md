...
```
[Source] → [Forward Model (Scanning Transmission Electron Microscopy (STEM))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Scanning Transmission Electron Microscopy (STEM) physics | operator model error |
...
**Dataset**: Scanning Transmission Electron Microscopy (STEM)
**Input**: HAADF image (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/stem.md`
...
plt.tight_layout(); plt.savefig('stem_recon.png'); plt.show()
```
...
python3 main.py --modality stem --period forward --prompt "your system description"
python3 main.py --modality stem --period reconstruction --prompt "your algorithm"
```
