...
```
[Source] → [Forward Model (X-ray NDT (Radiography))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | X-ray NDT (Radiography) physics | operator model error |
...
**Dataset**: X-ray NDT (Radiography)
**Input**: projection (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/xray_ndt.md`
...
plt.tight_layout(); plt.savefig('xray_ndt_recon.png'); plt.show()
```
...
python3 main.py --modality xray_ndt --period forward --prompt "your system description"
python3 main.py --modality xray_ndt --period reconstruction --prompt "your algorithm"
```
