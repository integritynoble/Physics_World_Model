...
```
[Source] → [Forward Model (Ultrasonic Phased Array (TFM/FMC))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Ultrasonic Phased Array (TFM/FMC) physics | operator model error |
...
**Dataset**: Ultrasonic Phased Array (TFM/FMC)
**Input**: FMC data (elem × elem × time, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/ultrasonic_phased_array.md`
...
plt.tight_layout(); plt.savefig('ultrasonic_phased_array_recon.png'); plt.show()
```
...
python3 main.py --modality ultrasonic_phased_array --period forward --prompt "your system description"
python3 main.py --modality ultrasonic_phased_array --period reconstruction --prompt "your algorithm"
```
