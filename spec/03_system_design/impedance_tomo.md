...
```
[Source] → [Forward Model (Electrical Impedance Tomography (EIT))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Electrical Impedance Tomography (EIT) physics | operator model error |
...
**Dataset**: Electrical Impedance Tomography (EIT)
**Input**: boundary voltages (M, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/impedance_tomo.md`
...
plt.tight_layout(); plt.savefig('impedance_tomo_recon.png'); plt.show()
```
...
python3 main.py --modality impedance_tomo --period forward --prompt "your system description"
python3 main.py --modality impedance_tomo --period reconstruction --prompt "your algorithm"
```
