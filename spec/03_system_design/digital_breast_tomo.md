...
```
[Source] → [Forward Model (Digital Breast Tomosynthesis (DBT))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Digital Breast Tomosynthesis (DBT) physics | operator model error |
...
**Dataset**: Digital Breast Tomosynthesis (DBT)
**Input**: projections (angles × H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/digital_breast_tomo.md`
...
plt.tight_layout(); plt.savefig('digital_breast_tomo_recon.png'); plt.show()
```
...
python3 main.py --modality digital_breast_tomo --period forward --prompt "your system description"
python3 main.py --modality digital_breast_tomo --period reconstruction --prompt "your algorithm"
```
