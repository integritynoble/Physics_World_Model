...
```
[Source] → [Forward Model (Proton Therapy Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Proton Therapy Imaging physics | operator model error |
...
**Dataset**: Proton Therapy Imaging
**Input**: dose distribution (H × W × D, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/proton_therapy_img.md`
...
plt.tight_layout(); plt.savefig('proton_therapy_img_recon.png'); plt.show()
```
...
python3 main.py --modality proton_therapy_img --period forward --prompt "your system description"
python3 main.py --modality proton_therapy_img --period reconstruction --prompt "your algorithm"
```
