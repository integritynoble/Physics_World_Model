...
```
[Source] → [Forward Model (Arterial Spin Labeling (ASL) MRI)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Arterial Spin Labeling (ASL) MRI physics | operator model error |
...
**Dataset**: Arterial Spin Labeling (ASL) MRI
**Input**: label-control pairs (2 × H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/asl_mri.md`
...
plt.tight_layout(); plt.savefig('asl_mri_recon.png'); plt.show()
```
...
python3 main.py --modality asl_mri --period forward --prompt "your system description"
python3 main.py --modality asl_mri --period reconstruction --prompt "your algorithm"
```
