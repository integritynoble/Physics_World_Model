...
```
[Source] → [Forward Model (Light-Sheet Fluorescence Microscopy (LSFM))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Light-Sheet Fluorescence Microscopy (LSFM) physics | operator model error |
...
**Dataset**: Light-Sheet Fluorescence Microscopy (LSFM)
**Input**: Z-stack (Z × H × W, float32)
**Algorithms**: 12 CPU, 4 GPU — see `spec/lightsheet.md`
...
plt.tight_layout(); plt.savefig('lightsheet_recon.png'); plt.show()
```
...
python3 main.py --modality lightsheet --period forward --prompt "your system description"
python3 main.py --modality lightsheet --period reconstruction --prompt "your algorithm"
```
