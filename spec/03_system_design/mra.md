...
```
[Source] → [Forward Model (MR Angiography (MRA))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | MR Angiography (MRA) physics | operator model error |
...
**Dataset**: MR Angiography (MRA)
**Input**: k-space (kx × ky × kz, complex64)
**Algorithms**: 11 CPU, 4 GPU — see `spec/mra.md`
...
plt.tight_layout(); plt.savefig('mra_recon.png'); plt.show()
```
...
python3 main.py --modality mra --period forward --prompt "your system description"
python3 main.py --modality mra --period reconstruction --prompt "your algorithm"
```
