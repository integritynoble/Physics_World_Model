...
```
[Source] → [Forward Model (X-ray Angiography)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | X-ray Angiography physics | operator model error |
...
**Dataset**: X-ray Angiography
**Input**: projection (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/angiography.md`
...
plt.tight_layout(); plt.savefig('angiography_recon.png'); plt.show()
```
...
python3 main.py --modality angiography --period forward --prompt "your system description"
python3 main.py --modality angiography --period reconstruction --prompt "your algorithm"
```
