...
```
[Source] → [Forward Model (Transmission Electron Microscopy (TEM))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Transmission Electron Microscopy (TEM) physics | operator model error |
...
**Dataset**: Transmission Electron Microscopy (TEM)
**Input**: TEM image (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/tem.md`
...
plt.tight_layout(); plt.savefig('tem_recon.png'); plt.show()
```
...
python3 main.py --modality tem --period forward --prompt "your system description"
python3 main.py --modality tem --period reconstruction --prompt "your algorithm"
```
