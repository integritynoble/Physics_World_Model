...
```
[Source] → [Forward Model (PET/CT Fusion)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | PET/CT Fusion physics | operator model error |
...
**Dataset**: PET/CT Fusion
**Input**: PET sino + CT proj (both float32)
**Algorithms**: 3 CPU, 0 GPU — see `spec/pet_ct.md`
...
plt.tight_layout(); plt.savefig('pet_ct_recon.png'); plt.show()
```
...
python3 main.py --modality pet_ct --period forward --prompt "your system description"
python3 main.py --modality pet_ct --period reconstruction --prompt "your algorithm"
```
