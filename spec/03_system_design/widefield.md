...
```
[Source] → [Forward Model (Widefield Fluorescence Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Widefield Fluorescence Microscopy physics | operator model error |
...
**Dataset**: Widefield Fluorescence Microscopy
**Input**: fluorescence image (H × W, float32)
**Algorithms**: 10 CPU, 7 GPU — see `spec/widefield.md`
...
plt.tight_layout(); plt.savefig('widefield_recon.png'); plt.show()
```
...
python3 main.py --modality widefield --period forward --prompt "your system description"
python3 main.py --modality widefield --period reconstruction --prompt "your algorithm"
```
