...
```
[Source] → [Forward Model (Particle Calorimetry)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Particle Calorimetry physics | operator model error |
...
**Dataset**: Particle Calorimetry
**Input**: deposits (N × 5, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/particle_calorimetry.md`
...
plt.tight_layout(); plt.savefig('particle_calorimetry_recon.png'); plt.show()
```
...
python3 main.py --modality particle_calorimetry --period forward --prompt "your system description"
python3 main.py --modality particle_calorimetry --period reconstruction --prompt "your algorithm"
```
