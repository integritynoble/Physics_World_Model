...
```
[Source] → [Forward Model (Photon-Counting Spectral CT)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Photon-Counting Spectral CT physics | operator model error |
...
**Dataset**: Photon-Counting Spectral CT
**Input**: energy-bin sinos (bins × angles × det, float32)
**Algorithms**: 3 CPU, 0 GPU — see `spec/spectral_ct.md`
...
plt.tight_layout(); plt.savefig('spectral_ct_recon.png'); plt.show()
```
...
python3 main.py --modality spectral_ct --period forward --prompt "your system description"
python3 main.py --modality spectral_ct --period reconstruction --prompt "your algorithm"
```
