...
```
[Source] → [Forward Model (Optical Diffraction Tomography (ODT))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Optical Diffraction Tomography (ODT) physics | operator model error |
...
**Dataset**: Optical Diffraction Tomography (ODT)
**Input**: holograms (angles × H × W, complex64)
**Algorithms**: 11 CPU, 4 GPU — see `spec/odt.md`
...
plt.tight_layout(); plt.savefig('odt_recon.png'); plt.show()
```
...
python3 main.py --modality odt --period forward --prompt "your system description"
python3 main.py --modality odt --period reconstruction --prompt "your algorithm"
```
