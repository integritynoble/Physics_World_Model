...
```
[Source] → [Forward Model (Atomic Force Microscopy (AFM))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Atomic Force Microscopy (AFM) physics | operator model error |
...
**Dataset**: Atomic Force Microscopy (AFM)
**Input**: force-distance map (H × W, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/afm.md`
...
plt.tight_layout(); plt.savefig('afm_recon.png'); plt.show()
```
...
python3 main.py --modality afm --period forward --prompt "your system description"
python3 main.py --modality afm --period reconstruction --prompt "your algorithm"
```
