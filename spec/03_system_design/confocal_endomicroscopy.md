...
```
[Source] → [Forward Model (Confocal Laser Endomicroscopy (CLE))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Confocal Laser Endomicroscopy (CLE) physics | operator model error |
...
**Dataset**: Confocal Laser Endomicroscopy (CLE)
**Input**: confocal frame (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/confocal_endomicroscopy.md`
...
plt.tight_layout(); plt.savefig('confocal_endomicroscopy_recon.png'); plt.show()
```
...
python3 main.py --modality confocal_endomicroscopy --period forward --prompt "your system description"
python3 main.py --modality confocal_endomicroscopy --period reconstruction --prompt "your algorithm"
```
