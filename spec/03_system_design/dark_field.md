...
```
[Source] → [Forward Model (Dark-Field Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Dark-Field Microscopy physics | operator model error |
...
**Dataset**: Dark-Field Microscopy
**Input**: grating images (2 × H × W, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/dark_field.md`
...
plt.tight_layout(); plt.savefig('dark_field_recon.png'); plt.show()
```
...
python3 main.py --modality dark_field --period forward --prompt "your system description"
python3 main.py --modality dark_field --period reconstruction --prompt "your algorithm"
```
