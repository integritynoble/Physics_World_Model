...
```
[Source] → [Forward Model (Susceptibility-Weighted Imaging (SWI))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Susceptibility-Weighted Imaging (SWI) physics | operator model error |
...
**Dataset**: Susceptibility-Weighted Imaging (SWI)
**Input**: phase image (H × W × slices, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/swi.md`
...
plt.tight_layout(); plt.savefig('swi_recon.png'); plt.show()
```
...
python3 main.py --modality swi --period forward --prompt "your system description"
python3 main.py --modality swi --period reconstruction --prompt "your algorithm"
```
