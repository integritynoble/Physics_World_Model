...
```
[Source] → [Forward Model (Stimulated Raman Scattering (SRS) Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Stimulated Raman Scattering (SRS) Microscopy physics | operator model error |
...
**Dataset**: Stimulated Raman Scattering (SRS) Microscopy
**Input**: SRS image (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/srs.md`
...
plt.tight_layout(); plt.savefig('srs_recon.png'); plt.show()
```
...
python3 main.py --modality srs --period forward --prompt "your system description"
python3 main.py --modality srs --period reconstruction --prompt "your algorithm"
```
