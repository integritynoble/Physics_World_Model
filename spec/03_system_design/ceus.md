...
```
[Source] → [Forward Model (Contrast-Enhanced Ultrasound (CEUS))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Contrast-Enhanced Ultrasound (CEUS) physics | operator model error |
...
**Dataset**: Contrast-Enhanced Ultrasound (CEUS)
**Input**: contrast frames (T × H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/ceus.md`
...
plt.tight_layout(); plt.savefig('ceus_recon.png'); plt.show()
```
...
python3 main.py --modality ceus --period forward --prompt "your system description"
python3 main.py --modality ceus --period reconstruction --prompt "your algorithm"
```
