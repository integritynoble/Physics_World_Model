...
```
[Source] → [Forward Model (Coherent Anti-Stokes Raman (CARS) Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Coherent Anti-Stokes Raman (CARS) Microscopy physics | operator model error |
...
**Dataset**: Coherent Anti-Stokes Raman (CARS) Microscopy
**Input**: CARS spectrum cube (H × W × λ, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/cars.md`
...
plt.tight_layout(); plt.savefig('cars_recon.png'); plt.show()
```
...
python3 main.py --modality cars --period forward --prompt "your system description"
python3 main.py --modality cars --period reconstruction --prompt "your algorithm"
```
