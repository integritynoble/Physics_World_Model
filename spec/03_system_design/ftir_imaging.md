...
```
[Source] → [Forward Model (FTIR Spectroscopic Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | FTIR Spectroscopic Imaging physics | operator model error |
...
**Dataset**: FTIR Spectroscopic Imaging
**Input**: interferogram (H × W × OPD, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/ftir_imaging.md`
...
plt.tight_layout(); plt.savefig('ftir_imaging_recon.png'); plt.show()
```
...
python3 main.py --modality ftir_imaging --period forward --prompt "your system description"
python3 main.py --modality ftir_imaging --period reconstruction --prompt "your algorithm"
```
