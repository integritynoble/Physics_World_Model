...
```
[Source] → [Forward Model (Polarization Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Polarization Microscopy physics | operator model error |
...
**Dataset**: Polarization Microscopy
**Input**: Stokes images (4 × H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/polarization.md`
...
plt.tight_layout(); plt.savefig('polarization_recon.png'); plt.show()
```
...
python3 main.py --modality polarization --period forward --prompt "your system description"
python3 main.py --modality polarization --period reconstruction --prompt "your algorithm"
```
