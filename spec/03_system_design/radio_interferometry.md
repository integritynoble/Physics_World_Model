...
```
[Source] → [Forward Model (Radio Interferometry (VLBI))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Radio Interferometry (VLBI) physics | operator model error |
...
**Dataset**: Radio Interferometry (VLBI)
**Input**: UV-plane data (N_baselines, complex64)
**Algorithms**: 11 CPU, 4 GPU — see `spec/radio_interferometry.md`
...
plt.tight_layout(); plt.savefig('radio_interferometry_recon.png'); plt.show()
```
...
python3 main.py --modality radio_interferometry --period forward --prompt "your system description"
python3 main.py --modality radio_interferometry --period reconstruction --prompt "your algorithm"
```
