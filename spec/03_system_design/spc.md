...
```
[Source] → [Forward Model (Single-Pixel Camera (SPC))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Single-Pixel Camera (SPC) physics | operator model error |
...
**Dataset**: Single-Pixel Camera (SPC)
**Input**: photon counts (T × H × W, uint16)
**Algorithms**: 15 CPU, 10 GPU — see `spec/spc.md`
...
plt.tight_layout(); plt.savefig('spc_recon.png'); plt.show()
```
...
python3 main.py --modality spc --period forward --prompt "your system description"
python3 main.py --modality spc --period reconstruction --prompt "your algorithm"
```
