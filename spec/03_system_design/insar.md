...
```
[Source] → [Forward Model (Interferometric SAR (InSAR))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Interferometric SAR (InSAR) physics | operator model error |
...
**Dataset**: Interferometric SAR (InSAR)
**Input**: interferometric phase (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/insar.md`
...
plt.tight_layout(); plt.savefig('insar_recon.png'); plt.show()
```
...
python3 main.py --modality insar --period forward --prompt "your system description"
python3 main.py --modality insar --period reconstruction --prompt "your algorithm"
```
