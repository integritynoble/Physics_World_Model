...
```
[Source] → [Forward Model (Correlative Light-Electron Microscopy (CLEM))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Correlative Light-Electron Microscopy (CLEM) physics | operator model error |
...
**Dataset**: Correlative Light-Electron Microscopy (CLEM)
**Input**: EM + fluorescence (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/clem.md`
...
plt.tight_layout(); plt.savefig('clem_recon.png'); plt.show()
```
...
python3 main.py --modality clem --period forward --prompt "your system description"
python3 main.py --modality clem --period reconstruction --prompt "your algorithm"
```
