...
```
[Source] → [Forward Model (Intravascular Ultrasound (IVUS))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Intravascular Ultrasound (IVUS) physics | operator model error |
...
**Dataset**: Intravascular Ultrasound (IVUS)
**Input**: RF pullback (frames × elements × samples, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/ivus.md`
...
plt.tight_layout(); plt.savefig('ivus_recon.png'); plt.show()
```
...
python3 main.py --modality ivus --period forward --prompt "your system description"
python3 main.py --modality ivus --period reconstruction --prompt "your algorithm"
```
