...
```
[Source] → [Forward Model (PALM/STORM Single-Molecule Localization)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | PALM/STORM Single-Molecule Localization physics | operator model error |
...
**Dataset**: PALM/STORM Single-Molecule Localization
**Input**: localisations (N × 4: x,y,σ,I)
**Algorithms**: 9 CPU, 6 GPU — see `spec/palm_storm.md`
...
plt.tight_layout(); plt.savefig('palm_storm_recon.png'); plt.show()
```
...
python3 main.py --modality palm_storm --period forward --prompt "your system description"
python3 main.py --modality palm_storm --period reconstruction --prompt "your algorithm"
```
