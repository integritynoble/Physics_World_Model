...
```
[Source] → [Forward Model (Terahertz Imaging (THz))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Terahertz Imaging (THz) physics | operator model error |
...
**Dataset**: Terahertz Imaging (THz)
**Input**: THz waveform (T × H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/terahertz.md`
...
plt.tight_layout(); plt.savefig('terahertz_recon.png'); plt.show()
```
...
python3 main.py --modality terahertz --period forward --prompt "your system description"
python3 main.py --modality terahertz --period reconstruction --prompt "your algorithm"
```
