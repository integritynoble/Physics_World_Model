...
```
[Source] → [Forward Model (Integral Photography)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Integral Photography physics | operator model error |
...
**Dataset**: Integral Photography
**Input**: integral image (H × W, float32)
**Algorithms**: 12 CPU, 4 GPU — see `spec/integral.md`
...
plt.tight_layout(); plt.savefig('integral_recon.png'); plt.show()
```
...
python3 main.py --modality integral --period forward --prompt "your system description"
python3 main.py --modality integral --period reconstruction --prompt "your algorithm"
```
