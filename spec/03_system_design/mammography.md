...
```
[Source] → [Forward Model (Mammography)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Mammography physics | operator model error |
...
**Algorithms**: 11 CPU, 4 GPU — see `spec/mammography.md`
...
plt.tight_layout(); plt.savefig('mammography_recon.png'); plt.show()
```
...
python3 main.py --modality mammography --period forward --prompt "your system description"
python3 main.py --modality mammography --period reconstruction --prompt "your algorithm"
```
