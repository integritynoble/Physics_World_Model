...
```
[Source] → [Forward Model (Solar EUV/X-ray Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Solar EUV/X-ray Imaging physics | operator model error |
...
**Dataset**: Solar EUV/X-ray Imaging
**Input**: EUV image (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/solar_imaging.md`
...
plt.tight_layout(); plt.savefig('solar_imaging_recon.png'); plt.show()
```
...
python3 main.py --modality solar_imaging --period forward --prompt "your system description"
python3 main.py --modality solar_imaging --period reconstruction --prompt "your algorithm"
```
