...
```
[Source] → [Forward Model (Fluoroscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Fluoroscopy physics | operator model error |
...
**Algorithms**: 11 CPU, 4 GPU — see `spec/fluoroscopy.md`
...
plt.tight_layout(); plt.savefig('fluoroscopy_recon.png'); plt.show()
```
...
python3 main.py --modality fluoroscopy --period forward --prompt "your system description"
python3 main.py --modality fluoroscopy --period reconstruction --prompt "your algorithm"
```
