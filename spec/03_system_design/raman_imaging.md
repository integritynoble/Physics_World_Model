...
```
[Source] → [Forward Model (Raman Imaging / Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Raman Imaging / Microscopy physics | operator model error |
...
**Dataset**: Raman Imaging / Microscopy
**Input**: Raman spectra (H × W × wn, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/raman_imaging.md`
...
plt.tight_layout(); plt.savefig('raman_imaging_recon.png'); plt.show()
```
...
python3 main.py --modality raman_imaging --period forward --prompt "your system description"
python3 main.py --modality raman_imaging --period reconstruction --prompt "your algorithm"
```
