...
```
[Source] → [Forward Model (Event Camera / Dynamic Vision Sensor (DVS))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Event Camera / Dynamic Vision Sensor (DVS) physics | operator model error |
...
**Dataset**: Event Camera / Dynamic Vision Sensor (DVS)
**Input**: event stream (N × 4: t,x,y,p)
**Algorithms**: 11 CPU, 4 GPU — see `spec/event_camera.md`
...
plt.tight_layout(); plt.savefig('event_camera_recon.png'); plt.show()
```
...
python3 main.py --modality event_camera --period forward --prompt "your system description"
python3 main.py --modality event_camera --period reconstruction --prompt "your algorithm"
```
