...
```
[Source] → [Forward Model (Flash LiDAR)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Flash LiDAR physics | operator model error |
...
**Algorithms**: 11 CPU, 4 GPU — see `spec/flash_lidar.md`
...
plt.tight_layout(); plt.savefig('flash_lidar_recon.png'); plt.show()
```
...
python3 main.py --modality flash_lidar --period forward --prompt "your system description"
python3 main.py --modality flash_lidar --period reconstruction --prompt "your algorithm"
```
