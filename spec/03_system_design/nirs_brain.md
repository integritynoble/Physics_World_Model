...
```
[Source] → [Forward Model (Functional Near-Infrared Spectroscopy (fNIRS))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Functional Near-Infrared Spectroscopy (fNIRS) physics | operator model error |
...
**Dataset**: Functional Near-Infrared Spectroscopy (fNIRS)
**Input**: optical signal (channels × T, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/nirs_brain.md`
...
plt.tight_layout(); plt.savefig('nirs_brain_recon.png'); plt.show()
```
...
python3 main.py --modality nirs_brain --period forward --prompt "your system description"
python3 main.py --modality nirs_brain --period reconstruction --prompt "your algorithm"
```
