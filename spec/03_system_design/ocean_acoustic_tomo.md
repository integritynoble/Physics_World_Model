...
```
[Source] → [Forward Model (Ocean Acoustic Tomography)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Ocean Acoustic Tomography physics | operator model error |
...
**Dataset**: Ocean Acoustic Tomography
**Input**: travel times (pairs, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/ocean_acoustic_tomo.md`
...
plt.tight_layout(); plt.savefig('ocean_acoustic_tomo_recon.png'); plt.show()
```
...
python3 main.py --modality ocean_acoustic_tomo --period forward --prompt "your system description"
python3 main.py --modality ocean_acoustic_tomo --period reconstruction --prompt "your algorithm"
```
