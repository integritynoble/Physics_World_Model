...
```
[Source] → [Forward Model (STEM-EDX Elemental Mapping)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | STEM-EDX Elemental Mapping physics | operator model error |
...
**Dataset**: STEM-EDX Elemental Mapping
**Input**: X-ray counts (H × W × channels, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/edx_mapping.md`
...
plt.tight_layout(); plt.savefig('edx_mapping_recon.png'); plt.show()
```
...
python3 main.py --modality edx_mapping --period forward --prompt "your system description"
python3 main.py --modality edx_mapping --period reconstruction --prompt "your algorithm"
```
