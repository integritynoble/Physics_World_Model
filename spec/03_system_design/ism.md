...
```
[Source] → [Forward Model (Image Scanning Microscopy (ISM))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Image Scanning Microscopy (ISM) physics | operator model error |
...
**Dataset**: Image Scanning Microscopy (ISM)
**Input**: raw stack (H_scan × W_scan × px × py, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/ism.md`
...
plt.tight_layout(); plt.savefig('ism_recon.png'); plt.show()
```
...
python3 main.py --modality ism --period forward --prompt "your system description"
python3 main.py --modality ism --period reconstruction --prompt "your algorithm"
```
