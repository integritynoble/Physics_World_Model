...
```
[Source] → [Forward Model (Light Field Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Light Field Imaging physics | operator model error |
...
**Dataset**: Light Field Imaging
**Input**: light field (u × v × s × t, float32)
**Algorithms**: 12 CPU, 4 GPU — see `spec/light_field.md`
...
plt.tight_layout(); plt.savefig('light_field_recon.png'); plt.show()
```
...
python3 main.py --modality light_field --period forward --prompt "your system description"
python3 main.py --modality light_field --period reconstruction --prompt "your algorithm"
```
