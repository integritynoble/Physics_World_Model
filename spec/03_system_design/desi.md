...
```
[Source] → [Forward Model (DESI Mass Spectrometry Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | DESI Mass Spectrometry Imaging physics | operator model error |
...
**Dataset**: DESI Mass Spectrometry Imaging
**Input**: mass image (H × W × m/z, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/desi.md`
...
plt.tight_layout(); plt.savefig('desi_recon.png'); plt.show()
```
...
python3 main.py --modality desi --period forward --prompt "your system description"
python3 main.py --modality desi --period reconstruction --prompt "your algorithm"
```
