...
```
[Source] → [Forward Model (Scanning Acoustic Microscopy (SAM))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Scanning Acoustic Microscopy (SAM) physics | operator model error |
...
**Dataset**: Scanning Acoustic Microscopy (SAM)
**Input**: RF data (H × W × T, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/acoustic_microscopy.md`
...
plt.tight_layout(); plt.savefig('acoustic_microscopy_recon.png'); plt.show()
```
...
python3 main.py --modality acoustic_microscopy --period forward --prompt "your system description"
python3 main.py --modality acoustic_microscopy --period reconstruction --prompt "your algorithm"
```
