...
```
[Source] → [Forward Model (Fiber Bundle Endoscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Fiber Bundle Endoscopy physics | operator model error |
...
**Dataset**: Fiber Bundle Endoscopy
**Input**: image (H × W × 3, uint8)
**Algorithms**: 9 CPU, 6 GPU — see `spec/endoscopy.md`
...
plt.tight_layout(); plt.savefig('endoscopy_recon.png'); plt.show()
```
...
python3 main.py --modality endoscopy --period forward --prompt "your system description"
python3 main.py --modality endoscopy --period reconstruction --prompt "your algorithm"
```
