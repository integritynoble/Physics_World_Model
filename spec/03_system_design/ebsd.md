...
```
[Source] → [Forward Model (Electron Backscatter Diffraction (EBSD))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Electron Backscatter Diffraction (EBSD) physics | operator model error |
...
**Dataset**: Electron Backscatter Diffraction (EBSD)
**Input**: Kikuchi pattern (H × W × px × py, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/ebsd.md`
...
plt.tight_layout(); plt.savefig('ebsd_recon.png'); plt.show()
```
...
python3 main.py --modality ebsd --period forward --prompt "your system description"
python3 main.py --modality ebsd --period reconstruction --prompt "your algorithm"
```
