...
```
[Source] → [Forward Model (Stellar Coronagraphy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Stellar Coronagraphy physics | operator model error |
...
**Dataset**: Stellar Coronagraphy
**Input**: image (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/coronagraphy.md`
...
plt.tight_layout(); plt.savefig('coronagraphy_recon.png'); plt.show()
```
...
python3 main.py --modality coronagraphy --period forward --prompt "your system description"
python3 main.py --modality coronagraphy --period reconstruction --prompt "your algorithm"
```
