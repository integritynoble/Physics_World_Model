...
```
[Source] → [Forward Model (Three-Photon Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Three-Photon Microscopy physics | operator model error |
...
**Dataset**: Three-Photon Microscopy
**Input**: Z-stack (Z × H × W, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/three_photon.md`
...
plt.tight_layout(); plt.savefig('three_photon_recon.png'); plt.show()
```
...
python3 main.py --modality three_photon --period forward --prompt "your system description"
python3 main.py --modality three_photon --period reconstruction --prompt "your algorithm"
```
