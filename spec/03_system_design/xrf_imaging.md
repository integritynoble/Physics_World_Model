...
```
[Source] → [Forward Model (X-ray Fluorescence (XRF) Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | X-ray Fluorescence (XRF) Imaging physics | operator model error |
...
**Dataset**: X-ray Fluorescence (XRF) Imaging
**Input**: fluorescence map (H × W × elements, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/xrf_imaging.md`
...
plt.tight_layout(); plt.savefig('xrf_imaging_recon.png'); plt.show()
```
...
python3 main.py --modality xrf_imaging --period forward --prompt "your system description"
python3 main.py --modality xrf_imaging --period reconstruction --prompt "your algorithm"
```
