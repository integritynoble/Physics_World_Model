...
```
[Source] → [Forward Model (Shearography)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Shearography physics | operator model error |
...
**Dataset**: Shearography
**Input**: shearograms (2 × H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/shearography.md`
...
plt.tight_layout(); plt.savefig('shearography_recon.png'); plt.show()
```
...
python3 main.py --modality shearography --period forward --prompt "your system description"
python3 main.py --modality shearography --period reconstruction --prompt "your algorithm"
```
