...
```
[Source] → [Forward Model (Scanning Tunneling Microscopy (STM))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Scanning Tunneling Microscopy (STM) physics | operator model error |
...
**Dataset**: Scanning Tunneling Microscopy (STM)
**Input**: tunneling map (H × W, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/stm.md`
...
plt.tight_layout(); plt.savefig('stm_recon.png'); plt.show()
```
...
python3 main.py --modality stm --period forward --prompt "your system description"
python3 main.py --modality stm --period reconstruction --prompt "your algorithm"
```
