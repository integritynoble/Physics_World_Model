...
```
[Source] → [Forward Model (Pump-Probe Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Pump-Probe Microscopy physics | operator model error |
...
**Dataset**: Pump-Probe Microscopy
**Input**: transient spectra (T × λ, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/pump_probe.md`
...
plt.tight_layout(); plt.savefig('pump_probe_recon.png'); plt.show()
```
...
python3 main.py --modality pump_probe --period forward --prompt "your system description"
python3 main.py --modality pump_probe --period reconstruction --prompt "your algorithm"
```
