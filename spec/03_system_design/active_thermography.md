...
```
[Source] → [Forward Model (Active Thermography (IR))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Active Thermography (IR) physics | operator model error |
...
**Dataset**: Active Thermography (IR)
**Input**: thermal sequence (T × H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/active_thermography.md`
...
plt.tight_layout(); plt.savefig('active_thermography_recon.png'); plt.show()
```
...
python3 main.py --modality active_thermography --period forward --prompt "your system description"
python3 main.py --modality active_thermography --period reconstruction --prompt "your algorithm"
```
