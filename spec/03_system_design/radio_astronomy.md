...
```
[Source] → [Forward Model (Radio Aperture Synthesis)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Radio Aperture Synthesis physics | operator model error |
...
**Dataset**: Radio Aperture Synthesis
**Input**: visibilities (baselines × freq × T, complex64)
**Algorithms**: 11 CPU, 4 GPU — see `spec/radio_astronomy.md`
...
plt.tight_layout(); plt.savefig('radio_astronomy_recon.png'); plt.show()
```
...
python3 main.py --modality radio_astronomy --period forward --prompt "your system description"
python3 main.py --modality radio_astronomy --period reconstruction --prompt "your algorithm"
```
