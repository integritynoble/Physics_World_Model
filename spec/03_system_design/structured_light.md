...
```
[Source] → [Forward Model (Structured-Light Depth Camera)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Structured-Light Depth Camera physics | operator model error |
...
**Dataset**: Structured-Light Depth Camera
**Input**: pattern images (N × H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/structured_light.md`
...
plt.tight_layout(); plt.savefig('structured_light_recon.png'); plt.show()
```
...
python3 main.py --modality structured_light --period forward --prompt "your system description"
python3 main.py --modality structured_light --period reconstruction --prompt "your algorithm"
```
