...
```
[Source] → [Forward Model (XFEL Serial Femtosecond Crystallography (SFX))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | XFEL Serial Femtosecond Crystallography (SFX) physics | operator model error |
...
**Dataset**: XFEL Serial Femtosecond Crystallography (SFX)
**Input**: diffraction patterns (N_shots × H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/xfel_sfx.md`
...
plt.tight_layout(); plt.savefig('xfel_sfx_recon.png'); plt.show()
```
...
python3 main.py --modality xfel_sfx --period forward --prompt "your system description"
python3 main.py --modality xfel_sfx --period reconstruction --prompt "your algorithm"
```
