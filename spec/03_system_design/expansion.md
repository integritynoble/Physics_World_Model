...
```
[Source] → [Forward Model (Expansion Microscopy (ExM))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Expansion Microscopy (ExM) physics | operator model error |
...
**Dataset**: Expansion Microscopy (ExM)
**Input**: confocal + expansion (H × W, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/expansion.md`
...
plt.tight_layout(); plt.savefig('expansion_recon.png'); plt.show()
```
...
python3 main.py --modality expansion --period forward --prompt "your system description"
python3 main.py --modality expansion --period reconstruction --prompt "your algorithm"
```
