...
```
[Source] → [Forward Model (Acoustic Emission Testing (AE))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Acoustic Emission Testing (AE) physics | operator model error |
...
**Dataset**: Acoustic Emission Testing (AE)
**Input**: waveform (samples, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/acoustic_emission.md`
...
plt.tight_layout(); plt.savefig('acoustic_emission_recon.png'); plt.show()
```
...
python3 main.py --modality acoustic_emission --period forward --prompt "your system description"
python3 main.py --modality acoustic_emission --period reconstruction --prompt "your algorithm"
```
