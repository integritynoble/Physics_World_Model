...
```
[Source] → [Forward Model (US/MRI Fusion)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | US/MRI Fusion physics | operator model error |
...
**Dataset**: US/MRI Fusion
**Input**: US + MRI combined data
**Algorithms**: 11 CPU, 4 GPU — see `spec/us_mri.md`
...
plt.tight_layout(); plt.savefig('us_mri_recon.png'); plt.show()
```
...
python3 main.py --modality us_mri --period forward --prompt "your system description"
python3 main.py --modality us_mri --period reconstruction --prompt "your algorithm"
```
