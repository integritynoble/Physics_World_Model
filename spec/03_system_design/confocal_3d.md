...
```
[Source] → [Forward Model (Confocal 3D Z-Stack)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Confocal 3D Z-Stack physics | operator model error |
...
**Dataset**: Confocal 3D Z-Stack
**Input**: Z-stack (Z × H × W, float32)
**Algorithms**: 11 CPU, 5 GPU — see `spec/confocal_3d.md`
...
plt.tight_layout(); plt.savefig('confocal_3d_recon.png'); plt.show()
```
...
python3 main.py --modality confocal_3d --period forward --prompt "your system description"
python3 main.py --modality confocal_3d --period reconstruction --prompt "your algorithm"
```
