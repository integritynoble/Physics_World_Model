...
```
[Source] → [Forward Model (Passive Microwave Radiometry)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Passive Microwave Radiometry physics | operator model error |
...
**Dataset**: Passive Microwave Radiometry
**Input**: brightness T (H × W × ch, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/passive_microwave.md`
...
plt.tight_layout(); plt.savefig('passive_microwave_recon.png'); plt.show()
```
...
python3 main.py --modality passive_microwave --period forward --prompt "your system description"
python3 main.py --modality passive_microwave --period reconstruction --prompt "your algorithm"
```
