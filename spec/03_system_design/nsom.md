...
```
[Source] → [Forward Model (Near-field Scanning Optical Microscopy (NSOM))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Near-field Scanning Optical Microscopy (NSOM) physics | operator model error |
...
**Dataset**: Near-field Scanning Optical Microscopy (NSOM)
**Input**: near-field signal (H × W, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/nsom.md`
...
plt.tight_layout(); plt.savefig('nsom_recon.png'); plt.show()
```
...
python3 main.py --modality nsom --period forward --prompt "your system description"
python3 main.py --modality nsom --period reconstruction --prompt "your algorithm"
```
