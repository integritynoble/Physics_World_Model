...
```
[Source] → [Forward Model (SPECT/CT Fusion)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | SPECT/CT Fusion physics | operator model error |
...
**Dataset**: SPECT/CT Fusion
**Input**: SPECT proj + CT sino (both float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/spect_ct.md`
...
plt.tight_layout(); plt.savefig('spect_ct_recon.png'); plt.show()
```
...
python3 main.py --modality spect_ct --period forward --prompt "your system description"
python3 main.py --modality spect_ct --period reconstruction --prompt "your algorithm"
```
