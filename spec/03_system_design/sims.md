...
```
[Source] → [Forward Model (Secondary Ion Mass Spectrometry (SIMS) Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Secondary Ion Mass Spectrometry (SIMS) Imaging physics | operator model error |
...
**Dataset**: Secondary Ion Mass Spectrometry (SIMS) Imaging
**Input**: ion images (H × W × m/z, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/sims.md`
...
plt.tight_layout(); plt.savefig('sims_recon.png'); plt.show()
```
...
python3 main.py --modality sims --period forward --prompt "your system description"
python3 main.py --modality sims --period reconstruction --prompt "your algorithm"
```
