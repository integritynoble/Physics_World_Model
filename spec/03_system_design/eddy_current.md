...
```
[Source] → [Forward Model (Eddy Current Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Eddy Current Imaging physics | operator model error |
...
**Dataset**: Eddy Current Imaging
**Input**: induced voltage (coils × time, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/eddy_current.md`
...
plt.tight_layout(); plt.savefig('eddy_current_recon.png'); plt.show()
```
...
python3 main.py --modality eddy_current --period forward --prompt "your system description"
python3 main.py --modality eddy_current --period reconstruction --prompt "your algorithm"
```
