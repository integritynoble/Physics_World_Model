...
```
[Source] → [Forward Model (Photometric Stereo)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Photometric Stereo physics | operator model error |
...
**Algorithms**: 11 CPU, 4 GPU — see `spec/photometric_stereo.md`
...
plt.tight_layout(); plt.savefig('photometric_stereo_recon.png'); plt.show()
```
...
python3 main.py --modality photometric_stereo --period forward --prompt "your system description"
python3 main.py --modality photometric_stereo --period reconstruction --prompt "your algorithm"
```
