...
```
[Source] → [Forward Model (Electron Tomography)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Electron Tomography physics | operator model error |
...
**Dataset**: Electron Tomography
**Input**: tilt series (angles × H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/electron_tomography.md`
...
plt.tight_layout(); plt.savefig('electron_tomography_recon.png'); plt.show()
```
...
python3 main.py --modality electron_tomography --period forward --prompt "your system description"
python3 main.py --modality electron_tomography --period reconstruction --prompt "your algorithm"
```
