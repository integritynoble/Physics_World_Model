...
```
[Source] → [Forward Model (High Dynamic Range (HDR) Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | High Dynamic Range (HDR) Imaging physics | operator model error |
...
**Dataset**: High Dynamic Range (HDR) Imaging
**Input**: multi-exposure (K × H × W × 3, uint8)
**Algorithms**: 11 CPU, 4 GPU — see `spec/hdr_imaging.md`
...
plt.tight_layout(); plt.savefig('hdr_imaging_recon.png'); plt.show()
```
...
python3 main.py --modality hdr_imaging --period forward --prompt "your system description"
python3 main.py --modality hdr_imaging --period reconstruction --prompt "your algorithm"
```
