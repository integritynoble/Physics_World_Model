...
```
[Source] → [Forward Model (Sonar Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Sonar Imaging physics | operator model error |
...
**Dataset**: Sonar Imaging
**Input**: echo data (elements × samples, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/sonar.md`
...
plt.tight_layout(); plt.savefig('sonar_recon.png'); plt.show()
```
...
python3 main.py --modality sonar --period forward --prompt "your system description"
python3 main.py --modality sonar --period reconstruction --prompt "your algorithm"
```
