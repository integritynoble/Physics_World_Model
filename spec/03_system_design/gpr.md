...
```
[Source] → [Forward Model (Ground-Penetrating Radar (GPR))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Ground-Penetrating Radar (GPR) physics | operator model error |
...
**Dataset**: Ground-Penetrating Radar (GPR)
**Input**: B-scan (traces × samples, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/gpr.md`
...
plt.tight_layout(); plt.savefig('gpr_recon.png'); plt.show()
```
...
python3 main.py --modality gpr --period forward --prompt "your system description"
python3 main.py --modality gpr --period reconstruction --prompt "your algorithm"
```
