...
```
[Source] → [Forward Model (Diffusion MRI (DTI))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Diffusion MRI (DTI) physics | operator model error |
...
**Dataset**: Diffusion MRI (DTI)
**Input**: DWI (N_dirs × H × W × D, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/diffusion_mri.md`
...
plt.tight_layout(); plt.savefig('diffusion_mri_recon.png'); plt.show()
```
...
python3 main.py --modality diffusion_mri --period forward --prompt "your system description"
python3 main.py --modality diffusion_mri --period reconstruction --prompt "your algorithm"
```
