...
```
[Source] → [Forward Model (Phase Contrast Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Phase Contrast Microscopy physics | operator model error |
...
**Dataset**: Phase Contrast Microscopy
**Input**: image (H × W, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/phase_contrast.md`
...
plt.tight_layout(); plt.savefig('phase_contrast_recon.png'); plt.show()
```
...
python3 main.py --modality phase_contrast --period forward --prompt "your system description"
python3 main.py --modality phase_contrast --period reconstruction --prompt "your algorithm"
```
