...
```
[Source] → [Forward Model (Gravitational Wave Detection)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Gravitational Wave Detection physics | operator model error |
...
**Dataset**: Gravitational Wave Detection
**Input**: strain (samples, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/gravitational_wave.md`
...
plt.tight_layout(); plt.savefig('gravitational_wave_recon.png'); plt.show()
```
...
python3 main.py --modality gravitational_wave --period forward --prompt "your system description"
python3 main.py --modality gravitational_wave --period reconstruction --prompt "your algorithm"
```
