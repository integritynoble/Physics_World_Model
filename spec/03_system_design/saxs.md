...
```
[Source] → [Forward Model (Small-Angle X-ray Scattering (SAXS))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Small-Angle X-ray Scattering (SAXS) physics | operator model error |
...
**Dataset**: Small-Angle X-ray Scattering (SAXS)
**Input**: scattering pattern (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/saxs.md`
...
plt.tight_layout(); plt.savefig('saxs_recon.png'); plt.show()
```
...
python3 main.py --modality saxs --period forward --prompt "your system description"
python3 main.py --modality saxs --period reconstruction --prompt "your algorithm"
```
