...
```
[Source] → [Forward Model (Portal Imaging (EPID))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Portal Imaging (EPID) physics | operator model error |
...
**Dataset**: Portal Imaging (EPID)
**Input**: EPID projection (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/portal_imaging.md`
...
plt.tight_layout(); plt.savefig('portal_imaging_recon.png'); plt.show()
```
...
python3 main.py --modality portal_imaging --period forward --prompt "your system description"
python3 main.py --modality portal_imaging --period reconstruction --prompt "your algorithm"
```
