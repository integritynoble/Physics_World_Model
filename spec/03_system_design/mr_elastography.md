...
```
[Source] → [Forward Model (MR Elastography (MRE))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | MR Elastography (MRE) physics | operator model error |
...
**Dataset**: MR Elastography (MRE)
**Input**: wave images (slices × H × W, complex64)
**Algorithms**: 11 CPU, 4 GPU — see `spec/mr_elastography.md`
...
plt.tight_layout(); plt.savefig('mr_elastography_recon.png'); plt.show()
```
...
python3 main.py --modality mr_elastography --period forward --prompt "your system description"
python3 main.py --modality mr_elastography --period reconstruction --prompt "your algorithm"
```
