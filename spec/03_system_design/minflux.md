...
```
[Source] → [Forward Model (MINFLUX Nanoscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | MINFLUX Nanoscopy physics | operator model error |
...
**Dataset**: MINFLUX Nanoscopy
**Input**: photon records (N × 5: t,x,y,z,id)
**Algorithms**: 9 CPU, 6 GPU — see `spec/minflux.md`
...
plt.tight_layout(); plt.savefig('minflux_recon.png'); plt.show()
```
...
python3 main.py --modality minflux --period forward --prompt "your system description"
python3 main.py --modality minflux --period reconstruction --prompt "your algorithm"
```
