...
```
[Source] → [Forward Model (Bioluminescence Tomography (BLT))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Bioluminescence Tomography (BLT) physics | operator model error |
...
**Dataset**: Bioluminescence Tomography (BLT)
**Input**: surface flux (H × W × angles, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/bioluminescence_tomo.md`
...
plt.tight_layout(); plt.savefig('bioluminescence_tomo_recon.png'); plt.show()
```
...
python3 main.py --modality bioluminescence_tomo --period forward --prompt "your system description"
python3 main.py --modality bioluminescence_tomo --period reconstruction --prompt "your algorithm"
```
