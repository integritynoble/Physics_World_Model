...
```
[Source] → [Forward Model (PET/MR Fusion)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | PET/MR Fusion physics | operator model error |
...
**Dataset**: PET/MR Fusion
**Input**: PET sino + MRI k-space (both float32)
**Algorithms**: 3 CPU, 0 GPU — see `spec/pet_mr.md`
...
plt.tight_layout(); plt.savefig('pet_mr_recon.png'); plt.show()
```
...
python3 main.py --modality pet_mr --period forward --prompt "your system description"
python3 main.py --modality pet_mr --period reconstruction --prompt "your algorithm"
```
