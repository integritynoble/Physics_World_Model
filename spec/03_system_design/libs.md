...
```
[Source] → [Forward Model (Laser-Induced Breakdown Spectroscopy (LIBS) Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Laser-Induced Breakdown Spectroscopy (LIBS) Imaging physics | operator model error |
...
**Dataset**: Laser-Induced Breakdown Spectroscopy (LIBS) Imaging
**Input**: emission spectrum (wavelengths, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/libs.md`
...
plt.tight_layout(); plt.savefig('libs_recon.png'); plt.show()
```
...
python3 main.py --modality libs --period forward --prompt "your system description"
python3 main.py --modality libs --period reconstruction --prompt "your algorithm"
```
