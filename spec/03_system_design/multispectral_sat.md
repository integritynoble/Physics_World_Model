...
```
[Source] → [Forward Model (Multispectral Satellite Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Multispectral Satellite Imaging physics | operator model error |
...
**Dataset**: Multispectral Satellite Imaging
**Input**: radiance (H × W × bands, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/multispectral_sat.md`
...
plt.tight_layout(); plt.savefig('multispectral_sat_recon.png'); plt.show()
```
...
python3 main.py --modality multispectral_sat --period forward --prompt "your system description"
python3 main.py --modality multispectral_sat --period reconstruction --prompt "your algorithm"
```
