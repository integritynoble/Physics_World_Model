...
```
[Source] → [Forward Model (STED Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | STED Microscopy physics | operator model error |
...
**Dataset**: STED Microscopy
**Input**: STED + confocal (2 × H × W, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/sted.md`
...
plt.tight_layout(); plt.savefig('sted_recon.png'); plt.show()
```
...
python3 main.py --modality sted --period forward --prompt "your system description"
python3 main.py --modality sted --period reconstruction --prompt "your algorithm"
```
