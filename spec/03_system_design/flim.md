...
```
[Source] → [Forward Model (Fluorescence Lifetime Imaging (FLIM))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Fluorescence Lifetime Imaging (FLIM) physics | operator model error |
...
**Dataset**: Fluorescence Lifetime Imaging (FLIM)
**Input**: photon arrivals (H × W × T, float32)
**Algorithms**: 12 CPU, 4 GPU — see `spec/flim.md`
...
plt.tight_layout(); plt.savefig('flim_recon.png'); plt.show()
```
...
python3 main.py --modality flim --period forward --prompt "your system description"
python3 main.py --modality flim --period reconstruction --prompt "your algorithm"
```
