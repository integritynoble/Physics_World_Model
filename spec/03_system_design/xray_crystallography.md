...
```
[Source] → [Forward Model (X-ray Crystallography)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | X-ray Crystallography physics | operator model error |
...
**Dataset**: X-ray Crystallography
**Input**: structure factors (hkl × F, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/xray_crystallography.md`
...
plt.tight_layout(); plt.savefig('xray_crystallography_recon.png'); plt.show()
```
...
python3 main.py --modality xray_crystallography --period forward --prompt "your system description"
python3 main.py --modality xray_crystallography --period reconstruction --prompt "your algorithm"
```
