...
```
[Source] → [Forward Model (Neutron Diffraction)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Neutron Diffraction physics | operator model error |
...
**Dataset**: Neutron Diffraction
**Input**: pattern (2θ × intensity, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/neutron_diffraction.md`
...
plt.tight_layout(); plt.savefig('neutron_diffraction_recon.png'); plt.show()
```
...
python3 main.py --modality neutron_diffraction --period forward --prompt "your system description"
python3 main.py --modality neutron_diffraction --period reconstruction --prompt "your algorithm"
```
