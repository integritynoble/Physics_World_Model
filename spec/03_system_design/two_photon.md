...
```
[Source] → [Forward Model (Two-Photon / Multiphoton Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Two-Photon / Multiphoton Microscopy physics | operator model error |
...
**Dataset**: Two-Photon / Multiphoton Microscopy
**Input**: Z-stack (Z × H × W, float32)
**Algorithms**: 9 CPU, 6 GPU — see `spec/two_photon.md`
...
plt.tight_layout(); plt.savefig('two_photon_recon.png'); plt.show()
```
...
python3 main.py --modality two_photon --period forward --prompt "your system description"
python3 main.py --modality two_photon --period reconstruction --prompt "your algorithm"
```
