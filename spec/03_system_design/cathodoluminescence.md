...
```
[Source] → [Forward Model (Cathodoluminescence (CL) Imaging)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Cathodoluminescence (CL) Imaging physics | operator model error |
...
**Dataset**: Cathodoluminescence (CL) Imaging
**Input**: spectrum image (H × W × λ, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/cathodoluminescence.md`
...
plt.tight_layout(); plt.savefig('cathodoluminescence_recon.png'); plt.show()
```
...
python3 main.py --modality cathodoluminescence --period forward --prompt "your system description"
python3 main.py --modality cathodoluminescence --period reconstruction --prompt "your algorithm"
```
