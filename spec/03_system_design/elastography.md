...
```
[Source] → [Forward Model (Shear-Wave Elastography)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Shear-Wave Elastography physics | operator model error |
...
**Dataset**: Shear-Wave Elastography
**Input**: displacement (H × W × 3, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/elastography.md`
...
plt.tight_layout(); plt.savefig('elastography_recon.png'); plt.show()
```
...
python3 main.py --modality elastography --period forward --prompt "your system description"
python3 main.py --modality elastography --period reconstruction --prompt "your algorithm"
```
