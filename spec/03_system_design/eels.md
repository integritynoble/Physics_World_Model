...
```
[Source] → [Forward Model (Electron Energy Loss Spectroscopy (EELS))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Electron Energy Loss Spectroscopy (EELS) physics | operator model error |
...
**Dataset**: Electron Energy Loss Spectroscopy (EELS)
**Input**: energy-loss spectrum (H × W × E, float32)
**Algorithms**: 12 CPU, 4 GPU — see `spec/eels.md`
...
plt.tight_layout(); plt.savefig('eels_recon.png'); plt.show()
```
...
python3 main.py --modality eels --period forward --prompt "your system description"
python3 main.py --modality eels --period reconstruction --prompt "your algorithm"
```
