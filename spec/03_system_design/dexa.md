...
```
[Source] → [Forward Model (Dual-Energy X-ray Absorptiometry (DEXA))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Dual-Energy X-ray Absorptiometry (DEXA) physics | operator model error |
...
**Dataset**: Dual-Energy X-ray Absorptiometry (DEXA)
**Input**: dual-energy projections (2 × H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/dexa.md`
...
plt.tight_layout(); plt.savefig('dexa_recon.png'); plt.show()
```
...
python3 main.py --modality dexa --period forward --prompt "your system description"
python3 main.py --modality dexa --period reconstruction --prompt "your algorithm"
```
