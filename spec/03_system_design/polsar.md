...
```
[Source] → [Forward Model (Polarimetric SAR (PolSAR))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Polarimetric SAR (PolSAR) physics | operator model error |
...
**Dataset**: Polarimetric SAR (PolSAR)
**Input**: scattering matrix (H × W × 4, complex64)
**Algorithms**: 11 CPU, 4 GPU — see `spec/polsar.md`
...
plt.tight_layout(); plt.savefig('polsar_recon.png'); plt.show()
```
...
python3 main.py --modality polsar --period forward --prompt "your system description"
python3 main.py --modality polsar --period reconstruction --prompt "your algorithm"
```
