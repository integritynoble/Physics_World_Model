...
```
[Source] → [Forward Model (Brillouin Microscopy)] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | Brillouin Microscopy physics | operator model error |
...
**Dataset**: Brillouin Microscopy
**Input**: spectral shift map (H × W, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/brillouin.md`
...
plt.tight_layout(); plt.savefig('brillouin_recon.png'); plt.show()
```
...
python3 main.py --modality brillouin --period forward --prompt "your system description"
python3 main.py --modality brillouin --period reconstruction --prompt "your algorithm"
```
