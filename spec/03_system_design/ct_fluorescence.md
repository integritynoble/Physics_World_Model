...
```
[Source] → [Forward Model (CT + Fluorescence (FLIT))] → [Detector] → y
      ↓                ↓
  [Noise]         [Mismatch]
```
...
| Forward model | CT + Fluorescence (FLIT) physics | operator model error |
...
**Dataset**: CT + Fluorescence (FLIT)
**Input**: XRF sinogram (angles × detectors × ch, float32)
**Algorithms**: 11 CPU, 4 GPU — see `spec/ct_fluorescence.md`
...
plt.tight_layout(); plt.savefig('ct_fluorescence_recon.png'); plt.show()
```
...
python3 main.py --modality ct_fluorescence --period forward --prompt "your system description"
python3 main.py --modality ct_fluorescence --period reconstruction --prompt "your algorithm"
```
