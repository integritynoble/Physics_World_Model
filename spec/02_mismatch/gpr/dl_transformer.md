# Ground-Penetrating Radar (GPR) — RS-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: B-scan (traces × samples, float32)
...
plt.tight_layout(); plt.savefig('gpr_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`Ground-Penetrating Radar (GPR) — RS-Transformer + gradient` corresponds to the leaderboard entry at
