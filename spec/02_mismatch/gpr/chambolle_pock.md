# Ground-Penetrating Radar (GPR) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: B-scan (traces × samples, float32)
...
import h5py
with h5py.File('gpr_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/gpr/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # B-scan (traces × samples, float32)
...
plt.tight_layout(); plt.savefig('gpr_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Ground-Penetrating Radar (GPR) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/gpr` (mismatch correction tier).
