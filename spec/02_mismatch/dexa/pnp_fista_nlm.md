# Dual-Energy X-ray Absorptiometry (DEXA) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('dexa_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`Dual-Energy X-ray Absorptiometry (DEXA) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
