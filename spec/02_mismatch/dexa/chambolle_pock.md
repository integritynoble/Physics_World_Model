# Dual-Energy X-ray Absorptiometry (DEXA) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: dual-energy projections (2 × H × W, float32)
...
import h5py
with h5py.File('dexa_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/dexa/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # dual-energy projections (2 × H × W, float32)
...
plt.tight_layout(); plt.savefig('dexa_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Dual-Energy X-ray Absorptiometry (DEXA) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/dexa` (mismatch correction tier).
