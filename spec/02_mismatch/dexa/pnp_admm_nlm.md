# Dual-Energy X-ray Absorptiometry (DEXA) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('dexa_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`Dual-Energy X-ray Absorptiometry (DEXA) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
