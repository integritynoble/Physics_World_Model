# Coded Exposure / Flutter Shutter — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: coded frames (N × H × W, float32)
...
import h5py
with h5py.File('coded_exposure_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/coded_exposure/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # coded frames (N × H × W, float32)
...
plt.tight_layout(); plt.savefig('coded_exposure_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Coded Exposure / Flutter Shutter — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/coded_exposure` (mismatch correction tier).
