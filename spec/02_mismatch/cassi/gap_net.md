# Coded Aperture Snapshot Spectral Imaging (CASSI) — GAP-Net + Gradient Mismatch Correction
...
x_wrong = run_solver('gap_net', y, cfg={'model_key': 'gap_net'})
...
cfg.update({'model_key': 'gap_net'})
x_corrected = run_solver('gap_net', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cassi_gap_net_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — GAP-Net + gradient` corresponds to the leaderboard entry at
