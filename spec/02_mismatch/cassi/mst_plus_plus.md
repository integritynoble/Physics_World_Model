# Coded Aperture Snapshot Spectral Imaging (CASSI) — MST++ + Gradient Mismatch Correction
...
x_wrong = run_solver('mst_plus_plus', y, cfg={'model_key': 'mst_plus_plus'})
...
cfg.update({'model_key': 'mst_plus_plus'})
x_corrected = run_solver('mst_plus_plus', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cassi_mst_plus_plus_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — MST++ + gradient` corresponds to the leaderboard entry at
