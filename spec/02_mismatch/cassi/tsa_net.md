# Coded Aperture Snapshot Spectral Imaging (CASSI) — TSA-Net + Gradient Mismatch Correction
...
x_wrong = run_solver('tsa_net', y, cfg={'model_key': 'tsa_net'})
...
cfg.update({'model_key': 'tsa_net'})
x_corrected = run_solver('tsa_net', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cassi_tsa_net_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — TSA-Net + gradient` corresponds to the leaderboard entry at
