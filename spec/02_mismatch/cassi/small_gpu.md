# Coded Aperture Snapshot Spectral Imaging (CASSI) — GAP-TV (fast) + Gradient Mismatch Correction
...
x_wrong = run_solver('small_gpu', y, cfg={'iters': 50, 'lam': 0.1, 'tv_iter': 5})
...
cfg.update({'iters': 50, 'lam': 0.1, 'tv_iter': 5})
...
plt.tight_layout(); plt.savefig('cassi_small_gpu_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — GAP-TV (fast) + gradient` corresponds to the leaderboard entry at
