# Coded Aperture Snapshot Spectral Imaging (CASSI) — MiJUN-5stg + Gradient Mismatch Correction
...
x_wrong = run_solver('mijun_5stg', y, cfg={'model_key': 'mijun_5stg'})
...
cfg.update({'model_key': 'mijun_5stg'})
x_corrected = run_solver('mijun_5stg', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cassi_mijun_5stg_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — MiJUN-5stg + gradient` corresponds to the leaderboard entry at
