# Coded Aperture Snapshot Spectral Imaging (CASSI) — λ-Net + Gradient Mismatch Correction
...
x_wrong = run_solver('lambda_net', y, cfg={'model_key': 'lambda_net'})
...
cfg.update({'model_key': 'lambda_net'})
x_corrected = run_solver('lambda_net', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cassi_lambda_net_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — λ-Net + gradient` corresponds to the leaderboard entry at
