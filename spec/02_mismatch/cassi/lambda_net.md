# Coded Aperture Snapshot Spectral Imaging (CASSI) — λ-Net + Gradient

**GPU**  **PSNR**: ~30.1 dB  **Mismatch**: dispersion step `[1, 5] px`
...
x_wrong = run_solver('lambda_net', y)           # no correction
disp = cassi_calibrate_step(y)
calib_cfg = {"disp_step": float(disp)}
x = run_solver('lambda_net', y, cfg={**calib_cfg, **{'model_key': 'lambda_net'}})        # corrected
