# Coded Aperture Snapshot Spectral Imaging (CASSI) — SSR-L + Gradient Mismatch Correction
...
x_wrong = run_solver('ssr_l', y, cfg={'model_key': 'ssr_l'})
...
cfg.update({'model_key': 'ssr_l'})
x_corrected = run_solver('ssr_l', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cassi_ssr_l_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — SSR-L + gradient` corresponds to the leaderboard entry at
