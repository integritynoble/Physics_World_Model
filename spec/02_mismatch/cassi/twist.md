# Coded Aperture Snapshot Spectral Imaging (CASSI) — TwIST + Gradient Mismatch Correction

**Device**: CPU  **Input**: coded snapshot (H × W, float32)
...
x_wrong = run_solver('twist', y, cfg={'iters': 100, 'lam': 0.01, 'tv_iter': 5})
...
cfg.update({'iters': 100, 'lam': 0.01, 'tv_iter': 5})
x_corrected = run_solver('twist', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cassi_twist_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — TwIST + gradient` corresponds to the leaderboard entry at
