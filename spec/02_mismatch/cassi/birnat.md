# Coded Aperture Snapshot Spectral Imaging (CASSI) — BIRNAT + Gradient Mismatch Correction
...
x_wrong = run_solver('birnat', y, cfg={'model_key': 'birnat'})
...
cfg.update({'model_key': 'birnat'})
x_corrected = run_solver('birnat', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cassi_birnat_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — BIRNAT + gradient` corresponds to the leaderboard entry at
