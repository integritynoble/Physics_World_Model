# Coded Aperture Snapshot Spectral Imaging (CASSI) — ADMM-Net + Gradient Mismatch Correction
...
x_wrong = run_solver('admm_net', y, cfg={'model_key': 'admm_net'})
...
cfg.update({'model_key': 'admm_net'})
x_corrected = run_solver('admm_net', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cassi_admm_net_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — ADMM-Net + gradient` corresponds to the leaderboard entry at
