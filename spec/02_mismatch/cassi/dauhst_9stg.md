# Coded Aperture Snapshot Spectral Imaging (CASSI) — DAUHST-9stg + Gradient Mismatch Correction
...
x_wrong = run_solver('dauhst_9stg', y, cfg={'model_key': 'dauhst_9stg'})
...
cfg.update({'model_key': 'dauhst_9stg'})
x_corrected = run_solver('dauhst_9stg', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cassi_dauhst_9stg_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — DAUHST-9stg + gradient` corresponds to the leaderboard entry at
