# Coded Aperture Snapshot Spectral Imaging (CASSI) — DGSMP + Gradient Mismatch Correction
...
x_wrong = run_solver('dgsmp', y, cfg={'model_key': 'dgsmp'})
...
cfg.update({'model_key': 'dgsmp'})
x_corrected = run_solver('dgsmp', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cassi_dgsmp_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — DGSMP + gradient` corresponds to the leaderboard entry at
