# Coded Aperture Snapshot Spectral Imaging (CASSI) — BiSRNet + Gradient Mismatch Correction
...
x_wrong = run_solver('bisrnet', y, cfg={'model_key': 'bisrnet'})
...
cfg.update({'model_key': 'bisrnet'})
x_corrected = run_solver('bisrnet', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cassi_bisrnet_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — BiSRNet + gradient` corresponds to the leaderboard entry at
