# Coded Aperture Snapshot Spectral Imaging (CASSI) — RDLUF-MixS2-9stg + Gradient Mismatch Correction
...
x_wrong = run_solver('rdluf_mixs2_9stg', y, cfg={'model_key': 'rdluf_mixs2_9stg'})
...
cfg.update({'model_key': 'rdluf_mixs2_9stg'})
x_corrected = run_solver('rdluf_mixs2_9stg', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cassi_rdluf_mixs2_9stg_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — RDLUF-MixS2-9stg + gradient` corresponds to the leaderboard entry at
