# Coded Aperture Snapshot Spectral Imaging (CASSI) — GAP-TV (200 iter) + Gradient Mismatch Correction
...
x_wrong = run_solver('best_quality', y, cfg={'iters': 200, 'lam': 0.01, 'tv_iter': 5})
...
cfg.update({'iters': 200, 'lam': 0.01, 'tv_iter': 5})
...
plt.tight_layout(); plt.savefig('cassi_best_quality_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Snapshot Spectral Imaging (CASSI) — GAP-TV (200 iter) + gradient` corresponds to the leaderboard entry at
