# Coded Aperture Snapshot Spectral Imaging (CASSI) — MST-L + Gradient

**GPU**  **PSNR**: ~34.81 dB  **Mismatch**: dispersion step `[1, 5] px`
...
x_wrong = run_solver('mst_l', y)           # no correction
disp = cassi_calibrate_step(y)
calib_cfg = {"disp_step": float(disp)}
x = run_solver('mst_l', y, cfg=calib_cfg)        # corrected
