# Electron Backscatter Diffraction (EBSD) — EBSD-DL (DictIndex) [proxy] + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('ebsd_best_quality_corrected.png'); plt.show()
```

## Benchmark

`Electron Backscatter Diffraction (EBSD) — EBSD-DL (DictIndex) [proxy] + gradient` corresponds to the leaderboard entry at
