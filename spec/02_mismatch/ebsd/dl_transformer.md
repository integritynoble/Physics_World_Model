# Electron Backscatter Diffraction (EBSD) — Probe-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: Kikuchi pattern (H × W × px × py, float32)
...
import h5py
with h5py.File('ebsd_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/ebsd/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # Kikuchi pattern (H × W × px × py, float32)
...
plt.tight_layout(); plt.savefig('ebsd_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`Electron Backscatter Diffraction (EBSD) — Probe-Transformer + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/ebsd` (mismatch correction tier).
