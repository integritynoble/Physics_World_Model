# Three-Photon Microscopy — 3P-Net (CARE) + Gradient Mismatch Correction
...
import h5py
with h5py.File('three_photon_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/three_photon/public/
...
x_wrong = run_solver('3p_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('3p_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('three_photon_3p_dl_corrected.png'); plt.show()
```

## Benchmark

`Three-Photon Microscopy — 3P-Net (CARE) + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/three_photon` (mismatch correction tier).
