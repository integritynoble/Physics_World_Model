# Radio Aperture Synthesis — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: visibilities (baselines × freq × T, complex64)
...
import h5py
with h5py.File('radio_astronomy_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/radio_astronomy/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # visibilities (baselines × freq × T, complex64)
...
plt.tight_layout(); plt.savefig('radio_astronomy_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Radio Aperture Synthesis — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/radio_astronomy` (mismatch correction tier).
