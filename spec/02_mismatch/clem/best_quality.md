# Correlative Light-Electron Microscopy (CLEM) — PnP-ADMM [proxy] + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('clem_best_quality_corrected.png'); plt.show()
```

## Benchmark

`Correlative Light-Electron Microscopy (CLEM) — PnP-ADMM [proxy] + gradient` corresponds to the leaderboard entry at
