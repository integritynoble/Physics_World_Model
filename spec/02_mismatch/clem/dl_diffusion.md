# Correlative Light-Electron Microscopy (CLEM) — DiffusionMicro + Gradient Mismatch Correction

**Device**: GPU  **Input**: EM + fluorescence (H × W, float32)
...
plt.tight_layout(); plt.savefig('clem_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Correlative Light-Electron Microscopy (CLEM) — DiffusionMicro + gradient` corresponds to the leaderboard entry at
