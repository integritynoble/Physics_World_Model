# Ocean Acoustic Tomography — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: travel times (pairs, float32)
...
import h5py
with h5py.File('ocean_acoustic_tomo_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/ocean_acoustic_tomo/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # travel times (pairs, float32)
...
plt.tight_layout(); plt.savefig('ocean_acoustic_tomo_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Ocean Acoustic Tomography — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/ocean_acoustic_tomo` (mismatch correction tier).
