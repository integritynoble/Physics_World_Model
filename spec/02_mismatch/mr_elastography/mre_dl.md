# MR Elastography (MRE) — MRE-Net [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('mre_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('mre_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('mr_elastography_mre_dl_corrected.png'); plt.show()
```

## Benchmark

`MR Elastography (MRE) — MRE-Net [proxy] + gradient` corresponds to the leaderboard entry at
