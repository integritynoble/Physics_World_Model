# MR Elastography (MRE) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: wave images (slices × H × W, complex64)
...
import h5py
with h5py.File('mr_elastography_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/mr_elastography/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # wave images (slices × H × W, complex64)
...
plt.tight_layout(); plt.savefig('mr_elastography_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`MR Elastography (MRE) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/mr_elastography` (mismatch correction tier).
