# MR Elastography (MRE) — DiffusionMed + Gradient Mismatch Correction

**Device**: GPU  **Input**: wave images (slices × H × W, complex64)
...
plt.tight_layout(); plt.savefig('mr_elastography_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`MR Elastography (MRE) — DiffusionMed + gradient` corresponds to the leaderboard entry at
