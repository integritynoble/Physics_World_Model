# Ultrasound B-mode Imaging — US-ViT (PnP-DRS DRUNet) + Gradient Mismatch Correction
...
x_wrong = run_solver('us_vit', y, cfg={})
...
cfg = {"c0": float(sos)}
cfg.update({})
x_corrected = run_solver('us_vit', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ultrasound_us_vit_corrected.png'); plt.show()
```

## Benchmark

`Ultrasound B-mode Imaging — US-ViT (PnP-DRS DRUNet) + gradient` corresponds to the leaderboard entry at
