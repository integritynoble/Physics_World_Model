# Ultrasound B-mode Imaging — US-Mamba (RED DRUNet) + Gradient Mismatch Correction
...
x_wrong = run_solver('us_mamba', y, cfg={})
...
cfg = {"c0": float(sos)}
cfg.update({})
x_corrected = run_solver('us_mamba', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ultrasound_us_mamba_corrected.png'); plt.show()
```

## Benchmark

`Ultrasound B-mode Imaging — US-Mamba (RED DRUNet) + gradient` corresponds to the leaderboard entry at
