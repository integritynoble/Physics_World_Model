# Ultrasound B-mode Imaging — Minimum-Variance Capon Beamformer + Gradient Mismatch Correction

**Device**: CPU  **Input**: RF data (elements × samples, float32)
...
x_wrong = run_solver('mv_capon', y, cfg={})
...
cfg = {"c0": float(sos)}
cfg.update({})
x_corrected = run_solver('mv_capon', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ultrasound_mv_capon_corrected.png'); plt.show()
```

## Benchmark

`Ultrasound B-mode Imaging — Minimum-Variance Capon Beamformer + gradient` corresponds to the leaderboard entry at
