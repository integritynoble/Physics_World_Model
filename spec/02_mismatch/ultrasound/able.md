# Ultrasound B-mode Imaging — ABLE (PnP-HQS DRUNet) + Gradient Mismatch Correction
...
cfg = {"c0": float(sos)}
cfg.update({})
x_corrected = run_solver('able', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ultrasound_able_corrected.png'); plt.show()
```

## Benchmark

`Ultrasound B-mode Imaging — ABLE (PnP-HQS DRUNet) + gradient` corresponds to the leaderboard entry at
