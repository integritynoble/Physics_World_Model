# Ultrasound B-mode Imaging — US-Diffusion (PnP-PGD DRUNet) + Gradient Mismatch Correction
...
x_wrong = run_solver('us_diffusion', y, cfg={})
...
cfg = {"c0": float(sos)}
cfg.update({})
x_corrected = run_solver('us_diffusion', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ultrasound_us_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Ultrasound B-mode Imaging — US-Diffusion (PnP-PGD DRUNet) + gradient` corresponds to the leaderboard entry at
