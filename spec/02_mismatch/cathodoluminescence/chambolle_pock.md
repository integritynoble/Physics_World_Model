# Cathodoluminescence (CL) Imaging — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: spectrum image (H × W × λ, float32)
...
import h5py
with h5py.File('cathodoluminescence_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/cathodoluminescence/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # spectrum image (H × W × λ, float32)
...
plt.tight_layout(); plt.savefig('cathodoluminescence_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Cathodoluminescence (CL) Imaging — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/cathodoluminescence` (mismatch correction tier).
