# Cathodoluminescence (CL) Imaging — CL-Net [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('cl_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('cl_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cathodoluminescence_cl_dl_corrected.png'); plt.show()
```

## Benchmark

`Cathodoluminescence (CL) Imaging — CL-Net [proxy] + gradient` corresponds to the leaderboard entry at
