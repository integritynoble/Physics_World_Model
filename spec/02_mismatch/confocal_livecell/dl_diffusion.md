# Confocal Live-Cell Microscopy — DiffusionMicro + Gradient Mismatch Correction

**Device**: GPU  **Input**: time-lapse (T × H × W, float32)
...
import h5py
with h5py.File('confocal_livecell_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/confocal_livecell/public/
...
plt.tight_layout(); plt.savefig('confocal_livecell_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Confocal Live-Cell Microscopy — DiffusionMicro + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/confocal_livecell` (mismatch correction tier).
