# Streak Camera Imaging — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: streak image (time × space, float32)
...
import h5py
with h5py.File('streak_camera_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/streak_camera/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # streak image (time × space, float32)
...
plt.tight_layout(); plt.savefig('streak_camera_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Streak Camera Imaging — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/streak_camera` (mismatch correction tier).
