# PALM/STORM Single-Molecule Localization — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: localisations (N × 4: x,y,σ,I)
...
import h5py
with h5py.File('palm_storm_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/palm_storm/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # localisations (N × 4: x,y,σ,I)
...
plt.tight_layout(); plt.savefig('palm_storm_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`PALM/STORM Single-Molecule Localization — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/palm_storm` (mismatch correction tier).
