# Diffusion MRI (DTI) — q-DL (qDiffusion) [proxy] + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('diffusion_mri_best_quality_corrected.png'); plt.show()
```

## Benchmark

`Diffusion MRI (DTI) — q-DL (qDiffusion) [proxy] + gradient` corresponds to the leaderboard entry at
