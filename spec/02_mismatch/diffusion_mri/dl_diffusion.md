# Diffusion MRI (DTI) — DiffusionMed + Gradient Mismatch Correction

**Device**: GPU  **Input**: DWI (N_dirs × H × W × D, float32)
...
plt.tight_layout(); plt.savefig('diffusion_mri_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Diffusion MRI (DTI) — DiffusionMed + gradient` corresponds to the leaderboard entry at
