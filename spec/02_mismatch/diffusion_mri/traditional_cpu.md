# Diffusion MRI (DTI) — SENSE (WLS tensor fit) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('diffusion_mri_traditional_cpu_corrected.png'); plt.show()
```

## Benchmark

`Diffusion MRI (DTI) — SENSE (WLS tensor fit) + gradient` corresponds to the leaderboard entry at
