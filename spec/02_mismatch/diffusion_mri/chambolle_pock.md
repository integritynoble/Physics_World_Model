# Diffusion MRI (DTI) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: DWI (N_dirs × H × W × D, float32)
...
import h5py
with h5py.File('diffusion_mri_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/diffusion_mri/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # DWI (N_dirs × H × W × D, float32)
...
plt.tight_layout(); plt.savefig('diffusion_mri_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Diffusion MRI (DTI) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/diffusion_mri` (mismatch correction tier).
