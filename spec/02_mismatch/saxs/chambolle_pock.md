# Small-Angle X-ray Scattering (SAXS) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: scattering pattern (H × W, float32)
...
import h5py
with h5py.File('saxs_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/saxs/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # scattering pattern (H × W, float32)
...
plt.tight_layout(); plt.savefig('saxs_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Small-Angle X-ray Scattering (SAXS) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/saxs` (mismatch correction tier).
