# Small-Angle X-ray Scattering (SAXS) — SAXS-VAE [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('saxs_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('saxs_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('saxs_saxs_dl_corrected.png'); plt.show()
```

## Benchmark

`Small-Angle X-ray Scattering (SAXS) — SAXS-VAE [proxy] + gradient` corresponds to the leaderboard entry at
