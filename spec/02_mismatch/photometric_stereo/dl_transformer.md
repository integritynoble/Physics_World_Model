# Photometric Stereo — CS-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: images under N lights (N × H × W, float32)
...
import h5py
with h5py.File('photometric_stereo_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/photometric_stereo/public/
...
plt.tight_layout(); plt.savefig('photometric_stereo_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`Photometric Stereo — CS-Transformer + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/photometric_stereo` (mismatch correction tier).
