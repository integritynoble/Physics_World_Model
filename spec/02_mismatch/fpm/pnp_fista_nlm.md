# Fourier Ptychographic Microscopy (FPM) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('fpm_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`Fourier Ptychographic Microscopy (FPM) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
