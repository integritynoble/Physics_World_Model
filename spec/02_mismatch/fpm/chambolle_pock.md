# Fourier Ptychographic Microscopy (FPM) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: LED images (N_leds × H × W, float32)
...
# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # LED images (N_leds × H × W, float32)
...
plt.tight_layout(); plt.savefig('fpm_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Fourier Ptychographic Microscopy (FPM) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/fpm` (mismatch correction tier).
