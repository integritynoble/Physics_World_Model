# Single-Pixel Camera (SPC) — IHT + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('iht', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('spc_iht_corrected.png'); plt.show()
```

## Benchmark

`Single-Pixel Camera (SPC) — IHT + gradient` corresponds to the leaderboard entry at
