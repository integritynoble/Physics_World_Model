# Single-Pixel Camera (SPC) — IST + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('ist', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('spc_ist_corrected.png'); plt.show()
```

## Benchmark

`Single-Pixel Camera (SPC) — IST + gradient` corresponds to the leaderboard entry at
