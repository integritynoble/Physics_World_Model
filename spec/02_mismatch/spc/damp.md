# Single-Pixel Camera (SPC) — D-AMP + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('damp', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('spc_damp_corrected.png'); plt.show()
```

## Benchmark

`Single-Pixel Camera (SPC) — D-AMP + gradient` corresponds to the leaderboard entry at
