# Single-Pixel Camera (SPC) — CSGM + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('csgm', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('spc_csgm_corrected.png'); plt.show()
```

## Benchmark

`Single-Pixel Camera (SPC) — CSGM + gradient` corresponds to the leaderboard entry at
