# Single-Pixel Camera (SPC) — CSNet+ + Gradient Mismatch Correction
...
x_wrong = run_solver('csnet_plus', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('csnet_plus', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('spc_csnet_plus_corrected.png'); plt.show()
```

## Benchmark

`Single-Pixel Camera (SPC) — CSNet+ + gradient` corresponds to the leaderboard entry at
