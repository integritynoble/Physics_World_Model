# Single-Pixel Camera (SPC) — TransCS + Gradient Mismatch Correction
...
x_wrong = run_solver('transcs', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('transcs', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('spc_transcs_corrected.png'); plt.show()
```

## Benchmark

`Single-Pixel Camera (SPC) — TransCS + gradient` corresponds to the leaderboard entry at
