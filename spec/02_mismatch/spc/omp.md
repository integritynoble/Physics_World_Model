# Single-Pixel Camera (SPC) — OMP + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('omp', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('spc_omp_corrected.png'); plt.show()
```

## Benchmark

`Single-Pixel Camera (SPC) — OMP + gradient` corresponds to the leaderboard entry at
