# Single-Pixel Camera (SPC) — FISTA-L1 + Gradient Mismatch Correction
...
x_wrong = run_solver('fista_l1', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('fista_l1', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('spc_fista_l1_corrected.png'); plt.show()
```

## Benchmark

`Single-Pixel Camera (SPC) — FISTA-L1 + gradient` corresponds to the leaderboard entry at
