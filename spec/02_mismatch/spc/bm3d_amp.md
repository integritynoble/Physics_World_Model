# Single-Pixel Camera (SPC) — BM3D-AMP + Gradient

**CPU**  **Mismatch**: detector gain/bias `gain [0.8, 1.2], bias [-0.1, 0.1]`
...
x_wrong = run_solver('bm3d_amp', y)           # no correction
gain, bias = spc_calibrate_gain_bias(y)
calib_cfg = {"gain": float(gain), "bias": float(bias)}
x = run_solver('bm3d_amp', y, cfg=calib_cfg)        # corrected
