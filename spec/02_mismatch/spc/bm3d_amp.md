# Single-Pixel Camera (SPC) — BM3D-AMP + Gradient Mismatch Correction
...
x_wrong = run_solver('bm3d_amp', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('bm3d_amp', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('spc_bm3d_amp_corrected.png'); plt.show()
```

## Benchmark

`Single-Pixel Camera (SPC) — BM3D-AMP + gradient` corresponds to the leaderboard entry at
