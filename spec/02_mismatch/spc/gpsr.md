# Single-Pixel Camera (SPC) — GPSR + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('gpsr', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('spc_gpsr_corrected.png'); plt.show()
```

## Benchmark

`Single-Pixel Camera (SPC) — GPSR + gradient` corresponds to the leaderboard entry at
