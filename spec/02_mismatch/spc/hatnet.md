# Single-Pixel Camera (SPC) — HATNet + Gradient Mismatch Correction
...
x_wrong = run_solver('hatnet', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('hatnet', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('spc_hatnet_corrected.png'); plt.show()
```

## Benchmark

`Single-Pixel Camera (SPC) — HATNet + gradient` corresponds to the leaderboard entry at
