# Single-Pixel Camera (SPC) — SCSNet + Gradient Mismatch Correction
...
x_wrong = run_solver('scsnet', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('scsnet', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('spc_scsnet_corrected.png'); plt.show()
```

## Benchmark

`Single-Pixel Camera (SPC) — SCSNet + gradient` corresponds to the leaderboard entry at
