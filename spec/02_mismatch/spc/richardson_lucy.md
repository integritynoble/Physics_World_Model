# Single-Pixel Camera (SPC) — Richardson-Lucy + Gradient Mismatch Correction

**Device**: CPU  **Input**: photon counts (T × H × W, uint16)
...
plt.tight_layout(); plt.savefig('spc_richardson_lucy_corrected.png'); plt.show()
```

## Benchmark

`Single-Pixel Camera (SPC) — Richardson-Lucy + gradient` corresponds to the leaderboard entry at
