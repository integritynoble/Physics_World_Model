# Functional Near-Infrared Spectroscopy (fNIRS) — DiffusionMed + Gradient Mismatch Correction

**Device**: GPU  **Input**: optical signal (channels × T, float32)
...
plt.tight_layout(); plt.savefig('nirs_brain_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Functional Near-Infrared Spectroscopy (fNIRS) — DiffusionMed + gradient` corresponds to the leaderboard entry at
