# Light Field Imaging — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: light field (u × v × s × t, float32)
...
import h5py
with h5py.File('light_field_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/light_field/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # light field (u × v × s × t, float32)
...
plt.tight_layout(); plt.savefig('light_field_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Light Field Imaging — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/light_field` (mismatch correction tier).
