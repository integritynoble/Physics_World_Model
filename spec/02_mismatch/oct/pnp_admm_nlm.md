# Optical Coherence Tomography (OCT) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('oct_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`Optical Coherence Tomography (OCT) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
