# Optical Coherence Tomography (OCT) — DiffusionMed + Gradient Mismatch Correction

**Device**: GPU  **Input**: spectrum (wavenumbers × A-scans, float32)
...
plt.tight_layout(); plt.savefig('oct_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Optical Coherence Tomography (OCT) — DiffusionMed + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/oct` (mismatch correction tier).
