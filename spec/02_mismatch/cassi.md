# Coded Aperture Snapshot Spectral Imaging (CASSI) — Mismatch Correction + Reconstruct

**Input**: coded snapshot (H × W, float32)
**Benchmark**: `gs://pwm-benchmark-datasets/datasets/Benchmark/cassi/public/`
**Algorithms**: 4 CPU, 18 GPU — reconstruct with `run_solver(key, y)`

## Mismatch

| Parameter | Range | Typical Error | Correction Method |
|-----------|-------|---------------|-------------------|
| dispersion step (px) | `[1, 5] px` | ±0.5 px drift | grid search on sparsity metric |
...
mismatch_cfg = {
    'disp_step': None,    # float if known; None = auto-estimate
    'search_range': '[1, 5] px',
    'search_steps': 20,
}
```
...
x_wrong = run_solver('best_quality', y)

# 2. Reconstruct WITH mismatch correction
x_corrected = run_solver('best_quality', y, cfg=mismatch_cfg)
...
plt.tight_layout(); plt.savefig('cassi_corrected.png'); plt.show()
```
