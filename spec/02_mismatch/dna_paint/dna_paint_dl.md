# DNA-PAINT Super-Resolution — DECODE-PAINT + Gradient Mismatch Correction

**Device**: GPU  **Input**: localisation list (N × 2, float32)
...
import h5py
with h5py.File('dna_paint_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/dna_paint/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # localisation list (N × 2, float32)
...
x_wrong = run_solver('dna_paint_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('dna_paint_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('dna_paint_dna_paint_dl_corrected.png'); plt.show()
```

## Benchmark

`DNA-PAINT Super-Resolution — DECODE-PAINT + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/dna_paint` (mismatch correction tier).
