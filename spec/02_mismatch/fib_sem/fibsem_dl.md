# Focused Ion Beam SEM (FIB-SEM) — FIB-SEM-Net + Gradient Mismatch Correction

**Device**: GPU  **Input**: cross-sections (Z × H × W, uint8)
...
import h5py
with h5py.File('fib_sem_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/fib_sem/public/
...
x_wrong = run_solver('fibsem_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('fibsem_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('fib_sem_fibsem_dl_corrected.png'); plt.show()
```

## Benchmark

`Focused Ion Beam SEM (FIB-SEM) — FIB-SEM-Net + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/fib_sem` (mismatch correction tier).
