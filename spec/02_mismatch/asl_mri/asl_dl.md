# Arterial Spin Labeling (ASL) MRI — ASL-Net [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('asl_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('asl_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('asl_mri_asl_dl_corrected.png'); plt.show()
```

## Benchmark

`Arterial Spin Labeling (ASL) MRI — ASL-Net [proxy] + gradient` corresponds to the leaderboard entry at
