# Arterial Spin Labeling (ASL) MRI — DiffusionMed + Gradient Mismatch Correction

**Device**: GPU  **Input**: label-control pairs (2 × H × W, float32)
...
plt.tight_layout(); plt.savefig('asl_mri_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Arterial Spin Labeling (ASL) MRI — DiffusionMed + gradient` corresponds to the leaderboard entry at
