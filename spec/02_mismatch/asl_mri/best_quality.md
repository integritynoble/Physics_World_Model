# Arterial Spin Labeling (ASL) MRI — DL-Recon [proxy] + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('asl_mri_best_quality_corrected.png'); plt.show()
```

## Benchmark

`Arterial Spin Labeling (ASL) MRI — DL-Recon [proxy] + gradient` corresponds to the leaderboard entry at
