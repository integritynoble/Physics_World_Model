# Arterial Spin Labeling (ASL) MRI — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: label-control pairs (2 × H × W, float32)
...
import h5py
with h5py.File('asl_mri_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/asl_mri/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # label-control pairs (2 × H × W, float32)
...
plt.tight_layout(); plt.savefig('asl_mri_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Arterial Spin Labeling (ASL) MRI — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/asl_mri` (mismatch correction tier).
