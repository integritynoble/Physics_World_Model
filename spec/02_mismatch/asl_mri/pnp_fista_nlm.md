# Arterial Spin Labeling (ASL) MRI — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('asl_mri_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`Arterial Spin Labeling (ASL) MRI — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
