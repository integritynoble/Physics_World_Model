# Functional MRI (BOLD fMRI) — DiffusionMed + Gradient Mismatch Correction

**Device**: GPU  **Input**: BOLD volumes (T × H × W × D, float32)
...
plt.tight_layout(); plt.savefig('fmri_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Functional MRI (BOLD fMRI) — DiffusionMed + gradient` corresponds to the leaderboard entry at
