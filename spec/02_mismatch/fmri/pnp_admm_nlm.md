# Functional MRI (BOLD fMRI) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('fmri_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`Functional MRI (BOLD fMRI) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
