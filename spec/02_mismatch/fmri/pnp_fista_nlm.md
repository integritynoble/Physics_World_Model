# Functional MRI (BOLD fMRI) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('fmri_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`Functional MRI (BOLD fMRI) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
