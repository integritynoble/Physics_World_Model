# Functional MRI (BOLD fMRI) — DeepBold [proxy] + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('fmri_famous_dl_corrected.png'); plt.show()
```

## Benchmark

`Functional MRI (BOLD fMRI) — DeepBold [proxy] + gradient` corresponds to the leaderboard entry at
