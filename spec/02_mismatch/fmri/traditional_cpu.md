# Functional MRI (BOLD fMRI) — SENSE (fMRI) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('fmri_traditional_cpu_corrected.png'); plt.show()
```

## Benchmark

`Functional MRI (BOLD fMRI) — SENSE (fMRI) + gradient` corresponds to the leaderboard entry at
