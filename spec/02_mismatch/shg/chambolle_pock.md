# Second Harmonic Generation (SHG) Microscopy — Chambolle-Pock + Gradient Mismatch Correction
...
import h5py
with h5py.File('shg_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/shg/public/
...
plt.tight_layout(); plt.savefig('shg_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Second Harmonic Generation (SHG) Microscopy — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/shg` (mismatch correction tier).
