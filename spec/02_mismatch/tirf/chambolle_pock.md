# TIRF Microscopy — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: TIRF frames (T × H × W, float32)
...
# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # TIRF frames (T × H × W, float32)
...
plt.tight_layout(); plt.savefig('tirf_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`TIRF Microscopy — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/tirf` (mismatch correction tier).
