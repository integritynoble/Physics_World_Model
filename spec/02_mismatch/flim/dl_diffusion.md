# Fluorescence Lifetime Imaging (FLIM) — DiffusionMicro + Gradient Mismatch Correction

**Device**: GPU  **Input**: photon arrivals (H × W × T, float32)
...
plt.tight_layout(); plt.savefig('flim_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Fluorescence Lifetime Imaging (FLIM) — DiffusionMicro + gradient` corresponds to the leaderboard entry at
