# Fluorescence Lifetime Imaging (FLIM) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: photon arrivals (H × W × T, float32)
...
import h5py
with h5py.File('flim_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/flim/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # photon arrivals (H × W × T, float32)
...
plt.tight_layout(); plt.savefig('flim_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Fluorescence Lifetime Imaging (FLIM) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/flim` (mismatch correction tier).
