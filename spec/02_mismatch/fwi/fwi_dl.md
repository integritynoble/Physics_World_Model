# Full-Waveform Inversion (FWI) — InversionNet [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('fwi_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('fwi_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('fwi_fwi_dl_corrected.png'); plt.show()
```

## Benchmark

`Full-Waveform Inversion (FWI) — InversionNet [proxy] + gradient` corresponds to the leaderboard entry at
