# Full-Waveform Inversion (FWI) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: seismic waveforms (receivers × time, float32)
...
import h5py
with h5py.File('fwi_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/fwi/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # seismic waveforms (receivers × time, float32)
...
plt.tight_layout(); plt.savefig('fwi_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Full-Waveform Inversion (FWI) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/fwi` (mismatch correction tier).
