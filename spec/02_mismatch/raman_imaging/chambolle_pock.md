# Raman Imaging / Microscopy — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: Raman spectra (H × W × wn, float32)
...
import h5py
with h5py.File('raman_imaging_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/raman_imaging/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # Raman spectra (H × W × wn, float32)
...
plt.tight_layout(); plt.savefig('raman_imaging_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Raman Imaging / Microscopy — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/raman_imaging` (mismatch correction tier).
