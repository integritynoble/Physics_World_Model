# X-ray Computed Tomography (CT) — Mismatch Correction + Reconstruct

**Input**: sinogram (angles × detectors, float32)
**Benchmark**: `gs://pwm-benchmark-datasets/datasets/Benchmark/ct/public/`
**Algorithms**: 26 CPU, 15 GPU — reconstruct with `run_solver(key, y)`

## Mismatch

| Parameter | Range | Typical Error | Correction Method |
|-----------|-------|---------------|-------------------|
| center-of-rotation (CoR) offset | `[-5, +5] px` | ±1 px | cross-correlation on 180°-opposed projections |
...
mismatch_cfg = {
    'cor_offset': None,    # float if known; None = auto-estimate
    'search_range': '[-5, +5] px',
    'search_steps': 20,
}
```
...
x_wrong = run_solver('pnp_admm_nlm', y)

# 2. Reconstruct WITH mismatch correction
x_corrected = run_solver('pnp_admm_nlm', y, cfg=mismatch_cfg)
...
plt.tight_layout(); plt.savefig('ct_corrected.png'); plt.show()
```
