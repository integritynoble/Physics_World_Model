# Hyperspectral Remote Sensing — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: radiance cube (H × W × bands, float32)
...
import h5py
with h5py.File('hyperspectral_remote_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/hyperspectral_remote/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # radiance cube (H × W × bands, float32)
...
plt.tight_layout(); plt.savefig('hyperspectral_remote_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Hyperspectral Remote Sensing — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/hyperspectral_remote` (mismatch correction tier).
