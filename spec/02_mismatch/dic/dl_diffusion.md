# Differential Interference Contrast (DIC) — DiffusionMicro + Gradient Mismatch Correction

**Device**: GPU  **Input**: DIC image (H × W, float32)
...
plt.tight_layout(); plt.savefig('dic_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Differential Interference Contrast (DIC) — DiffusionMicro + gradient` corresponds to the leaderboard entry at
