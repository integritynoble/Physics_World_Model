# Bioluminescence Tomography (BLT) — MambaRecon + Gradient Mismatch Correction

**Device**: GPU  **Input**: surface flux (H × W × angles, float32)
...
plt.tight_layout(); plt.savefig('bioluminescence_tomo_dl_mamba_corrected.png'); plt.show()
```

## Benchmark

`Bioluminescence Tomography (BLT) — MambaRecon + gradient` corresponds to the leaderboard entry at
