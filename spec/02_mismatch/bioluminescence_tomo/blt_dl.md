# Bioluminescence Tomography (BLT) — BLT-Net [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('blt_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('blt_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('bioluminescence_tomo_blt_dl_corrected.png'); plt.show()
```

## Benchmark

`Bioluminescence Tomography (BLT) — BLT-Net [proxy] + gradient` corresponds to the leaderboard entry at
