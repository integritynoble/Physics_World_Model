# Bioluminescence Tomography (BLT) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: surface flux (H × W × angles, float32)
...
import h5py
with h5py.File('bioluminescence_tomo_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/bioluminescence_tomo/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # surface flux (H × W × angles, float32)
...
plt.tight_layout(); plt.savefig('bioluminescence_tomo_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Bioluminescence Tomography (BLT) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/bioluminescence_tomo` (mismatch correction tier).
