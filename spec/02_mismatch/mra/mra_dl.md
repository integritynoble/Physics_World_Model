# MR Angiography (MRA) — MRA-VesselNet [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('mra_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('mra_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('mra_mra_dl_corrected.png'); plt.show()
```

## Benchmark

`MR Angiography (MRA) — MRA-VesselNet [proxy] + gradient` corresponds to the leaderboard entry at
