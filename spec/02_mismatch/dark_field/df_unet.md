# Dark-Field Microscopy — DF-UNet + Gradient Mismatch Correction
...
x_wrong = run_solver('df_unet', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('df_unet', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('dark_field_df_unet_corrected.png'); plt.show()
```

## Benchmark

`Dark-Field Microscopy — DF-UNet + gradient` corresponds to the leaderboard entry at
