# Dark-Field Microscopy — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: grating images (2 × H × W, float32)
...
import h5py
with h5py.File('dark_field_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/dark_field/public/
...
plt.tight_layout(); plt.savefig('dark_field_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Dark-Field Microscopy — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/dark_field` (mismatch correction tier).
