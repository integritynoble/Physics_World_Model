# Electron Energy Loss Spectroscopy (EELS) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: energy-loss spectrum (H × W × E, float32)
...
import h5py
with h5py.File('eels_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/eels/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # energy-loss spectrum (H × W × E, float32)
...
plt.tight_layout(); plt.savefig('eels_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Electron Energy Loss Spectroscopy (EELS) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/eels` (mismatch correction tier).
