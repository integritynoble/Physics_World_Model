# Acoustic Emission Testing (AE) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: waveform (samples, float32)
...
import h5py
with h5py.File('acoustic_emission_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/acoustic_emission/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # waveform (samples, float32)
...
plt.tight_layout(); plt.savefig('acoustic_emission_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Acoustic Emission Testing (AE) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/acoustic_emission` (mismatch correction tier).
