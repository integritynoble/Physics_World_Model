# Electrical Impedance Tomography (EIT) — MambaRecon + Gradient Mismatch Correction

**Device**: GPU  **Input**: boundary voltages (M, float32)
...
import h5py
with h5py.File('impedance_tomo_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/impedance_tomo/public/
...
plt.tight_layout(); plt.savefig('impedance_tomo_dl_mamba_corrected.png'); plt.show()
```

## Benchmark

`Electrical Impedance Tomography (EIT) — MambaRecon + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/impedance_tomo` (mismatch correction tier).
