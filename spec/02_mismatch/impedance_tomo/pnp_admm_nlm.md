# Electrical Impedance Tomography (EIT) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('impedance_tomo_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`Electrical Impedance Tomography (EIT) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
