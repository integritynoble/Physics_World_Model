# Ghost Imaging — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: bucket signal (N_patterns, float32)
...
# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # bucket signal (N_patterns, float32)
...
plt.tight_layout(); plt.savefig('ghost_imaging_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Ghost Imaging — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/ghost_imaging` (mismatch correction tier).
