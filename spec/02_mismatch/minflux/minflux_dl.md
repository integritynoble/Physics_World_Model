# MINFLUX Nanoscopy — MINFLUX-Net + Gradient Mismatch Correction

**Device**: GPU  **Input**: photon records (N × 5: t,x,y,z,id)
...
import h5py
with h5py.File('minflux_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/minflux/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # photon records (N × 5: t,x,y,z,id)
...
x_wrong = run_solver('minflux_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('minflux_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('minflux_minflux_dl_corrected.png'); plt.show()
```

## Benchmark

`MINFLUX Nanoscopy — MINFLUX-Net + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/minflux` (mismatch correction tier).
