# Scanning Tunneling Microscopy (STM) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('stm_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`Scanning Tunneling Microscopy (STM) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
