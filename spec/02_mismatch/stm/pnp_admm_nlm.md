# Scanning Tunneling Microscopy (STM) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('stm_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`Scanning Tunneling Microscopy (STM) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
