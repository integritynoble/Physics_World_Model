# Transmission Electron Microscopy (TEM) — TEM-DL (ePIE-Net) [proxy] + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('tem_best_quality_corrected.png'); plt.show()
```

## Benchmark

`Transmission Electron Microscopy (TEM) — TEM-DL (ePIE-Net) [proxy] + gradient` corresponds to the leaderboard entry at
