# Transmission Electron Microscopy (TEM) — 3D-Diffusion + Gradient Mismatch Correction

**Device**: GPU  **Input**: TEM image (H × W, float32)
...
plt.tight_layout(); plt.savefig('tem_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Transmission Electron Microscopy (TEM) — 3D-Diffusion + gradient` corresponds to the leaderboard entry at
