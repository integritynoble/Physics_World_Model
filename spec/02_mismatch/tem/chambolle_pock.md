# Transmission Electron Microscopy (TEM) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: TEM image (H × W, float32)
...
import h5py
with h5py.File('tem_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/tem/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # TEM image (H × W, float32)
...
plt.tight_layout(); plt.savefig('tem_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Transmission Electron Microscopy (TEM) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/tem` (mismatch correction tier).
