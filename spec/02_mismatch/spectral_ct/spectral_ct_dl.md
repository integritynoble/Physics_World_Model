# Photon-Counting Spectral CT — SpectralCT-Net [proxy] + Gradient Mismatch Correction

**Device**: CPU  **Input**: energy-bin sinos (bins × angles × det, float32)
...
import h5py
with h5py.File('spectral_ct_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/spectral_ct/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # energy-bin sinos (bins × angles × det, float32)
...
x_wrong = run_solver('spectral_ct_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('spectral_ct_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('spectral_ct_spectral_ct_dl_corrected.png'); plt.show()
```

## Benchmark

`Photon-Counting Spectral CT — SpectralCT-Net [proxy] + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/spectral_ct` (mismatch correction tier).
