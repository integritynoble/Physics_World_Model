# Low-Dose Widefield Microscopy — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: photon-limited image (H × W, float32)
...
import h5py
with h5py.File('widefield_lowdose_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/widefield_lowdose/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # photon-limited image (H × W, float32)
...
plt.tight_layout(); plt.savefig('widefield_lowdose_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Low-Dose Widefield Microscopy — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/widefield_lowdose` (mismatch correction tier).
