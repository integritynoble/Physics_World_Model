# Cryo-Electron Tomography (Cryo-ET) — Chambolle-Pock + Gradient Mismatch Correction
...
import h5py
with h5py.File('cryo_et_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/cryo_et/public/
...
plt.tight_layout(); plt.savefig('cryo_et_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Cryo-Electron Tomography (Cryo-ET) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/cryo_et` (mismatch correction tier).
