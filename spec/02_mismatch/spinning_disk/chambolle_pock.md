# Spinning Disk Confocal Microscopy — Chambolle-Pock + Gradient Mismatch Correction
...
import h5py
with h5py.File('spinning_disk_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/spinning_disk/public/
...
plt.tight_layout(); plt.savefig('spinning_disk_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Spinning Disk Confocal Microscopy — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/spinning_disk` (mismatch correction tier).
