# Event Horizon Telescope (EHT) Imaging — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: hologram (H × W, complex64)
...
import h5py
with h5py.File('eht_imaging_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/eht_imaging/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # hologram (H × W, complex64)
...
plt.tight_layout(); plt.savefig('eht_imaging_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Event Horizon Telescope (EHT) Imaging — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/eht_imaging` (mismatch correction tier).
