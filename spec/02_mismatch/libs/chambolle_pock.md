# Laser-Induced Breakdown Spectroscopy (LIBS) Imaging — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: emission spectrum (wavelengths, float32)
...
import h5py
with h5py.File('libs_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/libs/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # emission spectrum (wavelengths, float32)
...
plt.tight_layout(); plt.savefig('libs_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Laser-Induced Breakdown Spectroscopy (LIBS) Imaging — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/libs` (mismatch correction tier).
