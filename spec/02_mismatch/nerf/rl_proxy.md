# Neural Radiance Fields (NeRF) — Richardson-Lucy (proxy baseline) + Gradient Mismatch Correction
...
import h5py
with h5py.File('nerf_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/nerf/public/
...
x_wrong = run_solver('rl_proxy', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('rl_proxy', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('nerf_rl_proxy_corrected.png'); plt.show()
```

## Benchmark

`Neural Radiance Fields (NeRF) — Richardson-Lucy (proxy baseline) + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/nerf` (mismatch correction tier).
