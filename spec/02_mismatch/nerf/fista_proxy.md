# Neural Radiance Fields (NeRF) — FISTA-TV (proxy baseline) + Gradient Mismatch Correction
...
x_wrong = run_solver('fista_proxy', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('fista_proxy', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('nerf_fista_proxy_corrected.png'); plt.show()
```

## Benchmark

`Neural Radiance Fields (NeRF) — FISTA-TV (proxy baseline) + gradient` corresponds to the leaderboard entry at
