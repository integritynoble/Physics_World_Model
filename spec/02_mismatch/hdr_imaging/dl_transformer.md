# High Dynamic Range (HDR) Imaging — DL-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: multi-exposure (K × H × W × 3, uint8)
...
plt.tight_layout(); plt.savefig('hdr_imaging_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`High Dynamic Range (HDR) Imaging — DL-Transformer + gradient` corresponds to the leaderboard entry at
