# High Dynamic Range (HDR) Imaging — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: multi-exposure (K × H × W × 3, uint8)
...
import h5py
with h5py.File('hdr_imaging_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/hdr_imaging/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # multi-exposure (K × H × W × 3, uint8)
...
plt.tight_layout(); plt.savefig('hdr_imaging_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`High Dynamic Range (HDR) Imaging — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/hdr_imaging` (mismatch correction tier).
