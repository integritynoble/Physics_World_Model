# High Dynamic Range (HDR) Imaging — HDR-Net [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('hdr_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('hdr_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('hdr_imaging_hdr_dl_corrected.png'); plt.show()
```

## Benchmark

`High Dynamic Range (HDR) Imaging — HDR-Net [proxy] + gradient` corresponds to the leaderboard entry at
