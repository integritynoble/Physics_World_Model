# X-ray Fluorescence Tomography — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: XRF sinograms (elem × angles × det, float32)
...
import h5py
with h5py.File('xrf_tomo_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/xrf_tomo/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # XRF sinograms (elem × angles × det, float32)
...
plt.tight_layout(); plt.savefig('xrf_tomo_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`X-ray Fluorescence Tomography — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/xrf_tomo` (mismatch correction tier).
