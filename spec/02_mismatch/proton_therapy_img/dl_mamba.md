# Proton Therapy Imaging — MambaRecon + Gradient Mismatch Correction

**Device**: GPU  **Input**: dose distribution (H × W × D, float32)
...
import h5py
with h5py.File('proton_therapy_img_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/proton_therapy_img/public/
...
plt.tight_layout(); plt.savefig('proton_therapy_img_dl_mamba_corrected.png'); plt.show()
```

## Benchmark

`Proton Therapy Imaging — MambaRecon + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/proton_therapy_img` (mismatch correction tier).
