# Structured Illumination Microscopy (SIM) — DiffusionMicro + Gradient Mismatch Correction

**Device**: GPU  **Input**: raw frames (9 × H × W: 3 angles × 3 phases)
...
plt.tight_layout(); plt.savefig('sim_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Structured Illumination Microscopy (SIM) — DiffusionMicro + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/sim` (mismatch correction tier).
