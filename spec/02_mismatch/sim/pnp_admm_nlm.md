# Structured Illumination Microscopy (SIM) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('sim_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`Structured Illumination Microscopy (SIM) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
