# Structured Illumination Microscopy (SIM) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('sim_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`Structured Illumination Microscopy (SIM) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
