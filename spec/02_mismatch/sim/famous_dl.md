# Structured Illumination Microscopy (SIM) — fairSIM (open-source) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('sim_famous_dl_corrected.png'); plt.show()
```

## Benchmark

`Structured Illumination Microscopy (SIM) — fairSIM (open-source) + gradient` corresponds to the leaderboard entry at
