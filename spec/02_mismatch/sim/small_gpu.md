# Structured Illumination Microscopy (SIM) — Wiener-SIM (fast) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('sim_small_gpu_corrected.png'); plt.show()
```

## Benchmark

`Structured Illumination Microscopy (SIM) — Wiener-SIM (fast) + gradient` corresponds to the leaderboard entry at
