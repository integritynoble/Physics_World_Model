# Sonar Imaging — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: echo data (elements × samples, float32)
...
import h5py
with h5py.File('sonar_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/sonar/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # echo data (elements × samples, float32)
...
plt.tight_layout(); plt.savefig('sonar_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Sonar Imaging — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/sonar` (mismatch correction tier).
