# STEM-EDX Elemental Mapping — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: X-ray counts (H × W × channels, float32)
...
import h5py
with h5py.File('edx_mapping_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/edx_mapping/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # X-ray counts (H × W × channels, float32)
...
plt.tight_layout(); plt.savefig('edx_mapping_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`STEM-EDX Elemental Mapping — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/edx_mapping` (mismatch correction tier).
