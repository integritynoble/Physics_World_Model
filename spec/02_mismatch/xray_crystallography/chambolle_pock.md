# X-ray Crystallography — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: structure factors (hkl × F, float32)
...
import h5py
with h5py.File('xray_crystallography_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/xray_crystallography/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # structure factors (hkl × F, float32)
...
plt.tight_layout(); plt.savefig('xray_crystallography_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`X-ray Crystallography — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/xray_crystallography` (mismatch correction tier).
