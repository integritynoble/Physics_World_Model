# Scanning Transmission Electron Microscopy (STEM) — 3D-Diffusion + Gradient Mismatch Correction

**Device**: GPU  **Input**: HAADF image (H × W, float32)
...
plt.tight_layout(); plt.savefig('stem_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Scanning Transmission Electron Microscopy (STEM) — 3D-Diffusion + gradient` corresponds to the leaderboard entry at
