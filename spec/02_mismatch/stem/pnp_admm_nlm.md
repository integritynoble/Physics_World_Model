# Scanning Transmission Electron Microscopy (STEM) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('stem_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`Scanning Transmission Electron Microscopy (STEM) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
