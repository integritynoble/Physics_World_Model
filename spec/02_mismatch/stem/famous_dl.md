# Scanning Transmission Electron Microscopy (STEM) — STEM-UNet [proxy] + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('stem_famous_dl_corrected.png'); plt.show()
```

## Benchmark

`Scanning Transmission Electron Microscopy (STEM) — STEM-UNet [proxy] + gradient` corresponds to the leaderboard entry at
