# Scanning Transmission Electron Microscopy (STEM) — STEM-DL (AtomSegNet) [proxy] + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('stem_best_quality_corrected.png'); plt.show()
```

## Benchmark

`Scanning Transmission Electron Microscopy (STEM) — STEM-DL (AtomSegNet) [proxy] + gradient` corresponds to the leaderboard entry at
