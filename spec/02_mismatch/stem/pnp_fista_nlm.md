# Scanning Transmission Electron Microscopy (STEM) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('stem_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`Scanning Transmission Electron Microscopy (STEM) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
