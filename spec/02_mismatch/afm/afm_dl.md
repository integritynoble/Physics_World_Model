# Atomic Force Microscopy (AFM) — AFM-UNet + Gradient Mismatch Correction

**Device**: GPU  **Input**: force-distance map (H × W, float32)
...
import h5py
with h5py.File('afm_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/afm/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # force-distance map (H × W, float32)
...
x_wrong = run_solver('afm_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('afm_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('afm_afm_dl_corrected.png'); plt.show()
```

## Benchmark

`Atomic Force Microscopy (AFM) — AFM-UNet + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/afm` (mismatch correction tier).
