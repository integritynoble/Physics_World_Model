# Lensless (Diffuser Camera) Imaging — Mismatch Correction + Reconstruct

**Input**: diffuser measurement (H × W, float32)
**Benchmark**: `gs://pwm-benchmark-datasets/datasets/Benchmark/lensless/public/`
**Algorithms**: 9 CPU, 8 GPU — reconstruct with `run_solver(key, y)`

## Mismatch

| Parameter | Range | Typical Error | Correction Method |
|-----------|-------|---------------|-------------------|
| PSF shift (px) | `[-5, +5] px (x and y)` | ±1 px thermal drift | gradient calibration against nominal PSF |
...
mismatch_cfg = {
    'psf_shift': None,    # float if known; None = auto-estimate
...
plt.tight_layout(); plt.savefig('lensless_corrected.png'); plt.show()
```
