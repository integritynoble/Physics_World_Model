# Ultrasound B-mode Imaging — Mismatch Correction + Reconstruct

**Input**: RF data (elements × samples, float32)
**Benchmark**: `gs://pwm-benchmark-datasets/datasets/Benchmark/ultrasound/public/`
**Algorithms**: 11 CPU, 6 GPU — reconstruct with `run_solver(key, y)`

## Mismatch

| Parameter | Range | Typical Error | Correction Method |
|-----------|-------|---------------|-------------------|
| sound speed (m/s) | `[1400, 1600] m/s` | ±30 m/s tissue variation | coherence-based speed estimation |
...
mismatch_cfg = {
    'c0': None,    # float if known; None = auto-estimate
...
plt.tight_layout(); plt.savefig('ultrasound_corrected.png'); plt.show()
```
