# Digital Breast Tomosynthesis (DBT) — DBT-DL [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('dbt_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('dbt_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('digital_breast_tomo_dbt_dl_corrected.png'); plt.show()
```

## Benchmark

`Digital Breast Tomosynthesis (DBT) — DBT-DL [proxy] + gradient` corresponds to the leaderboard entry at
