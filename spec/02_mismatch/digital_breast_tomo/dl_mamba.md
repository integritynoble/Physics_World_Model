# Digital Breast Tomosynthesis (DBT) — MambaRecon + Gradient Mismatch Correction

**Device**: GPU  **Input**: projections (angles × H × W, float32)
...
import h5py
with h5py.File('digital_breast_tomo_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/digital_breast_tomo/public/
...
plt.tight_layout(); plt.savefig('digital_breast_tomo_dl_mamba_corrected.png'); plt.show()
```

## Benchmark

`Digital Breast Tomosynthesis (DBT) — MambaRecon + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/digital_breast_tomo` (mismatch correction tier).
