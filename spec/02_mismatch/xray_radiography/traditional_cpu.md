# X-ray Radiography — FBP (X-ray radiography) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('xray_radiography_traditional_cpu_corrected.png'); plt.show()
```

## Benchmark

`X-ray Radiography — FBP (X-ray radiography) + gradient` corresponds to the leaderboard entry at
