# Lensless (Diffuser Camera) Imaging — ADMM-TV (Lensless) + Gradient Mismatch Correction
...
x_wrong = run_solver('admm_tv', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('admm_tv', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('lensless_admm_tv_corrected.png'); plt.show()
```

## Benchmark

`Lensless (Diffuser Camera) Imaging — ADMM-TV (Lensless) + gradient` corresponds to the leaderboard entry at
