# Lensless (Diffuser Camera) Imaging — PnP-HQS (NLM) + Gradient Mismatch Correction
...
x_wrong = run_solver('pnp_hqs_nlm', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('pnp_hqs_nlm', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('lensless_pnp_hqs_nlm_corrected.png'); plt.show()
```

## Benchmark

`Lensless (Diffuser Camera) Imaging — PnP-HQS (NLM) + gradient` corresponds to the leaderboard entry at
