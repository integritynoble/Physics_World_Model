# Lensless (Diffuser Camera) Imaging — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('lensless_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`Lensless (Diffuser Camera) Imaging — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
