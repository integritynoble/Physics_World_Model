# Lensless (Diffuser Camera) Imaging — PhlatCam + Gradient Mismatch Correction
...
x_wrong = run_solver('phlatcam', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('phlatcam', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('lensless_phlatcam_corrected.png'); plt.show()
```

## Benchmark

`Lensless (Diffuser Camera) Imaging — PhlatCam + gradient` corresponds to the leaderboard entry at
