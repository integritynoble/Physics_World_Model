# Lensless (Diffuser Camera) Imaging — DiffuserDM + Gradient Mismatch Correction
...
x_wrong = run_solver('diffuser_dm', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('diffuser_dm', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('lensless_diffuser_dm_corrected.png'); plt.show()
```

## Benchmark

`Lensless (Diffuser Camera) Imaging — DiffuserDM + gradient` corresponds to the leaderboard entry at
