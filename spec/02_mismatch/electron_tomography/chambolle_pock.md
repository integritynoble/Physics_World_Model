# Electron Tomography — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: tilt series (angles × H × W, float32)
...
import h5py
with h5py.File('electron_tomography_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/electron_tomography/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # tilt series (angles × H × W, float32)
...
plt.tight_layout(); plt.savefig('electron_tomography_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Electron Tomography — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/electron_tomography` (mismatch correction tier).
