# Stellar Coronagraphy — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: image (H × W, float32)
...
import h5py
with h5py.File('coronagraphy_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/coronagraphy/public/
...
plt.tight_layout(); plt.savefig('coronagraphy_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Stellar Coronagraphy — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/coronagraphy` (mismatch correction tier).
