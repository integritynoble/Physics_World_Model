# Two-Photon / Multiphoton Microscopy — Chambolle-Pock + Gradient Mismatch Correction
...
import h5py
with h5py.File('two_photon_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/two_photon/public/
...
plt.tight_layout(); plt.savefig('two_photon_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Two-Photon / Multiphoton Microscopy — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/two_photon` (mismatch correction tier).
