# Stimulated Raman Scattering (SRS) Microscopy — SRS-DeepSpec [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('srs_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('srs_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('srs_srs_dl_corrected.png'); plt.show()
```

## Benchmark

`Stimulated Raman Scattering (SRS) Microscopy — SRS-DeepSpec [proxy] + gradient` corresponds to the leaderboard entry at
