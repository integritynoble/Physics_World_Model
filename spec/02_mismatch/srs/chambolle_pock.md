# Stimulated Raman Scattering (SRS) Microscopy — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: SRS image (H × W, float32)
...
import h5py
with h5py.File('srs_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/srs/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # SRS image (H × W, float32)
...
plt.tight_layout(); plt.savefig('srs_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Stimulated Raman Scattering (SRS) Microscopy — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/srs` (mismatch correction tier).
