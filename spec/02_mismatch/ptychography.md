# Ptychographic Imaging — Mismatch Correction + Reconstruct
...
|-----------|-------|---------------|-------------------|
| probe position error (px) | `[-3, +3] px` | ±1 px | annealing position refinement |
...
mismatch_cfg = {
    'pos_error': None,    # float if known; None = auto-estimate
    'search_range': '[-3, +3] px',
    'search_steps': 20,
}
```
...
plt.tight_layout(); plt.savefig('ptychography_corrected.png'); plt.show()
```
