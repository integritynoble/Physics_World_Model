# Solar EUV/X-ray Imaging — RS-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: EUV image (H × W, float32)
...
import h5py
with h5py.File('solar_imaging_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/solar_imaging/public/
...
plt.tight_layout(); plt.savefig('solar_imaging_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`Solar EUV/X-ray Imaging — RS-Transformer + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/solar_imaging` (mismatch correction tier).
