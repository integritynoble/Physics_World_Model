# Pump-Probe Microscopy — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: transient spectra (T × λ, float32)
...
import h5py
with h5py.File('pump_probe_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/pump_probe/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # transient spectra (T × λ, float32)
...
plt.tight_layout(); plt.savefig('pump_probe_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Pump-Probe Microscopy — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/pump_probe` (mismatch correction tier).
