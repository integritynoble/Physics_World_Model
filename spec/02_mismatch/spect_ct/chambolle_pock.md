# SPECT/CT Fusion — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: SPECT proj + CT sino (both float32)
...
import h5py
with h5py.File('spect_ct_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/spect_ct/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # SPECT proj + CT sino (both float32)
...
plt.tight_layout(); plt.savefig('spect_ct_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`SPECT/CT Fusion — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/spect_ct` (mismatch correction tier).
