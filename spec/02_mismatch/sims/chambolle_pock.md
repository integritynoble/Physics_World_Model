# Secondary Ion Mass Spectrometry (SIMS) Imaging — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: ion images (H × W × m/z, float32)
...
import h5py
with h5py.File('sims_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/sims/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # ion images (H × W × m/z, float32)
...
plt.tight_layout(); plt.savefig('sims_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Secondary Ion Mass Spectrometry (SIMS) Imaging — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/sims` (mismatch correction tier).
