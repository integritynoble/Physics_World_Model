# Positron Emission Tomography (PET) — Mismatch Correction + Reconstruct
...
|-----------|-------|---------------|-------------------|
| attenuation map (μ-map) | `μ ∈ [0, 0.3] cm⁻¹` | ±10% scaling error | CT-based attenuation correction |
...
mismatch_cfg = {
    'mu_map': None,    # float if known; None = auto-estimate
    'search_range': 'μ ∈ [0, 0.3] cm⁻¹',
    'search_steps': 20,
}
```
...
plt.tight_layout(); plt.savefig('pet_corrected.png'); plt.show()
```
