# Muon Tomography — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: muon tracks (N × 6, float32)
...
import h5py
with h5py.File('muon_tomo_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/muon_tomo/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # muon tracks (N × 6, float32)
...
plt.tight_layout(); plt.savefig('muon_tomo_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Muon Tomography — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/muon_tomo` (mismatch correction tier).
