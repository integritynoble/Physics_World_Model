# LiDAR Scanner — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: point cloud (N × 3, float32)
...
import h5py
with h5py.File('lidar_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/lidar/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # point cloud (N × 3, float32)
...
plt.tight_layout(); plt.savefig('lidar_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`LiDAR Scanner — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/lidar` (mismatch correction tier).
