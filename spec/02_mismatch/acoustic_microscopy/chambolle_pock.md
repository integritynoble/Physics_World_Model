# Scanning Acoustic Microscopy (SAM) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: RF data (H × W × T, float32)
...
import h5py
with h5py.File('acoustic_microscopy_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/acoustic_microscopy/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # RF data (H × W × T, float32)
...
plt.tight_layout(); plt.savefig('acoustic_microscopy_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Scanning Acoustic Microscopy (SAM) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/acoustic_microscopy` (mismatch correction tier).
