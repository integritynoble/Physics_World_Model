# Scanning Acoustic Microscopy (SAM) — SAFT-DL [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('saft_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('saft_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('acoustic_microscopy_saft_dl_corrected.png'); plt.show()
```

## Benchmark

`Scanning Acoustic Microscopy (SAM) — SAFT-DL [proxy] + gradient` corresponds to the leaderboard entry at
