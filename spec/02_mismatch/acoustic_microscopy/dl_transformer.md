# Scanning Acoustic Microscopy (SAM) — DL-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: RF data (H × W × T, float32)
...
plt.tight_layout(); plt.savefig('acoustic_microscopy_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`Scanning Acoustic Microscopy (SAM) — DL-Transformer + gradient` corresponds to the leaderboard entry at
