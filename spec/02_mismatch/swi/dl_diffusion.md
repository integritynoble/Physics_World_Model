# Susceptibility-Weighted Imaging (SWI) — DiffusionMed + Gradient Mismatch Correction

**Device**: GPU  **Input**: phase image (H × W × slices, float32)
...
import h5py
with h5py.File('swi_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/swi/public/
...
plt.tight_layout(); plt.savefig('swi_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Susceptibility-Weighted Imaging (SWI) — DiffusionMed + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/swi` (mismatch correction tier).
