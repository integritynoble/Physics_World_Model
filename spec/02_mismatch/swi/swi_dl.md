# Susceptibility-Weighted Imaging (SWI) — SWI-Net [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('swi_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('swi_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('swi_swi_dl_corrected.png'); plt.show()
```

## Benchmark

`Susceptibility-Weighted Imaging (SWI) — SWI-Net [proxy] + gradient` corresponds to the leaderboard entry at
