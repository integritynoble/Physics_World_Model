# Ultrasonic Phased Array (TFM/FMC) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: FMC data (elem × elem × time, float32)
...
import h5py
with h5py.File('ultrasonic_phased_array_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/ultrasonic_phased_array/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # FMC data (elem × elem × time, float32)
...
x_wrong = run_solver('chambolle_pock', y, cfg={'iters': 30, 'lam': 0.005})
...
cfg.update({'iters': 30, 'lam': 0.005})
x_corrected = run_solver('chambolle_pock', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ultrasonic_phased_array_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Ultrasonic Phased Array (TFM/FMC) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/ultrasonic_phased_array` (mismatch correction tier).
