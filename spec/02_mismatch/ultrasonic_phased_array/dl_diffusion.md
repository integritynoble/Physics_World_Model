# Ultrasonic Phased Array (TFM/FMC) — DiffusionMed + Gradient Mismatch Correction

**Device**: GPU  **Input**: FMC data (elem × elem × time, float32)
...
x_wrong = run_solver('dl_diffusion', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('dl_diffusion', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ultrasonic_phased_array_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Ultrasonic Phased Array (TFM/FMC) — DiffusionMed + gradient` corresponds to the leaderboard entry at
