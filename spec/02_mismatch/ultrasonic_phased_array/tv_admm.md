# Ultrasonic Phased Array (TFM/FMC) — TV-ADMM + Gradient Mismatch Correction
...
x_wrong = run_solver('tv_admm', y, cfg={'iters': 20, 'lam': 0.005, 'rho': 1.0})
...
cfg.update({'iters': 20, 'lam': 0.005, 'rho': 1.0})
x_corrected = run_solver('tv_admm', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ultrasonic_phased_array_tv_admm_corrected.png'); plt.show()
```

## Benchmark

`Ultrasonic Phased Array (TFM/FMC) — TV-ADMM + gradient` corresponds to the leaderboard entry at
