# Ultrasonic Phased Array (TFM/FMC) — Wiener Deconvolution + Gradient Mismatch Correction
...
x_wrong = run_solver('wiener', y, cfg={'reg': 0.01})
...
cfg.update({'reg': 0.01})
x_corrected = run_solver('wiener', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ultrasonic_phased_array_wiener_corrected.png'); plt.show()
```

## Benchmark

`Ultrasonic Phased Array (TFM/FMC) — Wiener Deconvolution + gradient` corresponds to the leaderboard entry at
