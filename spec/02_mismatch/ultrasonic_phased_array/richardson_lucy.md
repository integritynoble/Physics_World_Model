# Ultrasonic Phased Array (TFM/FMC) — Richardson-Lucy + Gradient Mismatch Correction
...
x_wrong = run_solver('richardson_lucy', y, cfg={'iters': 50})
...
cfg.update({'iters': 50})
x_corrected = run_solver('richardson_lucy', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ultrasonic_phased_array_richardson_lucy_corrected.png'); plt.show()
```

## Benchmark

`Ultrasonic Phased Array (TFM/FMC) — Richardson-Lucy + gradient` corresponds to the leaderboard entry at
