# Neutron Diffraction — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: pattern (2θ × intensity, float32)
...
import h5py
with h5py.File('neutron_diffraction_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/neutron_diffraction/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # pattern (2θ × intensity, float32)
...
plt.tight_layout(); plt.savefig('neutron_diffraction_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Neutron Diffraction — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/neutron_diffraction` (mismatch correction tier).
