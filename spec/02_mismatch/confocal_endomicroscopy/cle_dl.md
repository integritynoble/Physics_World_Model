# Confocal Laser Endomicroscopy (CLE) — CLE-Net (CARE) [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('cle_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('cle_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('confocal_endomicroscopy_cle_dl_corrected.png'); plt.show()
```

## Benchmark

`Confocal Laser Endomicroscopy (CLE) — CLE-Net (CARE) [proxy] + gradient` corresponds to the leaderboard entry at
