# Confocal Laser Endomicroscopy (CLE) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: confocal frame (H × W, float32)
...
import h5py
with h5py.File('confocal_endomicroscopy_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/confocal_endomicroscopy/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # confocal frame (H × W, float32)
...
plt.tight_layout(); plt.savefig('confocal_endomicroscopy_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Confocal Laser Endomicroscopy (CLE) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/confocal_endomicroscopy` (mismatch correction tier).
