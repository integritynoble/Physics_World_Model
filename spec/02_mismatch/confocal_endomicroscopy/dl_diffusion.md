# Confocal Laser Endomicroscopy (CLE) — DiffusionMicro + Gradient Mismatch Correction

**Device**: GPU  **Input**: confocal frame (H × W, float32)
...
plt.tight_layout(); plt.savefig('confocal_endomicroscopy_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Confocal Laser Endomicroscopy (CLE) — DiffusionMicro + gradient` corresponds to the leaderboard entry at
