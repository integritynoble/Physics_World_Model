# Wide-Angle X-ray Scattering (WAXS) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: wide-angle pattern (H × W, float32)
...
import h5py
with h5py.File('waxs_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/waxs/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # wide-angle pattern (H × W, float32)
...
plt.tight_layout(); plt.savefig('waxs_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Wide-Angle X-ray Scattering (WAXS) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/waxs` (mismatch correction tier).
