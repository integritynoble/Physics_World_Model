# Quantum Illumination — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: coincidence image (H × W, float32)
...
import h5py
with h5py.File('quantum_illumination_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/quantum_illumination/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # coincidence image (H × W, float32)
...
plt.tight_layout(); plt.savefig('quantum_illumination_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Quantum Illumination — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/quantum_illumination` (mismatch correction tier).
