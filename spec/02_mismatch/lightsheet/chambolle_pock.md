# Light-Sheet Fluorescence Microscopy (LSFM) — Chambolle-Pock + Gradient Mismatch Correction
...
import h5py
with h5py.File('lightsheet_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/lightsheet/public/
...
plt.tight_layout(); plt.savefig('lightsheet_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Light-Sheet Fluorescence Microscopy (LSFM) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/lightsheet` (mismatch correction tier).
