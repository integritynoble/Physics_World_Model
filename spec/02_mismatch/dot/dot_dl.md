# Diffuse Optical Tomography (DOT) — DOT-Net [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('dot_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('dot_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('dot_dot_dl_corrected.png'); plt.show()
```

## Benchmark

`Diffuse Optical Tomography (DOT) — DOT-Net [proxy] + gradient` corresponds to the leaderboard entry at
