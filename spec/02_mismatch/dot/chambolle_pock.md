# Diffuse Optical Tomography (DOT) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: boundary flux (sources × detectors, float32)
...
import h5py
with h5py.File('dot_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/dot/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # boundary flux (sources × detectors, float32)
...
plt.tight_layout(); plt.savefig('dot_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Diffuse Optical Tomography (DOT) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/dot` (mismatch correction tier).
