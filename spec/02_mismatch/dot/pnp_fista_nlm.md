# Diffuse Optical Tomography (DOT) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('dot_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`Diffuse Optical Tomography (DOT) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
