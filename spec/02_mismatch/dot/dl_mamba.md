# Diffuse Optical Tomography (DOT) — MambaRecon + Gradient Mismatch Correction

**Device**: GPU  **Input**: boundary flux (sources × detectors, float32)
...
plt.tight_layout(); plt.savefig('dot_dl_mamba_corrected.png'); plt.show()
```

## Benchmark

`Diffuse Optical Tomography (DOT) — MambaRecon + gradient` corresponds to the leaderboard entry at
