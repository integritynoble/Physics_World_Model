# Diffuse Optical Tomography (DOT) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('dot_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`Diffuse Optical Tomography (DOT) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
