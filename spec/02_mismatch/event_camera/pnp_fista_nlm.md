# Event Camera / Dynamic Vision Sensor (DVS) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('event_camera_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`Event Camera / Dynamic Vision Sensor (DVS) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
