# Event Camera / Dynamic Vision Sensor (DVS) — CS-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: event stream (N × 4: t,x,y,p)
...
plt.tight_layout(); plt.savefig('event_camera_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`Event Camera / Dynamic Vision Sensor (DVS) — CS-Transformer + gradient` corresponds to the leaderboard entry at
