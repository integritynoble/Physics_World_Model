# Event Camera / Dynamic Vision Sensor (DVS) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('event_camera_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`Event Camera / Dynamic Vision Sensor (DVS) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
