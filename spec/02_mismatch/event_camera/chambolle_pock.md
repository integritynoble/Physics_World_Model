# Event Camera / Dynamic Vision Sensor (DVS) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: event stream (N × 4: t,x,y,p)
...
import h5py
with h5py.File('event_camera_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/event_camera/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # event stream (N × 4: t,x,y,p)
...
plt.tight_layout(); plt.savefig('event_camera_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Event Camera / Dynamic Vision Sensor (DVS) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/event_camera` (mismatch correction tier).
