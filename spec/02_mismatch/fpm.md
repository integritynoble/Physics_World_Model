# Fourier Ptychographic Microscopy (FPM) — Mismatch Correction + Reconstruct
...
|-----------|-------|---------------|-------------------|
| LED position error (mm) | `[-2, +2] mm (x, y)` | ±0.5 mm | embedded pupil function recovery (EPRY) |
...
mismatch_cfg = {
    'led_pos_error': None,    # float if known; None = auto-estimate
    'search_range': '[-2, +2] mm (x, y)',
    'search_steps': 20,
}
```
...
plt.tight_layout(); plt.savefig('fpm_corrected.png'); plt.show()
```
