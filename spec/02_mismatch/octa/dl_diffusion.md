# OCT Angiography (OCTA) — DiffusionMed + Gradient Mismatch Correction

**Device**: GPU  **Input**: B-scans (T × depth × A-scans, float32)
...
import h5py
with h5py.File('octa_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/octa/public/
...
plt.tight_layout(); plt.savefig('octa_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`OCT Angiography (OCTA) — DiffusionMed + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/octa` (mismatch correction tier).
