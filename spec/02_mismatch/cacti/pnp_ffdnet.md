# Coded Aperture Compressive Temporal Imaging (CACTI) — PnP-FFDNet + Gradient Mismatch Correction

**Device**: CPU  **Input**: coded frames (B × H × W, float32)
...
x_wrong = run_solver('pnp_ffdnet', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('pnp_ffdnet', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cacti_pnp_ffdnet_corrected.png'); plt.show()
```

## Benchmark

`Coded Aperture Compressive Temporal Imaging (CACTI) — PnP-FFDNet + gradient` corresponds to the leaderboard entry at
