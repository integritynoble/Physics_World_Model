# Proton Radiography — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: fluence map (H × W, float32)
...
import h5py
with h5py.File('proton_radiography_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/proton_radiography/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # fluence map (H × W, float32)
...
plt.tight_layout(); plt.savefig('proton_radiography_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Proton Radiography — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/proton_radiography` (mismatch correction tier).
