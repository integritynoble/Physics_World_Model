# Positron Emission Tomography (PET) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('pet_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`Positron Emission Tomography (PET) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
