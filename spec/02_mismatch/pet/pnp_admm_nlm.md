# Positron Emission Tomography (PET) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('pet_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`Positron Emission Tomography (PET) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
