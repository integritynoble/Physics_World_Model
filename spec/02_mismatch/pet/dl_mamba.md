# Positron Emission Tomography (PET) — MambaRecon + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('pet_dl_mamba_corrected.png'); plt.show()
```

## Benchmark

`Positron Emission Tomography (PET) — MambaRecon + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/pet` (mismatch correction tier).
