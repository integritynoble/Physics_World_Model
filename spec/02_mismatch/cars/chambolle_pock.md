# Coherent Anti-Stokes Raman (CARS) Microscopy — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: CARS spectrum cube (H × W × λ, float32)
...
import h5py
with h5py.File('cars_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/cars/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # CARS spectrum cube (H × W × λ, float32)
...
plt.tight_layout(); plt.savefig('cars_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Coherent Anti-Stokes Raman (CARS) Microscopy — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/cars` (mismatch correction tier).
