# Cryo-EM Single Particle Analysis — CryoSPARC (PnP-PGD DRUNet) + Gradient
...
x_wrong = run_solver('cryosparc', y)           # no correction
defocus = cryoem_calibrate_defocus(y)
calib_cfg = {"defocus": float(defocus)}
x = run_solver('cryosparc', y, cfg=calib_cfg)        # corrected
