# Cryo-EM Single Particle Analysis — CryoMamba (RED DRUNet) + Gradient
...
x_wrong = run_solver('cryo_mamba', y)           # no correction
defocus = cryoem_calibrate_defocus(y)
calib_cfg = {"defocus": float(defocus)}
x = run_solver('cryo_mamba', y, cfg=calib_cfg)        # corrected
