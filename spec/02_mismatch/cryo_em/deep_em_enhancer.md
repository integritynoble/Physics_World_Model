# Cryo-EM Single Particle Analysis — DeepEMenhancer (DRUNet denoise) + Gradient Mismatch Correction
...
x_wrong = run_solver('deep_em_enhancer', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('deep_em_enhancer', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cryo_em_deep_em_enhancer_corrected.png'); plt.show()
```

## Benchmark

`Cryo-EM Single Particle Analysis — DeepEMenhancer (DRUNet denoise) + gradient` corresponds to the leaderboard entry at
