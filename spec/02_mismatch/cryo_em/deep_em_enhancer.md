# Cryo-EM Single Particle Analysis — DeepEMenhancer (DRUNet denoise) + Gradient

**GPU**  **Mismatch**: defocus value `[0.5, 3.0] μm`
...
x_wrong = run_solver('deep_em_enhancer', y)           # no correction
defocus = cryoem_calibrate_defocus(y)
calib_cfg = {"defocus": float(defocus)}
x = run_solver('deep_em_enhancer', y, cfg=calib_cfg)        # corrected
