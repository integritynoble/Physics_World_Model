# Cryo-EM Single Particle Analysis — Topaz-Denoise (DRUNet denoise) + Gradient
...
x_wrong = run_solver('topaz_denoise', y)           # no correction
defocus = cryoem_calibrate_defocus(y)
calib_cfg = {"defocus": float(defocus)}
x = run_solver('topaz_denoise', y, cfg=calib_cfg)        # corrected
