# Cryo-EM Single Particle Analysis — CryoSTAR (PnP-DRS DRUNet) + Gradient
...
x_wrong = run_solver('cryostar', y)           # no correction
defocus = cryoem_calibrate_defocus(y)
calib_cfg = {"defocus": float(defocus)}
x = run_solver('cryostar', y, cfg=calib_cfg)        # corrected
