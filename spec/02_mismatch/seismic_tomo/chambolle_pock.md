# Seismic Tomography — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: travel times (src-recv, float32)
...
import h5py
with h5py.File('seismic_tomo_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/seismic_tomo/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # travel times (src-recv, float32)
...
plt.tight_layout(); plt.savefig('seismic_tomo_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Seismic Tomography — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/seismic_tomo` (mismatch correction tier).
