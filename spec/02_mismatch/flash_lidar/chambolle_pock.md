# Flash LiDAR — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: range + intensity (H × W × 2, float32)
...
import h5py
with h5py.File('flash_lidar_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/flash_lidar/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # range + intensity (H × W × 2, float32)
...
plt.tight_layout(); plt.savefig('flash_lidar_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Flash LiDAR — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/flash_lidar` (mismatch correction tier).
