# MR Spectroscopy (MRS) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: FID (T, complex64)
...
import h5py
with h5py.File('mrs_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/mrs/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # FID (T, complex64)
...
plt.tight_layout(); plt.savefig('mrs_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`MR Spectroscopy (MRS) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/mrs` (mismatch correction tier).
