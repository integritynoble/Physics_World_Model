# FTIR Spectroscopic Imaging — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: interferogram (H × W × OPD, float32)
...
import h5py
with h5py.File('ftir_imaging_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/ftir_imaging/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # interferogram (H × W × OPD, float32)
...
plt.tight_layout(); plt.savefig('ftir_imaging_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`FTIR Spectroscopic Imaging — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/ftir_imaging` (mismatch correction tier).
