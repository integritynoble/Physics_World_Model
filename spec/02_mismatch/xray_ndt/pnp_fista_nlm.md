# X-ray NDT (Radiography) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('xray_ndt_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`X-ray NDT (Radiography) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
