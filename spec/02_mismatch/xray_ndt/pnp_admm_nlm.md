# X-ray NDT (Radiography) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('xray_ndt_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`X-ray NDT (Radiography) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
