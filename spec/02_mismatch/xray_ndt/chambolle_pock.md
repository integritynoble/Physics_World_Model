# X-ray NDT (Radiography) — Chambolle-Pock + Gradient Mismatch Correction
...
import h5py
with h5py.File('xray_ndt_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/xray_ndt/public/
...
plt.tight_layout(); plt.savefig('xray_ndt_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`X-ray NDT (Radiography) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/xray_ndt` (mismatch correction tier).
