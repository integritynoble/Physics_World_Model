# X-ray NDT (Radiography) — NDT-DefectNet [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('ndt_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('ndt_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('xray_ndt_ndt_dl_corrected.png'); plt.show()
```

## Benchmark

`X-ray NDT (Radiography) — NDT-DefectNet [proxy] + gradient` corresponds to the leaderboard entry at
