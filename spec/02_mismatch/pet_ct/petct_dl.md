# PET/CT Fusion — PET-CT-Fusion-Net [proxy] + Gradient Mismatch Correction

**Device**: CPU  **Input**: PET sino + CT proj (both float32)
...
import h5py
with h5py.File('pet_ct_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/pet_ct/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # PET sino + CT proj (both float32)
...
x_wrong = run_solver('petct_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('petct_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('pet_ct_petct_dl_corrected.png'); plt.show()
```

## Benchmark

`PET/CT Fusion — PET-CT-Fusion-Net [proxy] + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/pet_ct` (mismatch correction tier).
