# Radio Interferometry (VLBI) — R2D2 (interferometry) [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('ri_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('ri_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('radio_interferometry_ri_dl_corrected.png'); plt.show()
```

## Benchmark

`Radio Interferometry (VLBI) — R2D2 (interferometry) [proxy] + gradient` corresponds to the leaderboard entry at
