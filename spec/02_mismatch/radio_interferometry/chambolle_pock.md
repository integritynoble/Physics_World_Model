# Radio Interferometry (VLBI) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: UV-plane data (N_baselines, complex64)
...
import h5py
with h5py.File('radio_interferometry_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/radio_interferometry/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # UV-plane data (N_baselines, complex64)
...
plt.tight_layout(); plt.savefig('radio_interferometry_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Radio Interferometry (VLBI) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/radio_interferometry` (mismatch correction tier).
