# STED Microscopy — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: STED + confocal (2 × H × W, float32)
...
import h5py
with h5py.File('sted_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/sted/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # STED + confocal (2 × H × W, float32)
...
plt.tight_layout(); plt.savefig('sted_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`STED Microscopy — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/sted` (mismatch correction tier).
