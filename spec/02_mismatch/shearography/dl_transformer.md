# Shearography — Probe-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: shearograms (2 × H × W, float32)
...
import h5py
with h5py.File('shearography_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/shearography/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # shearograms (2 × H × W, float32)
...
plt.tight_layout(); plt.savefig('shearography_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`Shearography — Probe-Transformer + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/shearography` (mismatch correction tier).
