# Polarization Microscopy — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: Stokes images (4 × H × W, float32)
...
import h5py
with h5py.File('polarization_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/polarization/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # Stokes images (4 × H × W, float32)
...
plt.tight_layout(); plt.savefig('polarization_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Polarization Microscopy — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/polarization` (mismatch correction tier).
