# Brachytherapy Imaging — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: dose map (H × W × D, float32)
...
import h5py
with h5py.File('brachytherapy_img_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/brachytherapy_img/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # dose map (H × W × D, float32)
...
plt.tight_layout(); plt.savefig('brachytherapy_img_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Brachytherapy Imaging — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/brachytherapy_img` (mismatch correction tier).
