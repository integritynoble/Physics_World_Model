# Ptychographic Imaging — Ptychographic Iterative Engine (PIE) + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('pie', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ptychography_pie_corrected.png'); plt.show()
```

## Benchmark

`Ptychographic Imaging — Ptychographic Iterative Engine (PIE) + gradient` corresponds to the leaderboard entry at
