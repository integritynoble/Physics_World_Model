# Ptychographic Imaging — Wigner Distribution Deconvolution (WDD) + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('wdd', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ptychography_wdd_corrected.png'); plt.show()
```

## Benchmark

`Ptychographic Imaging — Wigner Distribution Deconvolution (WDD) + gradient` corresponds to the leaderboard entry at
