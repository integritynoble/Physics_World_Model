# Ptychographic Imaging — Ptychography Diffusion (DL-PGD) + Gradient

**GPU**  **Mismatch**: probe position error `[-3, +3] px`
...
x_wrong = run_solver('ptycho_diffusion', y)           # no correction
pos_err = ptycho_calibrate_offset(y)
calib_cfg = {"pos_error": pos_err}
x = run_solver('ptycho_diffusion', y, cfg=calib_cfg)        # corrected
