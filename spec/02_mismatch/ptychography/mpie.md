# Ptychographic Imaging — Momentum PIE (mPIE) + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('mpie', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ptychography_mpie_corrected.png'); plt.show()
```

## Benchmark

`Ptychographic Imaging — Momentum PIE (mPIE) + gradient` corresponds to the leaderboard entry at
