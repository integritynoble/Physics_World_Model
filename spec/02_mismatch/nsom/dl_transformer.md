# Near-field Scanning Optical Microscopy (NSOM) — Probe-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: near-field signal (H × W, float32)
...
import h5py
with h5py.File('nsom_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/nsom/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # near-field signal (H × W, float32)
...
plt.tight_layout(); plt.savefig('nsom_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`Near-field Scanning Optical Microscopy (NSOM) — Probe-Transformer + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/nsom` (mismatch correction tier).
