# Talbot-Lau X-ray Grating Interferometry — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: stepping images (N_steps × H × W, float32)
...
import h5py
with h5py.File('talbot_lau_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/talbot_lau/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # stepping images (N_steps × H × W, float32)
...
plt.tight_layout(); plt.savefig('talbot_lau_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Talbot-Lau X-ray Grating Interferometry — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/talbot_lau` (mismatch correction tier).
