# Widefield Fluorescence Microscopy — Richardson-Lucy with TV Regularisation + Gradient Mismatch Correction

**Device**: CPU  **Input**: fluorescence image (H × W, float32)
...
x_wrong = run_solver('rl_tv', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('rl_tv', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('widefield_rl_tv_corrected.png'); plt.show()
```

## Benchmark

`Widefield Fluorescence Microscopy — Richardson-Lucy with TV Regularisation + gradient` corresponds to the leaderboard entry at
