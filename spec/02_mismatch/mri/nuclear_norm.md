# Magnetic Resonance Imaging (MRI) — Nuclear Norm (SVT) + Gradient Mismatch Correction
...
x_wrong = run_solver('nuclear_norm', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('nuclear_norm', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('mri_nuclear_norm_corrected.png'); plt.show()
```

## Benchmark

`Magnetic Resonance Imaging (MRI) — Nuclear Norm (SVT) + gradient` corresponds to the leaderboard entry at
