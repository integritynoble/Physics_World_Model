# Magnetic Resonance Imaging (MRI) — POCS + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('pocs', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('mri_pocs_corrected.png'); plt.show()
```

## Benchmark

`Magnetic Resonance Imaging (MRI) — POCS + gradient` corresponds to the leaderboard entry at
