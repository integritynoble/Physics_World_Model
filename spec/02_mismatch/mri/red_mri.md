# Magnetic Resonance Imaging (MRI) — RED (Regularization by Denoising) + Gradient Mismatch Correction
...
x_wrong = run_solver('red_mri', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('red_mri', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('mri_red_mri_corrected.png'); plt.show()
```

## Benchmark

`Magnetic Resonance Imaging (MRI) — RED (Regularization by Denoising) + gradient` corresponds to the leaderboard entry at
