# Magnetic Resonance Imaging (MRI) — U-Net (fastMRI) + Gradient Mismatch Correction
...
x_wrong = run_solver('unet_mri', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('unet_mri', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('mri_unet_mri_corrected.png'); plt.show()
```

## Benchmark

`Magnetic Resonance Imaging (MRI) — U-Net (fastMRI) + gradient` corresponds to the leaderboard entry at
