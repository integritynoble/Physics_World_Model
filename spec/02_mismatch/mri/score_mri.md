# Magnetic Resonance Imaging (MRI) — Score-MRI (diffusion) + Gradient Mismatch Correction

**Device**: GPU  **Input**: k-space (H × W × 2: real+imag, float32)
...
x_wrong = run_solver('score_mri', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('score_mri', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('mri_score_mri_corrected.png'); plt.show()
```

## Benchmark

`Magnetic Resonance Imaging (MRI) — Score-MRI (diffusion) + gradient` corresponds to the leaderboard entry at
