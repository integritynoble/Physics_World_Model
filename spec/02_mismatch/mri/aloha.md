# Magnetic Resonance Imaging (MRI) — ALOHA (Hankel Low-Rank) + Gradient Mismatch Correction
...
x_wrong = run_solver('aloha', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('aloha', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('mri_aloha_corrected.png'); plt.show()
```

## Benchmark

`Magnetic Resonance Imaging (MRI) — ALOHA (Hankel Low-Rank) + gradient` corresponds to the leaderboard entry at
