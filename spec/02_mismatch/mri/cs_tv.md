# Magnetic Resonance Imaging (MRI) — CS-MRI (TV) + Gradient Mismatch Correction
...
x_wrong = run_solver('cs_tv', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('cs_tv', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('mri_cs_tv_corrected.png'); plt.show()
```

## Benchmark

`Magnetic Resonance Imaging (MRI) — CS-MRI (TV) + gradient` corresponds to the leaderboard entry at
