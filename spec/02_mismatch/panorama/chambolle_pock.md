# Panorama Multi-Focus Fusion — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: images (N × H × W × 3, uint8)
...
import h5py
with h5py.File('panorama_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/panorama/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # images (N × H × W × 3, uint8)
...
plt.tight_layout(); plt.savefig('panorama_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Panorama Multi-Focus Fusion — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/panorama` (mismatch correction tier).
