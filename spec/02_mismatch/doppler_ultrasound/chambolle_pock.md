# Doppler Ultrasound — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: IQ data (H × W × T, float32)
...
import h5py
with h5py.File('doppler_ultrasound_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/doppler_ultrasound/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # IQ data (H × W × T, float32)
...
plt.tight_layout(); plt.savefig('doppler_ultrasound_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Doppler Ultrasound — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/doppler_ultrasound` (mismatch correction tier).
