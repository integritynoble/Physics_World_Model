# Intravascular Ultrasound (IVUS) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: RF pullback (frames × elements × samples, float32)
...
import h5py
with h5py.File('ivus_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/ivus/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # RF pullback (frames × elements × samples, float32)
...
plt.tight_layout(); plt.savefig('ivus_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Intravascular Ultrasound (IVUS) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/ivus` (mismatch correction tier).
