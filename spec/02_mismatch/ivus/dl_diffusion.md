# Intravascular Ultrasound (IVUS) — DiffusionMed + Gradient Mismatch Correction

**Device**: GPU  **Input**: RF pullback (frames × elements × samples, float32)
...
plt.tight_layout(); plt.savefig('ivus_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Intravascular Ultrasound (IVUS) — DiffusionMed + gradient` corresponds to the leaderboard entry at
