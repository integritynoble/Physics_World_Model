# Single Photon Emission CT (SPECT) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: projections (angles × detectors, float32)
...
# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # projections (angles × detectors, float32)
...
plt.tight_layout(); plt.savefig('spect_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Single Photon Emission CT (SPECT) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/spect` (mismatch correction tier).
