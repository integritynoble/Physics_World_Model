# Generic Matrix Sensing — CS-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: partial matrix (M × N, float32, NaN=missing)
...
import h5py
with h5py.File('matrix_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/matrix/public/
...
plt.tight_layout(); plt.savefig('matrix_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`Generic Matrix Sensing — CS-Transformer + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/matrix` (mismatch correction tier).
