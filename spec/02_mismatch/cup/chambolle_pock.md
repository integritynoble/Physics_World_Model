# Compressed Ultrafast Photography (CUP) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: streak image (H × W_streak, float32)
...
import h5py
with h5py.File('cup_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/cup/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # streak image (H × W_streak, float32)
...
plt.tight_layout(); plt.savefig('cup_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Compressed Ultrafast Photography (CUP) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/cup` (mismatch correction tier).
