# Compressed Ultrafast Photography (CUP) — CS-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: streak image (H × W_streak, float32)
...
plt.tight_layout(); plt.savefig('cup_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`Compressed Ultrafast Photography (CUP) — CS-Transformer + gradient` corresponds to the leaderboard entry at
