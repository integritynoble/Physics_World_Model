# Active Thermography (IR) — Probe-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: thermal sequence (T × H × W, float32)
...
import h5py
with h5py.File('active_thermography_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/active_thermography/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # thermal sequence (T × H × W, float32)
...
plt.tight_layout(); plt.savefig('active_thermography_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`Active Thermography (IR) — Probe-Transformer + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/active_thermography` (mismatch correction tier).
