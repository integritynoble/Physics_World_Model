# Active Thermography (IR) — Pulsed-Phase TV [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('pulsed_phase_tv', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('pulsed_phase_tv', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('active_thermography_pulsed_phase_tv_corrected.png'); plt.show()
```

## Benchmark

`Active Thermography (IR) — Pulsed-Phase TV [proxy] + gradient` corresponds to the leaderboard entry at
