# Magnetic Particle Imaging (MPI) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: system function (freq × ch, complex64)
...
# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # system function (freq × ch, complex64)
...
plt.tight_layout(); plt.savefig('magnetic_particle_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Magnetic Particle Imaging (MPI) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/magnetic_particle` (mismatch correction tier).
