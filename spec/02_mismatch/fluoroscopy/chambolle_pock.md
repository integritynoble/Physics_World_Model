# Fluoroscopy — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: X-ray frames (T × H × W, float32)
...
import h5py
with h5py.File('fluoroscopy_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/fluoroscopy/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # X-ray frames (T × H × W, float32)
...
plt.tight_layout(); plt.savefig('fluoroscopy_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Fluoroscopy — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/fluoroscopy` (mismatch correction tier).
