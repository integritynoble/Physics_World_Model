# Digital Holographic Microscopy — Gerchberg-Saxton + Gradient Mismatch Correction
...
import h5py
with h5py.File('holography_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/holography/public/
...
x_wrong = run_solver('gerchberg_saxton', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('gerchberg_saxton', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('holography_gerchberg_saxton_corrected.png'); plt.show()
```

## Benchmark

`Digital Holographic Microscopy — Gerchberg-Saxton + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/holography` (mismatch correction tier).
