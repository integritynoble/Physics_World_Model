# Digital Holographic Microscopy — Hybrid Input-Output (HIO) + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('hio', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('holography_hio_corrected.png'); plt.show()
```

## Benchmark

`Digital Holographic Microscopy — Hybrid Input-Output (HIO) + gradient` corresponds to the leaderboard entry at
