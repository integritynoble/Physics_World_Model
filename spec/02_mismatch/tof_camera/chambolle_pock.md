# Time-of-Flight Depth Camera — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: depth + amplitude (H × W × 2, float32)
...
# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # depth + amplitude (H × W × 2, float32)
...
plt.tight_layout(); plt.savefig('tof_camera_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Time-of-Flight Depth Camera — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/tof_camera` (mismatch correction tier).
