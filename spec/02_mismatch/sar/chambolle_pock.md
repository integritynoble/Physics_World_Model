# Synthetic Aperture Radar (SAR) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: raw data (range × azimuth, complex64)
...
# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # raw data (range × azimuth, complex64)
...
plt.tight_layout(); plt.savefig('sar_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Synthetic Aperture Radar (SAR) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/sar` (mismatch correction tier).
