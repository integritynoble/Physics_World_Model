# Synthetic Aperture Radar (SAR) — FBP (SAR backprojection) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('sar_traditional_cpu_corrected.png'); plt.show()
```

## Benchmark

`Synthetic Aperture Radar (SAR) — FBP (SAR backprojection) + gradient` corresponds to the leaderboard entry at
