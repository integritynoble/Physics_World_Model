# Synthetic Aperture Radar (SAR) — SAR-DL (PolSF) [proxy] + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('sar_best_quality_corrected.png'); plt.show()
```

## Benchmark

`Synthetic Aperture Radar (SAR) — SAR-DL (PolSF) [proxy] + gradient` corresponds to the leaderboard entry at
