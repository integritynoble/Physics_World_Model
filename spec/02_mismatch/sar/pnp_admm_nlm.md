# Synthetic Aperture Radar (SAR) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('sar_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`Synthetic Aperture Radar (SAR) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
