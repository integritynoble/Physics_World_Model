# Synthetic Aperture Radar (SAR) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('sar_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`Synthetic Aperture Radar (SAR) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
