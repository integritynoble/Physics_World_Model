# Photoacoustic Imaging — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: time-series (elements × time, float32)
...
# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # time-series (elements × time, float32)
...
plt.tight_layout(); plt.savefig('photoacoustic_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Photoacoustic Imaging — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/photoacoustic` (mismatch correction tier).
