# Structured-Light Depth Camera — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: pattern images (N × H × W, float32)
...
import h5py
with h5py.File('structured_light_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/structured_light/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # pattern images (N × H × W, float32)
...
plt.tight_layout(); plt.savefig('structured_light_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Structured-Light Depth Camera — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/structured_light` (mismatch correction tier).
