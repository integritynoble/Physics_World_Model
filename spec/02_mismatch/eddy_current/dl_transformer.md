# Eddy Current Imaging — Probe-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: induced voltage (coils × time, float32)
...
import h5py
with h5py.File('eddy_current_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/eddy_current/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # induced voltage (coils × time, float32)
...
plt.tight_layout(); plt.savefig('eddy_current_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`Eddy Current Imaging — Probe-Transformer + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/eddy_current` (mismatch correction tier).
