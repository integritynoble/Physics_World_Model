# DESI Mass Spectrometry Imaging — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: mass image (H × W × m/z, float32)
...
import h5py
with h5py.File('desi_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/desi/public/
...
plt.tight_layout(); plt.savefig('desi_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`DESI Mass Spectrometry Imaging — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/desi` (mismatch correction tier).
