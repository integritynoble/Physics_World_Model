# XFEL Serial Femtosecond Crystallography (SFX) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: diffraction patterns (N_shots × H × W, float32)
...
import h5py
with h5py.File('xfel_sfx_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/xfel_sfx/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # diffraction patterns (N_shots × H × W, float32)
...
plt.tight_layout(); plt.savefig('xfel_sfx_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`XFEL Serial Femtosecond Crystallography (SFX) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/xfel_sfx` (mismatch correction tier).
