# Magnetic Force Microscopy (MFM) — Probe-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: magnetic force map (H × W, float32)
...
import h5py
with h5py.File('mfm_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/mfm/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # magnetic force map (H × W, float32)
...
plt.tight_layout(); plt.savefig('mfm_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`Magnetic Force Microscopy (MFM) — Probe-Transformer + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/mfm` (mismatch correction tier).
