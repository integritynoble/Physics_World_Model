# MALDI Mass Spectrometry Imaging — MSI-UNet [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('msi_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('msi_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('maldi_msi_msi_dl_corrected.png'); plt.show()
```

## Benchmark

`MALDI Mass Spectrometry Imaging — MSI-UNet [proxy] + gradient` corresponds to the leaderboard entry at
