# Portal Imaging (EPID) — PortalDL [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('portal_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('portal_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('portal_imaging_portal_dl_corrected.png'); plt.show()
```

## Benchmark

`Portal Imaging (EPID) — PortalDL [proxy] + gradient` corresponds to the leaderboard entry at
