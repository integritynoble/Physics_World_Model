# Portal Imaging (EPID) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: EPID projection (H × W, float32)
...
import h5py
with h5py.File('portal_imaging_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/portal_imaging/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # EPID projection (H × W, float32)
...
plt.tight_layout(); plt.savefig('portal_imaging_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Portal Imaging (EPID) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/portal_imaging` (mismatch correction tier).
