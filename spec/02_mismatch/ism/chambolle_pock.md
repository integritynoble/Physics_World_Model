# Image Scanning Microscopy (ISM) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: raw stack (H_scan × W_scan × px × py, float32)
...
import h5py
with h5py.File('ism_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/ism/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # raw stack (H_scan × W_scan × px × py, float32)
...
plt.tight_layout(); plt.savefig('ism_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Image Scanning Microscopy (ISM) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/ism` (mismatch correction tier).
