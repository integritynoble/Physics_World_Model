# Image Scanning Microscopy (ISM) — DiffusionMicro + Gradient Mismatch Correction

**Device**: GPU  **Input**: raw stack (H_scan × W_scan × px × py, float32)
...
plt.tight_layout(); plt.savefig('ism_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Image Scanning Microscopy (ISM) — DiffusionMicro + gradient` corresponds to the leaderboard entry at
