# Cone-Beam Computed Tomography (CBCT) — Mismatch Correction + Reconstruct

**Input**: projections (angles × H × W, float32)
**Benchmark**: `gs://pwm-benchmark-datasets/datasets/Benchmark/cbct/public/`
**Algorithms**: 17 CPU, 5 GPU — reconstruct with `run_solver(key, y)`

## Mismatch

| Parameter | Range | Typical Error | Correction Method |
|-----------|-------|---------------|-------------------|
| source-detector distance (SAD/SDD) | `SAD ±5 mm, SDD ±10 mm` | ±2 mm per session | phantom-based geometric calibration |
...
mismatch_cfg = {
    'geometry_error': None,    # float if known; None = auto-estimate
    'search_range': 'SAD ±5 mm, SDD ±10 mm',
    'search_steps': 20,
}
```
...
x_wrong = run_solver('tv_admm', y)

# 2. Reconstruct WITH mismatch correction
x_corrected = run_solver('tv_admm', y, cfg=mismatch_cfg)
...
plt.tight_layout(); plt.savefig('cbct_corrected.png'); plt.show()
```
