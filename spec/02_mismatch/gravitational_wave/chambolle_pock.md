# Gravitational Wave Detection — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: strain (samples, float32)
...
import h5py
with h5py.File('gravitational_wave_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/gravitational_wave/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # strain (samples, float32)
...
plt.tight_layout(); plt.savefig('gravitational_wave_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Gravitational Wave Detection — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/gravitational_wave` (mismatch correction tier).
