# Industrial X-ray CT — MambaRecon + Gradient Mismatch Correction

**Device**: GPU  **Input**: sinogram (angles × detectors, float32)
...
|-----------|-------|---------|---------------------|
| CoR offset (px) | `[-10, +10] px` | ±2 px | Sinogram symmetry; gradient descent on projection consistency metric |
...
plt.tight_layout(); plt.savefig('industrial_ct_dl_mamba_corrected.png'); plt.show()
```

## Benchmark

`Industrial X-ray CT — MambaRecon + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/industrial_ct` (mismatch correction tier).
