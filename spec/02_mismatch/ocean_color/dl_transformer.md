# Ocean Color Remote Sensing — RS-Transformer + Gradient Mismatch Correction

**Device**: GPU  **Input**: radiance (H × W × bands, float32)
...
import h5py
with h5py.File('ocean_color_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/ocean_color/public/
...
plt.tight_layout(); plt.savefig('ocean_color_dl_transformer_corrected.png'); plt.show()
```

## Benchmark

`Ocean Color Remote Sensing — RS-Transformer + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/ocean_color` (mismatch correction tier).
