# Scanning Electron Microscopy (SEM) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('sem_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`Scanning Electron Microscopy (SEM) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
