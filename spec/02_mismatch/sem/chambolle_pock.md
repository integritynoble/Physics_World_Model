# Scanning Electron Microscopy (SEM) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: SEM image (H × W, uint16)
...
import h5py
with h5py.File('sem_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/sem/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # SEM image (H × W, uint16)
...
plt.tight_layout(); plt.savefig('sem_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Scanning Electron Microscopy (SEM) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/sem` (mismatch correction tier).
