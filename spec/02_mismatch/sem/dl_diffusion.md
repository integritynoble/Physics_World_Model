# Scanning Electron Microscopy (SEM) — 3D-Diffusion + Gradient Mismatch Correction

**Device**: GPU  **Input**: SEM image (H × W, uint16)
...
plt.tight_layout(); plt.savefig('sem_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Scanning Electron Microscopy (SEM) — 3D-Diffusion + gradient` corresponds to the leaderboard entry at
