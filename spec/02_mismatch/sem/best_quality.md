# Scanning Electron Microscopy (SEM) — SEM-DL (SegNet) [proxy] + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('sem_best_quality_corrected.png'); plt.show()
```

## Benchmark

`Scanning Electron Microscopy (SEM) — SEM-DL (SegNet) [proxy] + gradient` corresponds to the leaderboard entry at
