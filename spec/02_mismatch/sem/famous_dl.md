# Scanning Electron Microscopy (SEM) — SEM-UNet [proxy] + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('sem_famous_dl_corrected.png'); plt.show()
```

## Benchmark

`Scanning Electron Microscopy (SEM) — SEM-UNet [proxy] + gradient` corresponds to the leaderboard entry at
