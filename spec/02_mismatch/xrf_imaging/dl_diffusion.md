# X-ray Fluorescence (XRF) Imaging — XR-Diffusion + Gradient Mismatch Correction

**Device**: GPU  **Input**: fluorescence map (H × W × elements, float32)
...
plt.tight_layout(); plt.savefig('xrf_imaging_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`X-ray Fluorescence (XRF) Imaging — XR-Diffusion + gradient` corresponds to the leaderboard entry at
