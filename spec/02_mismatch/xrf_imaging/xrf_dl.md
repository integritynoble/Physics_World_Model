# X-ray Fluorescence (XRF) Imaging — XRF-Net [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('xrf_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('xrf_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('xrf_imaging_xrf_dl_corrected.png'); plt.show()
```

## Benchmark

`X-ray Fluorescence (XRF) Imaging — XRF-Net [proxy] + gradient` corresponds to the leaderboard entry at
