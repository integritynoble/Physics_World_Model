# Weather / Doppler Radar — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: reflectivity (H × W, float32)
...
import h5py
with h5py.File('weather_radar_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/weather_radar/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # reflectivity (H × W, float32)
...
plt.tight_layout(); plt.savefig('weather_radar_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Weather / Doppler Radar — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/weather_radar` (mismatch correction tier).
