# Lattice Light-Sheet Microscopy — LLSM-CARE + Gradient Mismatch Correction

**Device**: GPU  **Input**: Z-stack (Z × H × W, float32)
...
import h5py
with h5py.File('lattice_lightsheet_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/lattice_lightsheet/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # Z-stack (Z × H × W, float32)
...
x_wrong = run_solver('llsm_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('llsm_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('lattice_lightsheet_llsm_dl_corrected.png'); plt.show()
```

## Benchmark

`Lattice Light-Sheet Microscopy — LLSM-CARE + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/lattice_lightsheet` (mismatch correction tier).
