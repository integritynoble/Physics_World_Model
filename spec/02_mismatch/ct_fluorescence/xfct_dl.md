# CT + Fluorescence (FLIT) — XFCT-Net [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('xfct_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('xfct_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_fluorescence_xfct_dl_corrected.png'); plt.show()
```

## Benchmark

`CT + Fluorescence (FLIT) — XFCT-Net [proxy] + gradient` corresponds to the leaderboard entry at
