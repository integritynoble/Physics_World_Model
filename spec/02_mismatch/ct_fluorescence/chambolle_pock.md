# CT + Fluorescence (FLIT) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: XRF sinogram (angles × detectors × ch, float32)
...
import h5py
with h5py.File('ct_fluorescence_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/ct_fluorescence/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # XRF sinogram (angles × detectors × ch, float32)
...
plt.tight_layout(); plt.savefig('ct_fluorescence_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`CT + Fluorescence (FLIT) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/ct_fluorescence` (mismatch correction tier).
