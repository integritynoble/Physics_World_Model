# X-ray Computed Tomography (CT) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
x_wrong = run_solver('pnp_fista_nlm', y, cfg={'iters': 20, 'sigma': 0.05})
...
cfg.update({'iters': 20, 'sigma': 0.05})
...
plt.tight_layout(); plt.savefig('ct_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
