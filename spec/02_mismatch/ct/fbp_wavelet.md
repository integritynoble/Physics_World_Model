# X-ray Computed Tomography (CT) — FBP + Wavelet + Gradient Mismatch Correction
...
x_wrong = run_solver('fbp_wavelet', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('fbp_wavelet', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_fbp_wavelet_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — FBP + Wavelet + gradient` corresponds to the leaderboard entry at
