# X-ray Computed Tomography (CT) — ART + Gradient Mismatch Correction
...
x_wrong = run_solver('art', y, cfg={'iters': 5, 'relaxation': 0.1})
...
cfg.update({'iters': 5, 'relaxation': 0.1})
...
plt.tight_layout(); plt.savefig('ct_art_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — ART + gradient` corresponds to the leaderboard entry at
