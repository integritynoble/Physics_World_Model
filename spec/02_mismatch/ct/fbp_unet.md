# X-ray Computed Tomography (CT) — FBP + U-Net + Gradient Mismatch Correction
...
x_wrong = run_solver('fbp_unet', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('fbp_unet', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_fbp_unet_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — FBP + U-Net + gradient` corresponds to the leaderboard entry at
