# X-ray Computed Tomography (CT) — FBP (Ram-Lak) + Gradient Mismatch Correction
...
x_wrong = run_solver('traditional_cpu', y, cfg={'filter': 'ramlak'})
...
plt.tight_layout(); plt.savefig('ct_traditional_cpu_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — FBP (Ram-Lak) + gradient` corresponds to the leaderboard entry at
