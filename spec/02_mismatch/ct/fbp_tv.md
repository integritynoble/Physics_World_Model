# X-ray Computed Tomography (CT) — FBP + TV + Gradient Mismatch Correction
...
x_wrong = run_solver('fbp_tv', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('fbp_tv', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_fbp_tv_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — FBP + TV + gradient` corresponds to the leaderboard entry at
