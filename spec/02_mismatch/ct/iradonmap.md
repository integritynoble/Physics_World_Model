# X-ray Computed Tomography (CT) — iRadonMAP + Gradient Mismatch Correction
...
x_wrong = run_solver('iradonmap', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('iradonmap', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_iradonmap_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — iRadonMAP + gradient` corresponds to the leaderboard entry at
