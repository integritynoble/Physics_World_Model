# X-ray Computed Tomography (CT) — FBP + Bilateral + Gradient
...
x_wrong = run_solver('fbp_bilateral', y)           # no correction
cor_offset = ct_calibrate_cor(y, shift_range=5)
calib_cfg = {"cor_offset": float(cor_offset)}
x = run_solver('fbp_bilateral', y, cfg=calib_cfg)        # corrected
