# X-ray Computed Tomography (CT) — FBP + Bilateral + Gradient Mismatch Correction
...
x_wrong = run_solver('fbp_bilateral', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('fbp_bilateral', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_fbp_bilateral_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — FBP + Bilateral + gradient` corresponds to the leaderboard entry at
