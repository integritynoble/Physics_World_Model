# X-ray Computed Tomography (CT) — CTformer + Gradient Mismatch Correction
...
x_wrong = run_solver('ctformer', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('ctformer', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_ctformer_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — CTformer + gradient` corresponds to the leaderboard entry at
