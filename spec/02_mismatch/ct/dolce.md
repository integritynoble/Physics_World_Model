# X-ray Computed Tomography (CT) — DOLCE + Gradient Mismatch Correction
...
x_wrong = run_solver('dolce', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('dolce', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_dolce_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — DOLCE + gradient` corresponds to the leaderboard entry at
