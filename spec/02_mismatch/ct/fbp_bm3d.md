# X-ray Computed Tomography (CT) — FBP + BM3D + Gradient Mismatch Correction
...
x_wrong = run_solver('fbp_bm3d', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('fbp_bm3d', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_fbp_bm3d_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — FBP + BM3D + gradient` corresponds to the leaderboard entry at
