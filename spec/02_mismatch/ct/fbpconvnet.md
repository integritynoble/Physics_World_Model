# X-ray Computed Tomography (CT) — FBPConvNet + Gradient Mismatch Correction
...
x_wrong = run_solver('fbpconvnet', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('fbpconvnet', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_fbpconvnet_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — FBPConvNet + gradient` corresponds to the leaderboard entry at
