# X-ray Computed Tomography (CT) — DPS + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('dps', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_dps_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — DPS + gradient` corresponds to the leaderboard entry at
