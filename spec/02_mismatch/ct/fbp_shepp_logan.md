# X-ray Computed Tomography (CT) — FBP (Shepp-Logan) + Gradient Mismatch Correction
...
x_wrong = run_solver('fbp_shepp_logan', y, cfg={'filter': 'shepp_logan'})
...
cfg.update({'filter': 'shepp_logan'})
x_corrected = run_solver('fbp_shepp_logan', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_fbp_shepp_logan_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — FBP (Shepp-Logan) + gradient` corresponds to the leaderboard entry at
