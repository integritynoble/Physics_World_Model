# X-ray Computed Tomography (CT) — DuDoTrans + Gradient

**GPU**  **PSNR**: ~42.1 dB  **Mismatch**: center-of-rotation offset `[-5, +5] px`
...
x_wrong = run_solver('dudotrans', y)           # no correction
cor_offset = ct_calibrate_cor(y, shift_range=5)
calib_cfg = {"cor_offset": float(cor_offset)}
x = run_solver('dudotrans', y, cfg=calib_cfg)        # corrected
