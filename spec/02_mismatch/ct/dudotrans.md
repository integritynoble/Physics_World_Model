# X-ray Computed Tomography (CT) — DuDoTrans + Gradient Mismatch Correction
...
x_wrong = run_solver('dudotrans', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('dudotrans', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_dudotrans_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — DuDoTrans + gradient` corresponds to the leaderboard entry at
