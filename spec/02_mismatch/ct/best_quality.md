# X-ray Computed Tomography (CT) — FBP + NLM + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('ct_best_quality_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — FBP + NLM + gradient` corresponds to the leaderboard entry at
