# X-ray Computed Tomography (CT) — FBP (Hann) + Gradient Mismatch Correction
...
x_wrong = run_solver('fbp_hann', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('fbp_hann', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_fbp_hann_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — FBP (Hann) + gradient` corresponds to the leaderboard entry at
