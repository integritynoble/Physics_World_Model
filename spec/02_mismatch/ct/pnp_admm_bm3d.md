# X-ray Computed Tomography (CT) — PnP-ADMM (BM3D) + Gradient Mismatch Correction
...
x_wrong = run_solver('pnp_admm_bm3d', y, cfg={'iters': 10, 'sigma': 0.05, 'rho': 0.5})
...
cfg.update({'iters': 10, 'sigma': 0.05, 'rho': 0.5})
x_corrected = run_solver('pnp_admm_bm3d', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_pnp_admm_bm3d_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — PnP-ADMM (BM3D) + gradient` corresponds to the leaderboard entry at
