# X-ray Computed Tomography (CT) — FBP (Cosine) + Gradient Mismatch Correction
...
x_wrong = run_solver('fbp_cosine', y, cfg={'filter': 'cosine'})
...
cfg.update({'filter': 'cosine'})
x_corrected = run_solver('fbp_cosine', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_fbp_cosine_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — FBP (Cosine) + gradient` corresponds to the leaderboard entry at
