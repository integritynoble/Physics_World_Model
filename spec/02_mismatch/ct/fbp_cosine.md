# X-ray Computed Tomography (CT) — FBP (Cosine) + Gradient
...
x_wrong = run_solver('fbp_cosine', y)           # no correction
cor_offset = ct_calibrate_cor(y, shift_range=5)
calib_cfg = {"cor_offset": float(cor_offset)}
x = run_solver('fbp_cosine', y, cfg={**calib_cfg, **{'filter': 'cosine'}})        # corrected
