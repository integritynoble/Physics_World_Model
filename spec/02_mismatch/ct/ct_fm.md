# X-ray Computed Tomography (CT) — CT-FM + Gradient Mismatch Correction
...
x_wrong = run_solver('ct_fm', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('ct_fm', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_ct_fm_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — CT-FM + gradient` corresponds to the leaderboard entry at
