# X-ray Computed Tomography (CT) — SART + Gradient Mismatch Correction
...
x_wrong = run_solver('sart', y, cfg={'iters': 10, 'relaxation': 0.25})
...
cfg.update({'iters': 10, 'relaxation': 0.25})
x_corrected = run_solver('sart', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_sart_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — SART + gradient` corresponds to the leaderboard entry at
