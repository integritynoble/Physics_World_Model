# X-ray Computed Tomography (CT) — FBP (Hamming) + Gradient Mismatch Correction
...
x_wrong = run_solver('fbp_hamming', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('fbp_hamming', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_fbp_hamming_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — FBP (Hamming) + gradient` corresponds to the leaderboard entry at
