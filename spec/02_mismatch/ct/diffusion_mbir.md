# X-ray Computed Tomography (CT) — DiffusionMBIR + Gradient Mismatch Correction
...
x_wrong = run_solver('diffusion_mbir', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('diffusion_mbir', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_diffusion_mbir_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — DiffusionMBIR + gradient` corresponds to the leaderboard entry at
