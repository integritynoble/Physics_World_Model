# X-ray Computed Tomography (CT) — DiffusionMBIR + Gradient

**GPU**  **PSNR**: ~43.8 dB  **Mismatch**: center-of-rotation offset `[-5, +5] px`
...
x_wrong = run_solver('diffusion_mbir', y)           # no correction
cor_offset = ct_calibrate_cor(y, shift_range=5)
calib_cfg = {"cor_offset": float(cor_offset)}
x = run_solver('diffusion_mbir', y, cfg=calib_cfg)        # corrected
