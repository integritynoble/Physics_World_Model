# X-ray Computed Tomography (CT) — Tikhonov + Gradient Mismatch Correction
...
x_wrong = run_solver('tikhonov', y, cfg={'iters': 30, 'lam': 0.01})
...
plt.tight_layout(); plt.savefig('ct_tikhonov_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — Tikhonov + gradient` corresponds to the leaderboard entry at
