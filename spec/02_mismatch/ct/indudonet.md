# X-ray Computed Tomography (CT) — InDuDoNet + Gradient Mismatch Correction
...
x_wrong = run_solver('indudonet', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('indudonet', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_indudonet_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — InDuDoNet + gradient` corresponds to the leaderboard entry at
