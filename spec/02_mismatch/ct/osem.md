# X-ray Computed Tomography (CT) — OSEM + Gradient Mismatch Correction
...
x_wrong = run_solver('osem', y, cfg={'iters': 5, 'n_subsets': 10})
...
cfg.update({'iters': 5, 'n_subsets': 10})
...
plt.tight_layout(); plt.savefig('ct_osem_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — OSEM + gradient` corresponds to the leaderboard entry at
