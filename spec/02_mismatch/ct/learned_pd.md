# X-ray Computed Tomography (CT) — Learned Primal-Dual + Gradient Mismatch Correction
...
x_wrong = run_solver('learned_pd', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('learned_pd', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_learned_pd_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — Learned Primal-Dual + gradient` corresponds to the leaderboard entry at
