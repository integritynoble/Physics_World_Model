# X-ray Computed Tomography (CT) — Learned Primal-Dual + Gradient

**GPU**  **PSNR**: ~36.2 dB  **Mismatch**: center-of-rotation offset `[-5, +5] px`
...
x_wrong = run_solver('learned_pd', y)           # no correction
cor_offset = ct_calibrate_cor(y, shift_range=5)
calib_cfg = {"cor_offset": float(cor_offset)}
x = run_solver('learned_pd', y, cfg=calib_cfg)        # corrected
