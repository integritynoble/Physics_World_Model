# X-ray Computed Tomography (CT) — DuDoNet + Gradient Mismatch Correction
...
x_wrong = run_solver('dudonet', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('dudonet', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('ct_dudonet_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — DuDoNet + gradient` corresponds to the leaderboard entry at
