# X-ray Computed Tomography (CT) — PnP-HQS (NLM) + Gradient Mismatch Correction
...
x_wrong = run_solver('pnp_hqs_nlm', y, cfg={'iters': 15, 'sigma': 0.05})
...
cfg.update({'iters': 15, 'sigma': 0.05})
...
plt.tight_layout(); plt.savefig('ct_pnp_hqs_nlm_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — PnP-HQS (NLM) + gradient` corresponds to the leaderboard entry at
