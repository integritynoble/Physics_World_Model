# X-ray Computed Tomography (CT) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('ct_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
