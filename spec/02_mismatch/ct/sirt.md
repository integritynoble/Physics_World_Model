# X-ray Computed Tomography (CT) — SIRT + Gradient Mismatch Correction
...
x_wrong = run_solver('sirt', y, cfg={'iters': 20})
...
plt.tight_layout(); plt.savefig('ct_sirt_corrected.png'); plt.show()
```

## Benchmark

`X-ray Computed Tomography (CT) — SIRT + gradient` corresponds to the leaderboard entry at
