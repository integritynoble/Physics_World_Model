# Fundus Camera — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: photograph (H × W × 3, uint8)
...
# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # photograph (H × W × 3, uint8)
...
plt.tight_layout(); plt.savefig('fundus_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Fundus Camera — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/fundus` (mismatch correction tier).
