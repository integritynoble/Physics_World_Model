# Machine Vision / AOI — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: image (H × W × 3, uint8)
...
import h5py
with h5py.File('machine_vision_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/machine_vision/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # image (H × W × 3, uint8)
...
plt.tight_layout(); plt.savefig('machine_vision_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Machine Vision / AOI — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/machine_vision` (mismatch correction tier).
