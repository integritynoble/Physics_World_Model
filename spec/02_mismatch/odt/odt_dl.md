# Optical Diffraction Tomography (ODT) — ODT-Net (PhaseNet) [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('odt_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('odt_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('odt_odt_dl_corrected.png'); plt.show()
```

## Benchmark

`Optical Diffraction Tomography (ODT) — ODT-Net (PhaseNet) [proxy] + gradient` corresponds to the leaderboard entry at
