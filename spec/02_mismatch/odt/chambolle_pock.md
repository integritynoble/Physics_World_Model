# Optical Diffraction Tomography (ODT) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: holograms (angles × H × W, complex64)
...
import h5py
with h5py.File('odt_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/odt/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # holograms (angles × H × W, complex64)
...
plt.tight_layout(); plt.savefig('odt_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Optical Diffraction Tomography (ODT) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/odt` (mismatch correction tier).
