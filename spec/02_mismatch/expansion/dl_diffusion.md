# Expansion Microscopy (ExM) — DiffusionMicro + Gradient Mismatch Correction

**Device**: GPU  **Input**: confocal + expansion (H × W, float32)
...
plt.tight_layout(); plt.savefig('expansion_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Expansion Microscopy (ExM) — DiffusionMicro + gradient` corresponds to the leaderboard entry at
