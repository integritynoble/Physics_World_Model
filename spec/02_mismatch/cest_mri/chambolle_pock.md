# CEST MRI — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: Z-spectrum (offsets × H × W, float32)
...
import h5py
with h5py.File('cest_mri_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/cest_mri/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # Z-spectrum (offsets × H × W, float32)
...
plt.tight_layout(); plt.savefig('cest_mri_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`CEST MRI — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/cest_mri` (mismatch correction tier).
