# Cone-Beam Computed Tomography (CBCT) — Simultaneous ART (SART) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('cbct_sart_corrected.png'); plt.show()
```

## Benchmark

`Cone-Beam Computed Tomography (CBCT) — Simultaneous ART (SART) + gradient` corresponds to the leaderboard entry at
