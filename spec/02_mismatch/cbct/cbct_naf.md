# Cone-Beam Computed Tomography (CBCT) — CBCT Neural Attenuation Fields (DL-DRS) + Gradient Mismatch Correction
...
x_wrong = run_solver('cbct_naf', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('cbct_naf', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cbct_cbct_naf_corrected.png'); plt.show()
```

## Benchmark

`Cone-Beam Computed Tomography (CBCT) — CBCT Neural Attenuation Fields (DL-DRS) + gradient` corresponds to the leaderboard entry at
