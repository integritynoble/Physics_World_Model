# Cone-Beam Computed Tomography (CBCT) — CBCT-UNet (DnCNN) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('cbct_small_gpu_corrected.png'); plt.show()
```

## Benchmark

`Cone-Beam Computed Tomography (CBCT) — CBCT-UNet (DnCNN) + gradient` corresponds to the leaderboard entry at
