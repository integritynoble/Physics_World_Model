# Cone-Beam Computed Tomography (CBCT) — PnP-ADMM with NLM + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('cbct_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`Cone-Beam Computed Tomography (CBCT) — PnP-ADMM with NLM + gradient` corresponds to the leaderboard entry at
