# Cone-Beam Computed Tomography (CBCT) — Conjugate Gradient Least Squares (CGLS) + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('cgls', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cbct_cgls_corrected.png'); plt.show()
```

## Benchmark

`Cone-Beam Computed Tomography (CBCT) — Conjugate Gradient Least Squares (CGLS) + gradient` corresponds to the leaderboard entry at
