# Cone-Beam Computed Tomography (CBCT) — FDK-DL (DL-PGD) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('cbct_famous_dl_corrected.png'); plt.show()
```

## Benchmark

`Cone-Beam Computed Tomography (CBCT) — FDK-DL (DL-PGD) + gradient` corresponds to the leaderboard entry at
