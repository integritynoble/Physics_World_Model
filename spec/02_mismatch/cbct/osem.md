# Cone-Beam Computed Tomography (CBCT) — Ordered Subsets EM (OS-EM) + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('osem', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cbct_osem_corrected.png'); plt.show()
```

## Benchmark

`Cone-Beam Computed Tomography (CBCT) — Ordered Subsets EM (OS-EM) + gradient` corresponds to the leaderboard entry at
