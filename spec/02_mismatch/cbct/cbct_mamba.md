# Cone-Beam Computed Tomography (CBCT) — CBCT-Mamba (RED-DRUNet) + Gradient Mismatch Correction
...
x_wrong = run_solver('cbct_mamba', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('cbct_mamba', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cbct_cbct_mamba_corrected.png'); plt.show()
```

## Benchmark

`Cone-Beam Computed Tomography (CBCT) — CBCT-Mamba (RED-DRUNet) + gradient` corresponds to the leaderboard entry at
