# Cone-Beam Computed Tomography (CBCT) — Simultaneous Iterative Reconstruction (SIRT) + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('sirt', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cbct_sirt_corrected.png'); plt.show()
```

## Benchmark

`Cone-Beam Computed Tomography (CBCT) — Simultaneous Iterative Reconstruction (SIRT) + gradient` corresponds to the leaderboard entry at
