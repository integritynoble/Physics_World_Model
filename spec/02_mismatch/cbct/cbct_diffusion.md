# Cone-Beam Computed Tomography (CBCT) — CBCT Diffusion (DL-PGD) + Gradient Mismatch Correction
...
x_wrong = run_solver('cbct_diffusion', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('cbct_diffusion', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cbct_cbct_diffusion_corrected.png'); plt.show()
```

## Benchmark

`Cone-Beam Computed Tomography (CBCT) — CBCT Diffusion (DL-PGD) + gradient` corresponds to the leaderboard entry at
