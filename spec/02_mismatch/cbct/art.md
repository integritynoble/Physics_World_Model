# Cone-Beam Computed Tomography (CBCT) — Algebraic Reconstruction Technique (ART) + Gradient Mismatch Correction
...
cfg.update({})
x_corrected = run_solver('art', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('cbct_art_corrected.png'); plt.show()
```

## Benchmark

`Cone-Beam Computed Tomography (CBCT) — Algebraic Reconstruction Technique (ART) + gradient` corresponds to the leaderboard entry at
