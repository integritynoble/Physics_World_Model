# 3D Gaussian Splatting (3DGS) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('gaussian_splatting_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`3D Gaussian Splatting (3DGS) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
