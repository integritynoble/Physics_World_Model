# 3D Gaussian Splatting (3DGS) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('gaussian_splatting_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`3D Gaussian Splatting (3DGS) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
