# 3D Gaussian Splatting (3DGS) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: posed images (N × H × W × 3, float32)
...
import h5py
with h5py.File('gaussian_splatting_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/gaussian_splatting/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # posed images (N × H × W × 3, float32)
...
plt.tight_layout(); plt.savefig('gaussian_splatting_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`3D Gaussian Splatting (3DGS) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/gaussian_splatting` (mismatch correction tier).
