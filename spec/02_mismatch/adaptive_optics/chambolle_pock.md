# Adaptive Optics (AO) Imaging — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: wavefront sensor (H × W, float32)
...
import h5py
with h5py.File('adaptive_optics_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/adaptive_optics/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # wavefront sensor (H × W, float32)
...
plt.tight_layout(); plt.savefig('adaptive_optics_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Adaptive Optics (AO) Imaging — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/adaptive_optics` (mismatch correction tier).
