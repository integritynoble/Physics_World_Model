# Adaptive Optics (AO) Imaging — Deep-AO [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('deep_ao', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('deep_ao', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('adaptive_optics_deep_ao_corrected.png'); plt.show()
```

## Benchmark

`Adaptive Optics (AO) Imaging — Deep-AO [proxy] + gradient` corresponds to the leaderboard entry at
