# Atom Probe Tomography (APT) — APT-Net [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('apt_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('apt_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('atom_probe_apt_dl_corrected.png'); plt.show()
```

## Benchmark

`Atom Probe Tomography (APT) — APT-Net [proxy] + gradient` corresponds to the leaderboard entry at
