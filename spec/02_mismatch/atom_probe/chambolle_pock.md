# Atom Probe Tomography (APT) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: hit positions (N × 3, float32)
...
import h5py
with h5py.File('atom_probe_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/atom_probe/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # hit positions (N × 3, float32)
...
plt.tight_layout(); plt.savefig('atom_probe_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Atom Probe Tomography (APT) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/atom_probe` (mismatch correction tier).
