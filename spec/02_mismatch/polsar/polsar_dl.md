# Polarimetric SAR (PolSAR) — PolSAR-Net [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('polsar_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('polsar_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('polsar_polsar_dl_corrected.png'); plt.show()
```

## Benchmark

`Polarimetric SAR (PolSAR) — PolSAR-Net [proxy] + gradient` corresponds to the leaderboard entry at
