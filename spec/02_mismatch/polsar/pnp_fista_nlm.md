# Polarimetric SAR (PolSAR) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('polsar_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`Polarimetric SAR (PolSAR) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
