# Polarimetric SAR (PolSAR) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: scattering matrix (H × W × 4, complex64)
...
import h5py
with h5py.File('polsar_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/polsar/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # scattering matrix (H × W × 4, complex64)
...
plt.tight_layout(); plt.savefig('polsar_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Polarimetric SAR (PolSAR) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/polsar` (mismatch correction tier).
