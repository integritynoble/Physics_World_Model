# Interferometric SAR (InSAR) — InSAR-Net [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('insar_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('insar_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('insar_insar_dl_corrected.png'); plt.show()
```

## Benchmark

`Interferometric SAR (InSAR) — InSAR-Net [proxy] + gradient` corresponds to the leaderboard entry at
