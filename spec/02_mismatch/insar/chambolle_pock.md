# Interferometric SAR (InSAR) — Chambolle-Pock + Gradient Mismatch Correction

**Device**: CPU  **Input**: interferometric phase (H × W, float32)
...
import h5py
with h5py.File('insar_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/insar/public/
    y, x_true = f['y'][0], f['x_true'][0]

# Option B — your data
import numpy as np
y      = np.load('your_measurement.npy').astype('float32')  # interferometric phase (H × W, float32)
...
plt.tight_layout(); plt.savefig('insar_chambolle_pock_corrected.png'); plt.show()
```

## Benchmark

`Interferometric SAR (InSAR) — Chambolle-Pock + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/insar` (mismatch correction tier).
