# MR Fingerprinting (MRF) — FBP [proxy] + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('mr_fingerprinting_traditional_cpu_corrected.png'); plt.show()
```

## Benchmark

`MR Fingerprinting (MRF) — FBP [proxy] + gradient` corresponds to the leaderboard entry at
