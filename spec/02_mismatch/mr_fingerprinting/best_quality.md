# MR Fingerprinting (MRF) — DL-Recon [proxy] + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('mr_fingerprinting_best_quality_corrected.png'); plt.show()
```

## Benchmark

`MR Fingerprinting (MRF) — DL-Recon [proxy] + gradient` corresponds to the leaderboard entry at
