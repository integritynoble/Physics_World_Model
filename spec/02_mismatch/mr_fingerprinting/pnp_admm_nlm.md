# MR Fingerprinting (MRF) — PnP-ADMM (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('mr_fingerprinting_pnp_admm_nlm_corrected.png'); plt.show()
```

## Benchmark

`MR Fingerprinting (MRF) — PnP-ADMM (NLM) + gradient` corresponds to the leaderboard entry at
