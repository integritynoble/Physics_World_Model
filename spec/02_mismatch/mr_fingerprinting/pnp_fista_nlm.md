# MR Fingerprinting (MRF) — PnP-FISTA (NLM) + Gradient Mismatch Correction
...
plt.tight_layout(); plt.savefig('mr_fingerprinting_pnp_fista_nlm_corrected.png'); plt.show()
```

## Benchmark

`MR Fingerprinting (MRF) — PnP-FISTA (NLM) + gradient` corresponds to the leaderboard entry at
