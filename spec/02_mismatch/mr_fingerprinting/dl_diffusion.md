# MR Fingerprinting (MRF) — DiffusionMed + Gradient Mismatch Correction

**Device**: GPU  **Input**: signal evolution (T × H × W, complex64)
...
import h5py
with h5py.File('mr_fingerprinting_public.h5', 'r') as f:   # GCS: gs://pwm-benchmark-datasets/datasets/Benchmark/mr_fingerprinting/public/
...
plt.tight_layout(); plt.savefig('mr_fingerprinting_dl_diffusion_corrected.png'); plt.show()
```

## Benchmark

`MR Fingerprinting (MRF) — DiffusionMed + gradient` corresponds to the leaderboard entry at
`https://pwm.platformai.org/benchmark/mr_fingerprinting` (mismatch correction tier).
