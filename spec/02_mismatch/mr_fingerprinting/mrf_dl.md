# MR Fingerprinting (MRF) — MRF-Net [proxy] + Gradient Mismatch Correction
...
x_wrong = run_solver('mrf_dl', y, cfg={})
...
cfg.update({})
x_corrected = run_solver('mrf_dl', y, cfg=cfg)
...
plt.tight_layout(); plt.savefig('mr_fingerprinting_mrf_dl_corrected.png'); plt.show()
```

## Benchmark

`MR Fingerprinting (MRF) — MRF-Net [proxy] + gradient` corresponds to the leaderboard entry at
